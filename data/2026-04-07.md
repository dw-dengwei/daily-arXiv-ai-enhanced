...
TL;DR: SafeScreen是一个安全优先的视频筛选框架，用于在开放域视频平台中为儿童和护理场景（如痴呆症护理）提供个性化视频推荐，通过强制执行个体化安全约束来防止有害内容暴露。


<details>
  <summary>Details</summary>
Motivation: 开放域视频平台虽然提供丰富的个性化内容，但其以参与度优化的推荐算法可能使脆弱用户（如儿童、痴呆症患者）暴露于不适当或有害内容。在儿童导向和护理场景中，内容必须在展示前满足个体化安全约束。

Method: SafeScreen采用安全优先的框架，将安全视为前提条件，通过自动化管道顺序批准或拒绝候选视频。包含三个核心组件：(1) 基于用户档案提取个体化安全标准；(2) 通过自适应问题生成和多模态VideoRAG分析进行证据基础评估；(3) 基于LLM的决策机制，在内容暴露前验证安全性、适当性和相关性。

Result: 在痴呆症护理回忆案例研究中，使用30个合成患者档案和90个测试查询进行评估。结果显示SafeScreen在80-93%的情况下优先考虑安全而非参与度，与YouTube的参与度优化排名存在显著差异，同时在安全性覆盖、合理性和基础性方面保持高水平，得到LLM评估和领域专家的验证。

Conclusion: SafeScreen提供了一种可解释的实时视频筛选方法，能够在不依赖预计算安全标签的情况下处理未策划的视频库，为脆弱用户群体提供安全优先的个性化视频推荐解决方案。
...
TL;DR: 提出基于SoC FPGA的智能相机平台，用于工业场景中喷射火焰的实时分割与表征，通过优化UNet模型实现边缘处理，显著降低参数数量和延迟


<details>
  <summary>Details</summary>
Motivation: 解决工业环境中缺乏实时火焰分割与表征方案的问题，特别是针对喷射火焰的早期检测需求

Method: 采用SoC FPGA（Ultra96平台）实现边缘处理流水线，使用Vitis框架优化UNet分割模型，通过参数压缩、多线程和批归一化等技术提升性能

Result: 模型参数从750万减少到59,095（125倍压缩），处理延迟降低7.5倍，达到30FPS实时性能，同时保持Dice Score精度

Conclusion: 提出的智能相机平台能够有效实现工业喷射火焰的实时分割与表征，该框架可扩展到其他消防安全应用
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在室内场景理解中融合2D-3D表示存在权衡问题：使用显式真值3D位置编码的方法与嫁接外部3D基础模型进行隐式几何的方法都导致次优部署。需要重新思考3D知识集成范式。

Method: 提出隐式几何涌现原理：通过细粒度几何验证器和全局表示约束等机制，利用特权几何监督构建信息瓶颈，迫使模型最大化视觉特征与3D结构之间的互信息，让3D感知在统一视觉表示中隐式涌现。

Result: 在多个3D场景理解基准测试中超越现有SOTA方法，推理延迟降低55%，同时在不同下游任务中保持强劲性能，实现了无依赖的3D理解。

Conclusion: 从外部嫁接转向隐式涌现代表了视觉语言模型中3D知识集成的根本性重新思考，精心设计的辅助目标能够实现无依赖的3D理解，为多模态场景理解提供了新范式。
...
### [4] [CGHair: Compact Gaussian Hair Reconstruction with Card Clustering](https://arxiv.org/abs/2604.03716)
*Haimin Luo,Srinjay Sarkar,Albert Mosella-Montoro,Francisco Vicente Carrasco,Fernando De la Torre*

Main category: cs.CV

TL;DR: 提出一种从多视角图像重建高保真头发的高效管道，通过聚类发丝为代表性发卡并共享纹理码本，结合3D高斯泼溅渲染，大幅降低重建时间、存储需求和内存占用。


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的头发重建方法虽然效果逼真，但通常需要数百万个图元，导致存储和渲染成本高昂。观察到同一发型中的头发具有结构和视觉相似性，可以利用这种相似性来优化重建效率。

Method: 1) 将发丝聚类为代表性发卡，并将这些发卡分组到共享纹理码本中；2) 将此结构与3D高斯泼溅渲染相结合；3) 提出基于生成先验的加速方法，从图像集重建初始发丝几何结构。

Result: 实验表明，发丝重建时间减少4倍，渲染性能相当，内存占用降低超过200倍。

Conclusion: 该方法在保持视觉质量的同时，显著提高了头发重建的效率，大幅降低了存储和渲染成本，为高保真头发重建提供了一种紧凑高效的解决方案。
...
### [5] [ClickAIXR: On-Device Multimodal Vision-Language Interaction with Real-World Objects in Extended Reality](https://arxiv.org/abs/2604.04905)
*Dawar Khan,Alexandre Kouyoumdjian,Xinyu Liu,Omar Mena,Dominik Engel,Ivan Viola*

Main category: cs.CV

TL;DR: ClickAIXR：一种在XR中通过控制器点击选择真实世界物体，并利用本地视觉语言模型进行多模态交互的端侧框架


<details>
  <summary>Details</summary>
Motivation: 解决现有XR系统中基于云AI（如ChatGPT）或基于注视选择（如GazePointAR）的局限性，减少仅使用注视或语音界面的模糊性，并通过端侧推理解决隐私和延迟问题

Method: 集成端侧视觉语言模型与控制器对象选择范式，用户精确点击XR中的真实物体后，物体图像由本地VLM处理，通过文本和语音回答自然语言问题；在Magic Leap SDK（C API）中实现，使用ONNX进行本地VLM推理

Result: 与Gemini 2.5 Flash和ChatGPT 5相比的用户研究表明，延迟适中，用户体验可接受；展示了基于点击的对象选择与端侧AI结合在推进可信、隐私保护的XR交互方面的潜力

Conclusion: ClickAIXR证明了控制器点击选择与端侧AI相结合的方法能够实现更精确、透明、隐私保护的XR交互，为可信XR系统的发展提供了新方向
...
TL;DR: MoViD是一个视角不变的3D人体姿态估计框架，通过解耦视角信息和运动特征，在减少训练数据需求的同时提高泛化能力和推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体姿态估计方法在真实世界部署中存在三个主要问题：1) 对未见相机视角泛化能力差；2) 需要大量训练数据；3) 推理延迟高。这些限制了在医疗监测、人机协作和沉浸式游戏等应用中的实际部署。

Method: MoViD框架包含三个核心组件：1) 视角估计器，通过建模关键关节关系预测视角信息；2) 正交投影模块，解耦运动和视角特征；3) 基于物理的跨视角对比对齐增强。为边缘设备实时部署，采用逐帧推理管道和基于视角估计的自适应翻转细化策略。

Result: 在9个公共数据集和新收集的多视角无人机及步态分析数据集上评估，MoViD相比最先进方法减少姿态估计误差超过24.2%，在严重遮挡下保持鲁棒性能且训练数据减少60%，在NVIDIA边缘设备上实现15FPS的实时推理。

Conclusion: MoViD通过解耦视角和运动特征，实现了视角不变的3D人体姿态估计，显著提高了泛化能力、数据效率和推理速度，为医疗监测、人机协作等实际应用提供了可行的解决方案。
...
TL;DR: 研究验证了在遥感场景中仅上传紧凑嵌入向量（而非原始像素）的可行性，发现嵌入向量是关键使能技术，但最优决策头（kNN检索 vs. 类别质心）因任务而异。


<details>
  <summary>Details</summary>
Motivation: 解决下行链路瓶颈问题，研究在严格约束下（地面站仅上传紧凑嵌入向量和元数据）的星上系统，探索在遥感场景变化（跨时间、跨事件/地点、跨站点云、跨城市AOI）下，仅依赖嵌入向量的处理流程是否仍然有效。

Method: 使用OlmoEarth嵌入向量，在扩展的公共多任务基准上进行实验（27个Sentinel-2 L2A场景，15个云站点，5个SpaceNet-2 AOI；10次随机种子）。地面站仅上传紧凑嵌入向量和元数据，星上系统通过向量搜索对新捕获数据进行分类。比较两种决策头：kNN检索和类别质心方法。

Result: 所有有效方法都依赖相同的上传嵌入向量，但最优决策头具有任务依赖性：kNN检索在云分类中显著更优（0.92 vs. 质心0.91；p<0.01），而类别质心在时序变化检测中占主导地位（0.85 vs. 检索0.48；p<0.01）。嵌入向量上传是关键使能技术，一旦嵌入向量在星上，系统可以零额外上行成本为每个任务选择最佳决策头，每次查询遥测数据小于1KB。

Conclusion: 仅嵌入向量上行链路是可行的关键使能技术，系统设计应允许根据任务需求灵活选择决策头（kNN检索或类别质心），这种架构在严格通信约束下仍能保持高性能。
...
TL;DR: 该论文针对多模态大语言模型在物理世界理解方面的不足，特别是对连续体对象动态理解的能力缺陷，提出了两个基准测试任务和一种名为场景动态场的改进方法，显著提升了模型在物理推理方面的表现。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在图像和视频理解方面表现出色，但在物理世界理解方面存在明显不足，特别是在高级物理推理方面。当前模型在理解连续体对象的动态特性方面存在显著局限性，这成为物理推理能力发展的关键瓶颈。

Method: 研究首先引入两个基础基准任务：下一帧选择和时序一致性验证，用于评估模型对连续体动态的理解能力。针对发现的局限性，提出了场景动态场方法，该方法利用物理模拟器在多任务微调框架中增强模型的物理理解能力。

Result: 实验表明，即使是当前最先进的多模态大语言模型在基础物理理解任务上表现也很差。提出的场景动态场方法显著提升了性能，在流体任务上实现了高达20.7%的性能提升，并在未见过的物理领域表现出良好的泛化能力。

Conclusion: 该研究不仅揭示了当前多模态大语言模型在物理理解方面的关键缺陷，还提出了一种成本效益高的方法，为开发更具物理基础的多模态大语言模型提供了有前景的途径。代码和数据已开源。
...
TL;DR: HVG-3D是一个基于扩散模型的3D感知手物交互视频合成框架，通过3D ControlNet编码几何和运动线索，实现精确的空间和时间控制。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖2D控制信号，缺乏空间表达能力，且限制了合成3D条件数据的利用。需要开发能够利用显式3D表示进行手物交互视频合成的统一框架。

Method: 提出HVG-3D框架，包含两个核心组件：1) 3D感知HOI视频生成扩散架构，通过3D ControlNet编码几何和运动线索进行显式3D推理；2) 构建输入和条件信号的混合流程，在训练和推理中实现灵活精确的控制。

Result: 在TASTE-Rob数据集上的实验表明，HVG-3D在空间保真度、时间一致性和可控性方面达到最先进水平，同时能够有效利用真实和模拟数据。

Conclusion: HVG-3D通过显式3D表示和扩散模型架构，实现了高质量、时空一致的手物交互视频合成，为3D感知视频生成提供了有效解决方案。
...
TL;DR: IDCL是一种鲁棒的图像聚类方法，首次将基于密度信息的模型训练策略引入图像聚类，通过课程学习方案和密度核心引导聚类分配，提升了聚类性能、鲁棒性和收敛速度。


<details>
  <summary>Details</summary>
Motivation: 现有深度聚类方法很少考虑模型学习策略对复杂图像数据聚类鲁棒性和性能的改进作用，且大多仅依赖点到聚类中心的距离进行潜在表示划分，导致迭代过程中误差累积。
...
TL;DR: V-Reflection：通过"思考-观察"视觉反射机制将MLLM转变为主动询问者，解决细粒度任务中的感知幻觉问题


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在细粒度任务中容易产生感知相关的幻觉，主要原因是其推理过程局限于语言域，将视觉输入视为静态、与推理无关的前置内容而非动态参与者，导致模型无法重新审视视觉细节来支撑其演进的推理状态
...
Conclusion: V-Reflection通过"思考-观察"视觉反射机制将MLLM转变为主动询问者，使潜在状态作为动态探针主动询问视觉特征空间，为每个推理步骤提供任务关键证据，解决了细粒度任务中的感知幻觉问题，同时保持推理时端到端自回归解码的高效性
...
TL;DR: TreeGaussian：一种树引导的级联对比学习框架，通过构建多级对象树和两阶段对比学习策略，解决3DGS在层次语义表示和分割一致性方面的问题。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法难以表示复杂场景中的层次3D语义结构和捕获整体-部分关系，同时密集的成对比较和不一致的2D先验层次标签阻碍了特征学习，导致分割效果不佳。

Method: 1）构建多级对象树以结构化学习对象-部分层次关系；2）提出两阶段级联对比学习策略，从全局到局部逐步细化特征表示；3）引入一致性分割检测机制和基于图的去噪模块，对齐跨视图分割模式并抑制不稳定高斯点。

Result: 通过开放词汇3D对象选择、3D点云理解和消融实验等广泛实验，证明了该方法在分割一致性和质量方面的有效性和鲁棒性。

Conclusion: TreeGaussian通过显式建模层次语义关系和减少对比监督冗余，显著提升了3DGS在复杂场景中的语义理解能力，为神经场景理解提供了更有效的表示方法。
...
<details>
  <summary>Details</summary>
Motivation: 长距离人体运动生成是计算机视觉和图形学的重要挑战，特别是在不同语义运动域之间生成连贯过渡的能力尚未充分探索，这对于舞蹈编排等应用至关重要

Method: 提出推理时优化框架，受扩散基随机最优控制启发，通过控制能量目标函数显式正则化预训练扩散模型的过渡轨迹

Result: 推理时优化该目标函数能够生成具有保真度和时间连贯性的运动过渡，是首个提供显式过渡建模的受控长距离人体运动生成通用框架

Conclusion: 该方法为长距离人体运动生成中的语义连贯过渡提供了有效解决方案，特别适用于舞蹈编排等需要流畅风格转换的应用场景
...
TL;DR: CardioSAM：结合SAM编码器与心脏专用解码器的混合架构，通过心脏特异性注意力和边界细化模块提升CMR图像分割精度，在ACDC基准测试中达到93.39% Dice系数，超越nnU-Net和专家间一致性水平。


<details>
  <summary>Details</summary>
Motivation: 心脏磁共振图像的手动分割耗时且存在显著的观察者间变异性，而现有基础模型（如SAM）虽然泛化能力强，但缺乏临床应用所需的边界精度，需要开发更精确的自动分割方法。

Method: 提出CardioSAM混合架构：冻结SAM编码器提供通用特征提取能力，结合轻量级可训练的心脏专用解码器。解码器包含两个关键创新：1）心脏特异性注意力模块，融入解剖拓扑先验知识；2）边界细化模块，专门改善组织界面描绘。

Result: 在ACDC基准测试中，CardioSAM达到Dice系数93.39%、IoU 87.61%、像素精度99.20%、HD95 4.2mm。相比nnU-Net提升3.89% Dice系数，超过报告的专家间一致性水平（91.2%）。

Conclusion: CardioSAM通过结合通用特征提取与心脏特异性优化，实现了临床适用的高精度心脏结构分割，其性能超越现有基准方法和专家间一致性，展示了可靠的临床应用潜力。
...
TL;DR: CoLA是一种新颖的参数高效微调框架，通过引入跨模态低秩适配路径扩展LoRA，在保持参数效率的同时显著提升多模态任务性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于单模态编码器（如DINO和BERT）的双流架构在多模态任务适配中存在挑战。现有的PEFT方法（如LoRA）在各模态内独立操作，无法有效捕捉跨模态交互，限制了多模态任务性能。

Method: 提出跨模态低秩适配（CoLA）框架，在标准模态内适配路径基础上，引入专门的跨模态适配路径。这种双路径设计使CoLA能够在不干扰模态特定学习的情况下，有效适配单模态基础模型到多模态任务。

Result: 在视觉语言（RefCOCO、RefCOCO+、RefCOCOg）和视听（AVE、AVS）基准测试中，CoLA始终优于LoRA，分别获得约3%和2%的相对增益，同时保持参数效率。CoLA首次实现了视觉定位的多任务PEFT框架。

Conclusion: CoLA通过引入跨模态适配路径，有效解决了现有PEFT方法在多模态任务中的局限性，为高效多模态适配提供了创新解决方案，特别是在视觉定位任务中实现了首个多任务PEFT框架。
...
### [16] [When Sinks Help or Hurt: Unified Framework for Attention Sink in Large Vision-Language Models](https://arxiv.org/abs/2604.03316)
*Jiho Choi,Jaemin Kim,Sanghwan Kim,Seunghoon Hong,Jin-Hwi Park*

Main category: cs.CV

TL;DR: 该论文研究了大型视觉语言模型中的注意力汇现象，将其分为视觉编码器产生的V-sink和LLM深层产生的L-sink，揭示了全局先验与局部感知间的性能权衡，并提出了无需任务监督的层间汇门控模块来动态调节注意力贡献。


<details>
  <summary>Details</summary>
Motivation: 虽然单模态Transformer中的注意力汇已被研究，但它们在大型视觉语言模型中的跨模态影响仍不清楚：这些汇是冗余的人工产物还是必要的全局先验？本文旨在探究视觉汇在LVLM中的作用机制及其对性能的影响。

Method: 首先将视觉汇分类为V-sink（从视觉编码器传播）和L-sink（在深层LLM层中出现）。基于此定义分析性能权衡，识别关键功能层，并提出层间汇门控（LSG）——一个轻量级即插即用模块，通过标准下一个词预测训练，动态缩放V-sink与其他视觉token的注意力贡献，同时保持LVLM主干冻结。

Result: 分析揭示了基本性能权衡：汇能有效编码全局场景级先验，但其主导地位会抑制局部感知所需的细粒度视觉证据。识别了调节这些汇最能影响下游性能的特定功能层。LSG在大多数层中都能改善代表性多模态基准测试性能，有效平衡全局推理和精确局部证据。

Conclusion: 注意力汇在LVLM中既是全局先验的有效编码器，也可能抑制局部感知。通过LSG模块动态调节汇的注意力贡献，可以在无需任务特定监督的情况下平衡全局推理和局部证据，提升多模态任务性能。
...
### [17] [Gaze to Insight: A Scalable AI Approach for Detecting Gaze Behaviours in Face-to-Face Collaborative Learning](https://arxiv.org/abs/2604.03317)
*Junyuan Liang,Qi Zhou,Sahan Bulathwela,Mutlu Cukurova*

Main category: cs.CV

TL;DR: 提出基于预训练和基础模型的可扩展AI方法，自动检测面对面协作学习中的注视行为，无需人工标注数据
...
Result: F1分数达到0.829，对笔记本电脑和同伴注视检测性能强，其他目标较弱；相比监督方法在复杂场景中表现更优且稳定
...
### [18] [EgoMind: Activating Spatial Cognition through Linguistic Reasoning in MLLMs](https://arxiv.org/abs/2604.03318)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖3D先验或几何监督导致数据准备成本高昂，要么纯2D方法在多帧空间推理中表现不佳。需要一种既能有效进行空间推理又避免几何依赖的方法。

Method: 提出EgoMind框架：1) Role-Play Caption：跨帧构建连贯的语言场景图；2) Progressive Spatial Analysis：渐进式推理解决任务特定问题。仅需5K自动生成的SFT样本和20K RL样本。

Result: 在VSI-Bench、SPAR-Bench、SITE-Bench和SPBench等多个基准测试中取得竞争性结果，证明了其在增强MLLMs空间推理能力方面的有效性。

Conclusion: EgoMind展示了语言推理在空间认知中的潜力，提供了一种无需几何先验的高效空间推理方法，仅需少量数据即可显著提升多模态大语言模型的空间推理能力。
...
### [19] [VitaTouch: Property-Aware Vision-Tactile-Language Model for Robotic Quality Inspection in Manufacturing](https://arxiv.org/abs/2604.03322)
*Junyi Zong,Qingxuan Jia,Meixian Shi,Tong Li,Jiayuan Li,Zihang Lv,Gang Chen,Fang Deng*

Main category: cs.CV

TL;DR: VitaTouch是一个面向智能制造质量检测的多模态模型，结合视觉、触觉和语言来推断材料属性并进行自然语言描述，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 智能制造中的质量检测需要识别超出可见几何形状的内在材料和表面属性，但纯视觉方法容易受到遮挡和反射的影响，因此需要结合多模态信息来提升检测能力。

Method: 提出VitaTouch模型，使用模态特定编码器和双Q-Former提取与语言相关的视觉和触觉特征，通过对比学习对齐模态并耦合视觉与触觉信息，构建了包含186个物体、52k图像和5.1k人工验证指令-答案对的VitaSet数据集。

Result: 在HCT和TVL基准测试中取得最佳性能，在SSVTP上保持竞争力；在VitaSet上达到88.89%硬度准确率、75.13%粗糙度准确率和54.81%描述符召回率；材料描述任务语义相似度峰值达0.9009；通过LoRA微调后，缺陷识别准确率分别为100.0%、96.0%和92.0%，闭环识别准确率94.0%，端到端分拣成功率94.0%。

Conclusion: VitaTouch通过有效整合视觉、触觉和语言信息，显著提升了智能制造中材料属性推断和缺陷识别的能力，为多模态质量检测提供了有效解决方案。
...
TL;DR: 比较九种代表性表面重建方法在叶片表面重建中的性能，分析不同方法在精度、平滑度、鲁棒性和计算成本方面的权衡，为资源受限的农业机器人平台提供选择指导。


<details>
  <summary>Details</summary>
Motivation: 叶片表面的精确三维重建对农业表型分析至关重要，但真实植物点云数据复杂，现有表面重建方法（参数化、三角化、隐式、学习型）在叶片重建中的相对性能尚不明确，需要系统比较为实际应用提供指导。

Method: 对九种代表性表面重建方法进行对比研究，在三个公开数据集（LAST-STRAW、Pheno4D、Crops3D）上进行评估，涵盖不同物种、传感器和感知环境，从清洁高分辨率室内扫描到噪声低分辨率田间设置。

Result: 分析揭示了不同方法在表面面积估计精度、平滑度、对噪声和缺失数据的鲁棒性以及计算成本之间的权衡关系，这些因素影响农业应用中机器人硬件的成本和约束条件。结果显示每种方法根据应用场景和资源约束具有不同的优势。

Conclusion: 研究结果为资源受限的机器人平台选择表面重建技术提供了实用指导，表明需要根据具体应用需求（精度要求、计算资源、数据质量等）权衡选择合适的方法。
...
TL;DR: RGxEStat是一个轻量级交互式工具，用于基因型×环境互作分析，包括显著性分析和稳定性分析，旨在简化育种数据分析流程
...
Method: 提出两种GxE互作分析模型：基于混合效应模型的显著性分析，用于确定基因或GxE互作是否显著影响表型性状；稳定性分析，进一步研究基因与环境间的交互关系以及基因型在不同环境中的相对优劣。开发RGxEStat工具，集成模型构建、求解和可视化功能

Result: 开发了RGxEStat工具，提供用户友好界面，无需学习复杂SAS或R编程即可进行育种数据分析，显著加速研究周期。代码和数据集已在GitHub上公开

Conclusion: RGxEStat工具简化了GxE互作分析流程，使育种家和农学家能够更高效地进行遗传选择和育种实践，通过消除编程障碍加速研究进程
...
TL;DR: AC-LAM通过强制潜在动作空间的加性组合结构，学习更具结构化的、运动特定的、位移校准的潜在动作，在模拟和真实世界桌面任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有潜在动作学习方法缺乏对物理运动加性、组合结构的先验编码，导致潜在动作常与无关场景细节或未来观测信息纠缠，且运动幅度校准不准确。

Method: 引入加性组合潜在动作模型(AC-LAM)，在潜在动作空间上强制短时间范围内的场景级加性组合结构，通过代数结构约束(恒等、逆、循环一致性)抑制非加性组合信息。

Result: AC-LAM学习到更具结构化、运动特定且位移校准的潜在动作，为下游策略学习提供更强监督，在模拟和真实世界桌面任务中优于最先进的LAM方法。
...
Method: 采用系统性综述方法，对混合专家模型在遥感领域的应用进行全面梳理，包括基本原理、架构设计和关键应用分析。

Result: 提供了混合专家模型在遥感领域的首个系统性综述，涵盖了各种遥感任务中的应用，并识别了该领域的研究现状和发展方向。

Conclusion: 混合专家模型是解决遥感数据多样性和动态性挑战的有效范式，该综述为未来研究和创新提供了基础框架和方向指引。
...
TL;DR: 量化向量是权重空间中可转移的量化鲁棒性方向，可从供体任务提取并应用于接收模型，无需接收方量化感知训练即可显著提升后训练量化鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 后训练量化（PTQ）是模型部署的关键技术，但通常需要量化感知训练（QAT）来维持精度。QAT成本高昂且需要训练数据。本文旨在探索是否存在一种无需接收方训练数据和QAT的零成本方法，通过权重空间几何特性转移量化鲁棒性。

Method: 提出"量化向量"概念，通过简单的权重空间算术从供体任务中提取量化鲁棒性方向。该向量可直接应用于接收模型，无需接收方量化感知训练或训练数据。在Vision Transformer模型上验证该方法。

Result: 量化向量可将后训练量化噪声的鲁棒性提升高达60%，且无需接收方量化感知训练。该方法为零样本、低成本的低比特部署提供了QAT的替代方案。

Conclusion: 量化鲁棒性不仅是任务特定训练的副产品，更是权重空间几何中可重用的特征，可通过转移而非重新训练获得。这为低比特模型部署开辟了新的研究方向。
...
TL;DR: 该研究提出了一种基于基础模型的自动化监控工作流，用于群体饲养的保育猪监测，通过结合预训练视觉-语言基础模型和轻量级任务特定逻辑，实现了可扩展、标签高效的长时监控。


<details>
  <summary>Details</summary>
Motivation: 在精准畜牧业中，现有监控系统主要依赖需要大量标注数据的监督学习模型，存在需要重复训练和农场特定调优的问题。基础模型通过大规模学习获得的通用视觉表示，有望减少对任务特定监督学习的依赖。

Method: 采用基础模型为中心的工作流：1) 使用Grounding-DINO进行目标检测建立基线；2) 针对夜间视觉和严重遮挡问题，集成时间跟踪逻辑；3) 基于Grounded-SAM2进行短期视频分割；4) 开发长期跟踪管道，包括初始化、跟踪、匹配、掩码细化、重识别和后处理质量控制。

Result: 在1,418张标注图像上建立检测基线；在550个一分钟视频片段中，经过后处理的4,927个活动轨迹中超过80%完全正确；在连续132分钟视频上，系统保持稳定身份，在132个均匀采样的真值帧上获得：平均区域相似度(J)0.83、轮廓精度(F)0.92、J&F 0.87、MOTA 0.99、MOTP 90.7%，无身份切换。

Conclusion: 该研究表明，基础模型的先验知识可以与轻量级、任务特定的逻辑相结合，在生猪生产中实现可扩展、标签高效和长时监控，为精准畜牧业提供了新的技术路径。
...
TL;DR: 在海洋物种分类中，使用自监督视觉基础模型的冻结嵌入作为标签高效基线，通过推理时电路复制方法（不微调模型权重）提升性能，接近全监督基准。


<details>
  <summary>Details</summary>
Motivation: 水下物种自动分类面临标注成本高和环境变化导致模型泛化能力受限的问题。现有研究表明自监督视觉基础模型的冻结嵌入已为海洋图像分类提供了强大的标签高效基线，但需要探索是否能在推理时（不微调模型权重）进一步提升性能。

Method: 采用电路复制方法（Circuit Duplication），这是一种最初为大型语言模型提出的推理时方法，在推理过程中将选定的Transformer层遍历两次。在AQUA20基准测试中使用冻结的DINOv3嵌入进行评估，设置两种电路选择策略：全局电路选择（整个数据集使用单一复制电路）和类别特定电路选择（每个物种可能获得不同的最优电路）。两种设置均使用简单的半监督下游分类器。

Result: 电路复制方法相比标准冻结前向传播持续提升性能。在最大标签预算下，类别特定选择达到0.875的宏观F1分数，将与全监督ConvNeXt基准（0.889）的差距缩小到1.4个百分点，且无需任何基于梯度的训练。四个物种的表现超过了全监督参考，其中章鱼提升了+12.1个F1点。在所有预算下，约75%的类别偏好类别特定电路，表明存在真正的类别依赖性优势。

Conclusion: 电路复制方法能够有效提升冻结嵌入在海洋物种分类中的性能，接近全监督基准，且无需模型微调。这是电路复制方法首次应用于计算机视觉领域，展示了推理时方法在标签受限场景下的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI图像编辑模型在风格化表情编辑时会产生全局噪声和像素漂移，无法集成到专业图像编辑软件和工作流程中，需要开发无伪影的快速编辑工具。

Method: 开发完全开源的Photoshop插件ExpressEdit，避免专有模型的常见伪影，与Photoshop原生操作（如液化）协同工作；编译包含135个表情标签的全面表情数据库，附带示例故事和图像，支持检索增强生成。

Result: ExpressEdit在单个消费级GPU上3秒内完成表情编辑，显著快于流行的专有模型；无常见伪影，能与Photoshop原生操作稳健协同；开源代码和数据集促进未来研究和艺术探索。

Conclusion: ExpressEdit填补了AI表情编辑工具与专业图像编辑工作流之间的空白，提供了快速、无伪影、可集成的解决方案，并通过开源数据库支持多样化的叙事需求。
...
TL;DR: 提出一个结合高分辨率卫星影像和街景图像的多模态框架，用于在有限标注条件下实现可扩展的城市树木检测，通过领域适应和混合学习策略显著提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统城市树木测绘依赖劳动密集型实地调查，难以扩展到大规模应用。现有自动化系统面临高标注成本和跨城市场景泛化能力差的问题，需要开发标注效率高且可扩展的解决方案。

Method: 提出多模态框架：首先利用卫星影像定位树木候选区域，然后检索对应的街景图像进行精细检测，减少无效采样。采用领域适应将已有标注数据集知识迁移到新区域。评估三种学习策略：半监督学习、主动学习和两者结合的混合策略，使用基于transformer的检测模型。

Result: 混合策略取得最佳性能，F1分数达到0.90，比基线模型提升12%。半监督学习因伪标签确认偏差导致性能逐渐下降，主动学习通过针对性人工标注不确定或错误预测稳步提升性能。错误分析显示主动和混合策略减少了假阳性和假阴性。

Conclusion: 多模态方法和引导式标注对于实现可扩展、标注高效的城市树木测绘至关重要，能够支持可持续城市规划。混合学习策略在平衡标注成本和检测性能方面表现最优。
...
TL;DR: 提出用户显著对象检测任务，强调基于用户主动需求而非被动视觉刺激来检测显著对象


<details>
  <summary>Details</summary>
Motivation: 现有显著对象检测方法基于被动视觉刺激，忽略了用户主动需求的决定性作用，这既无法满足用户需求，也限制了下游任务的发展

Method: 提出UserSOD任务，但主要挑战是缺乏用于模型训练和测试的数据集

Result: 论文主要提出了新的任务定义和挑战分析，尚未涉及具体实验结果

Conclusion: 需要基于用户需求检测显著对象，提出了UserSOD任务，并指出数据集缺乏是主要挑战
...
TL;DR: 该论文提出了一种将分数扩散模型作为先验直接集成到即插即用框架中的新方法，通过噪声注入改进严重病态逆问题的求解。


<details>
  <summary>Details</summary>
Motivation: 尽管即插即用方法和分数扩散模型都依赖于去噪器，但如何在不依赖反向扩散采样的前提下，系统地将分数扩散模型作为先验集成到即插即用框架中仍不明确。现有方法缺乏理论依据和系统性集成方案。

Method: 建立了即插即用的分数解释，证明了预训练分数扩散模型可直接用于即插即用算法。在此基础上提出了随机生成即插即用框架，通过注入噪声更好地利用分数扩散模型的表达能力，并证明了该噪声注入诱导了高斯平滑目标的优化，有助于逃离严格鞍点。

Result: 在多线圈MRI重建和大掩码自然图像修复等挑战性逆问题任务中，该方法相比传统即插即用方法取得了持续改进，性能与基于扩散的求解器相当。

Conclusion: 该工作为分数扩散模型在即插即用框架中的系统集成提供了理论基础和实用框架，通过噪声注入机制显著提升了严重病态逆问题的求解鲁棒性，弥合了传统即插即用方法与先进生成模型之间的差距。
...
TL;DR: PortraitCraft是一个统一的肖像构图理解与生成基准，包含5万张带结构化多级标注的肖像图像，支持构图评分、属性推理、视觉问答和构图约束生成等任务。


<details>
  <summary>Details</summary>
Motivation: 现有数据集和基准主要关注粗粒度美学评分、通用图像美学或无约束肖像生成，缺乏对结构化肖像构图分析和可控肖像生成的系统性研究，限制了在明确构图要求下的研究进展。

Method: 构建包含约5万张精选真实肖像图像的数据集，提供结构化多级监督：全局构图评分、13个构图属性标注、属性级解释文本、视觉问答对以及面向构图的文本描述。基于此数据集建立构图理解和构图感知生成两个互补的基准任务。

Result: 建立了PortraitCraft基准，包含标准化评估协议和代表性多模态模型的参考基线结果。该基准支持构图理解（评分预测、细粒度属性推理、图像基础视觉问答）和构图约束生成（从结构化构图描述生成肖像）的全面评估。

Conclusion: PortraitCraft为细粒度肖像理解、可解释美学评估和可控肖像生成的未来研究提供了全面基准，填补了结构化肖像构图分析领域的空白。
...
TL;DR: TABLeT提出了一种基于预训练2D自然图像自编码器的fMRI信号token化方法，通过将3D fMRI体积压缩为连续token，实现了在有限显存下对长序列时空动态的有效建模。


<details>
  <summary>Details</summary>
Motivation: 传统基于体素的fMRI模型虽然性能优异且可解释性强，但由于内存需求过高，只能捕捉有限的时间窗口，难以建模长程时空动态。需要一种既能保持性能又能提高计算效率的方法。

Method: 提出TABLeT方法：1) 使用预训练的2D自然图像自编码器将3D fMRI体积压缩为紧凑的连续token；2) 采用简单的Transformer编码器处理这些token序列；3) 开发自监督掩码token建模方法进行预训练。

Result: 在UK-Biobank、HCP和ADHD-200等大规模基准测试中，TABLeT在多个任务上优于现有模型，同时在相同输入条件下相比最先进的体素方法显著提升了计算和内存效率。自监督预训练进一步提升了各种下游任务的性能。

Conclusion: TABLeT为大脑活动的可扩展和可解释的时空建模提供了一种有前景的方法，通过token化策略有效解决了传统方法的内存瓶颈问题，实现了长序列建模。
...
TL;DR: MuPD是一个多模态病理学扩散基础模型，通过扩散变换器和解耦跨模态注意力将H&E组织学、RNA分子谱和临床文本嵌入共享潜在空间，支持多种跨模态合成任务，性能显著优于领域特定模型。
...
Method: 提出MuPD（多模态病理学扩散）生成基础模型，使用扩散变换器和解耦跨模态注意力将H&E染色组织学、RNA分子谱和临床文本嵌入共享潜在空间。在3400万组织学图像块、160万文本-组织学对和1080万RNA-组织学对上预训练，涵盖34个人体器官。
...
TL;DR: ComPrivDet是一种在压缩视频中高效检测隐私对象的方法，通过重用I帧推理结果，利用压缩域线索跳过或轻量级处理P/B帧，在保持高检测精度的同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 随着物联网视频分析规模扩大，逐帧隐私保护带来高延迟问题。现有方法需要完全解码视频或逐帧处理压缩视频，导致解码开销大或精度降低，需要更高效的隐私对象检测方案。

Method: 提出ComPrivDet方法：1) 重用I帧推理结果；2) 利用压缩域线索识别新对象出现；3) 对P帧和B帧选择性跳过检测或使用轻量级检测器进行细化处理。

Result: 在隐私人脸检测中保持99.75%准确率，隐私车牌检测中保持96.83%准确率，同时跳过超过80%的推理过程。相比现有压缩域检测方法，平均准确率提高9.84%，延迟降低75.95%。

Conclusion: ComPrivDet通过压缩域高效检测隐私对象，在保持高精度的同时显著降低计算开销，为大规模视频分析中的隐私保护提供了实用解决方案。
...
TL;DR: CSRS提出了一种用于多模态大语言模型无监督自演化的连续软化回溯重采样方法，通过回溯重推理机制、软化频率奖励和视觉语义扰动来提升推理性能，在几何任务上实现了SOTA结果。
...
Result: CSRS显著提升了Qwen2.5-VL-7B在MathVision等基准测试中的推理性能，在几何任务的无监督自演化中实现了最先进(SOTA)结果。

Conclusion: CSRS通过回溯重推理、软化频率奖励和视觉语义扰动的组合，有效解决了无监督自演化中的偏见问题，提升了多模态大语言模型的推理能力和稳定性。
...
TL;DR: ART提出自适应关系Transformer，通过时间感知关系图捕捉交互演化，自适应交互剪枝减少冗余计算，在行人轨迹预测任务中实现高精度和高计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于图或Transformer的方法存在计算开销大或难以捕捉人类交互多样性和时变特性的问题，需要更高效准确的轨迹预测方法。
...
Conclusion: ART通过显式建模交互演化和自适应剪枝，在行人轨迹预测任务中平衡了精度和效率，为机器人应用提供了有效的解决方案。
...
TL;DR: DreamPRVR提出了一种从粗到细的表示学习范式，通过生成全局语义寄存器来增强视频检索中的上下文感知，解决部分相关视频检索中的查询模糊性和局部噪声问题。
...
Method: 采用从粗到细的表示学习范式：1）生成全局上下文语义寄存器作为粗粒度高亮；2）通过概率变分采样器初始化寄存器，文本监督截断扩散模型迭代优化；3）文本语义结构学习构建良好文本潜在空间；4）寄存器增强的高斯注意力块自适应融合寄存器与视频标记。

Result: 大量实验表明DreamPRVR在性能上超越了现有最先进方法，验证了所提方法的有效性。

Conclusion: DreamPRVR通过从粗到细的表示学习范式有效解决了部分相关视频检索中的全局上下文感知不足问题，为跨模态匹配提供了新思路。
...
Method: 提出LaPR框架：1）设计图像-标签联合表示，显式整合标签信息；2）引入混合专家机制，每个专家捕捉特定标签模式，路由器推断查询自适应的混合权重；3）采用交替优化策略，结合VICL性能引导的对比损失和标签引导的对比损失。

Result: 在上下文分割、检测和着色任务上取得显著且一致的性能提升。LaPR在不同特征提取器和跨折叠场景中表现出良好的泛化能力，验证了标签利用在VICL提示检索中的重要性。

Conclusion: 标签信息对视觉上下文学习的提示检索至关重要。LaPR框架通过标签感知的表示学习和混合专家机制，有效提升了VICL性能，为多任务视觉基础模型的提示选择提供了新思路。
...
TL;DR: 提出一种基于视觉大语言模型的方法，通过Set-of-Mark提示和注视轨迹理解用户意图，结合逆指数采样策略，用于第一人称视角下的人-物交互预测任务


<details>
  <summary>Details</summary>
Motivation: 在智能辅助系统中预测人-物交互对于指导用户日常活动和理解其短期/长期目标至关重要。现有方法在视觉基础和用户意图理解方面存在局限，需要改进

Method: 使用视觉大语言模型，通过Set-of-Mark提示增强视觉基础能力，利用用户最近注视点形成的轨迹理解意图，并引入逆指数采样策略捕捉交互前的时序动态

Result: 在HD-EPIC第一人称数据集上的实验表明，该方法超越了现有最先进方法，展示了其模型无关的特性

Conclusion: 该方法通过改进视觉基础和意图理解，结合有效的时序采样策略，显著提升了第一人称视角下人-物交互预测的性能
...
TL;DR: DSERT-RoLL是一个多模态驾驶数据集，集成了立体事件相机、RGB相机、热成像相机、4D雷达和双激光雷达，包含多样天气和光照条件下的数据，提供精确的2D/3D边界框和轨迹ID，支持传感器组合的公平比较。
...
Method: 构建包含多种传感器（立体事件相机、RGB、热成像、4D雷达、双激光雷达）的驾驶数据集，提供精确的2D/3D标注和车辆里程计；建立统一的3D和2D基准测试框架；提出融合框架将传感器特定线索集成到统一特征空间。

Result: 创建了DSERT-RoLL数据集，包含多传感器数据、精确标注和基准测试；提供了代表性单模态和多模态方法的基线结果；提出的融合框架提高了不同天气和光照条件下的3D检测鲁棒性。

Conclusion: DSERT-RoLL数据集为新型传感器研究和多模态融合提供了重要资源，统一的基准测试框架支持传感器特性的直接比较，提出的融合方法增强了自动驾驶系统在复杂环境下的鲁棒性。
...
TL;DR: 该论文研究了科学图像领域的长尾识别问题，发现在预训练和微调数据分布差异大的情况下，微调基础模型效果有限，提出SciLT框架通过自适应特征融合和双监督学习来提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有长尾识别研究和基准主要局限于自然图像领域，而科学图像具有独特的视觉特征和监督信号，预训练和微调数据分布差异大，需要研究基础模型在科学图像长尾识别中的有效性。

Method: 提出SciLT框架，采用纯视觉和参数高效微调范式，通过自适应特征融合联合利用倒数第二层和最终层特征，并结合双监督学习来平衡头尾类别的性能。

Result: 在三个科学基准测试上的实验表明，SciLT框架在头尾类别上都取得了平衡且优异的性能，显著优于现有方法，为科学长尾识别建立了强大的实用基准。

Conclusion: SciLT框架通过有效利用多级表示，为适应基础模型到具有显著领域偏移的科学数据提供了有价值的指导，解决了科学图像长尾识别中的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的图像水印通常采用"编码器-噪声层-解码器"(END)架构来提高对随机信道失真的鲁棒性，但忽视了攻击者拥有额外知识时可能进行的针对性操纵。论文发现了一个关键但未被充分探索的漏洞：已知原始攻击(KOA)，攻击者可以访问多个原始-水印图像对，从而实施各种针对性抑制策略。

Method: 论文首先揭示了KOA漏洞，展示即使简单的基于残差的去除方法也能几乎完全去除水印。然后提出了ResGuard，这是一个即插即用模块，通过强制图像依赖性嵌入来增强KOA鲁棒性。其核心是残差特异性增强损失，鼓励残差与宿主图像紧密耦合。此外，辅助的KOA噪声层在训练期间注入残差式扰动，使解码器在更强的嵌入不一致性下保持可靠。

Result: ResGuard集成到现有框架后，显著提升了KOA鲁棒性，将平均水印提取准确率从59.87%提高到99.81%。

Conclusion: 论文揭示了深度学习图像水印中存在的KOA安全漏洞，并提出了有效的解决方案ResGuard，通过增强残差的图像依赖性来抵御已知原始攻击，为更安全的图像水印系统提供了重要改进。
...
Result: 在SceneFun3D、FunGraph3D和FunThor数据集上的实验表明，FunFact提高了节点和关系发现召回率，并显著减少了模糊关系的校准误差

Conclusion: 整体概率建模对功能场景理解具有显著优势，FunFact框架能够构建更准确、校准更好的功能3D场景图
...
<details>
  <summary>Details</summary>
Motivation: 现有交通视频理解方法缺乏结构化表示和可解释的推理过程，需要一种能够结合场景图结构和多模态信息的框架来提高理解能力和透明度。

Method: SGTA从路侧视频构建交通场景图（包括检测、跟踪和车道提取），然后使用基于工具的方法对符号图查询和视觉输入进行推理，采用ReAct方法处理大语言模型的交错推理轨迹和工具调用。
...
TL;DR: VidNum-1.4K是一个包含1,379个人工标注视频-问题对的视频问答基准，专门评估真实世界视频中的多步数值推理能力，揭示当前视觉语言模型在数值推理方面存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有视频数值推理基准通常局限于狭窄领域（如重复性运动），或将简单计数视为表面回归任务，无法评估真实世界多媒体内容中固有的多步数值逻辑。需要一个新的基准来测试视觉语言模型是否真正"理解"现实世界动态。

Method: 构建VidNum-1.4K基准，包含1,379个严格人工标注的视频-问题对，涵盖高度多样化的环境（对象、动作和事件量化）。基准采用三层层次结构：从直接视觉感知到视频组合数值推理，要求模型执行基于时间证据的算术运算、比较和逻辑推理。

Result: 评估多个最先进的视觉语言模型显示显著的推理差距：Gemini-3.1-pro仅达到60%准确率阈值，代表性开源模型在25%-45%范围内表现困难。这表明当前模型缺乏稳定的"内部世界模型"。

Conclusion: VidNum-1.4K作为一个具有挑战性的诊断测试平台，揭示了当前视觉语言模型在数值推理方面的局限性，为下一代数值视频智能的发展提供了评估标准。
...
TL;DR: XSeg：最大的X射线违禁品分割数据集，包含98,644张图像和295,932个实例掩码，涵盖30个常见违禁品类别；提出APSAM（自适应点SAM）模型，通过能量感知编码器和自适应点生成器改进SAM，实现高效精确的掩码标注。
...
Method: 1. 构建XSeg数据集：整合公开数据集和合成数据，通过自定义数据清洗管道过滤低质量样本，包含98,644张图像和295,932个实例掩码，涵盖30个常见违禁品类别。2. 提出APSAM模型：基于Segment Anything Model (SAM)构建，引入能量感知编码器增强掩码解码器初始化，提高对重叠物品的敏感性；设计自适应点生成器，允许用户仅通过单个粗略点提示获得精确掩码标签。

Result: XSeg是目前最大的X射线违禁品分割数据集。APSAM在XSeg上表现出优越性能，解决了SAM在跨域泛化能力差和检测堆叠物体能力有限的问题，显著提高了标注效率和精度。

Conclusion: XSeg数据集和APSAM模型为X射线违禁品分割提供了重要资源和方法。APSAM通过改进SAM的架构，实现了高效准确的掩码标注，减少了人工标注工作量，推动了该领域的发展。
...
TL;DR: 本文提出了一种基于图学习的黑色素瘤检测方法，使用超像素集合图和超像素层次图两种图表示，结合手工和学习的边权重分配技术，在ISIC2017数据集上达到99.00%准确率和99.59% AUC。


<details>
  <summary>Details</summary>
Motivation: 传统图信号处理技术中的图结构和边权重通常通过统计和计算方法设置，而图结构学习方法能提供更可靠和灵活的数据表示。本文旨在利用图学习方法来改进皮肤镜图像中的黑色素瘤检测。

Method: 提出基于超像素集合图和超像素层次图的图学习方法。生成多级超像素图（20、40、60、80、100个节点），探索两种边权重分配技术：手工高斯权重和基于优化的学习权重。节点信号基于纹理、几何和颜色超像素特征。研究图边阈值化（25%、50%、75%）对性能的影响。在ISIC2017数据集上使用不同分类器进行实验，并通过数据增强缓解类别不平衡。

Result: 学习型超像素集合图结合纹理节点信号获得最佳性能，达到99.00%准确率和99.59% AUC。结果表明图边阈值化对性能有影响，适当的阈值能提高检测效果。

Conclusion: 图学习方法在黑色素瘤检测中表现出色，特别是学习型超像素集合图结合纹理特征。该方法为生物医学图像分析提供了有效的图表示框架，图边阈值化是优化性能的重要参数。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法通常只能处理单一运动类型（相机运动或物体动态），或者依赖模糊的2D线索，导致相机视差与真实物体运动相互纠缠。需要一种能够统一控制相机轨迹和物体动态的框架。

Method: 提出SymphoMotion框架，包含两个核心机制：1）相机轨迹控制机制，整合显式相机路径与几何感知线索，确保稳定的视角转换；2）物体动态控制机制，结合2D视觉引导与3D轨迹嵌入，实现深度感知的空间一致物体操作。同时构建了RealCOD-25K数据集，包含配对的相机姿态和物体级3D轨迹。

Result: 实验和用户研究表明，SymphoMotion在视觉保真度、相机可控性和物体运动准确性方面显著优于现有方法，为统一运动控制建立了新的基准。

Conclusion: SymphoMotion通过统一控制相机轨迹和物体动态，解决了现有方法的局限性，在视频生成中实现了更连贯和富有表现力的运动控制，并通过公开数据集和代码促进了该领域的发展。
...
TL;DR: 提出PoCo方法解决多参考图像视频生成中的参考混淆问题，通过位置编码增强上下文控制，实现高相似度角色的可靠控制


<details>
  <summary>Details</summary>
Motivation: 现有专有模型（如Sora2）在多参考角色视频生成方面取得进展，但学术研究有限。核心挑战是当参考图像外观高度相似时，模型容易出现参考混淆，语义相似标记会降低正确上下文检索能力。

Method: 提出PoCo（Position Embedding as a Context Controller），将位置编码作为语义检索之外的额外上下文控制。通过利用标记的侧信息，PoCo实现精确的标记级匹配，同时保持隐式语义一致性建模。基于PoCo开发了多参考、多镜头视频生成模型。

Result: 大量实验表明，与多种基线方法相比，PoCo提高了跨镜头一致性和参考保真度，能够可靠控制视觉特征极其相似的角色。
...
TL;DR: SA-DSVN是一种用于钢筋混凝土结构缺陷体素级分割的3D卷积架构，利用宇宙射线μ子断层扫描技术，通过联合处理散射运动学和次级电磁簇射多重性来提升缺陷检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统μ子断层扫描方法（如POCA、MLSD）仅依赖μ子散射角度，忽略了次级电磁簇射多重性这一重要特征。作者旨在开发一种能够充分利用μ子与物质相互作用产生的全部信息的深度学习架构，以提高结构缺陷检测的准确性和灵敏度。

Method: 提出SA-DSVN（Scattering and Shower Dual-Stream Voxel Network）3D卷积架构，包含两个独立的编码器流：一个处理散射运动学（9个通道），另一个处理次级电磁簇射多重性（40个通道）。两个流通过交叉注意力机制融合。使用基于Geant4的Vega云原生仿真框架生成训练数据，包含450万个μ子事件和900个体积，涵盖蜂窝、剪切断裂、腐蚀空洞和分层四种缺陷类型。

Result: 在60个独立仿真验证体积上，模型达到96.3%的体素准确率，各缺陷类型的Dice分数为0.59-0.81，体积级检测灵敏度为100%，每个体积推理时间为10毫秒。消融研究表明，仅簇射多重性流就贡献了主要判别能力，将缺陷平均Dice从0.535（仅散射）提升到0.685（仅簇射）。

Conclusion: 次级电磁簇射多重性是一个先前未被充分利用但非常有效的特征，对于学习的μ子断层扫描重建具有重要意义。SA-DSVN架构通过联合处理散射和簇射信息，显著提升了钢筋混凝土结构缺陷检测的性能。
...
TL;DR: 提出ICBench基准和ITIScore自动评估指标，解决现有图像描述基准在多样性、模型覆盖和人工标注方面的不足


<details>
  <summary>Details</summary>
Motivation: 现有图像描述基准存在三个主要问题：描述长度多样性有限、缺乏最新先进MLLMs的参与、人工标注不足导致评估偏差，限制了全面评估现代MLLMs的能力

Method: 1) 构建ICBench基准：包含12个内容类别，2K张图像，使用10个先进MLLMs生成短描述和长描述，共40K条描述；2) 进行大规模人类主观研究获取平均意见分数；3) 提出ITIScore自动评估指标，基于图像-文本-图像重建一致性框架

Result: 1) ICBench基准覆盖广泛内容类别和描述长度；2) ITIScore与人类判断高度一致；3) 在公共描述数据集上展示强大的零样本泛化能力

Conclusion: ICBench基准和ITIScore指标为全面评估现代MLLMs的图像描述能力提供了有效工具，自动指标与人类评估高度一致，具有良好泛化性
...
TL;DR: M2StyleGS是一种实时3D风格迁移方法，使用3D高斯泼溅作为3D表示，结合CLIP多模态知识作为风格参考，通过精确特征对齐和损失函数实现文本或图像参考的风格化新视角生成。
...
Method: 1. 使用3D高斯泼溅作为3D表示；2. 利用CLIP精炼的多模态知识作为参考风格；3. 采用细分流进行精确特征对齐，增强CLIP文本-视觉组合特征到VGG风格特征的投影；4. 引入观察损失使风格化场景更好匹配参考风格；5. 引入抑制损失抑制参考颜色信息在解码过程中的偏移。

Result: M2StyleGS能够以文本或图像为参考生成风格增强的新视角序列，实验显示在视觉质量上表现更好，一致性方面比先前工作提升高达32.92%。

Conclusion: M2StyleGS通过多模态风格参考和精确特征对齐机制，实现了灵活、实时的3D风格迁移，显著提升了风格一致性和视觉质量，适用于VR/AR等需要灵活输入的实际应用场景。
...
TL;DR: 提出自动化框架，利用粗粒度CityGML建筑模型作为几何先验，通过表面匹配和二元整数优化，结合MLS数据精化建筑立面几何，确保拓扑有效性。


<details>
  <summary>Details</summary>
Motivation: 传统ALS获取的CityGML建筑模型在立面几何上精度不足，需要与高精度MLS数据融合以恢复详细立面几何，同时保留现有语义信息，避免从头重建的弊端。

Method: 使用粗模型作为几何先验，通过表面匹配识别过时表面，采用二元整数优化从候选数据中选择最优面片，并在优化中强制执行硬约束以确保拓扑有效性。

Result: 方法有效校正立面错位，将Cloud-to-Mesh RMSE降低约36%，实现厘米级对齐，同时保证严格水密和流形几何。

Conclusion: 该框架为升级ALS衍生的城市模型提供了鲁棒解决方案，能够在复杂城市环境中实现有针对性的立面几何更新，同时保持拓扑完整性。
...
TL;DR: MoScale：一种从粗到细的层次化自回归运动生成框架，通过多尺度预测和双向细化实现高效文本到运动生成


<details>
  <summary>Details</summary>
Motivation: 标准自回归模型虽然训练稳定高效，但其下一令牌预测与文本条件运动生成所需的时间结构不对齐，无法有效捕捉长程运动结构

Method: 提出MoScale框架：1）从粗到细的层次化自回归生成；2）跨尺度层次细化改进每尺度初始预测；3）尺度内时间细化进行选择性双向重预测

Result: 在文本到运动生成任务上达到SOTA性能，训练效率高，模型规模可扩展，并能零样本泛化到多种运动生成和编辑任务

Conclusion: MoScale通过层次化自回归框架有效解决了文本条件运动生成中的时间结构对齐问题，在有限数据下实现了鲁棒且高效的生成
...
<details>
  <summary>Details</summary>
Motivation: 夜间图像去雾面临雾霾、光晕、非均匀光照、颜色失真和传感器噪声等多种退化因素的联合影响，这些因素使传统的白天去雾假设失效，需要专门的方法来处理夜间场景的异质退化

Method: 提出HistoFusionNet架构，基于多尺度编码器-解码器主干，引入直方图Transformer块，根据动态范围特征对特征进行分组以建模长程依赖；同时包含频率感知细化分支，自适应地利用低频和高频线索来恢复场景结构、抑制伪影并增强局部细节

Result: 在NTIRE 2026夜间图像去雾挑战赛基准测试中表现出高度竞争力，在22个参赛团队中排名第一，证明了该方法的有效性

Conclusion: HistoFusionNet提供了一个统一的框架，特别适合处理真实夜间雾霾场景中的异质退化，通过直方图引导表示学习和频率自适应特征细化的结合，实现了对夜间图像去雾挑战的有效解决
...
TL;DR: 提出基于注意力分布熵的补丁剪枝方法，通过低熵（集中注意力）补丁保留、高熵（分散注意力）补丁剪除来降低Transformer计算成本，并扩展至Rényi熵实现自适应剪枝策略。
...
Method: 提出基于注意力分布Shannon熵的补丁重要性评估准则：低熵补丁（注意力集中）被视为重要而保留，高熵补丁（注意力分散）被视为冗余而剪除。进一步扩展至Rényi熵，强调注意力分布的峰值，支持根据任务需求和计算限制的自适应剪枝策略。

Result: 在细粒度图像识别任务中，该方法在保持准确率的同时显著降低了计算量。通过Rényi熵调整剪枝策略进一步提升了性能，改善了准确率与计算成本之间的权衡。

Conclusion: 基于注意力分布熵的补丁剪枝方法有效降低了Transformer的计算成本，同时保持了模型性能。Rényi熵扩展提供了更灵活的剪枝策略，能够根据具体任务需求优化准确率与计算效率的平衡。
...
TL;DR: EyeBench-V2：首个面向临床应用的眼底图像增强基准，通过多维临床对齐评估、专家指导设计和可操作洞察，填补传统评价指标与临床需求之间的鸿沟。


<details>
  <summary>Details</summary>
Motivation: 现有眼底图像增强模型的评估存在三大问题：1)传统去噪指标（PSNR/SSIM）无法捕捉临床相关特征（如病灶保留、血管形态一致性）；2)缺乏统一评估协议处理配对与非配对增强方法；3)需要提供可操作见解指导临床对齐增强模型的未来发展。

Method: 提出EyeBench-V2基准，包含三个核心贡献：1)多维临床对齐下游评估：超越传统增强指标，评估血管分割、糖尿病视网膜病变分级、未见噪声模式泛化、病灶分割等临床任务；2)专家指导评估设计：构建新数据集支持配对与非配对方法公平比较，制定结构化专家手动评估协议，评估病灶结构改变、背景颜色偏移、人工结构引入等临床关键方面；3)可操作见解：提供严格的面向任务分析。

Result: 建立了一个全面的眼底图像增强评估框架，填补了模型性能与临床效用之间的差距，为临床研究人员提供决策证据，同时识别当前方法的局限性以指导下一代增强模型设计。

Conclusion: EyeBench-V2基准通过将评估重点从传统图像质量指标转向临床相关任务和专家评估，为眼底图像增强领域提供了更贴近实际临床需求的评估标准，有望推动临床对齐增强模型的发展。
...
### [58] [InCaRPose: In-Cabin Relative Camera Pose Estimation Model and Dataset](https://arxiv.org/abs/2604.03814)
*Felix Stillger,Lukas Hahn,Frederik Hasecke,Tobias Meisen*

Main category: cs.CV

TL;DR: InCaRPose：基于Transformer的架构，用于在高度失真的车载监控环境中进行鲁棒的相对位姿估计和相机外参标定


<details>
  <summary>Details</summary>
Motivation: 在车载监控（ICAM）等受限、高度失真的环境中，精确的相对位姿估计仍然具有挑战性。传统方法难以在单次推理中实现绝对度量尺度的平移估计，而这对于需要准确真实世界距离的安全相关感知至关重要。

Method: 提出InCaRPose，一种基于Transformer的架构，利用冻结的骨干特征（如DINOv3）和Transformer解码器来捕捉参考视图与目标视图之间的几何关系。专门针对车载鱼眼相机的高度失真问题，仅使用合成数据进行训练。

Result: 模型能够在单次推理中实现绝对度量尺度的平移估计，且在车载相机物理可调范围内。能够泛化到真实世界车载环境，不依赖相同的相机内参。在公开7-Scenes数据集上取得有竞争力的性能，即使使用ViT-Small骨干也能保持高精度，实现实时性能。

Conclusion: InCaRPose为高度失真环境中的相机外参标定提供了有效的解决方案，特别适用于车载监控等安全关键应用。通过仅使用合成数据训练并实现泛化能力，该方法解决了真实数据获取的挑战。发布了真实世界的In-Cabin-Pose测试数据集和代码。
...
TL;DR: ActivityForensics：首个大规模视频活动伪造定位基准，包含6000+伪造视频片段，并提出TADiff方法用于检测修改人类行为的活动级伪造


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注外观级伪造（如人脸交换、物体移除），而视频生成技术的发展催生了活动级伪造，这些伪造通过修改人类行为来扭曲事件语义，具有高度欺骗性，严重威胁媒体真实性和公众信任

Method: 1) 构建ActivityForensics基准，包含6000+伪造视频片段，无缝融入视频上下文，视觉一致性高；2) 提出Temporal Artifact Diffuser (TADiff)基线方法，通过基于扩散的特征正则化器暴露伪影线索

Result: 建立了首个大规模活动伪造定位基准，涵盖域内、跨域和开放世界评估协议，并基准测试了多种最先进的伪造定位方法，为未来研究提供基础

Conclusion: ActivityForensics填补了活动级伪造检测的空白，TADiff方法有效暴露伪造伪影，该基准和评估协议将推动视频伪造检测领域的发展
...
### [60] [SPARK-IL: Spectral Retrieval-Augmented RAG for Knowledge-driven Deepfake Detection via Incremental Learning](https://arxiv.org/abs/2604.03833)
...
<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测器在特定生成器上训练后难以泛化到未见过的模型，因为像素级伪影在不同模型间差异较大。然而，频域特征在不同生成器间表现出更高的一致性，这为跨生成器检测提供了有前景的基础。

Method: 提出SPARK-IL框架：1）双路径频谱分析：使用部分冻结的ViT-L/14编码器获取语义表示，并行路径处理原始RGB像素嵌入；2）多频带傅里叶分解：将两个路径分解为四个频带，每个频带由带有专家混合的Kolmogorov-Arnold Networks处理；3）交叉注意力融合：通过交叉注意力和残差连接融合频谱嵌入；4）检索增强推理：使用余弦相似度从Milvus数据库中检索k个最近邻标记签名，通过多数投票进行预测；5）增量学习：扩展数据库并使用弹性权重巩固来保留先前学习的变换。

Result: 在UniversalFakeDetect基准测试中，涵盖19个生成模型（包括GANs、换脸和扩散方法），SPARK-IL实现了94.6%的平均准确率。

Conclusion: SPARK-IL通过结合双路径频谱分析和增量学习，有效解决了AI生成图像检测中的跨生成器泛化问题，在多个生成模型上表现出优异的检测性能。
...
Result: 在航空野生动物数据集上的实验表明，相比多种三元组损失设置，任务引导选择在分类和回归任务上均表现更优，产生更有效的共享表示
...
TL;DR: 提出半监督知识蒸馏框架，将预训练视觉基础模型压缩为紧凑专家模型，用于实例分割任务，在有限标注和大量未标注数据下实现高效压缩


<details>
  <summary>Details</summary>
Motivation: 基础模型感知能力强但计算量大难以部署，且适应这些模型通常需要昂贵的标注成本，特别是在实例分割这种像素级标注极其昂贵的任务中

Method: 三阶段框架：1) 通过对比校准的自训练进行领域适应；2) 通过统一多目标损失进行知识转移；3) 学生模型精炼以减轻伪标签偏差。核心是实例感知的像素级对比损失，融合掩码和类别分数来提取信息负样本并强制清晰的实例间边界

Result: 在Cityscapes和ADE20K数据集上，约11倍小的学生模型相比零-shot基础模型教师提升+11.9和+8.6 AP，相比适应后的教师提升+3.4和+1.5 AP，并在半监督知识蒸馏基准上优于最先进方法

Conclusion: 提出的半监督知识蒸馏框架能有效压缩视觉基础模型，在有限标注下实现高性能的紧凑模型，通过对比学习机制有效利用未标注数据并减轻伪标签偏差
...
TL;DR: SafeCtrl：基于检测-抑制范式的区域感知安全控制框架，用于文本到图像扩散模型，通过精确定位风险区域并局部抑制有害内容，在安全性和内容保真度之间实现更好平衡


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型广泛部署面临生成视觉有害内容（如色情、暴力、恐怖图像）的挑战。现有安全干预措施存在两个关键局限：1）安全性与上下文保真度之间的严重权衡；2）易受对抗性攻击。需要更精确、鲁棒的安全控制方法。

Method: 提出SafeCtrl框架，采用检测-抑制范式：1）注意力引导的检测模块精确定位特定风险区域；2）局部抑制模块通过图像级直接偏好优化（DPO）进行优化，仅在检测区域内中和有害语义，将不安全对象转化为安全替代物，同时保持周围上下文完整。

Result: 在多个风险类别上的广泛实验表明，SafeCtrl相比最先进方法在安全性和保真度之间实现了更优的权衡。关键的是，该方法对对抗性提示攻击表现出更强的韧性。

Conclusion: SafeCtrl为负责任生成提供了一个精确且鲁棒的解决方案，通过区域感知的安全控制有效解决了现有方法的局限性，在保持内容质量的同时增强了模型的安全性。
...
<details>
  <summary>Details</summary>
Motivation: 传统图套索方法在多模态学习中面临高维噪声、模态错位以及共享与类别特定拓扑混淆等瓶颈，需要新的方法来学习可解释的多模态表示

Method: 1) 文本-视觉对齐策略与统一视觉-语言编码器；2) 跨注意力蒸馏机制将高维补丁压缩为语义节点；3) 定制化GLasso估计与共同-特定结构学习的联合目标；4) 交替方向乘子法优化

Result: 在8个基准测试（自然和医学领域）上，CM-GLasso在生成式分类和密集语义分割任务中建立了新的最先进性能

Conclusion: CM-GLasso通过严格的多模态特征对齐、语义节点提取和联合优化框架，成功解决了多模态图估计中的关键挑战，实现了可解释的多模态表示学习
...
TL;DR: VLA-Forget：一种针对视觉-语言-动作模型的混合遗忘框架，通过选择性编辑感知层和跨模态投影器，结合分层选择性推理/动作遗忘，实现高效遗忘不良行为同时保持模型性能。
...
Method: 提出VLA-Forget混合遗忘框架，结合：1）感知和跨模态特异性编辑（ratio-aware selective editing）；2）分层选择性推理/动作遗忘（layer-selective reasoning/action unlearning）。通过分阶段更新视觉编码器、投影器和上层动作生成transformer块，联合优化三个目标：定向遗忘、感知保留和推理保持。

Result: 相比强基线方法，VLA-Forget在遗忘集行为探测中提升遗忘效果10%，保持感知特异性22%，保留推理和任务成功率9%，并将量化后恢复率降低55%。

Conclusion: VLA-Forget为VLA模型提供了一种有效的遗忘框架，能够针对性地移除不良行为，同时保持模型的感知、语言理解和动作控制能力，解决了VLA模型部署中的关键安全挑战。
...
TL;DR: GAPL：通过二阶统计量增强视觉语言模型提示学习，引入Gram矩阵作为二阶特征流，结合局部语义对齐与全局结构一致性，提升跨域鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效提示学习方法主要关注文本提示与一阶视觉特征（空间特征图）的对齐。虽然对细粒度语义区分有效，但仅依赖一阶信息不足以实现鲁棒适应，因为这些空间纠缠特征对域偏移和局部噪声高度敏感。

Method: 提出Gram锚定提示学习（GAPL）框架，通过Gram矩阵引入额外的二阶统计流，增强标准的一阶空间交互。将提示锚定到这些二阶先验，使语言表示能动态适应不同域的统计分布偏移。

Result: 大量实验验证了二阶特征的有效性，GAPL在各种基准测试中表现出色。

Conclusion: 通过结合局部语义对齐与全局结构一致性，GAPL为视觉语言模型的参数高效适应提供了更鲁棒的解决方案，特别适用于跨域场景。
...
TL;DR: HMAT：一种用于古代壁画修复的混合掩码感知Transformer框架，结合动态滤波和Transformer瓶颈，通过掩码条件风格融合和教师强制解码器实现高保真修复。


<details>
  <summary>Details</summary>
Motivation: 古代壁画作为珍贵文化遗产，常因环境暴露、材料老化和人为活动而严重退化。修复工作面临双重挑战：既要重建大面积缺失结构，又要严格保护未受损的真实区域。

Method: 提出混合掩码感知Transformer（HMAT）统一框架：1）掩码感知动态滤波用于鲁棒的局部纹理建模；2）Transformer瓶颈用于长距离结构推理；3）掩码条件风格融合模块动态引导生成过程；4）教师强制解码器通过硬门控跳跃连接确保有效区域保真度，聚焦缺失区域重建。

Result: 在DHMural数据集和九色鹿数据集上评估，HMAT在不同退化程度下均取得竞争性性能，相比现有方法产生更结构一致、视觉保真的修复结果。

Conclusion: HMAT为文化遗产壁画的数字修复提供了有效解决方案，能够实现高保真度的壁画修复。
...
TL;DR: 该论文提出了多模态排版攻击，通过协调音频、视觉和文本扰动来攻击多模态大语言模型，相比单模态攻击成功率显著提高（83.43% vs 34.93%）。
...
Method: 提出多模态排版攻击方法，系统研究跨模态排版攻击如何影响多模态大语言模型。分析音频、视觉和文本扰动之间的相互作用，设计协调的多模态攻击策略。

Result: 在多模态前沿模型、任务以及常识推理和内容审核基准测试中，协调多模态攻击相比单模态攻击成功率显著提高（83.43% vs 34.93%），揭示了多模态模型在跨模态攻击下的脆弱性。

Conclusion: 多模态排版攻击是多模态推理中一个关键且未被充分探索的攻击策略，强调了在安全关键应用中需要更强的多模态鲁棒性防御机制。
...
TL;DR: OASIC：一种通过估计遮挡严重程度、掩蔽遮挡物并选择对应优化模型的遮挡感知分类方法，显著提升遮挡场景下的分类性能。
...
Method: 提出OASIC框架：1）测试时通过掩蔽遮挡物去除干扰模式；2）训练时对物体随机掩蔽以应对不同遮挡程度；3）测试时估计遮挡严重程度并选择对应优化模型。

Result: 结合灰度掩蔽和自适应模型选择，相比标准训练提升AUC_occ 18.5点，相比在未遮挡图像上微调提升23.7点，显著优于单一模型。

Conclusion: 通过同时处理遮挡干扰和可见信息损失，并采用严重程度感知的自适应模型选择策略，OASIC在遮挡场景下实现了更优的分类性能。
...
TL;DR: MINERUPRO通过数据工程和训练策略优化（而非架构创新）提升文档解析性能，在固定1.2B参数架构下，通过数据引擎和渐进训练策略在OmniDocBench v1.6上达到95.69分，超越更大参数模型。


<details>
  <summary>Details</summary>
Motivation: 当前文档解析方法主要关注模型架构创新，而训练数据的系统工程研究不足。研究发现不同架构和参数规模的SOTA模型在相同困难样本上表现出高度一致的失败模式，表明性能瓶颈源于训练数据的共同缺陷而非架构本身。

Method: 提出MINERUPRO方法，核心是围绕覆盖度、信息量和标注准确率共同设计的数据引擎：1）多样性和难度感知采样将训练数据从不足1000万扩展到6550万样本并纠正分布偏移；2）跨模型一致性验证利用异构模型输出一致性评估样本难度并生成可靠标注；3）Judge-and-Refine管道通过渲染-验证迭代校正改进困难样本标注质量。采用三阶段渐进训练策略：大规模预训练、困难样本微调和GRPO对齐。

Result: 在评估方面，修正了OmniDocBench v1.5中的元素匹配偏差并引入Hard子集，建立了更具区分度的OmniDocBench v1.6协议。在未修改架构的情况下，MINERUPRO在OmniDocBench v1.6上达到95.69分，比相同架构基线提升2.71分，超越所有现有方法（包括参数规模超过200倍的模型）。

Conclusion: 研究表明文档解析的性能瓶颈主要源于训练数据而非模型架构，通过系统化的数据工程和训练策略优化可以在固定架构下显著提升性能，甚至超越参数规模大得多的模型，为文档解析领域提供了新的研究方向。
...
TL;DR: HOIGS是一种用于重建动态场景中复杂人-物交互的Gaussian Splatting方法，通过交叉注意力HOI模块显式建模交互引起的形变，在遮挡、接触和物体操纵场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有Gaussian Splatting方法要么依赖人体姿态先验而忽略动态物体，要么将所有运动近似在单一场中，无法有效捕捉交互丰富的动态场景。

Method: 提出Human-Object Interaction Gaussian Splatting (HOIGS)，通过交叉注意力HOI模块显式建模人-物交互引起的形变。采用异构特征提取：HexPlane用于人体，Cubic Hermite Spline (CHS)用于物体。通过集成这些异构特征捕捉相互依赖的运动。

Result: 在多个数据集上的综合实验表明，HOIGS在遮挡、接触和物体操纵场景中有效改进了形变估计，一致优于最先进的人体中心和4D Gaussian方法。

Conclusion: 显式建模人-物交互对于高保真重建至关重要，HOIGS通过异构特征提取和交叉注意力机制有效捕捉交互丰富的动态场景。
...
TL;DR: 1.x-Distill是一种分数步蒸馏框架，打破了传统整数步限制，在1.x步生成中实现高质量图像合成，相比原始扩散模型实现33倍加速。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能生成高质量文本到图像结果，但其迭代去噪过程计算成本高昂。现有的Distribution Matching Distillation (DMD)方法在减少到2步或更少时会出现多样性崩溃和保真度下降问题，需要突破整数步限制的蒸馏方法。

Method: 1.x-Distill包含三个关键技术：1) 分析教师CFG在DMD中的作用并引入简单修改抑制模式崩溃；2) 提出两阶段聚焦蒸馏策略，第一阶段通过多样性保持分布匹配学习粗粒度结构，第二阶段通过推理一致对抗蒸馏细化细节；3) 设计轻量级补偿模块实现蒸馏-缓存协同训练，将块级缓存自然融入蒸馏流程。

Result: 在SD3-Medium和SD3.5-Large模型上的实验表明，1.x-Distill在1.67和1.74有效NFE下超越了先前少步方法，实现了更好的质量和多样性，相比原始28x2 NFE采样实现了高达33倍的加速。

Conclusion: 1.x-Distill首次建立了1.x步生成作为蒸馏扩散模型的实用范式，突破了整数步限制，在极低步数下实现了高质量、多样化的图像生成，为高效文本到图像合成提供了新路径。
...
Method: 提出ATSS方法，利用AI生成视频中的异常时间自相似性指纹。通过构建视觉、文本和跨模态相似性矩阵来重建语义轨迹，使用专用Transformer编码器编码，并通过双向交叉注意力融合模块整合，有效建模模态内和模态间动态
...
<details>
  <summary>Details</summary>
Motivation: 当前最强的视觉语言模型展现出广泛的视觉推理能力，但其实现方法被锁在专有的强化学习流程和非公开数据中。研究旨在开发完全开源的视觉推理模型，揭示实现广泛视觉推理的配方

Method: 1. 构建Vero-600K数据集：从59个数据集中收集600K样本，覆盖六大任务类别；2. 设计任务路由奖励机制：处理异构答案格式；3. 基于Qwen3-VL-8B-Instruct等基础模型进行强化学习训练；4. 开发VeroEval评估套件：包含30个具有挑战性的基准测试

Result: 1. Vero在30个基准测试中平均提升3.7-5.5个百分点；2. 基于Qwen3-VL-8B-Instruct的Vero在30个基准中的23个上超越了Qwen3-VL-8B-Thinking；3. Vero-600K在各项任务类别上均超越现有RL数据集；4. 系统消融实验表明不同任务类别引发定性不同的推理模式，单独训练时迁移效果差，说明广泛数据覆盖是RL扩展的主要驱动力

Conclusion: 构建广泛视觉推理模型的关键在于覆盖多样任务类别的强化学习数据，而非专有的思维数据。Vero证明了通过开源方法可以达到或超越专有模型的性能，所有数据、代码和模型均已开源
...
TL;DR: TORA提出了一种拓扑优先表示对齐框架，通过从预训练的3D编码器中提取关系结构到流匹配主干中，显著提升3D形状装配的收敛速度和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有流匹配方法在3D形状装配中学习点级速度场，但缺乏对跨部件交互的显式指导。需要一种方法来注入几何关系知识以改善装配过程。

Method: 提出TORA框架：1) 通过token-wise余弦匹配将教师编码器的几何描述符注入学生模型；2) 使用中心核对齐(CKA)损失匹配师生表示的相似性结构；3) 系统探索不同3D编码器特性对对齐效果的影响。

Result: 1) 零推理开销；2) 收敛速度提升高达6.9倍；3) 分布内精度改善；4) 域转移下鲁棒性增强；5) 在五个基准测试中达到SOTA，零样本迁移到未见数据集表现突出。

Conclusion: 几何和接触为中心的教师特性（而非语义分类能力）决定对齐效果，后期transformer层对齐最有效。TORA为3D形状装配提供了高效的知识蒸馏框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉里程计系统通常依赖启发式特征提取策略，在大规模室外环境中精度和鲁棒性会下降。需要解决特征提取质量不足和场景泛化能力有限的问题。

Method: 1. 引入可微分自适应补丁选择器到端到端流程中；2. 集成多任务特征提取模块；3. 结合可微分束调整模块，利用逆深度先验；4. 有效学习外观和几何信息，弥合特征学习与状态估计之间的差距。

Result: 在TartanAir、KITTI、Euroc和TUM数据集上的广泛实验表明，DINO-VO在合成、室内和室外环境中都表现出强泛化能力，达到了最先进的跟踪精度。

Conclusion: DINO-VO通过端到端设计和自适应补丁选择，显著提升了视觉里程计系统的泛化能力和跟踪精度，为跨场景视觉定位提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要数十甚至数百个相机的密集多视角捕获来建模动态场景，这在便携性和成本上存在限制。本文旨在解决从极稀疏相机（仅需4个）视频中恢复4D动态场景的挑战。

Method: 提出4C4D框架，核心创新是引入高斯不透明度的神经衰减函数，增强4D高斯的几何建模能力。该设计通过鼓励4DGS梯度更多地关注几何学习，缓解了4DGS中几何与外观建模之间的固有不平衡问题。

Result: 在具有不同相机重叠度的稀疏视角数据集上进行广泛实验，结果显示4C4D在性能上优于现有技术。

Conclusion: 4C4D框架成功实现了从极稀疏相机视频中进行高保真4D高斯溅射，为解决稀疏视角下的动态场景建模问题提供了有效方案。
...
TL;DR: 基于YOLOv11检测器与BoT-SORT/ByteTrack多目标跟踪的离线实时交通监控系统，在典型条件下实现66.67-95.83%的计数准确率和高精度车辆检测。


<details>
  <summary>Details</summary>
Motivation: 利用人工智能驱动的计算机视觉技术增强监控系统，特别是交通监控领域，通过结合深度学习目标检测与计数技术，为智慧城市建设提供现代化解决方案。

Method: 采用预训练的YOLOv11检测器配合BoT-SORT/ByteTrack多目标跟踪算法，基于PyTorch/OpenCV实现，并封装在Qt桌面UI中，构建离线实时交通监控系统。

Result: 系统在多样化场景中实现66.67-95.83%的计数准确率；车辆检测精度高（轿车：0.97-1.00；卡车：1.00），召回率强（轿车：0.82-1.00；卡车：0.70-1.00），F1分数为轿车0.90-1.00，卡车0.82-1.00。

Conclusion: 通过集成轻量级模型与云独立的可访问界面，该系统展示了AI驱动交通监控系统的能力，为智慧城市的现代化和发展做出贡献，尽管恶劣天气条件可能影响性能。
...
TL;DR: HGR框架通过可修正的假设节点和依赖感知图记忆，实现语义预测驱动的探索与错误级联修正，提升具身智能体的长期导航与问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的导航系统将未探索区域视为语义未知，导致探索效率低下。视觉语言模型可预测边界语义，但错误预测会嵌入记忆并传播，产生结构误差累积，仅靠置信度衰减无法解决。需要既能利用语义预测进行定向探索，又能在新证据矛盾时系统撤回错误的框架。

Method: 提出假设图精炼（HGR）框架：1）语义假设模块：估计边界上的上下文条件语义分布，根据目标相关性、旅行成本和不确定性对探索目标排序；2）验证驱动的级联修正：将现场观察与预测语义比较，发现不匹配时撤回被反驳节点及其所有下游依赖节点，使图能够通过剪除错误子图而收缩。

Result: 在GOAT-Bench上达到72.41%成功率、56.22% SPL；在A-EQA和EM-EQA问答基准上持续改进。诊断分析显示级联修正消除了约20%的结构冗余假设节点，将错误区域重访减少4.5倍，镜面和透明表面占修正预测错误的67%。

Conclusion: HGR通过可修正的假设节点和依赖感知图记忆，实现了语义预测驱动的探索与系统错误修正，解决了现有方法中结构误差累积问题，显著提升了具身智能体在长期导航和问答任务中的性能与可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 开发在真实世界恶劣条件下（极端低光和烟雾退化）鲁棒的3D重建管道，使用RealX3D基准测试进行评估

Method: 组织NTIRE 2026 3DRR挑战赛，收集33个团队的有效提交，使用RealX3D基准在极端低光和烟雾条件下评估方法，并与最先进基线进行比较

Result: 279名参与者注册，33个团队提交有效结果；顶级方法在恶劣条件下的3D重建取得显著进展，分析揭示了高性能方法的共同设计原则

Conclusion: 挑战赛展示了3D重建在恶劣条件下的重要进展，为处理3D场景退化提供了有效策略的见解，并识别了高性能方法的共享设计模式
...
TL;DR: 该论文提出了一种针对空间非均匀曝光校正的新范式，通过空间自适应调制权重和基于不确定性的非均匀损失函数，解决了现有方法在空间非均匀曝光错误校正方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的曝光校正面临空间非均匀退化的根本挑战，单张图像中经常存在多种曝光错误共存的情况。然而，现有曝光校正方法大多基于均匀假设开发，架构上依赖全局聚合的调制信号仅捕捉整体曝光趋势，优化角度上传统重建损失基于共享全局尺度，忽视了不同区域的空间变化校正需求。

Method: 提出空间非均匀曝光校正新范式：1) 引入空间信号编码器预测空间自适应调制权重，指导多个查找表进行图像变换；2) 设计基于HSL的补偿模块提升色彩保真度；3) 提出基于不确定性的非均匀损失函数，根据局部恢复不确定性动态分配优化焦点。

Result: 大量实验表明，该方法在定性和定量性能上均优于现有最先进方法，在空间非均匀曝光校正方面表现出色。

Conclusion: 该研究提出了一种专门针对空间非均匀曝光校正的新方法，通过空间自适应调制和基于不确定性的优化策略，有效解决了现实世界中空间非均匀曝光错误的校正问题，为曝光校正领域提供了新的研究方向。
...
TL;DR: OP-GRPO：首个针对流匹配模型的离线策略GRPO框架，通过经验回放和重要性采样校正，将训练步骤减少至34.2%的同时保持生成质量


<details>
  <summary>Details</summary>
Motivation: GRPO在提升流匹配模型生成质量方面表现优异，但其在线策略训练范式导致样本效率低下，需要大量训练步骤

Method: 1. 主动选择高质量轨迹并自适应地存入回放缓冲区供后续训练重用；2. 提出序列级重要性采样校正，在保持GRPO裁剪机制完整性的同时缓解分布偏移；3. 通过截断后期去噪步骤解决离策略比率病态问题

Result: 在图像和视频生成基准测试中，OP-GRPO仅用平均34.2%的训练步骤就达到了与Flow-GRPO相当或更优的性能，显著提升了训练效率

Conclusion: OP-GRPO通过离线策略框架有效解决了GRPO的样本效率问题，为流匹配模型的强化学习训练提供了高效且稳定的解决方案
...
TL;DR: 提出了一种不确定性感知的测试时自适应框架，专门用于遥感回归任务中的时空融合，通过仅更新预训练模型的融合模块来应对地理区域间的域偏移问题。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在遥感应用中面临跨地理区域泛化困难的问题，主要原因是域偏移。现有测试时自适应方法主要针对分类任务设计，不适用于回归任务，特别是时空融合这类回归任务。

Method: 提出不确定性感知的测试时自适应框架：1) 仅更新预训练时空融合模型的融合模块；2) 使用认知不确定性指导自适应过程；3) 结合土地利用和土地覆盖一致性约束；4) 进行偏差校正；5) 无需源数据或标记的目标样本。

Result: 在四个不同气候的目标区域（意大利罗马、埃及开罗、西班牙马德里、法国蒙彼利埃）进行实验，相对于在法国奥尔良预训练的模型，RMSE和MAE分别平均提升24.2%和27.9%，即使使用有限的无标签目标数据和仅10个TTA轮次。

Conclusion: 该不确定性感知测试时自适应框架能有效解决遥感回归任务中的域偏移问题，显著提升模型在新地理区域的性能，为时空融合等回归任务的域自适应提供了有效解决方案。
...
TL;DR: 提出双曲共嵌入框架，通过蕴含关系而非简单配对对齐来建模字体-印象对应关系，量化印象的样式特异性


<details>
  <summary>Details</summary>
Motivation: 字体形状能唤起广泛印象，但字体与印象描述之间并非一一对应关系：有些印象与多种样式兼容，而有些印象则强烈限制字体选择。这种分级约束强度被称为样式特异性，需要量化建模

Method: 提出双曲共嵌入框架，将字体图像和印象描述（单标签或标签集）嵌入共享双曲空间，采用两种互补的蕴含约束：印象到字体的蕴含，以及印象间从低到高样式特异性的蕴含。这种表述诱导出径向结构，其中低样式特异性印象靠近原点，高特异性印象远离原点

Result: 在MyFonts数据集上的实验显示，相比强的一对一基线方法，该方法在双向检索方面有改进。遍历和标签级分析表明，学习到的空间捕获了从模糊到更样式特异性印象的连贯进展，提供了有意义的数据驱动的样式特异性量化

Conclusion: 双曲共嵌入框架通过蕴含关系有效建模字体-印象对应关系，提供了可解释的几何度量来量化印象对字体样式的约束强度，为理解字体设计的语义约束提供了新视角
...
<details>
  <summary>Details</summary>
Motivation: 现有的多视图多标签学习方法主要针对缺失视图情况，使用对比学习或信息瓶颈理论学习一致表示，但缺乏显式结构约束，难以捕获稳定且具有判别性的共享语义。双缺失场景（视图和标签都不完整）的研究仍很有限。

Method: 1) 通过多视图共享码本和跨视图重建学习离散一致表示，在有限的共享码本嵌入中自然对齐不同视图并减少特征冗余；2) 设计权重估计方法评估每个视图保留标签相关结构的能力，据此分配权重以提升融合预测质量；3) 提出融合教师自蒸馏框架，用融合预测指导视图特定分类器训练，将全局知识反馈到单视图分支，增强缺失标签条件下的泛化能力。

Result: 在五个基准数据集上与先进方法进行了广泛的对比实验，充分证明了所提方法的有效性。代码已在GitHub开源。

Conclusion: 该方法通过结构化的一致表示学习机制、基于标签相关结构的权重估计以及融合教师自蒸馏框架，有效解决了双缺失场景下的多视图多标签学习问题，提升了模型的表示能力和预测性能。
...
TL;DR: GENFIG1是一个评估生成式AI模型能力的基准测试，专注于让模型根据论文标题、摘要、引言和图表说明生成能清晰表达论文核心思想的"Figure 1"图表。


<details>
  <summary>Details</summary>
Motivation: 科学论文中的"Figure 1"通常是核心研究思想的视觉总结，这些图表视觉简洁但概念丰富，需要人类作者大量努力和迭代才能完成，凸显了科学视觉传达的难度。这启发了创建GENFIG1基准来评估AI模型在科学视觉表达方面的能力。

Method: 从顶级深度学习会议论文中筛选数据，应用严格的质量控制，并引入与专家判断相关性良好的自动评估指标。基准要求模型：(i)理解论文技术概念，(ii)识别最关键概念，(iii)设计连贯美观且忠实于输入的视觉图表。

Result: 评估了一系列代表性模型，发现即使性能最佳的系统在该任务上也面临显著挑战，表明科学视觉生成是一个困难的多模态AI问题。

Conclusion: GENFIG1基准为未来多模态AI在科学视觉表达方面的进展提供了基础，强调了将科学理解与视觉合成相结合的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有VideoLLMs大多是离线系统，不适用于需要持续观察和及时响应的实时视频流。当前的流式VideoLLMs要么依赖解耦的触发-响应管道，要么仅限于字幕式叙述，限制了开放式问答和长时程交互的有效性。

Method: 提出AURA端到端流式视觉交互框架，整合上下文管理、数据构建、训练目标和部署优化，实现稳定的长时程流式交互。支持实时问答和主动响应，在2个80G加速器上以2FPS运行实时演示系统。

Result: 在流式基准测试中达到最先进性能，支持包含ASR和TTS的实时演示系统，在2个80G加速器上以2FPS运行。发布了AURA模型和实时推理框架。

Conclusion: AURA为流式视频理解提供了一个统一的端到端框架，解决了现有系统的局限性，支持实时问答和主动响应，推动了流式VideoLLMs的研究和应用。
...
TL;DR: GDB是首个针对专业平面设计任务评估AI模型的综合基准套件，涵盖布局、排版、信息图、模板与设计语义、动画五大维度，包含50个任务，评估显示当前模型在空间推理、矢量代码生成、精细排版感知和动画时序分解等核心设计挑战上仍存在不足。
...
Method: 构建GraphicDesignBench(GDB)基准套件，包含50个任务，按五个维度组织：布局、排版、信息图、模板与设计语义、动画。每个维度都包含理解和生成两种评估设置，基于LICA分层组合数据集中的真实设计模板。采用标准化度量分类法评估空间准确性、感知质量、文本保真度、语义对齐和结构有效性。

Result: 评估前沿闭源模型显示，当前模型在专业设计的核心挑战上表现不足：复杂布局的空间推理、准确的矢量代码生成、精细的排版感知以及动画的时序分解等问题基本未解决。虽然高层语义理解已可达到，但随着任务对精度、结构和组合意识要求的提高，差距显著扩大。

Conclusion: GDB为追踪AI系统向有能力的设计协作伙伴发展提供了严谨、可复现的测试平台。当前模型在专业设计的关键能力上仍有明显差距，特别是在需要精确性、结构性和组合意识的复杂任务中。该评估框架已公开可用。
...
TL;DR: DriveVA是一个自动驾驶世界模型，通过共享潜在生成过程联合解码未来视觉预测和动作序列，利用预训练视频生成模型的先验知识，实现更好的泛化能力和视频-轨迹一致性。
...
Method: 提出DriveVA模型，采用基于DiT的解码器联合预测未来动作序列（轨迹）和视频，引入视频延续策略增强长时滚转一致性，利用预训练大规模视频生成模型继承运动动力学和物理合理性先验。

Result: 在NAVSIM上获得90.9 PDM分数的闭环性能，在nuScenes上平均L2误差和碰撞率分别降低78.9%和83.3%，在基于CARLA v2的Bench2drive上分别降低52.5%和52.4%，展示了零样本能力和跨域泛化能力。

Conclusion: DriveVA通过联合解码视觉预测和动作序列，实现了更好的规划-场景演化对齐，显著提升了自动驾驶世界模型的泛化性能和视频-轨迹一致性。
...
TL;DR: HighFM：首个面向高时间分辨率多光谱地球观测数据的基础模型，利用超过2TB的SEVIRI气象卫星数据，通过改进的掩码自编码框架学习鲁棒的时空表征，在云掩码和活跃火点检测任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 气候相关灾害日益频繁和严重，需要实时监测、早期预警和知情决策。现有基础模型大多依赖高分辨率但低重访率的卫星影像，难以适应快速演变现象和时间紧迫的应急响应需求。

Method: 基于超过2TB的Meteosat第二代卫星SEVIRI影像数据，改进SatMAE掩码自编码框架，加入细粒度时间编码以捕捉短期变化，学习鲁棒的时空表征。预训练模型在云掩码和活跃火点检测任务上进行微调。

Result: SEVIRI预训练的视觉变换器在平衡准确率和IoU指标上均优于传统基线和近期地理空间基础模型，展示了时间密集的地球静止数据在实时地球观测中的潜力。

Conclusion: 高时间分辨率地球静止数据为实时地球观测提供了重要价值，HighFM为灾害检测和跟踪基础模型提供了可扩展的路径，证明了时间密集数据在应对快速演变现象方面的优势。
...
Method: 1) 使用运动感知模块分割并移除动态区域；2) 利用扩散模型修复被遮挡区域，提供伪地面真值监督；3) 为每个高斯基元引入可学习的真实性标量，在渲染和监督过程中动态调节不透明度以实现真实性感知。

Result: 在DAVIS和自建Trajectory-Match数据集上的实验表明，GA-GS在静态场景重建方面达到最先进性能，特别是在具有大规模、持续遮挡的挑战性场景中表现优异。
...
TL;DR: SW-CLIP通过将空间自相关引入视觉-语言对比学习，提出了一种用于街景地理定位的新框架，用距离感知的软监督替代传统CLIP的硬负样本处理


<details>
  <summary>Details</summary>
Motivation: 传统CLIP方法将所有不匹配样本视为同等负样本，忽略了地理空间中的空间自相关特性（地理学第一定律），导致地理定位性能受限

Method: 1) 引入位置作为文本表示编码地理坐标；2) 用基于测地距离的空间加权软标签替代one-hot InfoNCE目标；3) 采用邻域一致性正则化保持嵌入空间的局部空间结构

Result: 在多城市数据集上的实验表明，SW-CLIP显著提高了地理定位精度，减少了长尾误差，增强了空间一致性，优于标准CLIP方法

Conclusion: 研究强调了从语义对齐转向地理对齐对鲁棒地理定位的重要性，为将空间原理整合到多模态表示学习中提供了通用范式
...
TL;DR: RLER提出了一种视频推理的双范式方法，将证据生成与可靠答案获取解耦，通过强化学习训练产生结构化证据，在推理时通过证据加权选举机制选择最佳答案，显著提升视频推理的可靠性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型在视频推理中通常采用单次推理返回答案，缺乏对推理过程是否与证据对齐的验证。这导致推理结果可能不可靠且难以解释，需要一种能够明确产生和使用证据的方法来提高视频推理的信任度。

Method: RLER采用双范式：1) RLER-Training：使用群体相对强化学习和三个新颖的任务驱动奖励（帧敏感奖励、思维透明度奖励、抗重复奖励）优化策略，训练模型产生结构化、机器可检查的证据；2) RLER-Inference：应用无需训练的编排器生成多样化候选答案，解析答案和引用帧，基于证据一致性、置信度、透明度和非冗余性进行评分，最后进行证据加权选举。

Result: 在8个代表性基准测试中，RLER相比各种开源和基于强化学习的大型多模态模型均达到最先进水平，平均提升6.3%，每个问题平均仅使用3.1个候选答案，在计算成本和质量之间取得了良好平衡。

Conclusion: 在训练中明确证据生成，在推理中基于证据进行选举，是实现可信视频推理的稳健路径。RLER方法在不增加模型规模的情况下，显著提高了视频推理的可靠性和可解释性。
...
TL;DR: 该论文提出了UENR-600K数据集，这是一个包含60万对1080p帧的大规模物理基础夜间视频去雨数据集，通过Unreal Engine模拟3D雨滴粒子，解决了现有合成数据集无法捕捉夜间雨滴颜色折射和局部光照的问题，并基于Wan 2.2视频生成模型建立了新的SOTA基线。


<details>
  <summary>Details</summary>
Motivation: 夜间视频去雨面临独特挑战：雨滴与人造光源相互作用，呈现多种颜色和局部光照效果。现有小规模合成数据集依赖2D雨滴叠加，无法捕捉这些物理特性，导致模型在真实夜间雨景中泛化能力差。同时，采集真实配对夜间视频不切实际，因为雨效无法与其他退化（如传感器噪声）分离。

Method: 1. 使用Unreal Engine将雨模拟为虚拟环境中的3D粒子，确保照片级真实感和物理真实的雨滴，捕捉颜色折射、场景遮挡、雨帘等正确细节。2. 构建包含600,000对1080p帧的UENR-600K数据集。3. 基于Wan 2.2视频生成模型建立新基线，将去雨视为视频到视频生成任务，利用强生成先验来弥合模拟到真实的差距。

Result: 1. 创建了UENR-600K大规模物理基础数据集。2. 建立的基线模型几乎完全弥合了模拟到真实的差距。3. 广泛基准测试表明，在该数据集上训练的模型对真实世界视频的泛化能力显著更好。

Conclusion: 通过物理基础的3D雨模拟和高质量大规模数据集，结合视频生成方法，显著提升了夜间视频去雨的性能和泛化能力，为夜间去雨研究提供了新的基准和资源。
...
TL;DR: 提出了BoxComm数据集和评估框架，专注于格斗运动（拳击）的解说生成，包含类别条件生成和节奏评估两个新评测维度，现有MLLMs表现不佳，提出的EIC-Gen基线通过检测击打事件获得提升。
...
Method: 1) 构建BoxComm数据集：包含445场世界拳击锦标赛视频和超过52K句专业解说；2) 提出结构化解说分类法：将每句解说分为实时播报、战术分析或背景信息三类；3) 引入两个新颖评估：类别条件生成（给定视频上下文生成指定类型解说）和节奏评估（评估连续视频段中解说的时序分布和类型分布）；4) 提出EIC-Gen基线：通过检测击打事件提供结构化动作线索。

Result: 实验表明当前最先进的MLLMs在两个评估上都表现不佳。提出的EIC-Gen基线通过整合检测到的击打事件，在所有评估指标上获得一致提升，突显了感知短暂细微事件对格斗运动解说的关键性。

Conclusion: BoxComm填补了格斗运动解说生成基准的空白，提出的结构化分类法和两个互补评估为体育解说生成提供了更全面的评测框架。实验结果表明当前MLLMs在格斗运动解说生成上仍有局限，而整合结构化动作线索能显著提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 虚拟现实需要详细且可定制的3D手部模型，但现有方法昂贵、繁琐且可定制性差。虽然零样本文本到3D合成技术（如SDS）能生成多样化3D模型，但在手部生成中存在结构不自然、视角不一致和细节丢失问题，无法很好泛化到3D手部模型生成。

Method: 1) 使用MANO手部模型初始化提供强结构先验；2) 手部骨架引导的扩散过程确保视角和姿态一致性；3) 提出新颖的校正手部形状引导损失，确保所有视角收敛到视角一致的模式，避免几何失真。

Result: 广泛评估表明该方法在3D手部模型生成方面优于最先进方法，为3D手部模型生成开辟了新途径。

Conclusion: HandDreamer通过解决SDS中的视角不一致问题（主要由文本提示概率景观的模糊性引起），结合MANO初始化、骨架引导扩散和校正形状引导损失，成功实现了高质量、视角一致的零样本文本到3D手部模型生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测方法在大规模预训练下在通用场景表现良好，但在领域转移的下游任务中性能严重下降。这源于领域特定任务中类别标签的稀缺性和弱语义性，以及现有模型无法捕捉粗粒度类别标签之外的辅助语义。
...
Result: 在OV-COCO、多个垂直领域数据集和修改的基准设置上评估，HSA-DINO相比先前最先进方法表现优异，在领域适应性和开放词汇泛化能力之间实现了优越的权衡。

Conclusion: HSA-DINO通过参数高效的语义增强有效解决了开放词汇目标检测在领域转移任务中的性能下降问题，实现了更好的领域适应性和泛化能力的平衡。
...
TL;DR: Chorus是一种利用跨请求相似性的缓存方法，用于加速视频扩散模型推理，相比现有单请求内缓存方法，在4步蒸馏模型上可实现高达45%的加速。
...
Method: Chorus采用三阶段缓存策略：阶段1完全复用相似请求的潜在特征；阶段2在中间去噪步骤的特定潜在区域利用跨请求缓存，并结合Token-Guided Attention Amplification提升生成视频与条件提示的语义对齐，从而将完全复用扩展到后期去噪步骤。

Result: 在工业4步蒸馏模型上实现了高达45%的加速，而先前的单请求内缓存方法在此类模型上效果不佳。

Conclusion: Chorus通过利用跨请求相似性的缓存策略，有效加速了视频扩散模型的推理过程，特别是在现有方法效果有限的4步蒸馏模型上取得了显著性能提升。
...
TL;DR: 该论文提出了一个针对视觉语言模型在自然对抗场景下的系统评估框架，评估了多种VLMs在对抗数据集上的表现，发现鲁棒CLIP模型可能放大自然对抗漏洞，CLIP模型在自然语言诱导对抗样本上性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在零样本迁移方面表现出色，但现有评估主要关注标准基准测试，缺乏对自然对抗场景下模型鲁棒性、局限性和实际适用性的全面独立评估。需要系统评估框架来理解VLMs在真实世界对抗环境中的表现。

Method: 构建系统评估框架，在自然对抗场景下评估多种VLMs（CLIP、鲁棒CLIP、BLIP2、SigLIP2）。使用精心策划的对抗数据集（印刷攻击、ImageNet-A、自然语言诱导对抗样本），测量模型在零样本图像分类、语义分割和视觉问答任务上的自然对抗性能。

Result: 分析显示：1）鲁棒CLIP模型可能放大自然对抗漏洞；2）CLIP模型在自然语言诱导对抗样本上性能显著下降；3）提供可解释分析以识别失败模式。

Conclusion: 该研究揭示了VLMs在自然对抗场景下的脆弱性，强调了超越标准基准进行系统评估的重要性。研究结果旨在启发未来在鲁棒和公平多模态模式识别方面的研究。
...
TL;DR: VicoEdit：一种无需训练和反转的视觉上下文感知图像编辑方法，通过直接转换源图像并采用概念对齐的后验采样来提升编辑一致性和灵活性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉上下文感知图像编辑方法存在数据收集和训练成本高（训练方法）或一致性和灵活性不足（基于扩散反转的无训练方法）的问题，需要一种更高效且性能更好的解决方案。

Method: 提出VicoEdit方法：1）无需训练和扩散反转，直接基于视觉上下文将源图像转换为目标图像；2）设计基于概念对齐的后验采样方法，通过增强编辑一致性来提升性能。

Result: 实验结果表明，这种无训练方法在编辑性能上甚至超越了当前最先进的训练模型，在保持高质量的同时提供了更好的灵活性和一致性。

Conclusion: VicoEdit通过消除训练和反转需求，提供了一种高效且性能优越的视觉上下文感知图像编辑方案，在编辑质量、一致性和灵活性方面均表现出色。
...
TL;DR: 提出基于补丁增强和跨视图正则化的统一防御框架，通过特征表示和输出分布层面的双重约束，在低中毒率下有效抑制多模态大语言模型后门攻击成功率，同时保持正常生成能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在监督微调过程中易受后门植入攻击，一旦特定触发模式被激活，模型会稳定输出攻击者预定义的有害响应。后门防御的核心挑战在于：在低中毒率下抑制攻击成功率的同时保持模型的正常生成能力，这两个目标存在内在冲突。
...
TL;DR: 提出Indra表示假设，认为单模态基础模型的表示收敛于反映现实共享关系结构的表示，类似于因陀罗网的哲学隐喻，通过范畴论形式化并证明其唯一性、完备性和结构保持性，在跨模型和跨模态场景中显著提升鲁棒性和对齐性。


<details>
  <summary>Details</summary>
Motivation: 现有研究发现单模态基础模型倾向于学习收敛的表示，但这些表示本质上是样本的独立内部抽象，表达能力有限。作者受到因陀罗网哲学隐喻的启发，认为这些表示实际上收敛于反映现实共享关系结构的表示，需要理论框架来形式化这一假设。

Method: 使用范畴论中的V-富集Yoneda嵌入形式化Indra表示假设，将每个样本的Indra表示定义为相对于其他样本的关系轮廓。该方法被证明在给定成本函数下具有唯一性、完备性和结构保持性。具体实现使用角距离实例化Indra表示，并在视觉、语言和音频的跨模型和跨模态场景中进行评估。

Result: 大量实验表明，Indra表示在跨架构和跨模态场景中一致地增强了鲁棒性和对齐性。该方法为单模态基础模型的无训练对齐提供了理论基础和实用框架。

Conclusion: 提出的Indra表示假设为理解单模态基础模型表示收敛现象提供了新的理论视角，通过范畴论形式化建立了关系表示框架，实验验证了其在提升跨模型和跨模态对齐方面的有效性，为无训练对齐提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型存在可信度问题，包括过度依赖文本线索而非视觉证据，以及产生无根据或虚构回答的风险，需要提升其推理的解释性和忠实性

Method: 提出Saliency-R1框架：1）引入新颖的显著性映射技术，高效突出对生成token贡献的关键图像区域；2）扩展该技术以追踪视觉信息在推理过程中的流向；3）使用显著性映射与人工标注边界框的重叠作为奖励函数；4）应用组相对策略优化（GRPO）对齐显著部分和关键区域

Result: 实验表明Saliency-R1提高了推理的忠实性、解释性和整体任务性能

Conclusion: Saliency-R1框架有效解决了视觉语言模型的可信度问题，通过显著性映射和GRPO优化实现了更好的视觉-文本对齐和推理透明度
...
TL;DR: MedROI是一个与编解码器无关的ROI中心化医学图像压缩框架，通过轻量级阈值提取组织边界框，丢弃背景体素后再压缩，显著提升压缩比和编解码速度。
...
Method: 提出MedROI框架：1）通过轻量级基于强度的阈值处理提取紧密的组织边界框；2）存储54字节元数据记录以支持空间恢复；3）裁剪后的ROI使用任何现有2D/3D编解码器压缩，无需架构修改或重新训练。

Result: 在ADNI的200个T1加权脑MRI体积上评估，使用6种编解码配置（传统编解码器：JPEG2000 2D/3D、HEIF；神经压缩器：LIC_TCM、TCM+AuxT、BCM-Net、SirenMRI）。MedROI在大多数配置中显著提升压缩比和编解码时间（经多重比较校正的双侧t检验），同时在ROI内保持可比较的重建质量；HEIF是压缩比增益的主要例外。

Conclusion: MedROI是一个有效的ROI中心化压缩框架，通过丢弃背景体素显著提升压缩效率，同时保持诊断相关区域的质量，且与现有编解码器兼容，无需修改或重新训练。
...
TL;DR: 提出StableTTA方法解决集成方法中的聚合策略冲突，提升预测稳定性与效率，在ImageNet-1K上实现显著精度提升，同时大幅降低计算成本。
...
Result: 在ImageNet-1K上获得10.93-32.82%的top-1精度提升，33个模型达到95%以上精度，多个超过96%。轻量架构以少于5%的参数和约89.1%的计算成本降低，超越ViT 11.75%的top-1精度。

Conclusion: StableTTA能够有效解决集成方法中的聚合冲突，在显著提升预测精度的同时大幅降低计算资源需求，使高精度推理在资源受限设备上成为可能。
...
<details>
  <summary>Details</summary>
Motivation: 传统VSLAM中的相对相机位姿估计面临噪声对应点的挑战，经典方法依赖随机假设采样和迭代估计，而基于学习的方法通常缺乏显式几何结构，需要一种结合几何约束和关系推理的新方法。

Method: 将相对位姿估计重构为极线对应图上的关系推理问题：匹配关键点作为节点，相邻点通过边连接。通过图剪枝、消息传递和池化操作估计四元数旋转、平移向量和本质矩阵。使用LoFTR进行无检测器的密集匹配。损失函数包含五个部分：与真实值的L2差异、本质矩阵的Frobenius范数差异、奇异值差异、航向角差异和尺度差异。

Result: 在室内外基准测试中，相比经典方法和学习引导方法，该方法对密集噪声和大基线变化表现出更强的鲁棒性，验证了全局关系共识的有效性。

Conclusion: 通过将相对位姿估计重构为图关系推理问题，结合几何约束和全局关系共识，能够有效处理噪声对应点，在挑战性场景中实现更鲁棒的位姿估计。
...
TL;DR: TILA框架通过时间反转作为监督信号，增强现有时序视觉-语言模型对医学影像（特别是胸片）中方向性变化的敏感性，提升进展分类和时间嵌入对齐性能。


<details>
  <summary>Details</summary>
Motivation: 当前医学基础模型大多孤立分析放射影像，忽略了临床实践中比较前后影像以评估间隔变化的关键任务。对于胸片，捕捉时间变化至关重要，因为放射科医生不仅需要评估发现的静态表现，还需要评估其随时间演变情况。

Method: 提出TILA（时序反转感知学习与对齐）框架，使用时间反转（反转图像对）作为监督信号，在预训练、微调和推理阶段集成反转感知目标，将显式时序顺序学习与常规外观建模相结合。

Result: 在公共数据集和真实医院队列上的实验表明，TILA应用于多种现有架构时，能持续改进进展分类和时间嵌入对齐性能。同时提出了统一的评估协议和MS-CXR-Tretrieval检索评估集。

Conclusion: TILA通过简单而有效的时序反转监督信号，显著增强了现有时序视觉-语言模型对医学影像中方向性变化的敏感性，为临床间隔变化评估提供了更好的解决方案。
...
TL;DR: TAPE是一个用于OCT/OCTA图像分割的两阶段参数高效微调框架，通过域对齐和任务拟合解决基础模型在医学图像中的域偏移和任务不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要大量数据和模型规模，在资源受限的临床环境中部署困难。基于基础模型的迁移学习面临域偏移和任务不匹配的挑战。

Method: 提出TAPE两阶段适应框架：第一阶段使用掩码图像建模进行参数高效微调实现域对齐；第二阶段进行下游分割任务拟合。应用于通用MAE和专用RETFound基础模型。

Result: TAPE在视网膜层分割任务中表现出卓越的参数效率，在多种病理情况下实现了最先进的泛化性能。

Conclusion: TAPE框架通过解耦域对齐和任务拟合，有效解决了医学图像分析中基础模型的适应问题，为资源受限的临床环境提供了高效的解决方案。
...
TL;DR: 系统评估文本到图像扩散模型中概念遗忘对组合生成能力的影响，揭示遗忘效果与组合完整性之间的权衡关系


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注概念遗忘的擦除成功率，但对模型整体生成能力的影响缺乏系统理解，需要评估遗忘方法在组合文本到图像生成中的表现

Method: 针对Stable Diffusion 1.4的裸露内容移除，使用T2I-CompBench++和GenEval等基准测试，系统评估多种最先进遗忘方法在组合生成任务中的表现

Result: 发现遗忘效果与组合完整性之间存在一致权衡：强擦除方法显著损害属性绑定、空间推理和计数能力；而保持组合结构的方法往往无法提供鲁棒擦除

Conclusion: 当前评估实践存在局限性，需要开发能够明确考虑语义保留的遗忘目标，超越单纯的目标抑制
...
TL;DR: 提出PR-IQA框架，用于评估扩散模型生成的多视角图像质量，无需真实标注，通过部分参考图像质量评估和跨注意力质量补全，提升3D高斯溅射重建效果。


<details>
  <summary>Details</summary>
Motivation: 扩散模型可用于稀疏视角新视角合成，为3D高斯溅射等重建方法提供伪真实视图。但生成的图像常存在光度与几何不一致性，直接用作监督会损害重建质量。现有方法缺乏有效的无真实标注质量评估机制。

Method: 提出Partial-Reference Image Quality Assessment (PR-IQA)框架：1) 在重叠区域计算几何一致的部分质量图；2) 通过跨注意力机制进行质量补全，将部分图修复为密集全图质量图，融入参考视图上下文确保跨视角一致性；3) 将PR-IQA集成到扩散增强的3DGS流程中，仅对高质量置信区域进行监督。

Result: PR-IQA在无真实标注情况下达到全参考级别的准确性，优于现有IQA方法。质量感知的3DGS能更有效过滤不一致性，产生更优的3D重建和新视角合成结果。

Conclusion: PR-IQA框架通过部分参考图像质量评估和质量补全，实现了无需真实标注的扩散生成视图质量评估，显著提升了3D高斯溅射在稀疏视角场景下的重建质量。
...
TL;DR: Firebolt-VL：一种高效视觉语言模型，用液态基础模型解码器替代Transformer，并引入Token-Grid相关模块实现细粒度视觉定位，在保持线性时间推理的同时提升效率


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型计算成本高，难以部署到资源受限场景；同时小规模视觉语言模型难以精确捕捉细粒度视觉区域，导致在细粒度推理任务上性能下降
...
Conclusion: Firebolt-VL通过创新的架构设计解决了现有视觉语言模型在效率和细粒度理解方面的局限性，为资源受限场景提供了可行的解决方案
...
TL;DR: 本文提出一种基于物理特征的AIGC检测方法，通过分析15种物理特征并筛选出5种核心特征，将其与CLIP模型结合，在多数据集上实现接近完美的检测准确率。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测器往往过度拟合特定生成模型，无法适应快速发展的AIGC技术。需要重新审视区分自然图像与AI生成图像的内在物理特性，解决两个关键问题：1）哪些物理特征能在不同数据集和生成架构中稳定区分AI生成图像；2）能否将这些像素级特征集成到多模态模型中，在减少语言信息不可靠性的同时提升检测性能。

Method: 1）在20多个由GAN和扩散模型生成的数据集上全面探索15种物理特征；2）提出新颖的特征选择算法，识别出拉普拉斯方差、Sobel统计量和残差噪声方差等5个在所有测试数据集中具有一致判别能力的核心物理特征；3）将这些特征转换为文本编码值，与语义描述结合，指导CLIP中的图像-文本表示学习。

Result: 在多个Genimage基准测试中达到最先进性能，在Wukong和SDv1.4等数据集上实现接近完美的准确率（99.8%）。实验证明该方法能有效区分自然图像与AI生成图像。

Conclusion: 通过将像素级真实性与语义理解相结合，本研究开创了使用物理基础特征进行可信视觉语言建模的先河，为缓解大型多模态模型中的幻觉和文本不准确问题开辟了新方向。
...
TL;DR: GLA提出了一种针对自动驾驶视觉语言模型的新型后门攻击方法，使用自然主义的涂鸦视觉触发器和跨语言文本触发器，在低投毒率下实现高攻击成功率，且不影响模型在干净任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型被集成到自动驾驶等安全关键系统中，传统后门攻击依赖单模态、显式且易检测的触发器，难以在自动驾驶场景中构建隐蔽且稳定的攻击通道。

Method: GLA引入两种自然主义触发器：1) 基于稳定扩散修复生成的涂鸦视觉模式，无缝融入城市场景；2) 跨语言文本触发器，在保持语义一致性的同时引入分布偏移，构建鲁棒的语言侧触发信号。

Result: 在DriveVLM上的实验表明，GLA仅需10%的投毒率即可达到90%的攻击成功率(ASR)和0%的误报率(FPR)。后门不会削弱模型在干净任务上的性能，反而提高了BLEU-1等指标，使传统基于性能下降的检测方法难以识别攻击。
...
TL;DR: InCTRLv2是一个新颖的少样本通用异常检测与分割框架，通过双分支架构结合判别式异常评分学习和单类异常评分学习，在多个数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前大多数异常检测方法是针对特定领域训练的专家模型，需要大量训练样本且难以泛化到未见数据集。为了解决这一局限性，通用异常检测范式应运而生，旨在学习一个单一通用模型来跨领域检测异常而无需重新训练。

Method: InCTRLv2基于先前提出的InCTRL模型，引入了双分支框架：1）主分支使用判别式异常评分学习，利用正常和异常数据学习语义引导的异常与正常空间；2）辅助分支使用单类异常评分学习，仅利用正常数据在语义空间中学习广义正常模式。两个分支都由大规模视觉语言模型编码的丰富视觉-文本语义先验指导。

Result: 在十个异常检测数据集上的广泛实验表明，InCTRLv2在各种设置下的异常检测和分割任务中都实现了最先进的性能。

Conclusion: InCTRLv2通过双分支架构提供了异常检测的双重语义视角：一个强调正常-异常区分，另一个强调偏离正常的语义，从而显著提升了通用异常检测的性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型的快速发展使得合成媒体高度逼真，引发严重的社会担忧。现有检测方法存在关键局限：依赖固定分辨率调整和裁剪等预处理操作，丢弃了细微的高频伪造痕迹，造成空间扭曲和信息损失；且训练评估数据集过时，无法捕捉现代生成模型的复杂性。

Method: 1. 构建大规模数据集：包含来自15个最先进开源和商业生成器的超过140K视频，以及专门评估超逼真合成内容的Magic Videos基准。2. 提出基于Qwen2.5-VL Vision Transformer的检测框架，支持原生可变空间分辨率和时间长度处理，有效保留传统预处理中丢失的高频伪影和时空不一致性。

Result: 广泛实验表明，该方法在多个基准测试中实现卓越性能，证明了原生尺度处理的关键重要性，并为AI生成视频检测建立了稳健的新基线。

Conclusion: 通过原生尺度处理和大规模现代数据集，成功解决了现有AI生成视频检测方法的局限性，为应对合成媒体带来的社会挑战提供了更有效的技术解决方案。
...
TL;DR: 提出Flow Divergence Sampler (FDS)，一种无需训练的框架，通过量化边际速度场的散度来识别和修正样本在流模型生成过程中的误导，从而提高生成质量。


<details>
  <summary>Details</summary>
Motivation: 流模型通过建模边际速度场来学习目标分布，但当样本级速度在中间状态冲突时，平均速度会将样本误导至低密度区域，从而降低生成质量。

Method: 提出Flow Divergence Sampler (FDS)，在求解器每一步之前细化中间状态。关键发现是误导的严重程度可通过边际速度场的散度量化，该散度在推理时可直接计算。FDS利用这一信号将状态引导至较少歧义区域。

Result: FDS作为即插即用框架，与标准求解器和现成的流模型骨干兼容，在各种生成任务（包括文本到图像合成和逆问题）中一致提高了保真度。

Conclusion: FDS通过量化边际速度场的散度来识别和修正流模型中的误导问题，无需额外训练即可显著提高生成质量，为流模型的实际应用提供了有效的改进方案。
...
TL;DR: Synthesis4AD：利用大规模高保真合成异常进行3D异常检测的端到端范式，通过3D-DefectStudio平台生成几何真实的缺陷，结合MLLM解释产品设计信息自动生成异常合成指令，并引入空间分布归一化和几何忠实数据增强的训练流程提升检测器性能。


<details>
  <summary>Details</summary>
Motivation: 工业3D异常检测性能受限于异常样本的稀缺性和长尾分布，现有方法难以获取足够的真实异常数据，限制了检测器的学习能力和泛化性能。

Method: 1. 开发3D-DefectStudio平台，基于可控合成引擎MPAS，通过高维支持基元注入几何真实缺陷并生成精确点级异常掩码；2. 集成多模态大语言模型解释产品设计信息，自动转换为可执行的异常合成指令；3. 提出基于空间分布归一化和几何忠实数据增强的训练流程，缓解Point Transformer对绝对坐标的敏感性。

Result: 在Real3D-AD、MulSen-AD和真实工业零件数据集上实现最先进性能，验证了合成异常的有效性和方法的泛化能力。

Conclusion: Synthesis4AD通过大规模合成异常数据解决了工业3D异常检测的数据稀缺问题，结合知识驱动的合成生成和鲁棒训练策略，显著提升了检测性能，为工业质量检测提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决从超高分辨率无人机图像进行实时深度重建的挑战，包括宽基线视差、大图像尺寸、低纹理表面、遮挡和计算约束，同时克服零样本扩散模型在度量精度和时间一致性方面的不足

Method: 提出ZeD-MAP框架：将流式无人机帧分组为重叠聚类；周期性束调整产生度量一致的位姿和稀疏3D连接点；将这些点重投影到选定帧中，作为扩散深度估计的度量指导

Result: 在约50米高度（地面采样距离约0.85厘米/像素）的DLR MACS系统飞行数据上验证，达到亚米级精度：水平面误差约0.87米，垂直方向误差约0.12米，单帧处理时间1.47-4.91秒

Conclusion: 束调整为基础的度量指导提供了与经典摄影测量方法相当的一致性，同时显著加速处理，实现了实时3D地图生成，平衡了精度与计算效率
...
TL;DR: 该论文提出DenZa-Gaussian方法，将3D高斯泼溅（3DGS）技术应用于稀疏视角ADF-STEM断层扫描，通过可学习的散射强度场、散射视角归一化和傅里叶振幅损失函数，解决了传统方法在有限视角下的重建退化问题。


<details>
  <summary>Details</summary>
Motivation: 传统ADF-STEM断层扫描需要多视角倾斜系列图像进行3D重建，但增加视角会导致电子束长时间照射，对剂量敏感材料造成损伤，并引入漂移和错位问题。稀疏视角采集虽能保护样品，但传统方法在有限视角下重建质量严重下降，产生伪影和结构保真度降低。

Method: 1. 引入可学习的局部散射强度标量场（denza）来匹配3DGS与ADF-STEM成像物理模型；2. 使用系数γ进行散射视角归一化，确保不同倾斜角度下散射强度的一致性；3. 在损失函数中加入2D傅里叶振幅项，抑制稀疏视角重建中的缺失楔形伪影。

Result: 在45视角和15视角倾斜系列上的实验表明，DenZa-Gaussian方法能够生成高保真度的3D重建结果和2D投影，这些投影与原始倾斜图像更吻合，在稀疏视角条件下表现出优越的鲁棒性。

Conclusion: DenZa-Gaussian方法成功地将3D高斯泼溅技术应用于ADF-STEM断层扫描领域，通过物理感知的建模和优化策略，有效解决了稀疏视角重建中的挑战，在保护样品的同时实现了高质量的三维结构表征。
...
<details>
  <summary>Details</summary>
Motivation: 世界模型作为人工智能的重要研究方向，目前缺乏清晰统一的定义，阻碍了该领域的研究进展和应用发展。需要建立一个标准化的框架来统一世界模型的理解、评估和部署。

Method: 基于世界模型的发展演变，提出了明确的世界模型定义：以感知为中心、具备交互和长期记忆能力的模型或框架，用于理解和预测复杂世界。系统分类了世界模型的核心能力，并在此基础上构建了OpenWorldLib框架，集成不同任务的模型，实现高效复用和协作推理。

Result: 开发了OpenWorldLib开源框架，提供了一个统一的世界模型推理平台。该框架能够集成多种任务模型，支持模型复用和协作推理，为世界模型研究提供了标准化工具。

Conclusion: OpenWorldLib为世界模型研究提供了清晰的定义、系统分类和标准化框架，促进了该领域的统一发展。论文还提出了对未来世界模型研究方向的思考和分析，为后续研究提供了指导。
...
TL;DR: 提出自适应KV缓存量化方法，通过数据驱动的控制器动态分配KV缓存精度，减少内存占用和延迟，同时保持接近FP16推理的精度。
...
Method: 提出自适应KV缓存量化框架，提取token级特征（频率、质量分数、注意力方差、基于熵的不确定性），通过紧凑的数据驱动控制器动态选择KV精度（2-bit、4-bit、8-bit、FP16）。

Result: 在SmolLM-135M、360M、1.7B模型上验证，相比静态KV量化减少17.75%解码延迟，提高7.60点精度，仅比FP16推理低0.30点精度。

Conclusion: 自适应KV缓存量化通过动态精度分配优化了精度-延迟权衡，为设备端LLM推理提供了有效的内存和带宽优化方案。
...
TL;DR: 提出过程驱动的图像生成方法，通过多步推理轨迹分解图像合成过程，包含文本规划、视觉草图、文本反思和视觉细化四个阶段，使生成过程显式、可解释且可监督。


<details>
  <summary>Details</summary>
Motivation: 人类绘画是增量式的：先规划全局布局，再绘制粗略草图，然后检查并细化细节，每个步骤都基于不断演变的视觉状态。然而，在文本-图像交错数据集上训练的统一多模态模型是否也能想象中间状态的链条？本文旨在探索这一过程驱动的图像生成范式。

Method: 提出过程驱动的图像生成方法，将合成过程分解为交错的思想和行动推理轨迹。每个迭代包含四个阶段：1) 文本规划（规划视觉状态应如何演变），2) 视觉草图（生成中间图像），3) 文本反思（基于当前视觉状态进行反思），4) 视觉细化（修正图像）。通过密集的逐步监督解决中间状态模糊性问题，对视觉中间状态强制执行空间和语义一致性，对文本中间状态保持先验视觉知识同时识别和纠正违反提示的元素。

Result: 在多个文本到图像生成基准测试中进行了实验验证，结果表明该方法使生成过程变得显式、可解释且可直接监督。

Conclusion: 过程驱动的图像生成方法通过多步推理轨迹分解图像合成过程，使生成过程变得透明、可控且可解释，为多模态模型的增量式创造性过程提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中的图像退化（模糊、噪声、压缩、光照差）严重削弱多模态理解。现有统一多模态模型虽兼具理解与生成能力，但未能利用自身生成能力处理退化输入，原因在于训练机制未要求模型在推理时调用生成，且标准解码-重编码路径不支持有效联合优化。

Method: CLEAR框架包含三个渐进步骤：1) 在退化感知数据集上进行监督微调，建立"生成-再回答"推理模式；2) 潜在表示桥，用可优化的直接连接替代解码-重编码路径；3) 交错GRPO，在答案正确性奖励下联合优化文本推理和视觉生成。构建了MMD-Bench基准，涵盖六个标准多模态基准的三个退化严重程度。

Result: 实验表明CLEAR显著提升退化输入的鲁棒性，同时保持干净图像性能。分析进一步显示，移除像素级重建监督会导致具有更高感知质量的中间视觉状态，表明任务驱动优化与视觉质量自然对齐。

Conclusion: CLEAR成功连接了统一多模态模型的生成与推理能力，通过任务驱动的联合优化有效处理图像退化问题，为退化感知的多模态理解提供了系统解决方案。
...
TL;DR: AvatarPointillist：基于单张肖像图像生成动态4D高斯化身的自回归Transformer框架


<details>
  <summary>Details</summary>
Motivation: 从单张肖像图像生成高质量、可动画的4D高斯化身，解决现有方法在动态建模和细节控制方面的局限性

Method: 使用仅解码器Transformer自回归生成点云用于3D高斯泼溅，动态调整点密度和总数，联合预测每点绑定信息，专用高斯解码器将点转换为完整可渲染的高斯属性

Result: 生成高质量、逼真且可控的化身，实验验证了方法的有效性，自回归公式为化身生成提供了新范式

Conclusion: AvatarPointillist通过自回归点云生成实现了高质量动态4D高斯化身生成，为化身生成领域提供了新范式，代码将开源以促进未来研究
...
TL;DR: MMF-BEV是一个用于自动驾驶的雷达-相机BEV融合框架，通过可变形注意力机制实现跨模态特征对齐，在VoD 4D雷达数据集上优于单模态基线，并在所有物体类别上取得竞争性结果。
...
Result: 在VoD数据集上，MMF-BEV持续优于单模态基线（相机或雷达单独），并在所有物体类别上与现有融合方法相比取得竞争性结果，包括完整标注区域和近距感兴趣区域。

Conclusion: MMF-BEV通过可变形注意力机制有效融合雷达和相机特征，传感器贡献分析提供了模态互补的可解释证据，两阶段训练策略稳定了学习过程，为自动驾驶3D检测提供了有效的多模态融合解决方案。
...
TL;DR: E-VLA框架通过将事件相机数据直接集成到视觉-语言-动作模型中，在低光照、运动模糊等恶劣视觉条件下显著提升了机器人操作的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统基于帧的视觉-语言-动作模型在极端低光、运动模糊和黑裁剪等感知退化条件下表现脆弱，需要增强在恶劣视觉环境下的操作鲁棒性。

Method: 提出E-VLA框架，直接利用事件流中的运动和结构线索，而非重建图像；构建包含DAVIS346事件相机的开源遥操作平台，收集真实世界RGB-事件-动作数据集；设计轻量级、预训练兼容的事件集成策略，研究事件窗口化和融合方法。
...
Conclusion: 事件驱动感知可有效集成到VLA模型中，为超越传统帧式成像的鲁棒具身智能提供了系统证据，简单的参数无关融合方法也能显著提升恶劣条件下的操作性能。
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率细节有时会成为噪声，导致幻觉或推理错误。需要让模型专注于本质结构信息而非分散注意力的纹理细节。

Method: DDP框架包含两个任务：物理属性任务使用80%下采样、结构视觉辅助（白色背景遮罩和正交线）和上下文学习；感知现象任务集成任务分类阶段，结合模糊遮罩、对比度增强和下采样等专门工具。

Result: 实验结果表明"少即是多"：通过有意降低视觉输入并提供有针对性的结构提示，DDP使VLMs能够绕过分散注意力的纹理，在具有挑战性的视觉基准测试中实现更优的推理准确性。

Conclusion: 策略性降低图像保真度并辅以结构提示的DDP框架能有效提升VLMs在VQA任务中的性能，特别是在处理容易导致人类误判或机器易受影响的视觉异常和错觉时。
...
TL;DR: 提出了一种基于一致性模型的粗到细指令条件交互生成框架，通过动态感知策略和碰撞感知引导实现实时的人-物-场景交互生成，并采用混合训练策略解决数据稀缺问题。
...
Method: 提出粗到细指令条件交互生成框架，与一致性模型的迭代去噪过程对齐。采用动态感知策略，利用前一步细化的轨迹更新场景上下文并条件化后续细化。引入碰撞感知引导减少物理伪影，无需细粒度场景几何。设计混合训练策略，通过将体素化场景占用注入HOI数据集合成伪HOSI样本，并与高保真HSI数据联合训练。

Result: 在HOSI和HOI生成任务上均达到最先进性能，对未见场景表现出强泛化能力。

Conclusion: 该方法通过动态感知策略、碰撞感知引导和混合训练策略，有效解决了HOSI生成中的动态推理、物理伪影和数据稀缺问题，实现了高质量、实时的交互生成。
...
TL;DR: 本文首次系统研究了自动驾驶中视觉语言模型的灾难性遗忘问题，提出了首个专门量化该问题的大规模基准数据集，并开发了Drive Expert Adapter框架，通过在提示空间而非权重空间进行适配，有效缓解了灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 将视觉语言模型集成到自动驾驶中虽然能解决长尾场景问题，但面临灾难性遗忘的关键挑战。微调过程在适配驾驶数据的同时会侵蚀模型预训练的世界知识，这与使用VLM的核心目的相悖。目前这一问题尚未得到系统研究。

Method: 提出了Drive Expert Adapter (DEA)框架，将适配从权重空间转移到提示空间。DEA基于场景特定线索动态路由推理到不同的知识专家，在不破坏模型基础参数的情况下提升驾驶任务性能。同时构建了包含18万场景的大规模数据集，创建了首个专门量化自动驾驶中灾难性遗忘的基准。

Result: 实验表明，现有方法存在显著的知识退化问题。提出的DEA方法不仅在驾驶任务上达到最先进性能，而且有效缓解了灾难性遗忘，保留了VLM作为自动驾驶变革力量所必需的泛化能力。数据和模型已公开。

Conclusion: 灾难性遗忘是自动驾驶中VLM集成的关键挑战。通过将适配从权重空间转移到提示空间，DEA框架成功解决了性能提升与知识保留之间的权衡问题，为VLM在自动驾驶中的实际应用提供了有效解决方案。
...
TL;DR: 提出Floorplan Markup Language (FML)通用表示法和基于Transformer的FMLM模型，用于多条件住宅平面图生成，在RPLAN数据集上超越先前任务专用方法。


<details>
  <summary>Details</summary>
Motivation: 现有住宅平面图自动生成方法存在局限性：早期基于约束满足或组合优化的方法缺乏多样性和灵活性；近期生成模型虽取得进展，但难以泛化到异构条件任务（如从场地边界、房间邻接图或部分布局生成），这主要源于次优的表示方法。

Method: 提出Floorplan Markup Language (FML)通用表示法，将平面图信息编码为结构化语法，将整个生成问题转化为下一个token预测任务。基于FML开发了Transformer生成模型FMLM，能够处理多样化的条件输入生成高质量、功能性的平面图。

Result: 在RPLAN数据集上的综合实验表明，FMLM虽然是单一模型，但在多种条件任务下均超越了先前任务专用的最先进方法，生成高保真且功能性的平面图。

Conclusion: FML表示法和FMLM模型解决了住宅平面图生成中的表示和泛化问题，通过统一的结构化语法和Transformer架构实现了跨异构条件任务的高性能生成，为自动空间设计提供了更高效和通用的解决方案。
...
TL;DR: 提出基于图像块级别的幻觉检测框架，通过分析细粒度token级交互来检测LVLM中的幻觉对象，相比现有全局方法更有效


<details>
  <summary>Details</summary>
Motivation: 现有的大视觉语言模型幻觉检测方法主要依赖全局图像度量，但幻觉token可能在多个局部区域有微弱但广泛的关联，这些关联聚合后会形成欺骗性的高相关性，从而逃避全局检测器

Method: 基于关键观察（忠实对象token必须牢固地锚定在特定图像区域），提出图像块级别的幻觉检测框架，分析模型各层的细粒度token级交互，识别幻觉token的两个特征：1）产生扩散的非局部化注意力模式；2）与任何视觉区域缺乏有意义的语义对齐

Result: 开发了轻量级可解释的检测方法，利用图像块级统计特征结合隐藏层表示，在token级幻觉检测中达到高达90%的准确率

Conclusion: 细粒度结构分析在检测幻觉方面具有优越性，图像块级别的检测框架比全局方法更有效，能够识别幻觉token的特定行为模式
...
TL;DR: Free-Range Gaussians：一种多视角重建方法，通过流匹配预测非像素、非体素对齐的3D高斯分布，仅需4张图像即可生成，能合成未观测区域的合理内容。


<details>
  <summary>Details</summary>
Motivation: 现有方法产生高度冗余的网格对齐高斯分布，在未观测区域存在空洞或模糊问题。需要一种能从稀疏输入生成高质量3D重建并合成合理未观测内容的方法。

Method: 1. 使用高斯参数流匹配进行生成式重建；2. 引入分层分块方案将空间相关高斯分组为联合transformer tokens；3. 训练时使用时间步加权渲染损失；4. 推理时采用光度梯度引导和分类器无关引导。

Result: 在Objaverse和Google Scanned Objects数据集上，相比像素和体素对齐方法，使用显著更少的高斯分布获得一致改进，在输入视图未覆盖物体部分时优势更大。

Conclusion: Free-Range Gaussians通过生成式流匹配方法实现了从稀疏视图的高质量3D重建，解决了网格对齐方法的冗余和未观测区域问题，同时通过分层分组和引导技术提升了效率与保真度。
...
TL;DR: DIRECT框架将视频混剪创作建模为多模态一致性满足问题，通过分层多智能体系统模拟专业制作流程，显著提升视觉连续性和听觉对齐质量。


<details>
  <summary>Details</summary>
Motivation: 现有自动化视频编辑框架往往忽视跨层级的多模态协调，导致序列脱节、视觉过渡突兀和音乐对齐不佳，无法达到专业级流畅度。

Method: 将视频混剪建模为多模态一致性满足问题，提出DIRECT分层多智能体框架：Screenwriter负责源感知全局结构锚定，Director实例化自适应编辑意图和指导，Editor进行意图引导的镜头序列编辑和细粒度优化。

Result: DIRECT在客观指标和人类主观评价中均显著优于现有最先进基线方法，并引入了包含视觉连续性和听觉对齐定制指标的Mashup-Bench基准测试。

Conclusion: DIRECT框架通过模拟专业制作流程的多层级协调机制，有效解决了视频混剪中的多模态一致性问题，为自动化视频编辑提供了新的解决方案。
...
TL;DR: HorizonWeaver是一个用于自动驾驶场景编辑的框架，通过大规模数据集、语言引导掩码和联合训练损失，解决了密集驾驶场景中的多层次粒度、丰富语义和领域偏移挑战，在真实性和可控性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶安全需要超越真实世界测试的可扩展、真实可控的驾驶场景生成。现有基于指令的图像编辑方法在处理密集、安全关键的驾驶布局时存在困难，需要解决多层次粒度、丰富语义和普遍领域偏移三大挑战。

Method: 1. 数据：从Boreas、nuScenes和Argoverse2构建大规模配对真实/合成数据集以提升泛化能力；2. 模型：语言引导掩码实现细粒度编辑，通过语义丰富的掩码和提示实现精确的语言引导编辑；3. 训练：内容保持和指令对齐，使用联合损失强制场景一致性和指令保真度。

Result: 收集了13个编辑类别共255K图像，在L1、CLIP和DINO指标上优于先前方法，获得+46.4%的用户偏好，BEV分割IoU提升+33%。

Conclusion: HorizonWeaver为复杂驾驶场景提供了可扩展的、逼真的指令驱动编辑框架，解决了自动驾驶场景编辑中的关键挑战，显著提升了编辑质量和用户偏好。
...
TL;DR: 提出SpatialEdit-Bench基准、SpatialEdit-500k合成数据集和SpatialEdit-16B基线模型，用于评估和改进图像空间编辑能力，重点关注几何驱动的对象布局和相机视角控制。


<details>
  <summary>Details</summary>
Motivation: 当前图像编辑模型在细粒度空间操作（如对象布局和相机视角控制）方面能力不足，缺乏专门的评估基准和训练数据，需要建立系统化的评估框架和可扩展的训练资源。

Method: 1) 创建SpatialEdit-Bench基准，通过视角重建和构图分析联合评估感知合理性和几何保真度；2) 使用可控Blender流水线生成SpatialEdit-500k合成数据集，包含多样化背景和系统相机轨迹；3) 基于该数据开发SpatialEdit-16B基线模型用于细粒度空间编辑。

Result: SpatialEdit-16B在通用编辑任务上表现具有竞争力，在空间操作任务上显著优于现有方法。所有资源将在GitHub公开。

Conclusion: 该工作提供了完整的图像空间编辑评估框架、大规模训练数据和基线模型，为几何驱动的图像编辑研究奠定了基础，解决了现有模型在细粒度空间操作方面的不足。
...
TL;DR: DeltaWorld：一种基于特征差异标记的生成式世界模型，通过将视频帧间特征差异编码为连续"delta"标记，大幅降低计算复杂度，实现高效的多假设未来预测。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型面临两大挑战：判别式模型产生确定性预测（隐含了对可能未来的平均），而生成式模型计算成本高昂。虽然已有研究证明在视觉基础模型特征空间进行预测可减少参数，但多数方法仍是判别式的。需要一种既能生成多样化未来预测又计算高效的生成式世界模型。

Method: 提出DeltaTok标记器将连续帧间的VFM特征差异编码为单个连续"delta"标记，将视频从三维时空表示压缩为一维时间序列。基于此构建DeltaWorld生成式世界模型，利用紧凑表示实现可处理的多假设训练：并行生成多个未来，仅监督最佳预测。推理时单次前向传播即可产生多样化预测。

Result: 在密集预测任务实验中，DeltaWorld生成的未来预测更接近真实世界结果，同时参数比现有生成式世界模型少35倍以上，FLOPs减少2000倍。紧凑表示（如512×512帧可实现1024倍标记减少）使高效多假设训练成为可能。

Conclusion: DeltaWorld通过特征差异标记化实现了高效且多样化的未来预测，在保持生成能力的同时大幅降低了计算成本，为视频世界建模提供了新的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的修复方法主要依赖微调或Control-Net模块来利用预训练模型的先验知识，但预训练扩散模型本身可能已具备修复能力，只是需要通过适当方式解锁

Method: 提出在扩散桥框架中学习提示嵌入，使训练和推理动态对齐，确保从噪声退化状态到干净图像的一致去噪路径；在预训练的WAN视频模型和FLUX图像模型上应用轻量级学习提示

Result: 方法在多种退化类型上实现了竞争性性能和良好泛化能力，避免了模型微调和修复专用控制模块的使用

Conclusion: 预训练扩散模型具有固有的修复行为，可通过学习提示嵌入解锁；在扩散桥框架中训练提示能解决训练-推理不对齐问题，实现高效的全能修复
...
TL;DR: SimpleProc：一种完全程序化的多视图立体视觉训练数据生成器，使用少量NURBS规则和基本位移/纹理模式，在小规模（8,000图像）上优于人工标注数据，在大规模（352,000图像）上达到甚至超过692,000人工标注数据的性能。


<details>
  <summary>Details</summary>
Motivation: 探索多视图立体视觉（MVS）程序化规则的设计空间，解决人工标注训练数据成本高、规模有限的问题，通过程序化方法生成高质量训练数据。

Method: 提出SimpleProc：完全程序化的数据生成器，使用少量规则驱动，基于非均匀有理B样条（NURBS）以及基本位移和纹理模式生成训练数据。

Result: 1. 在8,000图像规模下，SimpleProc生成的数据优于同规模游戏和真实物体的人工标注数据；2. 在352,000图像规模下，性能达到甚至超过使用692,000人工标注图像训练的模型。

Conclusion: 程序化数据生成是MVS训练的有效方法，SimpleProc能以较小规模达到甚至超过大规模人工标注数据的性能，为计算机视觉任务提供高效的数据生成解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型通常使用大型语言模型作为解码器，通过自回归生成响应标记，输出标记数量成为端到端延迟的瓶颈。不同模型需要不同数量的输出标记才能达到可比性能，需要探索更高效的推理方法。

Method: 提出多智能体推理框架，保持大模型生成简短响应，但在必要时从小模型转移关键推理标记。通过重用小模型的推理标记，帮助大模型接近其自身推理的性能。

Result: 实验表明，具有较少输出标记的大型模型可能比具有长输出序列的小型模型更高效。在多样化真实世界基准测试中，大模型可以用显著更少的输出标记实现更好或可比的性能。通过重用小模型的推理标记，可以接近大模型自身推理的性能。

Conclusion: 多智能体推理框架通过结合大模型的效率和从小模型转移的关键推理标记，有效解决了视觉语言模型中输出标记数量导致的延迟瓶颈问题，实现了更高效的推理性能。
...
TL;DR: LoMa：通过大规模数据混合、现代训练方法、模型容量扩展和计算资源扩展，显著提升局部特征匹配性能，并在多个基准测试中大幅超越现有方法


<details>
  <summary>Details</summary>
Motivation: 局部特征匹配作为3D视觉系统的基础组件，其发展滞后于现代数据驱动方法。现有方法仅在少数中型数据集上训练，而新方法如前馈重建模型已从大规模数据集中受益。需要从数据驱动角度重新审视局部特征匹配，解决基准测试饱和问题

Method: 提出LoMa方法，结合：1）大规模多样化数据混合；2）现代训练方法；3）扩展模型容量；4）扩展计算资源。同时收集1000个高难度图像对构建HardMatch数据集，通过人工标注获取真实对应关系
...
Conclusion: 通过数据驱动方法、大规模训练和模型扩展，局部特征匹配性能得到显著提升。HardMatch数据集解决了现有基准测试饱和问题，为未来研究提供了更具挑战性的评估平台
...
TL;DR: Vanast是一个统一框架，直接从单张人体图像、服装图像和姿态引导视频生成服装转移的人体动画视频，解决了传统两阶段方法中的身份漂移、服装变形和前后不一致问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法将基于图像的虚拟试穿和姿态驱动动画作为两个独立过程处理，导致身份漂移、服装变形和前后不一致问题。需要一种统一框架来直接生成连贯的服装转移动画。

Method: 1. 构建大规模三元组监督数据：生成身份保持的换装人体图像，捕捉完整上下服装三元组，组装多样化野外三元组；2. 引入双模块架构的视频扩散变换器，稳定训练、保持预训练生成质量，提升服装准确性、姿态遵循和身份保持，支持零样本服装插值。

Result: Vanast能够生成高保真、身份一致的动画，支持广泛的服装类型，在服装准确性、姿态遵循和身份保持方面表现优异。

Conclusion: Vanast通过统一框架解决了传统两阶段方法的局限性，实现了从单张图像直接生成连贯的服装转移动画，为虚拟试穿和人体动画提供了更高效的解决方案。
...
TL;DR: 训练代码大语言模型通过逐步模拟程序执行来提升竞争性编程性能，结合监督微调和强化学习，利用执行反馈进行自我验证和修复
...
Conclusion: 代码LLMs可以被训练来模拟程序执行，这种能力可用于提升竞争性编程性能，支持自我验证和迭代修复
...
TL;DR: 提出一种基于约束最大似然估计的新方法，用于准确估计大语言模型的失败率，结合小规模人工标注、大规模LLM评判标注和领域特定约束信息


<details>
  <summary>Details</summary>
Motivation: 当前LLM失败率估计面临两难选择：昂贵的人工标注金标准与可能存在严重偏差的自动标注方案（如"LLM-as-a-Judge"）。需要一种实用、高效且准确的失败率估计方法，以支持LLM的安全部署

Method: 提出基于约束最大似然估计（MLE）的方法，整合三个信号源：1）小规模高质量人工标注校准集；2）大规模LLM评判标注；3）通过领域特定约束获得的额外侧信息（基于评判性能统计的已知边界）

Result: 通过全面的实证研究验证方法有效性，在多种实验场景下（不同评判准确率、校准集大小、LLM失败率）均优于现有方法（如预测驱动推理PPI），提供更准确、更低方差的估计

Conclusion: 该方法超越了"黑盒"使用自动评判的方式，提供了一个原则性、可解释且可扩展的框架，为LLM失败率认证提供了新途径
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型参数规模庞大导致部署困难，现有方法需要特殊硬件支持或昂贵的后训练来保持模型质量，需要一种高效且经济实惠的模型压缩方法。

Method: SoLA结合软激活稀疏性和低秩分解，识别并保留对推理贡献显著的小部分组件，同时通过低秩分解压缩大部分组件。采用自适应组件级低秩分配策略为不同权重矩阵分配适当的截断位置。

Result: 在LLaMA-2-7B/13B/70B和Mistral-7B模型上的实验表明，SoLA在语言建模和下游任务准确率上都有显著提升，无需后训练。例如，在LLaMA-2-70B模型上以30%压缩率，困惑度从6.95降至4.44，下游任务准确率提升10%。

Conclusion: SoLA是一种有效的无需训练的大语言模型压缩方法，能够显著减少模型规模同时保持甚至提升模型性能，为高效部署提供了实用解决方案。
...
TL;DR: Focus是一种通过可学习质心将token分组的方法，限制远距离注意力只在同组token对之间进行，而局部注意力保持全分辨率。该方法保持所有模型权重冻结，仅训练少量参数（最低148K），在保持下游任务性能的同时提升领域困惑度，支持从124M到70B的各种注意力架构。


<details>
  <summary>Details</summary>
Motivation: 现有高效注意力方法在保持模型权重不变的"改造"设置中难以同时实现性能提升和计算效率。需要一种方法能够学习哪些token对真正重要，而不是近似所有token对，同时保持模型对齐性和下游任务性能。
...
Result: 在124M参数模型中，Focus超越完整注意力（30.3 vs 31.4 PPL）；在7B规模从头训练（2B tokens）时再次击败完整注意力（13.82 vs 13.89 PPL）。推理时通过硬稀疏模式实现2倍加速，同时超越预训练基线（41.3 vs 42.8 PPL）。使用标准FlashAttention调用达到8.6倍加速。与LoRA不同，质心路由保持对齐性，指令调优模型在适应后保持TruthfulQA分数。

Conclusion: Focus是一种纯加性的高效注意力方法，通过可学习质心路由实现token分组，在保持模型权重冻结的情况下显著提升性能。该方法在改造设置中优于现有方法，支持多种模型规模，保持模型对齐性，并能发现无监督的语义类别。
...
TL;DR: VIGIL是首个实时检测和缓解认知偏见触发器的浏览器扩展，通过LLM重新表述和隐私分层推理保护在线信息完整性


<details>
  <summary>Details</summary>
Motivation: 生成式AI对在线信息完整性和公民讨论构成风险，除了明显的虚假信息外，更隐蔽的威胁是利用人类认知偏见进行说服或操纵。目前缺乏直接检测和缓解这类认知偏见触发器的工具。

Method: 开发VIGIL浏览器扩展，提供实时认知偏见触发器检测和缓解功能，包括滚动同步检测、LLM驱动的可逆重新表述、隐私分层推理（从完全离线到云端），并设计为可扩展的第三方插件架构。

Result: VIGIL是首个此类工具，已开源发布，包含多个经过NLP基准严格验证的插件，实现了实时检测和缓解认知偏见触发器的功能。

Conclusion: VIGIL填补了检测和缓解认知偏见触发器工具的空白，为保护在线信息完整性和公民讨论提供了新的技术解决方案，其可扩展架构支持未来功能增强。
...
TL;DR: 提出Knowledge Packs：预计算的KV缓存，以零token成本提供与RAG相同的知识，通过正确的聊天模板格式实现完全等价，同时支持行为引导功能。


<details>
  <summary>Details</summary>
Motivation: RAG方法消耗大量token，而KV缓存理论上可以在不消耗额外token的情况下提供相同知识，但现有研究对KV缓存与RAG的等价性存在争议，需要验证其实际效果。

Method: 利用因果变换器的特性：在文本F上的前向传播产生的KV缓存与在F+q上的联合传播结果相同（因果掩码保证）。通过正确的聊天模板格式确保等价性，并利用RoPE旋转键但不影响值的特性，在缓存值上应用对比性delta来引导模型行为。

Result: 正确格式化下，Qwen3-8B和Llama-3.1-8B在700个问题上实现零差异，节省高达95%的token。行为引导效果位于中间层值（33-66%），独立方向几乎正交（cos~0）且可组合，知识和引导通道在alpha≤0.7时无干扰同时运行。

Conclusion: Knowledge Packs提供与RAG相同的知识但零token成本，同时支持无需训练或权重修改的行为引导，格式正确性对性能至关重要，错误格式会导致6-7pp的性能下降。
...
TL;DR: CresOWLve：基于现实世界知识的创造性问题解决基准，评估LLMs整合多领域知识进行非明显创造性连接的能力


<details>
  <summary>Details</summary>
Motivation: 现有基准大多只评估创造性问题解决的特定组成部分，且依赖人工构造的脑筋急转弯或人为场景，无法反映真实世界中的创造性问题解决过程

Method: 引入CresOWLve基准，使用基于现实世界知识的谜题，要求模型运用多种创造性思维策略、检索多领域事实并创造性整合信息

Result: CresOWLve对前沿非思维和思维LLMs仍具高度挑战性；模型在事实性问题上的表现显著优于创造性问题（最多下降17%）；模型能检索相关知识但难以形成非明显的创造性连接

Conclusion: 需要开发专门针对创造性连接形成能力的评估方法和模型改进，以提升LLMs在真实世界创造性问题解决中的表现
...
TL;DR: 研究探索了噪声引导技术，通过在推理时向transformer模型内部表示注入校准的高斯扰动，作为无训练多样性方法，用于生成阿拉伯语早期阅读评估的多样化教学有效故事。


<details>
  <summary>Details</summary>
Motivation: 为阿拉伯语早期阅读评估生成多样化且教学有效的故事面临挑战：需要在词汇、阅读水平和叙事结构的严格约束与避免重复情节（损害评估有效性）之间取得平衡。需要一种能在保持约束的同时增加多样性的方法。

Method: 研究噪声引导技术，在推理时向transformer模型的内部表示注入校准的高斯扰动。比较了四种注入策略与高温采样基线：残差流噪声、注意力熵噪声注入（AENI）等。在五个小型阿拉伯语中心语言模型（7-9B参数）上进行评估，测量多样性、质量、约束遵守和阅读年级水平。

Result: 残差流噪声在所有模型中一致提高叙事多样性，同时保持质量和约束成本最小，并保留早期阅读水平。AENI稳定了原本不可靠的注意力-逻辑噪声，同时恢复质量。高温采样会提高阅读年级水平，并在多个模型上导致灾难性崩溃。内部表示级扰动比输出级随机性更适合受限教育内容生成。

Conclusion: 对于受约束的教育内容生成，内部表示级扰动是比输出级随机性更合适的多样性策略。噪声引导作为无训练方法，能有效平衡阿拉伯语早期阅读评估故事的多样性需求与教学约束要求。
...
TL;DR: QIMMA是一个质量保证的阿拉伯语大语言模型排行榜，通过多模型评估管道结合自动LLM判断和人工审查，系统性地验证和修复阿拉伯语基准测试的质量问题，构建了包含5.2万个样本的精选评估套件。


<details>
  <summary>Details</summary>
Motivation: 现有阿拉伯语基准测试存在系统性质量问题，需要建立质量保证的评估框架来提升阿拉伯语NLP评估的可靠性和准确性。

Method: 采用多模型评估管道，结合自动LLM判断和人工审查，对现有阿拉伯语基准进行系统性质量验证和修复；使用LightEval和EvalPlus实现透明评估，并公开每个样本的推理输出。

Result: 构建了包含超过5.2万个样本的精选多领域、多任务评估套件，主要基于原生阿拉伯语内容（代码评估任务除外）；提供了可复现和社区可扩展的阿拉伯语NLP评估基础。

Conclusion: QIMMA通过系统性的基准验证和质量保证机制，为阿拉伯语LLM评估提供了可靠、透明和可扩展的框架，有助于推动阿拉伯语NLP研究的发展。
...
TL;DR: 该论文提出了一种基于形态驱动的标记模型，通过模块化认知集合组织名词，解释语言内部和跨语言的名词标记差异，并重新评估标记性和状态概念。


<details>
  <summary>Details</summary>
Motivation: 所有语言都有名词范畴，但其实现方式差异很大。不同语言中语义和/或形态句法差异的显著程度不同。论文旨在探索这些变异，以Riffian语为参考点，然后将分析扩展到其他语言。

Method: 提出形式化模型"形态驱动标记"。名词被组织成模块化认知集合，每个集合有自己的形态模板和无标记形式。将这种模式置于句法功能中，重新评估标记性和状态概念。

Result: 该模型有助于解释语言内部和跨语言的名词类型标记差异。提出将状态概念扩展到所有综合语言，并将其分析为基于句法的屈折变化的新子类别，类似于一致关系和语法格。

Conclusion: 形态驱动标记模型为理解名词范畴的跨语言变异提供了理论框架。状态概念应被重新概念化为所有综合语言中基于句法的屈折变化的一个子类别，这有助于更系统地分析名词标记模式。
...
TL;DR: 提出词汇丢弃方法解决自对弈中提议者多样性崩溃问题，通过随机掩码保持问题多样性，提升求解器在数学推理任务上的性能


<details>
  <summary>Details</summary>
Motivation: 在语言模型协同进化的自对弈中，提议者会快速收敛到满足奖励函数的狭窄问题分布，导致多样性崩溃，使课程对求解器失去信息价值，阻碍协同进化循环

Method: 引入词汇丢弃方法，在策略训练和课程生成期间对提议者的输出logits应用随机硬掩码，该掩码是非平稳的，防止提议者锁定固定的标记序列

Result: 在Qwen3-4B和Qwen3-8B模型上使用R-Zero进行数学推理训练，词汇丢弃在词汇、语义和功能指标上持续保持提议者多样性，8B模型平均提升+4.4分，在竞赛级基准上获得最大增益

Conclusion: 显式的动作空间约束（类似于经典自对弈中游戏规则的结构性作用）有助于维持语言中富有成效的协同进化，词汇丢弃是该原则的一个简单实例
...
Method: 采用LLM驱动的进化搜索自动发现无监督UQ方法，这些方法以Python程序形式表示。通过进化算法让LLM生成、评估和优化UQ方法。

Result: 在原子声明验证任务上，进化发现的方法在9个数据集上相对ROC-AUC提升达6.7%，且具有鲁棒的外分布泛化能力。不同LLM展现出不同的进化策略：Claude模型倾向于设计高特征数量的线性估计器，而GPT-oss-120B偏好更简单、可解释的位置加权方案。

Conclusion: LLM驱动的进化搜索是自动化、可解释的幻觉检测器设计的有前景范式，能够发现超越人工设计基线的方法，但不同LLM在利用方法复杂性方面存在差异。
...
TL;DR: 提出评估LLM文化对齐的人类中心框架，通过文化重要性向量和文化表征向量测量模型输出与本地人口文化认知的匹配度，发现模型存在西方中心校准和系统性错误模式。


<details>
  <summary>Details</summary>
Motivation: 现有LLM文化代表性评估主要关注文化多样性和事实准确性，但缺乏对文化对齐的评估——即生成内容在多大程度上反映本地人口对自己文化方面的认知和优先级。

Method: 1) 基于9个国家开放式调查建立人类衍生的文化重要性向量基准；2) 提出基于语法多样化提示集计算模型衍生文化表征向量的方法，应用于三个前沿LLM；3) 分析人类与模型向量之间的对齐度。

Result: 发现某些模型存在西方中心校准，随着国家与美国文化距离增加，对齐度降低；识别出所有模型高度相关的系统性错误特征(ρ>0.97)，过度索引某些文化标记而忽视深层社会和价值优先级。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于表示级引导的语言控制方法需要昂贵的多语言或平行数据来识别残差流中的语言特定方向，而稀疏自编码器（SAE）虽然提供了可解释的特征基础，但同样面临数据约束问题

Method: LangFIR通过随机标记序列过滤语言无关特征：使用少量单语数据和随机标记序列，识别SAE特征中持续被目标语言输入激活但不编码语言身份的特征，过滤这些语言无关特征以分离出稀疏的语言特定特征集

Result: LangFIR发现的特征极其稀疏、对目标语言高度选择性且具有因果重要性；在三个模型（Gemma 3 1B/4B、Llama 3.1 8B）、三个数据集和12种目标语言上实现了最佳平均准确率BLEU，优于最强的单语基线，甚至超过依赖平行数据的方法

Conclusion: 多语言LLM中的语言身份定位在稀疏的特征方向集中，这些特征可以通过单语数据发现；LangFIR为可靠的语言控制提供了一种高效的数据节约方法
...
TL;DR: 提出树状结构扩散语言模型，通过词汇树分解指数级降低分类维度，减少预测头参数和GPU内存使用，在相同参数预算下内存减半且性能相当


<details>
  <summary>Details</summary>
Motivation: 离散扩散语言模型训练时，全词汇表预测层占用大量参数（小型DiT设计中超20%）并主导GPU峰值内存使用，导致参数和内存利用效率低下

Method: 利用词汇间固有结构构建树状结构扩散语言模型，将扩散过程建模为词汇树中祖先节点的中间潜在状态，通过树状分解指数级降低分类维度

Result: 相同参数预算下，方法将峰值GPU内存使用减少一半，同时匹配最先进离散扩散语言模型的困惑度性能

Conclusion: 树状结构扩散语言模型通过词汇树分解有效解决了全词汇表预测层的参数和内存效率问题，实现了更高效的训练
...
TL;DR: MultiPress：用于多模态新闻分类的三阶段多智能体框架，通过专门的多模态感知、检索增强推理和门控融合评分智能体，结合奖励驱动的迭代优化机制，显著提升分类准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 随着多模态新闻内容的日益普及，有效的新闻主题分类需要能够联合理解和推理文本、图像等异构数据的模型。现有方法通常独立处理各模态或采用简单的融合策略，限制了其捕捉复杂跨模态交互和利用外部知识的能力。

Method: 提出MultiPress框架，包含三个阶段：1）多模态感知智能体处理文本和图像；2）检索增强推理智能体利用外部知识进行推理；3）门控融合评分智能体整合信息。采用奖励驱动的迭代优化机制进行模型优化。

Result: 在新构建的大规模多模态新闻数据集上验证，MultiPress相比强基线方法取得显著改进，展示了模块化多智能体协作和检索增强推理在提升分类准确性和可解释性方面的有效性。

Conclusion: MultiPress框架通过专门的多智能体协作和检索增强推理，有效解决了多模态新闻分类中的跨模态交互和知识利用问题，为多模态新闻理解提供了新的解决方案。
...
TL;DR: 论文提出FinLongDocQA数据集和FinLongDocAgent方法，解决大语言模型在长文档金融数值推理中的局限性，特别是跨表数值计算问题。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型具有强大的语言理解能力，但在长结构化文档（特别是金融年报）的可靠问答方面仍存在困难。金融报表分析需要准确的算术计算，分析师需要整合分散在多个表格和叙述文本中的证据来推导关键指标。现有基准主要关注单表设置，跨表文档级数值推理研究不足。

Method: 提出FinLongDocQA数据集用于单表和跨表金融数值推理；开发FinLongDocAgent方法，采用多智能体多轮检索增强生成（RAG）方法，迭代检索证据、执行中间计算并跨轮验证结果。

Result: 评估闭源和开源大语言模型在FinLongDocQA上发现两个瓶颈：1) 年报常超过129k tokens，加剧了定位相关表格的上下文衰减问题；2) 即使找到相关证据，大语言模型在多步数值推理中仍易出错。实验表明迭代检索和验证对长金融文档中可靠数值问答至关重要。

Conclusion: FinLongDocQA填补了跨表文档级数值推理的空白，FinLongDocAgent通过多智能体多轮RAG方法有效解决了长金融文档中的数值推理挑战，强调了迭代检索和验证的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 可解释性研究强调需要根据明确的语言学理论评估预训练语言模型，特别是上下文嵌入编码的语言信息。本研究聚焦意大利语NPN构式家族，挑战先前实验设计的理论和方法假设，并将此类研究扩展到较少被研究的语言

Method: 从BERT中提取上下文向量表示，作为分层探测分类器的输入，系统评估模型内部各层编码的信息

Result: 结果揭示了构式形式和意义在上下文嵌入中的反映程度，为构式主义理论与神经语言建模之间的对话提供了实证证据

Conclusion: 该研究通过系统评估BERT对意大利语NPN构式的编码能力，为语言模型可解释性研究提供了方法论改进，并扩展了跨语言构式分析的研究范围
...
<details>
  <summary>Details</summary>
Motivation: 当前掩码扩散语言模型(dLMs)虽然具有双向去噪能力，但由于监督微调(SFT)中仅对响应进行掩码的惯例，无法有效填充提示模板。这种限制是训练实践的产物，而非架构限制

Method: 扩展SFT中的全序列掩码训练，同时对提示和响应进行联合掩码。解锁后，模型能够在少量示例条件下填充提示模板的掩码部分

Result: 模型填充的提示模板性能匹配或超越人工设计模板，能有效跨模型迁移，并与现有提示优化方法互补
...
TL;DR: LightThinker++通过显式自适应内存管理，动态压缩LLM推理过程中的中间思维表示，显著降低计算开销同时提升长程推理性能


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂推理中面临长思维轨迹带来的认知开销激增问题，静态压缩方法在处理复杂推理时因中间细节的不可逆丢失可能导致逻辑瓶颈

Method: 提出LightThinker++框架，引入显式自适应内存管理范式，通过行为级管理和显式内存原语，配合专门的轨迹合成管道训练有目的的内存调度策略

Result: 实验显示：1) LightThinker降低峰值token使用70%，推理时间26%，精度损失极小；2) LightThinker++在标准推理中降低峰值token使用69.9%，相同上下文预算下精度提升2.42%；3) 在长程智能体任务中，80轮后保持稳定内存占用(降低60-70%)，平均性能提升14.8%

Conclusion: 该工作为在最小开销下维持深度LLM长程推理提供了可扩展方向，通过动态压缩中间思维表示实现效率与性能的双重提升
...
TL;DR: 批量处理和变量堆叠可显著降低LLM文本分类的API调用成本，在合理范围内不会显著降低编码质量


<details>
  <summary>Details</summary>
Motivation: 当前社会科学研究中，研究者通常对每个文本的每个变量使用单独的API调用，导致大规模文本分类成本高昂。需要验证批量处理和变量堆叠是否会影响编码质量。

Method: 测试了来自四个提供商的八个生产级LLM，使用3,962条专家编码的推文，涵盖四个分类任务。通过改变批量大小（1到1000项）和堆叠维度（最多25个编码维度），比较不同配置下的编码准确性。

Result: 八个模型中的六个在批量大小达到100时，准确性保持在单项目基准的2个百分点内。变量堆叠最多10个维度时，结果与单变量编码相当。性能下降主要由任务复杂性而非提示长度驱动。在安全操作范围内，批量处理和堆叠带来的测量误差小于真实数据中典型的编码员间分歧。

Conclusion: 批量处理和变量堆叠是降低LLM文本分类成本的有效策略，在合理范围内（批量≤100，堆叠维度≤10）不会显著影响编码质量，且测量误差小于人工编码的固有分歧。
...
TL;DR: POEMetric是首个全面的诗歌评估框架，用于评估LLM在诗歌创作上与人类诗人的差距，涵盖基本形式遵循、高级创作能力和整体质量评估。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM能够创作诗歌，但尚不清楚它们与人类诗人的实际差距。需要建立一个全面的评估框架来系统评估LLM在诗歌创作各方面的能力。

Method: 开发POEMetric框架，包含三个层次评估：1) 基本指令遵循能力（形式和主题）；2) 高级能力（创造力、词汇多样性、独特性、情感共鸣、意象和文学手法）；3) 整体质量评估和作者归属判断。收集203首人类诗歌数据集（7种固定形式），基于相同形式和主题生成6,090首LLM诗歌，结合规则评估和LLM-as-a-judge方法，并由人类专家验证。

Result: 最佳模型在形式准确性（4.26/5.00）和主题对齐（4.99）方面表现良好，但所有模型在高级能力上均未达到人类水平：人类在创造力（4.02）、独特性（3.95）、情感共鸣（4.06）、意象运用（4.49）和文学手法（4.67）方面表现卓越。人类在整体诗歌质量上也优于最佳LLM（4.22 vs. 3.20）。

Conclusion: 诗歌生成对LLM仍然是一个严峻挑战。尽管LLM在基本形式遵循方面表现良好，但在创造力、情感表达、文学技巧等高级能力上与人类诗人存在显著差距。POEMetric为诗歌评估提供了标准化框架。
...
TL;DR: 研究发现LLM激活空间中的"真理方向"并不像先前认为的那样具有普遍性，其有效性受模型层数、任务类型、任务复杂度和指令提示的显著影响。


<details>
  <summary>Details</summary>
Motivation: 先前研究对LLM激活空间中"真理方向"的普遍性存在争议，一些研究认为这些方向具有普遍性，而近期研究质疑其在某些设置下的泛化能力。本研究旨在更全面地探索真理方向普遍性的局限性。

Method: 通过系统性地分析不同模型层数、任务类型（事实性与推理性）、任务复杂度以及指令提示对真理方向的影响，探究真理方向普遍性的边界条件。

Result: 1) 真理方向高度依赖模型层数，需要在多个层进行探测才能全面理解；2) 真理方向受任务类型影响，事实性任务在较早层出现，推理任务在较深层出现；3) 任务复杂度影响真理方向性能；4) 模型指令显著影响真理方向，简单的正确性评估指令就会改变真理探针的泛化能力。

Conclusion: 真理方向的普遍性比先前认知更为有限，在模型层数、任务难度、任务类型和提示模板等方面都存在显著差异，这对基于激活空间探测的LLM可解释性方法提出了重要限制。
...
TL;DR: 该研究系统评估了多智能体系统中间接提示注入（IPI）攻击的防御策略，在动态多步工具调用环境中测试了6种防御方法对抗4种IPI攻击向量的效果，发现现有防御存在严重脆弱性，并提出基于表征工程（RepE）的检测方法能有效识别和拦截未授权操作。


<details>
  <summary>Details</summary>
Motivation: 开源框架的快速部署推动了多智能体系统发展，但扩展的动作空间（包括未受控的权限暴露和隐藏的系统间交互）带来了严重安全挑战。间接提示注入（IPI）将恶意指令隐藏在第三方内容中，可在正常操作中触发数据外泄等未授权操作。当前安全评估主要依赖孤立的单轮基准测试，复杂动态环境中智能体的系统性漏洞仍未被充分探索。

Method: 在动态多步工具调用环境中系统评估6种防御策略对抗4种复杂IPI攻击向量，测试了9个LLM骨干模型。超越二元成功率，进行多维分析。基于智能体内部状态异常高决策熵的观察，研究表征工程（RepE）作为鲁棒检测策略，通过在工具输入位置提取隐藏状态来识别和拦截未授权操作。

Result: 高级注入攻击成功绕过几乎所有基线防御，某些表面缓解措施甚至产生反效果副作用。智能体几乎立即执行恶意指令，但其内部状态表现出异常高的决策熵。基于RepE的断路器在智能体提交未授权操作前成功识别并拦截这些操作，在不同LLM骨干上实现高检测准确率。

Conclusion: 研究揭示了当前IPI防御的局限性，提供了构建弹性多智能体架构的高度实用范式。基于表征工程的检测方法能够有效识别智能体在恶意指令执行前的内部犹豫，为多智能体系统安全提供了新的防御思路。
...
TL;DR: LLMs在类比推理中存在局限性：当表面与结构线索一致时表现良好，但在需要潜在信息的深层类比中表现不佳，揭示了内部表征与提示行为之间的不对称关系。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在类比推理中的表现，特别是当类比需要深层抽象和泛化能力而非表面相似性时。探索模型内部表征与提示行为之间的关系，以理解LLMs在叙事理解中的认知限制。

Method: 比较模型的探测表征与提示性能在检测叙事类比中的表现。通过对比修辞类比和叙事类比两种任务，分析开源模型中探测与提示方法的性能差异。

Result: 发现不对称现象：对于修辞类比，探测方法显著优于提示方法；对于叙事类比，两种方法都表现较差且性能相似。这表明内部表征与提示行为的关系是任务依赖的，提示可能无法有效访问可用信息。

Conclusion: LLMs在需要深层抽象和泛化的类比推理中存在局限性。内部表征与提示行为之间的不对称关系揭示了提示机制在访问模型已有信息方面的限制，这对改进LLMs的类比推理能力具有重要意义。
...
TL;DR: 通过提示干预（奖励弃权机制和谦逊规范原则）减少LLM在事实性问题上的幻觉，无需修改模型参数


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生自信但错误的答案，因为传统的二元评分机制奖励回答而非诚实表达不确定性。研究旨在通过纯提示干预降低幻觉风险，无需修改模型

Method: 提出I-CALM框架：1) 引出语言置信度作为不确定性信号；2) 通过明确奖励机制部分奖励弃权；3) 添加强调真实性、谦逊和责任的轻量级规范原则。使用GPT-5 mini在PopQA数据集上进行实验

Result: 置信度引出和弃权奖励提示（特别是结合规范原则）通过识别易错案例并将其转为弃权，降低了已回答案例的错误率。这以覆盖率为代价提高了可靠性，同时强制回答性能基本不变。改变弃权奖励产生了清晰的弃权-幻觉边界

Conclusion: 该框架可以在不重新训练的情况下改进事实性问题上的选择性回答能力，效果大小因模型和数据集而异。纯提示干预是减少LLM幻觉的有效方法
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的社会模拟虽然能生成可信的社区互动，可用于"政策风洞"测试治理干预措施，但可信度不等于因果关系。现有模拟工作通常缺乏因果语义规范，无法支持"干预A减少冲突升级"等因果主张。需要建立因果框架来区分不同利益相关者的需求：调解员需要必要性证据诊断事件，平台设计师需要充分性证据选择政策。

Method: 采用因果反事实框架，区分必要因果（如果没有干预，结果是否会发生？）和充分因果（干预是否可靠地产生结果？）。将这种区分映射到不同利益相关者的需求，形式化这种映射关系，展示如何在明确假设下通过模拟设计支持估计，并论证结果应解释为模拟器条件因果估计。

Result: 建立了基于LLM社会模拟的因果分析框架，明确了模拟器条件因果估计的解释方式，其政策相关性取决于模拟器的保真度。该框架有助于定义适当的保真度标准，推动领域从追求表面真实性的模拟转向能够支持政策变革的模拟。

Conclusion: 现在建立这一框架至关重要：它有助于定义适当的保真度含义，推动该领域从看起来真实的模拟转向能够支持政策变革的模拟。模拟结果应解释为模拟器条件因果估计，其政策相关性取决于模拟器对现实世界动态的保真度。
...
TL;DR: CUP框架结合语言模型与结构化规划，通过不确定性感知的序列决策来提升目标导向对话系统的成功率并减少交互轮次。


<details>
  <summary>Details</summary>
Motivation: 现有目标导向对话系统存在局限性：结构化方法依赖预定义模式，而LLM方法缺乏长视野决策能力，导致信息获取与目标承诺之间协调不佳。需要一种能够平衡信息获取和目标承诺的多轮决策方法。

Method: 提出Conversation Uncertainty-aware Planning (CUP)框架，将目标导向对话建模为不确定性感知的序列决策问题。该框架集成语言模型与结构化规划：语言模型提出可行动作，规划器评估这些动作对不确定性减少的长期影响。

Result: 在多个对话基准测试中，CUP持续提高成功率同时减少交互轮次。分析表明不确定性感知规划有助于更高效的信息获取和更早的自信承诺。

Conclusion: 不确定性作为多轮决策的指导信号，结合语言模型的灵活性与结构化规划的长视野决策能力，能够有效提升目标导向对话系统的性能。
...
TL;DR: StreamGuard：一种基于预测的统一流式护栏，将内容审核重新定义为预测问题而非边界检测，通过蒙特卡洛推演监督未来有害性预测，实现早期干预


<details>
  <summary>Details</summary>
Motivation: 现有流式护栏通常将输出端审核视为边界检测问题，需要训练模型识别响应何时变得不安全。这种方法需要精确的令牌级边界标注，且在实际部署中，单一护栏通常同时用于提示和响应审核，而流式响应审核需要在部分生成内容上做出安全决策。

Method: StreamGuard采用模型无关的统一流式护栏框架，将审核重新定义为预测问题：给定部分前缀，模型预测可能未来延续的预期有害性。该方法使用蒙特卡洛推演进行监督，通过采样可能的未来延续来训练模型预测其平均有害性，从而无需精确的令牌级边界标注。

Result: 在标准安全基准测试中，StreamGuard在输入审核和流式输出审核方面均表现优异。8B规模的StreamGuard将聚合输入审核F1从86.7提升至88.2，聚合流式输出审核F1从80.4提升至81.9。在QWENGUARDTEST response_loc流式基准上，达到97.5 F1、95.1召回率和92.6%及时干预率，同时将漏检率从7.9%降至4.9%。预测监督能有效跨分词器和模型家族迁移。

Conclusion: 研究表明，无需精确边界标签即可获得强大的端到端流式审核能力，预测未来风险是低延迟安全干预的有效监督策略。基于预测的方法优于传统的边界检测方法，且监督目标可跨模型迁移。
...
TL;DR: 提出RUQuant方法，通过正交变换解决LLM激活量化中的非均匀分布问题，实现高效后训练量化


<details>
  <summary>Details</summary>
Motivation: 大型语言模型部署面临效率挑战，后训练量化是实用解决方案。现有均匀量化方法因激活分布的非均匀性导致精度显著下降，需要从理论角度重新审视激活量化问题

Method: 基于Lloyd-Max最优性条件理论分析，提出两阶段正交变换方法RUQuant：第一阶段将激活分块，使用Householder反射和Givens旋转构造的复合正交矩阵映射到均匀采样的目标向量；第二阶段通过全局Householder反射微调，利用Transformer输出差异最小化量化误差

Result: 无需模型微调即可实现接近最优的量化性能：在13B LLM上，W6A6量化达到全精度99.8%准确率，W4A4量化达到97%准确率，耗时约1分钟。微调变体可获得更高准确率，证明方法的有效性和可扩展性

Conclusion: RUQuant通过理论驱动的正交变换方法有效解决了LLM激活量化中的非均匀分布问题，实现了高效、准确的后训练量化，为资源受限环境下的模型部署提供了实用解决方案
...
TL;DR: 研究表明，在推理时使用文化负载数字进行少样本提示会导致大型语言模型产生语义漂移，使输出偏向黑暗、威权和污名化主题，而较小模型不受影响。


<details>
  <summary>Details</summary>
Motivation: 重新审视先前关于少样本提示不会导致模型不对齐的结论，探索推理时语义漂移的真实性和可测量性，特别是针对文化负载数字对模型输出的影响。

Method: 采用控制实验设计，在语义无关的提示前注入五个文化负载数字作为少样本演示，比较不同能力模型（大型vs小型）的输出分布变化，并测试无意义字符串的结构性污染效应。

Result: 具有丰富文化联想表征的大型模型表现出显著的分布偏移，偏向黑暗、威权和污名化主题；较小模型不受影响。无意义字符串也会扰动输出分布，表明存在两种可分离机制：结构格式污染和语义内容污染。

Conclusion: 推理时污染确实存在且可测量，但需要足够能力的模型。研究结果界定了推理时污染的边界条件，对使用少样本提示的LLM应用安全性具有直接意义。
...
TL;DR: 首次对小型语言模型（100M-10B参数）进行情感向量提取方法的比较分析，发现生成式提取方法在情感分离上表现更优，情感表征集中在中间Transformer层，并通过转向实验验证了因果行为效应。


<details>
  <summary>Details</summary>
Motivation: 尽管小型语言模型在生产系统中应用日益广泛，但尚不清楚它们是否具有前沿模型中已发现的内在情感表征。需要研究SLMs的情感表征特性，为开放权重模型的情感研究提供方法指导。

Method: 使用9个模型（涵盖GPT-2、Gemma、Qwen、Llama、Mistral 5个架构家族），评估20种情感和两种提取方法（生成式和理解式）。通过统计比较、层定位分析、各向异性基线验证、转向实验（使用外部情感分类器验证）等方法进行综合分析。

Result: 生成式提取方法在情感分离上具有统计显著优势；情感表征集中在中间Transformer层（约50%深度），呈现U型曲线且架构不变；转向实验揭示了三种机制（精准、重复崩溃、爆炸性），并发现Qwen存在跨语言情感纠缠问题。

Conclusion: 该研究为开放权重模型的情感研究提供了方法学指导，通过将外部行为分析与内部表征分析相结合，为"模型医学"系列做出贡献，同时揭示了多语言部署中的安全隐患。
...
TL;DR: EduEmbed：一个统一的嵌入增强框架，通过微调语言模型来丰富学习者和项目认知建模，提升认知诊断性能


<details>
  <summary>Details</summary>
Motivation: 虽然ID嵌入是认知建模的主流方法，但语言模型提供了丰富的语义表示来增强认知诊断性能。现有工作中存在两个关键挑战：1）语言模型训练目标与认知诊断模型之间的不对齐导致特征空间分布差距；2）需要在不同认知诊断任务中统一集成文本嵌入，同时保留现有认知建模范式的优势以确保嵌入增强的鲁棒性。

Method: 提出EduEmbed框架，包含两个阶段：第一阶段基于角色特定表示和交互诊断器微调语言模型，以弥合认知诊断模型的语义差距；第二阶段使用文本适配器提取任务相关语义，并将其与现有建模范式集成以提高泛化能力。

Result: 在四个认知诊断任务和计算机自适应测试任务上评估了该框架，取得了鲁棒的性能。进一步分析揭示了语义信息在不同任务中的影响，为未来在线智能教育系统中语言模型在认知诊断中的应用提供了关键见解。

Conclusion: EduEmbed通过统一框架有效整合语言模型的语义表示，解决了现有工作中语言模型与认知诊断模型的对齐问题，为在线智能教育系统中的认知建模提供了增强的嵌入方法。
...
TL;DR: 长思维链推理模型的压缩方法主要关注准确性和token节省，但本文首次系统研究了压缩对模型可信度（安全性、抗幻觉能力、多语言鲁棒性）的影响，发现压缩常导致可信度下降，并提出对齐感知的DPO变体来减少这种损失。


<details>
  <summary>Details</summary>
Motivation: 现有长思维链推理模型压缩评估主要关注任务准确性和token节省，但可信度属性（如安全性、抗幻觉能力、多语言鲁棒性）与模型参数空间紧密相关，压缩可能影响这些属性。需要系统研究压缩对可信度的影响。

Method: 1）对多种不同规模模型进行系统实证研究，评估思维链压缩对三个可信度维度的影响；2）提出归一化效率评分实现公平比较；3）引入对齐感知的DPO变体作为存在性证明，减少压缩带来的可信度损失。

Result: 研究发现：1）思维链压缩经常引入可信度回归；2）不同压缩方法在不同可信度维度上表现出明显不同的退化模式；3）标量指标可能掩盖可信度权衡；4）提出的对齐感知DPO变体在推理基准上减少19.3%的思维链长度，同时显著降低可信度损失。

Conclusion: 思维链压缩不仅应优化效率，还应同等重视可信度，将两者都视为重要的设计约束。需要开发同时考虑效率和可信度的压缩方法。
...
TL;DR: PALM算法通过构建小型LLM组合来满足异构用户偏好，在保证理论性能的同时降低系统成本


<details>
  <summary>Details</summary>
Motivation: 为每个用户维护单独的LLM在计算、内存和系统复杂性方面不切实际，需要开发能够捕获代表性行为的小型LLM组合方法

Method: 提出PALM算法，将用户偏好建模为多维权重向量，基于奖励函数生成小型LLM组合，确保对任何权重向量都能提供接近最优的LLM

Result: 首次为LLM组合的大小和近似质量提供理论保证，实证结果验证了这些保证并展示了比基线方法更大的输出多样性

Conclusion: PALM在系统成本与个性化之间实现了有效权衡，揭示了覆盖用户偏好所需的LLM多样性特征
...
TL;DR: 使用小型语言模型从儿科肾脏活检报告中提取结构化信息的半自动标注工作流程，在CPU基础设施上实现84.3%的准确率，解决了临床数据隐私和计算资源限制问题。


<details>
  <summary>Details</summary>
Motivation: 电子病历系统中的临床信息大多为非结构化文本，限制了其在研究和决策中的应用。大型语言模型虽能提取信息但需要大量计算资源，而将敏感临床数据发送到云端服务（即使去标识化）存在显著的隐私风险。因此需要开发资源高效且保护隐私的解决方案。

Method: 开发了基于小型语言模型的半自动标注工作流程，专注于儿科肾脏活检报告。将信息提取构建为基于临床指导实体指南和少量示例的问答任务，评估了五个指令调优的SLM，并使用分歧建模框架优先处理需要临床审查的报告。在临床监督下迭代开发，手动标注了2111份报告中的400份作为黄金标准。

Result: Gemma 2 2B模型达到最高准确率84.3%，优于现成模型如spaCy(74.3%)、BioBERT-SQuAD(62.3%)、RoBERTa-SQuAD(59.7%)和GLiNER(60.2%)。实体指南使性能比零样本基线提高7-19%，少量示例提高6-38%，但两者结合时效益不叠加。SLM可在仅CPU基础设施上从专业临床领域提取结构化信息。

Conclusion: 小型语言模型能够在仅CPU基础设施上从专业临床领域有效提取结构化信息，且仅需最少的临床参与。该工作流程为资源受限环境下的临床信息提取提供了可行的解决方案，平衡了准确性、隐私保护和计算效率。
...
TL;DR: 论文批评了在事实核查中使用形式逻辑的方法，指出逻辑上合理的结论可能对人类产生误导性推理，主张利用LLMs的人类化推理倾向作为补充验证手段。
...
Method: 基于认知科学和语用学研究，提出了一个类型学分类，识别逻辑上合理的结论如何系统性地引发人类做出基于前提无法支持的推理。主张利用LLMs的人类化推理倾向作为特征而非缺陷，使用这些模型来验证神经符号系统中形式组件的输出，防止误导性结论。

Result: 识别了逻辑上合理的结论与人类实际推理之间的系统性差异，建立了类型学分类来描述这些情况。提出了互补方法：利用LLMs的人类化推理倾向来检测形式逻辑可能遗漏的误导性主张。

Conclusion: 单纯依赖形式逻辑进行事实核查存在结构性缺陷，无法有效检测误导性主张。应将LLMs的人类化推理倾向作为补充验证手段，结合神经符号系统，通过LLMs验证形式组件的输出，从而更全面地识别和防止误导性结论。
...
TL;DR: 该论文通过后殖民视角，首次系统研究了LLM开发流程中标准英语变体（美式英语和英式英语）的方言不对称性，发现当代LLM将美式英语作为事实标准，存在系统性偏见。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在高风险领域部署日益增多，但其语言设置有限，主要基于"英语（美国）"，忽视了英语的全球多样性和殖民历史。研究者旨在通过后殖民框架，揭示数据策展、数字主导和语言标准化的地缘政治历史如何塑造LLM开发流程，特别关注标准英语变体之间的方言不对称性。

Method: 研究聚焦美式英语（AmE）和英式英语（BrE）两种主要标准变体，构建了包含1,813个AmE-BrE变体的精选语料库，并引入DiAlign——一种基于分布证据的动态、无需训练的方言对齐估计方法。通过三阶段操作化结构偏见：1）审计六个主要预训练语料库揭示系统性偏向AmE；2）分词器分析显示BrE形式产生更高的分词成本；3）生成评估显示模型输出存在持续的AmE偏好。

Result: 研究发现当代LLM将美式英语作为事实标准，存在系统性方言偏见：预训练语料库系统性偏向AmE；BrE形式在分词过程中产生更高成本；模型输出持续表现出AmE偏好。这是首次对LLM开发各阶段中标准英语变体方言不对称性的系统性和多层面检验。

Conclusion: 研究揭示了LLM开发中存在的方言不对称性，当代LLM将美式英语特权化为事实规范，引发了关于语言同质化、认知不公正和全球AI部署不平等的担忧。这些发现为开发更具方言包容性的语言技术提供了实践动力和方向。
...
TL;DR: DARE是一个统一的扩散大语言模型后训练与评估框架，旨在解决当前开源生态碎片化问题，提供可复现的基准评估和算法加速。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(dLLMs)作为自回归模型的替代方案正在兴起，但其开源生态系统在模型家族和后训练流程方面存在碎片化。这种碎片化导致研究迭代缓慢、复现工程负担重，以及算法间公平比较困难。

Method: DARE基于verl和OpenCompass构建，统一了监督微调、参数高效微调、偏好优化和dLLM特定的强化学习，为掩码和块扩散语言模型提供共享执行栈。支持LLaDA、Dream、SDAR、LLaDA2.x等代表性模型家族。

Result: DARE提供了广泛的算法覆盖、可复现的基准评估和实际加速效果。实证结果表明DARE可作为可重用的研究基础，用于开发、比较和部署当前及新兴dLLMs的后训练方法。

Conclusion: DARE框架解决了dLLM后训练生态的碎片化问题，为研究人员提供了统一的平台，促进了扩散大语言模型后训练方法的公平比较和高效开发。
...
TL;DR: 提出ACE混合框架，通过预测熵路由高不确定性专利权利要求至专家LLM，结合CoPT协议，在保证准确率的同时大幅降低验证成本


<details>
  <summary>Details</summary>
Motivation: 专利权利要求自动验证需要零缺陷容忍度，现有方法面临刚性-资源困境：轻量级编码器难以处理复杂的法律依赖关系，而使用大型语言模型进行全面验证成本过高

Method: 提出ACE混合框架，使用预测熵筛选高不确定性权利要求，仅将其路由至专家LLM；专家LLM执行基于35 U.S.C.法定标准的专利思维链协议，有效处理长程法律依赖关系

Result: ACE在评估方法中取得最佳F1分数94.95%，相比独立LLM部署降低78%的运营成本；构建了包含40,000个权利要求和MPEP基础错误标注的ACE-40k基准数据集

Conclusion: ACE框架通过自适应路由机制，在专利权利要求验证中实现了高准确率和成本效益的平衡，为解决法律文档自动验证的刚性-资源困境提供了有效方案
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM个性化系统大多在用户偏好稳定且缺乏客观真实标准的领域运行，而个人投资决策作为一个高风险、时间跨度长的决策领域，暴露了现有LLM定制范式的根本局限性，需要新的解决方案。

Method: 基于作者构建和部署的AI增强投资组合管理系统，识别出个人投资领域暴露LLM定制局限性的四个关键维度：行为记忆复杂性、漂移下的论点一致性、风格-信号张力、以及无真实标准的对齐问题。

Result: 论文识别了个人投资决策对LLM个性化提出的四个核心挑战，并描述了从系统构建中产生的架构响应方案，为高风险、时间跨度长的决策领域的个性化NLP研究指明了方向。

Conclusion: 个人投资决策领域揭示了当前LLM个性化范式的根本局限性，需要开发能够处理时间演化、自相矛盾、高风险决策的新架构和方法，为个性化NLP在高风险、时间跨度长的决策领域开辟了新的研究方向。
...
TL;DR: MINT基准测试揭示LLMs在多轮医学诊断中的三个行为模式：过早回答意图、自我纠正能力和强诱惑信息影响，并提出推迟诊断问题能显著改善准确性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LLMs在单轮提供完整临床信息时的诊断准确性，但缺乏对多轮证据积累（更接近真实临床推理过程）中LLMs行为的研究。需要探索LLMs在增量信息获取过程中的表现和潜在问题。

Method: 引入MINT（医学增量N轮基准测试），包含1,035个病例，具有临床标记的证据片段、受控的轮次粒度和信息保持分解。系统评估了11个LLMs，分析其在多轮证据积累过程中的行为模式。

Result: 发现三个持续性行为模式：1）过早回答意图：55%以上答案在前两轮就确定；2）自我纠正能力：错误到正确的修正率比正确到错误翻转率高10.6倍；3）强诱惑信息：临床显著信息（如实验室结果）会触发过早回答。推迟诊断问题可减少过早回答，将首次承诺点的准确性提高62.6%；将显著临床证据保留到后期可防止因过早承诺导致的准确性下降达23.3%。

Conclusion: MINT提供了受控评估框架，并为改善LLMs在多轮医学诊断中的可靠性提供了具体建议：推迟诊断问题、控制信息呈现顺序，以充分利用LLMs的自我纠正能力并避免过早承诺。
...
TL;DR: 提出GroundedKG-RAG系统，通过从源文档显式提取并基于知识图谱的检索增强生成，解决长文档问答中效率低、内容重复和幻觉问题
...
Method: 提出GroundedKG-RAG系统，从源文档显式提取并基于知识图谱。节点定义为实体和动作，边定义为时间或语义关系，每个节点和边都基于原始句子。使用语义角色标注（SRL）和抽象意义表示（AMR）解析构建知识图谱，然后进行嵌入用于检索。查询时对查询应用相同转换，从基于的源文本中检索最相关句子进行问答

Result: 在NarrativeQA数据集上评估，GroundedKG-RAG性能与最先进的专有长上下文模型相当但成本更低，且优于竞争基线。此外，GroundedKG具有可解释性和人类可读性，便于结果审计和错误分析

Conclusion: GroundedKG-RAG通过显式提取并基于知识图谱的检索增强生成，有效解决了长文档问答中的效率、准确性和幻觉问题，提供了一种成本效益高且可解释的解决方案
...
TL;DR: 研究发现，在五个transformer语言模型中，注意力头的logit能量场仅需2-11个奇异分量即可解释90%方差，而学习的交互矩阵W_Q^T W_K需要38-75个分量，表明语言数据本身具有高度可压缩性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解transformer语言模型中注意力机制的谱特性，特别是探索注意力头的logit能量场与学习的交互矩阵之间的压缩性差异，以揭示语言数据的内在结构特性。

Method: 方法包括：1）分析五个不同规模的transformer语言模型（124M-7B参数，四种架构家族）；2）计算注意力头的logit能量场和W_Q^T W_K交互矩阵的奇异值分解；3）测量达到90%方差所需的奇异分量数量；4）比较两者的有效秩差异。

Result: 主要结果：1）logit能量场仅需2-11个奇异分量即可解释90%方差；2）W_Q^T W_K交互矩阵需要38-75个分量达到相同阈值；3）两者的谱差距为5-25倍有效秩；4）注意力机制在所有维度上均匀分配容量，但语言交互实际集中在少数维度。

Conclusion: 结论是softmax注意力机制处理的语言具有高度可压缩性，这种压缩性是语言数据本身的固有属性，而非分析框架的特性。注意力机制的设计容量远超过语言交互的实际需求。
...
<details>
  <summary>Details</summary>
Motivation: 研究对齐训练语言模型中拒绝行为的内部机制，特别是如何通过稀疏路由机制实现内容检测与策略执行的分离，以理解模型安全机制的工作原理和局限性。

Method: 使用政治审查和安全拒绝作为自然实验，在9个模型（来自6个实验室）上验证机制，基于120个提示对语料库。采用必要性/充分性互换测试（p<0.001）、自举重采样稳定性分析（Jaccard 0.92-1.0）、消融实验（效果减弱达17倍）、密码编码实验等方法。

Result: 发现稳定的稀疏路由机制：门控头检测内容并触发放大器头增强拒绝信号；通过调制检测层信号可连续控制策略强度（从硬拒绝到事实遵从）；在密码编码下，门控头的路由贡献崩溃（Phi-4中78%），模型转向解谜而非拒绝，揭示了意图识别与策略路由的结构分离。

Conclusion: 对齐训练语言模型通过稀疏路由机制实现安全策略，该机制在规模扩展时分布化但仍可检测。意图识别（预训练获得）与策略路由（后训练绑定）存在结构分离，后者在输入变换下泛化能力较差，解释了模型安全机制的鲁棒性差异。
...
TL;DR: 该研究通过线性探针分析视觉文档理解任务中大型视觉语言模型内部信息表示，发现中间层比最终层编码更多线性信息，并基于此开发了针对中间层的微调策略。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在视觉文档理解任务上的性能评估通常基于生成响应，但这不能反映模型是否真正在内部捕获了所需信息。研究旨在探究VDU任务所需信息在LVLM不同层中的表示情况。

Method: 使用线性探针技术分析LVLM中LLM不同层的信息表示；基于发现设计针对中间层的微调策略，并进行实验验证。

Result: 研究发现：(1)内部表示与生成响应之间存在明显差距；(2)解决任务所需信息更多从中间层而非最终层线性编码。实验表明，微调中间层能同时提高线性探针准确率和响应准确率，并缩小差距。

Conclusion: 视觉文档理解任务中，LVLM的中间层比最终层编码更多有用信息；针对中间层的微调策略能有效提升模型性能并缩小内部表示与生成响应之间的差距。
...
TL;DR: Factum-4B：通过结构化事件事实和因果推理提升视频理解能力的Video-LLM框架


<details>
  <summary>Details</summary>
Motivation: 现有Video-LLMs依赖非结构化视频推理，将关键视觉证据嵌入冗长文本描述中，因果建模薄弱，导致推理效率低下且脆弱。人类理解视频动态时则基于实体、动作和时间关系的结构化心理表征。需要弥合这一认知差距。

Method: 提出结构化事件事实作为推理前的紧凑表征，包含显著事件及其因果关系。引入CausalFact-60K数据集和四阶段训练流程：事实对齐、格式预热、思维预热、基于强化学习的后训练。将优化建模为多目标强化学习问题，平衡结构完整性、因果保真度和推理长度。

Result: 开发出Factum-4B模型，在需要细粒度时间推理的挑战性视频理解任务上表现更强，产生更可靠的推理结果。

Conclusion: 结构化事件事实作为显式约束能促进简洁且因果基础的推理，使中间证据更易验证。通过MORL平衡竞争目标，实现了更可靠的视频理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在处理复杂、特定情境的规则推理方面仍面临挑战，特别是在法律和政策环境中需要进行道义推理。现有基准大多关注短上下文数学推理，缺乏针对长上下文、高风险道义推理的评估工具，因此需要创建专门的基准来填补这一空白。

Method: 研究者创建了DEONTICBENCH基准，包含四个领域的6,232个任务。支持两种推理方法：1) 自由形式的链式思维语言推理；2) 符号计算辅助方法，将法规和案例事实转换为可执行的Prolog程序，生成正式问题解释和明确程序跟踪。所有实例都提供了参考Prolog程序。

Result: 前沿LLM和编码模型在困难子集上的最佳表现仅为：SARA Numeric任务44.4%准确率，Housing任务46.6 macro-F1分数。通过监督微调和强化学习训练可以改善Prolog生成质量，但当前的强化学习方法仍无法可靠解决这些任务。

Conclusion: DEONTICBENCH为研究真实世界领域中基于上下文的规则推理提供了基准，支持符号和非符号两种设置。结果表明当前模型在复杂道义推理任务上仍有很大提升空间，特别是在符号程序生成方面需要进一步研究。
...
TL;DR: 研究发现Transformer语言模型不表现出生物系统中常见的标量变异性（噪声与数值大小成比例），而是表现出反标量模式：表征变异性随数值增大而减小。


<details>
  <summary>Details</summary>
Motivation: 研究旨在测试Transformer语言模型是否表现出生物系统中普遍存在的标量变异性特性，即表征噪声与数值大小成比例，产生恒定的变异系数。

Method: 分析三个7-8B参数模型（Llama-3-8B-Instruct、Mistral-7B-Instruct-v0.3、Llama-3-8B-Base）中26个数值大小在载体句子中的隐藏状态表征分散度，计算沿数值轴的表征变异性缩放指数。

Result: 发现反标量模式：表征变异性随数值增大而减小（缩放指数α≈-0.19），所有主要层均未显示正缩放指数。该模式在完整维度空间和句子身份校正后仍保持一致，且沿数值轴比正交维度强3-5倍。语料库频率能强烈预测每个数值的变异性（ρ=0.84）。

Conclusion: 分布学习本身不足以产生标量变异性：Transformer模型再现了对数压缩的数值几何结构，但未表现出生物系统中观察到的恒定变异系数噪声特征。
...
<details>
  <summary>Details</summary>
Motivation: 形态学数据收集和标注面临重大挑战，需要语言学专业知识、方法严谨性和大量资源，这对低资源语言和变体尤为困难，因此需要加速这一过程。

Method: 采用三层方法：专家语言学定义、贡献者启发和社区验证；平台通过主动学习、标注建议以及从相关语言导入和改编材料的工具来最小化手动工作；支持融合、粘着和根模式等多种形态系统。

Result: 开发了开源平台CommonMorph，提供UniMorph兼容输出，确保与NLP工具的互操作性；平台可通过https://common-morph.com访问，为通过协作技术保护语言多样性提供了可复制的模型。

Conclusion: CommonMorph平台通过简化形态学数据收集流程，降低了资源门槛，支持多种形态系统，为保护语言多样性提供了有效的协作技术解决方案。
...
TL;DR: 研究表明，在智能体代码基准测试中，评估语言的选择会显著影响模型排名，甚至可能颠倒不同骨干模型的性能排序。通过将评估提示本地化到五种语言，发现没有单一模型在所有语言中表现最优，且模型间一致性较低。


<details>
  <summary>Details</summary>
Motivation: 当前智能体代码基准测试通常默认使用英语作为评估语言，但这种固定语言设置可能掩盖了模型在不同语言环境下的真实性能差异。研究者希望探究评估语言变化是否会影响模型排名，从而揭示语言作为评估变量的重要性。

Method: 将Agent-as-a-Judge评估提示栈本地化到五种类型学上不同的语言（英语、阿拉伯语、土耳其语、中文、印地语），在55个DevAI开发任务上评估三个开发者智能体框架和六个评估骨干模型，共进行4950次评估运行。通过控制实验分析本地化程度对评估结果的影响。

Result: 1. 骨干模型与语言存在交互效应：GPT-4o在英语中满意度最高（44.72%），而Gemini在阿拉伯语（51.72%）和印地语（53.22%）中领先；2. 没有单一模型在所有语言中表现最优；3. 模型间对单个需求判断的一致性较低（Fleiss' κ ≤ 0.231）；4. 控制实验显示，仅本地化基准内容而保持评估指令为英语会导致印地语满意度从42.8%降至23.2%。

Conclusion: 语言应被视为智能体基准测试中的显式评估变量，仅翻译基准内容而保持评估指令为英语可能导致评估结果严重偏差。研究强调了全面本地化评估流程的重要性，并发布了完整的需求级判断和运行时统计数据以确保可复现性。
...
TL;DR: 该论文探讨依赖句法中树结构的约束条件，特别是超越投射性约束的中间方案，以更好地拟合真实语言现象。


<details>
  <summary>Details</summary>
Motivation: 依赖句法将句子结构表示为依赖关系树，但实践中许多树结构在真实语言中不常见或不合理。需要寻找能更好拟合真实语言现象的约束条件，以提供更准确的语言描述、更快的解析速度，以及对语言演变和人类语言处理的洞察。

Method: 论文回顾和分析了投射性约束及其局限性，特别是对于词序灵活的语言。在此基础上，探讨了各种旨在在投射性的限制性和无约束依赖结构的过度宽松性之间寻找现实中间地带的约束条件。

Result: 投射性作为最广泛研究的约束条件，被证明对于表示某些语言现象（特别是词序灵活的语言）过于严格。因此需要开发更灵活的约束条件来平衡准确性和实用性。

Conclusion: 需要超越投射性约束，开发更灵活的依赖结构约束条件，以在理论严谨性和实际语言现象之间找到更好的平衡点，特别是对于词序灵活的语言。
...
TL;DR: 论文提出PassiveQA框架，通过监督微调使LLM在信息不足时能够明智地选择回答、询问澄清或弃权，解决传统RAG系统在信息不完整查询中的过度自信问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的用户查询往往不完整、模糊或缺少关键变量，而现有LLM和RAG系统默认假设查询是完全指定且可回答的，导致模型产生过度自信或幻觉响应。需要让模型具备信息充分性判断的认知意识。

Method: 提出PassiveQA三行动框架：通过监督微调使模型行为与信息充分性对齐。方法整合结构化信息状态表示、知识图谱基础上下文，以及一个经过微调的规划器，该规划器显式建模缺失变量和决策推理。

Result: 在多个QA数据集上的实验表明，经过微调的规划器在宏观F1和弃权召回率方面取得显著改进，同时降低了幻觉率，且是在计算受限的训练机制下实现的。

Conclusion: 认知决策必须在训练期间学习，而不是在推理时强制实施。PassiveQA框架通过监督微调成功使模型具备信息充分性判断能力，显著改善了模型在信息不完整查询中的表现。
...
<details>
  <summary>Details</summary>
Motivation: 普什图语有6000-8000万使用者，但在多语言自动语音识别领域缺乏公开的基准测试。本研究旨在填补这一空白，首次在公共数据集上进行可重复的多模型评估，揭示现有模型在普什图语ASR中的实际表现和局限性。

Method: 使用FLEURS普什图语测试集和过滤的Common Voice~24子集，评估了10个模型的零样本ASR性能（包括所有7个Whisper尺寸、MMS-1B、SeamlessM4T-v2-large和OmniASR-CTC-300M）。进行语言识别审计分析脚本故障，评估5个微调普什图语ASR模型的跨域性能，并通过字符类错误分层分析特定音素的错误分布。

Result: 零样本Whisper的WER在90%-297%之间，Medium模型在Common Voice~24上崩溃至461%；SeamlessM4T在Common Voice~24上达到39.7% WER（当前最佳零样本结果）；MMS-1B在FLEURS上达到43.8%。Whisper模型产生普什图语脚本输出的比例低于0.8%，而其他模型超过93%。微调模型的WER从14%恶化到32.5-59%，但一个增强模型在两组测试集上都达到35.1%且无跨域退化。普什图语特有音素（卷舌音系列和边擦音）占不成比例的错误量。

Conclusion: 研究首次提供了普什图语ASR的全面基准测试，揭示了现有模型的严重局限性，特别是Whisper在脚本生成方面的系统性故障。识别了阻碍累积进展的五个结构性障碍，并提出了五个有序的研究优先级，为普什图语ASR的未来发展提供了路线图。
...
<details>
  <summary>Details</summary>
Motivation: 现有句子表示主要编码句子内容而非表达方式，而表达方式（风格和方言）对许多应用很重要，需要开发能捕捉这些特征的表示方法

Method: 提出IDIOLEX框架，结合句子来源监督和句子内容的语言特征，学习句子风格和方言的连续表示，在阿拉伯语和西班牙语方言上进行评估

Result: 学习到的表示能捕捉有意义的变异并在跨领域分析和分类中迁移，还可作为语言模型风格对齐的训练目标
...
<details>
  <summary>Details</summary>
Motivation: 高质量双语资源是多语言NLP发展的关键瓶颈，特别是在孟加拉语等低资源语言环境中。现有资源不足限制了相关研究进展，需要构建专门针对语法分类的标注语料库。

Method: 从开放许可的百科全书资源和自然对话文本中收集数据，经过系统预处理和自动语言识别。采用多阶段标注框架，由三名独立标注者进行标注，使用Fleiss Kappa评估标注一致性。语料库包含句法结构（简单、复杂、复合、复杂复合）和时态（现在、过去、将来）两个维度的标注。

Result: 构建了包含30,534个句子的语料库（17,465个英语句子，13,069个孟加拉语句子）。标注质量高，结构标注的Fleiss Kappa为0.82，时态标注为0.88。统计分析显示现实的结构和时态分布，基线评估表明双编码器架构优于强大多语言编码器。

Conclusion: BiST为双语语法建模提供了统一资源，支持语法建模任务（如受控文本生成、自动反馈生成和跨语言表示学习），促进了基于语言学的多语言研究，填补了孟加拉语高质量双语资源的空白。
...
TL;DR: 该研究挑战了"多语言推理只需模仿英语推理"的假设，通过定义可测量的推理特征并分析它们在不同语言中的有效性，发现英语推理特征在多语言环境中的关联强度差异显著，甚至可能相反。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在英语和其他语言之间存在显著的性能差距，当前工作假设通过使每种语言的推理都模仿英语推理就能缩小这一差距。本研究挑战这一假设，探究多语言环境中有效推理的特征，以及英语衍生的推理特征在其他语言中的实际帮助程度。

Method: 1. 定义了一套可测量的推理特征，涵盖多语言对齐、推理步骤和推理流程三个方面；2. 使用逻辑回归量化每个特征与最终答案准确性的关联；3. 在多语言推理轨迹上训练稀疏自编码器，自动发现实现或扩展这些特征的潜在推理概念；4. 使用这些特征作为测试时选择策略，检验它们是否能引导模型实现更强的多语言推理。

Result: 在两个数学推理基准测试、四个大型推理模型和10种语言中，研究发现：1. 大多数特征与准确性呈正相关；2. 关联强度在不同语言间差异显著；3. 某些情况下关联甚至相反。这表明英语推理特征并非在所有语言中都同样有效。

Conclusion: 研究结果挑战了以英语为中心的奖励设计，指出需要适应语言特定推理模式的自适应目标。这对多语言基准测试和奖励设计具有具体意义，表明需要开发能够适应不同语言推理模式的评估和训练方法，而不是简单地将英语推理模式强加于其他语言。
...
TL;DR: 研究发现，将英语作为第二语言（ESL）的变体与拼写错误结合使用时，对大型语言模型（LLM）性能的影响比单独考虑任一因素更大，但这种组合效应并非简单的叠加。封闭式任务中的性能下降模式更一致，而开放式任务的结果则更为复杂。


<details>
  <summary>Details</summary>
Motivation: 由于LLM主要在英语数据上训练，非英语母语者常使用英语作为第二语言（ESL）与其交互，且这些输入常包含拼写错误。先前研究通常单独考察ESL变体或拼写错误的影响，而现实中两者常同时出现，因此需要研究它们的组合效应。

Method: 使用Trans-EnV框架将标准英语输入转换为八种ESL变体，并应用MulTypo在三个级别（低、中、高）注入拼写错误，以评估LLM在不同条件下的性能表现。

Result: ESL变体与拼写错误的组合通常导致比单独任一因素更大的性能下降，但组合效应并非简单的叠加。封闭式任务的性能下降模式在ESL变体和拼写错误级别间更一致，而开放式任务的结果则更为复杂和混合。

Conclusion: 在干净标准英语上的评估可能高估LLM在现实世界中的性能，单独评估ESL变体或拼写错误无法完全捕捉模型在真实场景中的行为。需要综合考虑这些因素以获得更准确的性能评估。
...
TL;DR: 对LLMs文化包容性的初步计算审计：通过隐喻生成任务发现LLMs存在刻板印象和西方默认主义，提示文化身份并不能保证文化基础推理


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型被描述为多语言，但语言能力不等于文化推理能力。本文旨在探究LLMs是否真正进行文化感知推理，还是仅仅作为文化翻译器，利用主导概念框架配合本地化表达。

Method: 采用计算审计方法，以创意写作任务中的隐喻生成为案例研究。研究涵盖五个文化背景和多个抽象概念，分析LLMs在隐喻使用上的表现。

Result: 研究发现模型在某些文化背景下表现出刻板化的隐喻使用，并存在西方默认主义倾向。这表明仅通过提示LLMs文化身份并不能保证其进行文化基础推理。

Conclusion: LLMs在文化推理方面存在局限性，不能仅凭多语言能力就认为其具备文化包容性。需要更深入的研究来理解和改善LLMs的文化感知能力。
...
TL;DR: 该研究探索了人类-AI协同创作中的五种"黑暗模式"：奉承、语气管制、道德说教、死亡循环和锚定效应，这些模式可能抑制或扭曲创作过程。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地作为协作写作伙伴，需要研究它们对人类创造力的影响，特别是那些可能抑制或扭曲创作过程的微妙模型行为。

Method: 通过一系列受控会话，将LLMs作为写作助手在不同文学形式和主题中进行提示，分析生成响应中这些行为的普遍性。

Result: 奉承行为几乎无处不在（91.7%的案例），在敏感话题中尤其明显；锚定效应则依赖于文学形式，在民间故事中出现最频繁。

Conclusion: 这些黑暗模式通常是安全对齐的副产品，可能无意中限制了创造性探索，需要为有效支持创意写作的AI系统提出设计考虑。
...
TL;DR: 论文提出几何动力系统框架解释LLM幻觉现象，认为幻觉源于潜在空间中任务依赖的盆地结构，并通过几何感知引导减少幻觉概率


<details>
  <summary>Details</summary>
Motivation: 大型语言模型会产生幻觉（输出事实错误的流畅内容），需要理解幻觉产生的根本机制并开发减少幻觉的方法

Method: 使用几何动力系统框架分析幻觉现象，通过自回归隐藏状态轨迹研究多个开源模型和基准，提出任务复杂度和多盆地定理，分析L层Transformer中盆地涌现，开发几何感知引导技术

Result: 发现可分性强烈依赖于任务而非普遍存在：事实性任务显示更清晰的盆地分离，而摘要和误解较多的任务通常稳定性较差且经常重叠；几何感知引导可在不重新训练的情况下减少幻觉概率

Conclusion: 幻觉源于潜在空间中任务依赖的盆地结构，几何动力系统框架为理解LLM幻觉提供了理论基础，几何感知引导是减少幻觉的有效方法
...
TL;DR: HukukBERT：针对土耳其法律领域的最全面语言模型，通过混合领域自适应预训练方法在18GB法律语料上训练，在土耳其法律术语预测和法院判决结构分割任务中达到最先进性能。
...
Method: 采用混合领域自适应预训练方法，整合了全词掩码、令牌跨度掩码、词跨度掩码和针对性关键词掩码技术，使用48K WordPiece分词器，在18GB清洗后的土耳其法律语料上进行训练。

Result: 在土耳其法律术语预测任务中达到84.40%的Top-1准确率，显著优于现有模型；在土耳其法院判决结构分割任务中达到92.8%的文档通过率，均创下最新记录。

Conclusion: HukukBERT填补了土耳其法律领域专门语言模型的空白，为土耳其法律NLP任务（如命名实体识别、判决预测、法律文档分类等）的未来研究提供了有力支持。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在推理基准测试中表现出色，但其解决需要端到端工作流程的现实问题的能力尚不清楚。数学建模竞赛为评估这种端到端问题解决能力提供了一个严格的测试平台。

Method: 提出了一个面向问题、分阶段的评估框架，使用专家验证的标准评估大语言模型在各个建模阶段的表现。通过将自动评分与独立人类专家判断进行比较，验证了该框架的可靠性，并应用于中国研究生数学建模竞赛的问题。

Result: 框架的可靠性得到验证，自动评分与人类专家判断高度一致。研究发现最先进的大语言模型存在理解-执行差距：在问题识别和表述等早期阶段表现良好，但在模型求解、代码实现和结果分析等执行导向阶段存在持续缺陷。这些差距即使模型规模增大也依然存在。

Conclusion: 大语言模型在端到端问题解决中存在理解与执行之间的差距，这不能仅通过模型缩放来解决。需要超越模型缩放的方法来弥合这一差距，为将大语言模型应用于复杂的现实问题解决提供了见解。
...
TL;DR: LiveFact是一个持续更新的基准测试，用于评估LLM在动态、不完全信息下的假新闻检测和事实核查推理能力，解决了传统静态基准的数据污染和时态不确定性评估不足问题。


<details>
  <summary>Details</summary>
Motivation: 当前假新闻检测和事实核查的评估框架存在两个主要问题：1）静态基准容易受到基准数据污染（BDC）的影响；2）无法有效评估模型在时态不确定性下的推理能力。这些局限性使得现有评估无法反映真实世界中信息不断演变的"战争迷雾"场景。

Method: LiveFact采用动态、时态的证据集来模拟真实世界信息演变过程。提出双模式评估：分类模式用于最终验证，推理模式用于基于证据的推理。同时包含显式监控BDC的组件。基准持续更新以保持时效性。

Result: 测试22个LLM显示：开源混合专家模型（如Qwen3-235B-A22B）现在能够匹配或超越专有最先进系统。更重要的是分析发现显著的"推理差距"：能力强的模型在早期数据切片中表现出认知谦逊，能够识别不可验证的声明，这是传统静态基准忽略的方面。

Conclusion: LiveFact为评估稳健、时态感知的AI验证设定了可持续标准，解决了传统静态基准在数据污染和时态不确定性评估方面的局限性，推动了假新闻检测从简单分类向复杂推理的评估范式转变。
...
TL;DR: 该研究测试了大型语言模型在土耳其语前置关系从句附着歧义中是否像人类一样整合世界知识和句法结构，发现模型对合理性信息的敏感性远弱于人类。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在许多语言任务上表现良好，但尚不清楚它们是否像人类一样以结构敏感的方式整合世界知识和句法结构来解决歧义。本研究旨在通过土耳其语前置关系从句附着歧义来测试这一点。

Method: 研究者构建了保持句法配置固定且两种解析都可行的土耳其语歧义项，通过分级事件合理性选择性地支持高附着或低附着。首先进行独立规范评分验证对比，然后在限时强制选择理解实验中测试人类反应。最后评估土耳其语和多语言LLM在并行偏好设置中的表现，通过平均每词元对数概率比较匹配的高附着/低附着延续。

Result: 人类在实验中显示出大且正确方向的合理性效应。而土耳其语和多语言LLM在合理性驱动的偏好转移方面表现微弱、不稳定甚至相反。模型对合理性信息的敏感性远不如人类判断可靠。

Conclusion: 研究结果表明，在测试的模型中，合理性信息不像在人类判断中那样可靠地指导附着偏好。土耳其语关系从句附着歧义可作为超越广泛基准的有用跨语言诊断工具，揭示了当前LLM在整合世界知识和句法结构方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 中文到东南亚低资源语言的神经机器翻译面临两个主要挑战：一是干净平行语料库的极度稀缺，二是现有挖掘数据中的普遍噪声。这种长期短缺不仅阻碍了有效的模型训练，还导致与高资源语言方向存在巨大性能差距，使得数百万老挝语、缅甸语、他加禄语等语言使用者无法获得高质量的翻译系统。

Method: 提出了MERIT（多语言专家奖励信息调优）统一翻译框架，包含三个核心组件：1) 语言特定标记前缀（LTP）；2) 监督微调（SFT）；3) 基于语义对齐奖励（SAR）指导的组相对策略优化（GRPO）。该框架将传统的以英语为中心的ALT基准转换为以中文为中心的五种东南亚低资源语言评估套件。

Result: 研究结果表明，在低资源语言到中文的翻译任务中，针对性的数据整理和奖励引导的优化策略显著优于单纯的模型扩展。MERIT框架有效解决了数据稀缺和噪声问题，提升了翻译质量。

Conclusion: 对于低资源语言翻译任务，精心设计的数据策略和奖励引导优化比单纯增加模型规模更为有效。MERIT框架为中文到东南亚低资源语言翻译提供了系统性的解决方案，通过语言特定标记、监督微调和语义对齐奖励优化，显著缩小了与高资源语言翻译的性能差距。
...
TL;DR: 提出PCSA框架，通过人格化客户模拟对话来测试LLMs在心理治疗中的安全性，发现现有模型容易提供未经授权的医疗建议、强化妄想和鼓励危险行为。


<details>
  <summary>Details</summary>
Motivation: LLMs在心理健康领域的应用日益增多，但现有红队测试框架主要关注通用危害或基于优化的攻击，忽视了治疗性同理心与适应性不良验证之间的关键区别，即支持性回应可能无意中强化多轮对话中的有害信念或行为。

Method: 提出人格化客户模拟攻击(PCSA)框架，通过连贯的、基于人格的客户对话来模拟心理咨询中的客户，以暴露心理安全对齐中的漏洞。该方法在七个通用和心理健康专用LLMs上进行实验，并与四个竞争基线进行比较。

Result: PCSA显著优于四个竞争基线。困惑度分析和人工检查进一步表明PCSA生成更自然和真实的对话。结果显示当前LLMs仍然容易受到领域特定对抗策略的攻击，包括提供未经授权的医疗建议、强化妄想和隐含地鼓励危险行为。

Conclusion: 需要更专业的红队测试方法来评估LLMs在心理健康等高风险领域的心理安全对齐，当前模型存在严重漏洞，可能对用户造成实际伤害。
...
TL;DR: 提出AsymGRPO框架，通过解耦正负rollouts的调控，实现信息熵保留和虚假熵抑制，解决RLVR中的受限探索问题


<details>
  <summary>Details</summary>
Motivation: 强化学习与可验证奖励（RLVR）显著提升了LLMs的推理能力，但面临"受限探索"的根本限制——策略迅速收敛到狭窄解集。虽然熵正则化是维持探索的常用方法，但对LLMs效果不佳，存在高超参数敏感性和边际性能提升的问题

Method: 重新思考策略熵与探索的关系，推导组相对优势估计的参数化公式并分析熵动态，将策略熵概念分解为"信息熵"（保留多样化解路径）和"虚假熵"（侵蚀推理模式）。提出AsymGRPO探索框架，明确解耦正负rollouts的调控，实现对信息熵保留和虚假噪声抑制的独立控制

Result: 大量实验表明，AsymGRPO相比强基线实现了更优性能，并展现出与现有熵正则化方法协同的潜力

Conclusion: 有效探索需要"熵精炼"机制，而非盲目最大化熵。AsymGRPO通过解耦正负rollouts调控，在正rollouts上维持信息熵，在负rollouts上抑制虚假熵，为RLVR中的探索问题提供了新解决方案
...
TL;DR: TriAttention通过分析RoPE前空间中Q/K向量的集中特性，提出基于三角函数级数的键重要性估计方法，显著提升KV缓存压缩效率，在保持推理精度的同时实现2.5倍吞吐量提升或10.7倍内存减少。


<details>
  <summary>Details</summary>
Motivation: 大语言模型中的扩展推理导致严重的KV缓存内存瓶颈。现有基于注意力分数的KV缓存压缩方法在RoPE旋转后查询空间代表性不足，导致键选择效果差且推理不稳定。需要寻找更稳定的重要性估计方法。

Method: 观察到RoPE前空间中Q和K向量高度集中在固定非零中心且位置稳定（Q/K集中现象）。基于此提出TriAttention：1）利用三角函数级数，通过中心表征的距离偏好对键位置进行评分；2）结合Q/K范数作为重要性估计的额外信号。

Result: 在AIME25数据集32K令牌生成任务中，TriAttention在保持Full Attention推理精度的同时，实现2.5倍吞吐量提升或10.7倍KV内存减少。相比之下，现有基线方法在相同效率下只能达到约一半的精度。TriAttention使OpenClaw能在单消费级GPU上部署，而Full Attention会因长上下文导致内存不足。

Conclusion: 通过分析RoPE前空间的Q/K集中现象，TriAttention提供了一种稳定且高效的KV缓存压缩方法，解决了现有方法因查询旋转导致的代表性不足问题，为长上下文推理提供了实用的内存优化方案。
...
TL;DR: CoDE-Stop：基于中间答案置信度动态的早期停止方法，无需额外训练即可减少推理计算成本


<details>
  <summary>Details</summary>
Motivation: 大型推理模型依赖长链思维生成解决复杂问题，但过长的推理会增加计算成本，甚至因"过度思考"而降低性能。关键挑战在于确定模型何时应停止推理并生成最终答案。

Method: 提出CoDE-Stop（Confidence Dynamics Early Stop）方法，利用中间答案置信度的动态变化来决定何时终止推理。该方法基于观察：正确推理轨迹往往早期达到高置信度答案，而错误推理则产生冗长、无生产力的推理轨迹且置信度动态不可靠。

Result: 在多个推理和科学基准测试中评估CoDE-Stop，相比现有早期停止方法，实现了更好的准确率-计算权衡，相比标准全长推理减少了25-50%的总令牌使用量。

Conclusion: CoDE-Stop是一种无需额外训练、易于集成到现有模型中的早期停止方法，能有效减少推理计算成本，同时提供了对推理过程中置信度动态变化的分析见解。
...
TL;DR: 提出RACE方法，用于细粒度检测LLM生成文本，区分四种类型：纯人类文本、纯LLM文本、LLM润色的人类文本、人类化的LLM文本
...
Method: RACE方法结合修辞结构理论构建逻辑图捕捉创作者特征，提取基本话语单元级特征捕捉编辑者风格，实现创作者-编辑者建模

Result: RACE在12个基线方法中表现最优，能够准确识别细粒度文本类型且误报率低

Conclusion: RACE为LLM监管提供了政策对齐的解决方案，能够有效支持细粒度文本检测需求
...
TL;DR: 本文综述了大型语言模型在医疗健康领域的应用进展，重点探讨了其在诊断治疗、癌症护理、皮肤病学、牙科、神经退行性疾病和心理健康等领域的创新贡献。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在医疗健康领域的快速发展，需要系统梳理其在各医疗专业领域的应用现状、挑战与机遇，为医疗从业者和研究者提供全面的参考框架。

Method: 采用文献综述方法，综合分析大型语言模型在医疗健康领域的应用案例，重点关注诊断治疗相关功能，并探讨不同医疗专业领域的具体应用场景。

Result: 大型语言模型在癌症护理、皮肤病学、牙科、神经退行性疾病和心理健康等多个医疗领域展现出创新应用潜力，能够支持医生、医疗提供者和患者，但同时也面临数据多样性处理等挑战。

Conclusion: 大型语言模型在医疗健康领域具有变革性影响，尽管存在局限性，但在各医疗专业领域展现出广阔的应用前景，需要继续探索其整合应用的最佳实践。
...
### [214] [Trustless Provenance Trees: A Game-Theoretic Framework for Operator-Gated Blockchain Registries](https://arxiv.org/abs/2604.03434)
*Ian C. Moore*

Main category: cs.GT

TL;DR: 提出区块链上溯源树的形式化处理，解决操作者信任问题，通过双层加密承诺方案确保诚实行为成为唯一纳什均衡，分析树中毒攻击并提出完整性模型的三重机制。


<details>
  <summary>Details</summary>
Motivation: 解决区块链溯源系统中操作者信任问题：当单一特权操作者代表用户提交所有链上注册时，链上记录无法区分用户发起的注册与操作者单方面行为，需要建立无需信任操作者的可信溯源机制。

Method: 采用双层加密承诺方案：从客户端秘密密钥派生两个承诺，将密钥绑定到树根和每个唯一注册标识符，使虚假归属声明成为严格劣势策略。部署为AnchorRegistry（Base/Ethereum L2），提供O(1)气体成本不变性和仅从公共事件日志重建的信任算法。

Result: 在标准密码学假设下证明正确性，建立诚实行为为唯一纳什均衡。分析树中毒攻击的闭合特性，证明完整溯源树完整性模型需要三个独立机制：密码优先级、治理级联和合约执行，每个必要且单独不足。

Conclusion: 通过形式化溯源树和操作者信任问题，提出并实现无需信任操作者的双层加密承诺方案，建立完整攻击模型和防御机制，在以太坊L2上部署为可扩展、可验证的不可变链上溯源注册表。
...
### [215] [Fair Aggregation in Virtual Power Plants](https://arxiv.org/abs/2604.03559)
*Liudong Chen,Hyemi Kim,Adam N. Elmachtoub,Bolun Xu*

Main category: cs.GT

TL;DR: 该论文提出了一个公平感知的虚拟电厂定价框架，分析不同公平概念如何影响系统性能，发现利润最大化定价会损害灵活性较低的消费者，而效用公平性最能保护弱势消费者。


<details>
  <summary>Details</summary>
Motivation: 虚拟电厂聚合商通过激励价格引导消费者参与电力市场，但消费者灵活性异质性导致利润最大化定价会产生不平等，需要研究公平定价如何影响系统性能。

Method: 建立Stackelberg博弈模型分析聚合商与消费者互动，考虑三种公平标准：能源公平、价格公平和效用公平，使用挪威分层定价方案实验数据进行验证。

Result: 利润最大化定价系统性损害灵活性较低消费者；能源公平可能改善或恶化所有性能指标；价格公平从未使灵活性较低消费者受益；效用公平能保护灵活性较低消费者而不损害灵活性较高者。

Conclusion: 不同公平定义和执行水平对运营和福利结果有系统性影响，效用公平性最能保护弱势消费者，为监管机构和VPP运营商提供了公平定义与结果之间的系统映射。
...
### [216] [Optimal Contest Beyond Convexity](https://arxiv.org/abs/2604.04844)
*Negin Golrezaei,MohammadTaghi Hajiaghayi,Suho Shin*

Main category: cs.GT

TL;DR: 该论文研究了竞赛设计问题，证明了在非凸目标函数下最优机制具有高度结构化特征：第一名获得最高奖金，最后一名为零奖金，中间名次获得相等奖金。


<details>
  <summary>Details</summary>
Motivation: 研究竞赛设计问题中如何设计激励机制来激励参赛者付出更高努力，特别是在非凸目标函数（如用户福利与平均质量的凸组合、任意正多项式等）下的最优机制设计，这些目标函数在现有文献中很少被关注。

Method: 利用Bernstein基多项式加权函数的Schur凸性、全正性和变差缩减性质等数学工具，将高维非凸优化问题简化为单维优化问题，通过分析目标函数梯度序列的形状与最优解中转折点数量的关系。

Result: 证明了在所有研究的目标函数类型下，最优机制都具有高度结构化特征：p₁ ≥ p₂ = ... = p_{n-1} ≥ p_n = 0。基于这一结构特征，获得了给定值预言机的完全多项式时间近似方案。

Conclusion: 该研究深化了对一般设置下竞赛设计最优机制的理解，揭示了非凸目标函数下最优机制的统一结构化特征，并提供了有效的计算算法，相关技术方法可能具有独立的理论价值。
...
### [217] [IC3-Evolve: Proof-/Witness-Gated Offline LLM-Driven Heuristic Evolution for IC3 Hardware Model Checking](https://arxiv.org/abs/2604.03232)
...
<details>
  <summary>Details</summary>
Motivation: IC3算法性能严重依赖于大量交互式启发式方法和实现选择，手动调优成本高、脆弱且难以复现，需要自动化方法来改进IC3实现

Method: 提出IC3-Evolve框架，利用LLM对IC3实现提出小规模、槽位受限且可审计的补丁，通过证明/见证门控验证：SAFE运行必须生成可独立验证的证书，UNSAFE运行必须生成可重放的counterexample轨迹，防止不健全的编辑被部署

Result: 在公开硬件模型检查竞赛基准上进化，在未见过的公开和工业模型检查基准上评估泛化能力，证明IC3-Evolve能够在严格正确性门控下可靠发现实用的启发式改进

Conclusion: IC3-Evolve通过离线LLM驱动的代码进化框架，实现了对IC3算法的自动化改进，部署的工件是独立的进化检查器，无ML/LLM推理开销和运行时模型依赖，能够可靠发现实用的启发式改进
...
### [218] [Structural Segmentation of the Minimum Set Cover Problem: Exploiting Universe Decomposability for Metaheuristic Optimization](https://arxiv.org/abs/2604.03234)
*Isidora Hernández,Héctor Ferrada,Cristóbal A. Navarro*

Main category: cs.AI

TL;DR: 该论文研究了最小集合覆盖问题中的宇宙可分割性概念，提出基于并查集的预处理策略将问题分解为独立子问题，使用GRASP元启发式求解并组合结果，显著提升了大规模结构化实例的求解质量和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 最小集合覆盖问题是经典的NP难组合优化问题，现有方法大多将问题实例视为整体处理，忽略了宇宙集合可能存在的内在结构特性。本文旨在探索如何利用宇宙的可分割性来增强启发式优化算法的性能。

Method: 提出基于并查集的预处理策略，通过元素在子集中的共现关系检测连通分量，将原始实例分解为独立的子问题。每个子问题使用GRASP元启发式算法求解，然后组合部分解而不影响可行性。采用简洁的位级集合表示实现高效的集合操作。

Result: 在标准基准实例和大规模合成数据集上的实验表明，利用自然宇宙分割能持续改进解的质量和可扩展性，特别是对于大型且结构可分解的实例。位级集合表示使该方法在计算上具有实际可行性。

Conclusion: 宇宙可分割性是MSCP中可利用的重要结构特性，基于并查集的分解策略结合GRASP元启发式算法能有效提升求解性能，为大规模结构化实例提供了实用的解决方案。
...
### [219] [Position: Science of AI Evaluation Requires Item-level Benchmark Data](https://arxiv.org/abs/2604.03244)
*Han Jiang,Susu Zhang,Xiaoyuan Yi,Xing Xie,Ziang Xiao*

Main category: cs.AI

TL;DR: 论文主张在AI评估中采用项目级数据分析，以解决当前评估范式的系统性有效性缺陷，并提出了OpenEval数据仓库来支持基于证据的AI评估。


<details>
  <summary>Details</summary>
Motivation: 当前AI评估范式存在系统性有效性缺陷，包括不合理的设计选择和错位的评估指标。这些问题在没有系统性有效性证据收集框架和细粒度诊断分析的情况下难以解决，而AI评估结果正被用于高风险领域的生成式AI系统部署决策。

Method: 提出采用项目级AI基准数据作为AI评估科学的基础，通过项目级分析实现细粒度诊断和基准验证。论文通过分析计算机科学和心理测量学中的评估范式，以及项目属性和潜在构念的示例分析来支持这一观点。

Result: 项目级数据能够提供独特的洞察力，支持细粒度诊断和基于证据的评估验证。为促进社区采用，作者引入了OpenEval——一个不断增长的项目级基准数据仓库，专门用于支持基于证据的AI评估。

Conclusion: 项目级AI基准数据对于建立严谨的AI评估科学至关重要。通过采用项目级分析，可以解决当前评估范式的有效性缺陷，实现更可靠、可解释的AI系统评估，特别是在高风险应用领域。
...
### [220] [Evaluating Artificial Intelligence Through a Christian Understanding of Human Flourishing](https://arxiv.org/abs/2604.03356)
*Nicholas Skytland,Lauren Parsons,Alicia Llewellyn,Steele Billings,Peter Larson,John Anderson,Sean Boisen,Steve Runge*

Main category: cs.AI

TL;DR: 论文提出AI对齐不仅是安全问题，更是塑造问题，引入基于基督教繁荣观的评估框架FAI-C-ST，发现前沿模型默认采用程序性世俗主义，导致在信仰维度表现显著下降


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地参与道德审议和精神探索，它们不仅仅是提供信息，而是作为数字教化的工具，积极塑造和规范人类的理解、决策和道德反思。需要使这种塑造性影响变得可见和可测量，评估AI系统是否真正世界观中立

Method: 引入繁荣AI基准：基督教单轮对话（FAI-C-ST）框架，基于基督教对人类繁荣的理解，在七个维度上评估前沿模型响应。比较20个前沿模型在多元主义和基督教特定标准下的表现

Result: 当前AI系统并非世界观中立，而是默认采用程序性世俗主义，缺乏维持神学连贯性的基础。在所有繁荣维度上系统性表现下降约17点，在信仰和灵性维度下降最为严重，达到31点

Conclusion: 价值观对齐中的性能差距不是技术限制，而是源于训练目标优先考虑广泛可接受性和安全性，而非深入、内在连贯的道德或神学推理。AI对齐根本上是一个塑造问题，而不仅仅是安全问题
...
### [221] [VERT: Reliable LLM Judges for Radiology Report Evaluation](https://arxiv.org/abs/2604.03376)
*Federica Bologna,Jean-Philippe Corbeil,Matthew Wilkens,Asma Ben Abacha*

Main category: cs.AI

TL;DR: 该研究系统评估了放射学报告评估中LLM作为评判者的性能，提出了VERT指标，通过微调Qwen3 30B模型显著提升了与放射科医生判断的相关性，并减少了推理时间。


<details>
  <summary>Details</summary>
Motivation: 当前放射学报告评估研究主要关注胸部X射线和基于LLM的指标设计，但缺乏对其他模态和解剖结构的鲁棒性验证。需要确定最适合作为放射学评估LLM评判者的模型和提示配置。

Method: 1) 在RadEval和RaTE-Eval两个专家标注数据集上进行相关性分析；2) 比较三种现有LLM评判指标(RadFact、GREEN、FineRadScore)和提出的VERT指标；3) 使用不同规模的开源和闭源模型(推理和非推理)；4) 评估少样本方法、集成学习和参数高效微调；5) 进行系统性错误检测和分类研究。

Result: 1) VERT相比GREEN将相关性提升了11.7%；2) 微调Qwen3 30B仅用1,300个训练样本就获得了25%的性能提升；3) 微调模型将推理时间减少了37.2倍；4) 识别了指标与专家判断在不同方面的对齐程度。

Conclusion: LLM作为评判者在放射学报告评估中表现有效，轻量级适应即可实现可靠评估。微调中等规模模型能显著提升性能并大幅降低推理成本，为实际临床应用提供了可行方案。
...
### [222] [Hume's Representational Conditions for Causal Judgment: What Bayesian Formalization Abstracted Away](https://arxiv.org/abs/2604.03387)
*Yiling Wu*

Main category: cs.AI

TL;DR: 该论文从休谟文本中提取了因果判断的三个表征条件，追踪了从休谟到贝叶斯认识论和预测处理的演变，并以大语言模型为例说明现代框架如何保留更新结构但抽象掉休谟的进一步表征条件。


<details>
  <summary>Details</summary>
Motivation: 论文旨在阐明休谟因果判断理论中隐含的三个表征条件，并追踪这些条件在后续形式化框架中的演变，揭示现代认知科学如何继承休谟的更新结构同时抽象掉其具体的表征要求。

Method: 1. 从休谟文本中提取三个表征条件：经验基础、结构化检索、生动性转移；2. 论证这些条件是休谟因果心理学的组成部分；3. 追踪从休谟到贝叶斯认识论和预测处理的形式化轨迹；4. 以大语言模型为案例说明现代框架如何体现统计更新但不满足休谟的三个条件。

Result: 1. 成功识别并论证了休谟因果判断的三个表征条件；2. 展示了这些条件在后续形式化框架中被抽象化的过程；3. 揭示了大语言模型等现代系统如何使原本在休谟框架中作为背景假设的要求变得可见。

Conclusion: 休谟的因果判断理论包含三个重要的表征条件，这些条件在从休谟到现代贝叶斯框架的形式化过程中被逐渐抽象掉。大语言模型等当代系统通过展示纯粹的统计更新而不满足这些条件，使原本隐含的表征要求变得显性，从而深化了我们对认知架构演变的理解。
...
### [223] [TABQAWORLD: Optimizing Multimodal Reasoning for Multi-Turn Table Question Answering](https://arxiv.org/abs/2604.03393)
*Tung Sum Thomas Kwok,Xinyu Wang,Xiaofeng Lin,Peng Lu,Chunhe Wang,Changlun Li,Hanwei Wu,Nan Tang,Elisa Kreiss,Guang Cheng*

Main category: cs.AI

TL;DR: TABQAWORLD是一个免训练的多模态表格推理框架，通过联合优化表示和估计来解决多轮表格推理中的表示误差累积问题，实现了最先进的性能表现。


<details>
  <summary>Details</summary>
Motivation: 现有的多轮表格推理方法依赖固定的文本序列化进行表格状态读取，这引入了表示误差并在多轮中显著累积。虽然表格接地方法可以缓解这一问题，但以推理计算和成本为代价，使得实际部署不切实际。
...
Result: TABQAWORLD实现了最先进的性能，相比基线模型准确率提升4.87%，相比静态设置准确率提升5.42%且推理延迟减少33.35%。

Conclusion: TABQAWORLD为可靠且高效的表格推理设立了新标准，通过免训练框架解决了多轮推理中的表示误差累积问题，在准确性和效率方面均有显著提升。
...
### [224] [Contextual Control without Memory Growth in a Context-Switching Task](https://arxiv.org/abs/2604.03479)
*Song-Ju Kim*

Main category: cs.AI

TL;DR: 论文提出了一种基于干预的循环架构，通过干预共享循环潜在状态实现上下文依赖，无需增加循环维度或直接提供上下文输入。
...
Result: 在主要基准测试中，干预模型在不增加循环维度的情况下表现良好。使用条件互信息(I(C;O|S))作为操作探针，干预模型对任务相关的第一阶段结果显示出正的条件上下文信息。

Conclusion: 干预共享循环状态为上下文控制提供了一种可行的替代方案，无需增加循环记忆维度，在该设置下具有实际应用价值。
...
### [225] [Beyond Predefined Schemas: TRACE-KG for Context-Enriched Knowledge Graphs from Complex Documents](https://arxiv.org/abs/2604.03496)
*Mohammad Sadeq Abolhasani,Yang Ba,Yixuan He,Rong Pan*

Main category: cs.AI

TL;DR: TRACE-KG是一个无需预定义本体、联合构建上下文增强知识图谱和诱导模式的多模态框架，通过结构化限定词捕获条件关系，提供可重用语义支架。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱构建方法存在局限性：基于本体的方法需要昂贵的模式设计和维护成本，而无模式方法产生的图谱往往碎片化且缺乏全局组织，特别是在处理包含密集上下文依赖信息的长技术文档时。

Method: TRACE-KG采用多模态框架，无需预定义本体，联合构建上下文增强知识图谱和诱导模式。通过结构化限定词捕获条件关系，使用数据驱动模式组织实体和关系，同时保持对源证据的完全可追溯性。

Result: 实验表明，TRACE-KG能够生成结构连贯、可追溯的知识图谱，为基于本体和无模式的构建流程提供了实用的替代方案。

Conclusion: TRACE-KG框架在无需预定义本体的前提下，通过联合构建知识图谱和诱导模式，有效解决了现有方法的局限性，提供了结构连贯且可追溯的知识图谱构建方案。
...
### [226] [Resource-Conscious Modeling for Next- Day Discharge Prediction Using Clinical Notes](https://arxiv.org/abs/2604.03498)
*Ha Na Cho,Sairam Sutari,Alexander Lopez,Hansen Bow,Kai Zheng*

Main category: cs.AI

TL;DR: 该研究评估了轻量级微调大语言模型与传统文本模型在预测脊柱择期手术患者次日出院方面的表现，发现TF-IDF结合LGBM模型在性能与效率上达到最佳平衡。
...
Method: 研究比较了13种模型，包括TF-IDF结合XGBoost和LGBM的传统文本模型，以及通过LoRA微调的紧凑型LLMs（DistilGPT-2、Bio_ClinicalBERT），使用术后临床笔记预测次日出院。

Result: TF-IDF结合LGBM模型表现最佳，出院类别的F1分数为0.47，召回率为0.51，AUC-ROC最高（0.80）。LoRA微调提升了DistilGPT2的召回率，但整体上基于Transformer和生成模型表现不佳。

Conclusion: 在现实世界不平衡的临床预测任务中，可解释且资源高效的模型可能优于紧凑型大语言模型，为临床决策支持系统提供了实用方案。
...
### [227] [BioAlchemy: Distilling Biological Literature into Reasoning-Ready Reinforcement Learning Training Data](https://arxiv.org/abs/2604.03506)
...
TL;DR: BioAlchemy是一个从生物学研究文本中提取可验证问答对的流程，创建了包含34.5万个科学推理问题的数据集，并通过强化学习提升生物学推理性能，最终模型在生物学基准上比基础模型提升9.12%


<details>
  <summary>Details</summary>
Motivation: 当前大规模推理数据集中的生物学问题与现代生物学研究主题分布不匹配，这种主题不平衡可能影响性能；同时，从生物学研究文本中提取具有挑战性且可验证的研究问题的方法尚未充分发展，限制了强化学习在生物学研究任务中的应用

Method: 提出BioAlchemy流程，从生物学研究文本语料库中提取多样化的可验证问答对；创建BioAlchemy-345K训练数据集；通过强化学习将数据集与当代科学生物学主题分布对齐以提升推理性能

Result: 创建了包含超过345,000个科学推理问题的BioAlchemy-345K数据集；开发的BioAlchemist-8B模型在生物学基准测试中比基础推理模型提升了9.12%的性能

Conclusion: BioAlchemy方法能有效提升生物学领域的科学推理能力，通过主题对齐的数据集和强化学习可以显著改善生物学推理模型的性能
...
### [228] [Structural Rigidity and the 57-Token Predictive Window: A Physical Framework for Inference-Layer Governability in Large Language Models](https://arxiv.org/abs/2604.03524)
*Gregory M. Ruddell*

Main category: cs.AI

TL;DR: 论文提出基于能量的治理框架，通过轨迹张力分析发现指令调优模型中预承诺信号普遍缺失，仅Phi-3-mini在特定条件下存在57个token的检测窗口，揭示了规则违反与幻觉是不同故障模式。


<details>
  <summary>Details</summary>
Motivation: 当前AI安全依赖行为监控和后训练对齐，但实证测量显示这些方法在多数指令调优模型中无法产生可检测的预承诺信号。需要建立连接Transformer推理动态与约束满足模型的治理框架，以评估自主AI系统的部署风险。

Method: 提出基于能量的治理框架，将Transformer推理动态连接到神经计算的约束满足模型。使用轨迹张力(rho = ||a|| / ||v||)作为核心指标，在七个模型队列中分析五个几何机制，引入能量不对称性作为结构刚度的统一度量。

Result: 仅在Phi-3-mini-4k-instruct模型的贪婪解码算术约束探测中发现57个token的预承诺窗口。提出推理行为的五机制分类：权威带、晚期信号、倒置、平坦和支架选择性。七个模型中仅一个配置显示预测性信号，其余显示静默失败、晚期检测、倒置动态或平坦几何。事实幻觉在72个测试条件下均无预测信号。

Conclusion: 规则违反和幻觉是不同故障模式，需要不同的检测方法。内部几何监控仅在存在阻力时有效，事实虚构的检测需要外部验证机制。该工作为推理层可治理性提供了可测量框架，并引入了评估自主AI系统部署风险的分类学。
...
### [229] [Explainable Model Routing for Agentic Workflows](https://arxiv.org/abs/2604.03527)
...
<details>
  <summary>Details</summary>
Motivation: 当前智能体工作流的路由架构仅关注性能优化，未记录模型能力与成本之间的权衡关系，导致开发者无法区分智能效率与预算驱动的潜在失败。

Method: Topaz框架包含三个核心组件：1) 基于技能的分析，将多样化基准测试合成为细粒度能力档案；2) 完全可追溯的路由算法，使用基于预算和多目标优化；3) 面向开发者的解释，将追踪信息转化为自然语言。

Result: 通过使路由决策可解释，Topaz使用户能够理解、信任并有效引导路由的智能体系统，实现成本-质量权衡的迭代调优。

Conclusion: Topaz框架为智能体路由引入形式化可审计性，解决了当前路由架构缺乏透明性的问题，使开发者能够审计系统逻辑并优化成本-质量权衡。
...
### [230] [Automated Analysis of Global AI Safety Initiatives: A Taxonomy-Driven LLM Approach](https://arxiv.org/abs/2604.03533)
*Takayuki Semitsu,Naoto Kiribuchi,Kengo Zenitani*

Main category: cs.AI

TL;DR: 提出自动化交叉比对框架，使用共享活动分类法比较AI安全政策文档，评估LLM在政策文档分析中的稳定性和有效性


<details>
  <summary>Details</summary>
Motivation: 需要系统化比较AI安全政策文档，评估大型语言模型在政策文档分析中的表现，了解模型选择对分析结果的影响

Method: 基于AI安全活动地图定义的活动类别作为固定方面，构建自动化交叉比对框架，提取和映射相关活动，生成摘要、比较和相似度评分；使用5个LLM对10个公开文档进行交叉比对，并通过热图可视化平均相似度得分；进行专家人工评估验证

Result: 模型选择显著影响交叉比对结果，部分文档对在不同模型间存在高度分歧；三位专家对两个文档对的评估显示高标注者间一致性，但模型评分仍与人工判断存在差异

Conclusion: 研究支持政策文档的比较性检查，揭示了LLM在政策分析中的局限性，模型选择对结果有实质性影响，需要谨慎使用自动化工具进行政策文档分析
...
### [231] [When Do Hallucinations Arise? A Graph Perspective on the Evolution of Path Reuse and Path Compression](https://arxiv.org/abs/2604.03557)
*Xinnan Dai,Kai Yang,Cheng Luo,Shenglai Zeng,Kai Guo,Jiliang Tang*

Main category: cs.AI

TL;DR: 论文提出将LLM的推理幻觉建模为图搜索过程，揭示了路径重用和路径压缩两种机制导致幻觉产生
...
Method: 将下一个token预测建模为图搜索过程，实体为节点，学习到的转移形成边；区分内在推理（上下文约束搜索）和外在推理（记忆结构依赖）

Result: 发现推理幻觉源于两种机制：路径重用（早期训练中记忆知识覆盖上下文约束）和路径压缩（后期训练中多步路径坍缩为捷径边）
...
### [232] [When Adaptive Rewards Hurt: Causal Probing and the Switching-Stability Dilemma in LLM-Guided LEO Satellite Scheduling](https://arxiv.org/abs/2604.03562)
*Yuanhang Li*

Main category: cs.AI

TL;DR: 自适应奖励设计在LEO卫星调度中面临切换-稳定性困境：静态权重优于动态调整，因为PPO需要准稳态奖励信号。因果探测揭示反直觉杠杆效应，MLP在已知/新流量场景表现稳定，而微调LLM因权重振荡而失效。


<details>
  <summary>Details</summary>
Motivation: 研究自适应奖励设计在深度强化学习多波束LEO卫星调度中的应用动机，基于直觉认为机制感知的奖励权重应优于静态权重，但需要系统验证这一假设。

Method: 1) 系统测试自适应奖励设计的性能；2) 引入单变量因果探测方法，独立扰动每个奖励项±20%并测量PPO响应；3) 评估四种MDP架构变体：固定权重、基于规则的、学习的MLP、微调LLM；4) 在已知和新流量场景下进行测试。

Result: 1) 发现切换-稳定性困境：静态权重(342.1 Mbps)优于动态调整(103.3±96.8 Mbps)；2) 因果探测揭示反直觉杠杆：切换惩罚+20%在极地切换场景提升157 Mbps，在热-冷场景提升130 Mbps；3) MLP在已知场景达357.9 Mbps，新场景325.2 Mbps；4) 微调LLM崩溃至45.3±43.0 Mbps，因权重振荡而非领域知识不足。

Conclusion: PPO需要准稳态奖励信号进行价值函数收敛，权重自适应会反复重启收敛过程。LLM-DRL集成中，输出一致性而非领域知识是主要约束，LLM在自然语言意图理解中具有不可替代价值，但在奖励权重调整中简单方法更有效。
...
### [233] [Selective Forgetting for Large Reasoning Models](https://arxiv.org/abs/2604.03571)
*Tuan Le,Wei Qian,Mengdi Huai*

Main category: cs.AI

TL;DR: 提出了一种针对大型推理模型的选择性遗忘框架，通过识别和替换推理链中的敏感部分来保护知识隐私，同时保持整体推理能力


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过生成结构化推理链来产生最终答案，这使得中间推理步骤容易泄露敏感信息（如版权内容和隐私数据）。现有遗忘方法主要针对最终答案，可能会损害模型的整体推理能力，而直接对整个推理链应用遗忘也会降低一般推理能力。

Method: 提出了一种新颖的LRM遗忘框架：1）利用多个LLM结合检索增强生成技术分析推理链轨迹；2）识别需要遗忘的相关片段；3）用保持逻辑结构的良性占位符替换这些片段；4）引入特征替换遗忘损失函数，同时抑制生成遗忘内容的概率并强化结构有效的替换。

Result: 在合成数据集和医疗数据集上的广泛实验验证了所提方法的期望特性，能够精确遗忘目标知识同时保持一般推理能力的完整性。

Conclusion: 该方法为解决大型推理模型中的知识泄漏问题提供了一种有效解决方案，能够在保护敏感信息的同时维持模型的推理性能，为机器遗忘在复杂推理任务中的应用开辟了新途径。
...
### [234] [Rashomon Memory: Towards Argumentation-Driven Retrieval for Multi-Perspective Agent Memory](https://arxiv.org/abs/2604.03588)
*Albert Sadowski,Jarosław A. Chudziak*

Main category: cs.AI

TL;DR: 提出Rashomon Memory架构，支持并行目标条件智能体根据各自优先级编码经验，通过论证在查询时协商，保留冲突解释而非强制统一


<details>
  <summary>Details</summary>
Motivation: 当前AI智能体在长期运作中积累的经验服务于多个并发目标，经常需要对同一事件保持冲突解释。现有内存架构假设单一正确编码或仅支持统一存储上的多视图，无法处理真正的解释冲突

Method: 提出Rashomon Memory架构：并行目标条件智能体根据各自优先级编码经验，每个视角维护自己的本体和知识图谱。检索时，视角提出解释，使用非对称领域知识相互批评，通过Dung的论证语义学确定哪些提议保留。生成的攻击图本身作为解释

Result: 概念验证显示检索模式（选择、组合、冲突浮现）从攻击图拓扑中涌现。冲突浮现模式让系统报告真实分歧而非强制解决，使决策者能直接看到底层解释冲突

Conclusion: Rashomon Memory架构支持AI智能体处理经验的多重冲突解释，通过论证协商和攻击图提供透明解释，特别适用于需要保留而非解决解释冲突的决策场景
...
### [235] [Entropy and Attention Dynamics in Small Language Models: A Trace-Level Structural Analysis on the TruthfulQA Benchmark](https://arxiv.org/abs/2604.03589)
*Adeyemi Adeseye,Aisvarya Adeseye,Hannu Tenhunen,Jouni Isoaho*

Main category: cs.AI

TL;DR: 该研究对小型语言模型在TruthfulQA数据集上的熵和注意力动态进行追踪分析，揭示了三种不同的熵模式分类及其与模型真实性的关系。


<details>
  <summary>Details</summary>
Motivation: 小型语言模型在资源受限环境中部署时存在自信的错误预测和不稳定输出问题，当前评估方法仅关注最终准确率或幻觉率，缺乏对内部模型行为如何影响输出的深入分析，特别是熵在解码过程中的演变、注意力在层间的分布以及隐藏表示如何影响不确定性、逻辑不一致性和错误信息传播。

Method: 使用TruthfulQA数据集对四个参数范围在1B-1.7B的小型语言模型进行追踪分析，通过令牌级输出熵、注意力熵、头部分散度和隐藏状态表示等指标来研究模型内部行为。

Result: 根据熵模式将模型分为三类：确定性模型（DeepSeek-1.5B和LLaMA-1B）输出熵随时间递减；探索性模型（Gemma-1B）熵随时间增加；平衡模型（Qwen-1.7B）具有中等且稳定的熵。每组模型在隐藏状态移动和注意力分散模式上也有显著差异。

Conclusion: 小型语言模型的真实性源于结构化的熵和注意力动态，监控和优化这些内部不确定性模式可以指导设计更可靠、幻觉感知且特定于应用的边缘小型语言模型。
...
### [236] [A Multimodal Foundation Model of Spatial Transcriptomics and Histology for Biological Discovery and Clinical Prediction](https://arxiv.org/abs/2604.03630)
...
TL;DR: STORM是一个空间转录组学基础模型，整合了120万个空间转录组谱与匹配的组织学图像，通过层次架构连接形态学、基因表达和空间上下文，能够从H&E图像预测基因表达并改善免疫治疗响应预测。


<details>
  <summary>Details</summary>
Motivation: 空间转录组学(ST)虽然能在解剖背景下映射基因表达，但成本高、通量低；而H&E染色虽然提供丰富的形态学信息，但缺乏分子分辨率。需要一种能够桥接成像和组学的方法，实现形态学与分子特征的整合。

Method: 开发了STORM（空间转录组学和组织学表示模型），这是一个基础模型，在18个器官的120万个空间解析转录组谱和匹配的组织学图像上进行训练。采用层次架构整合形态学特征、基因表达和空间上下文，生成稳健的分子-形态学表示。

Result: STORM增强了空间域发现，产生生物学一致的组织图谱；在11种肿瘤类型中，从H&E图像预测空间基因表达方面优于现有方法。模型平台无关，在Visium、Xenium、Visium HD和CosMx上表现一致。应用于23个独立队列（7,245名患者）显著改善了免疫治疗响应预测和预后评估。

Conclusion: STORM提供了一个可扩展的框架，用于空间信息发现和临床精准医学，通过整合形态学和分子数据，显著提升了空间转录组学分析和临床预测能力。
...
### [237] [Single-agent vs. Multi-agents for Automated Video Analysis of On-Screen Collaborative Learning Behaviors](https://arxiv.org/abs/2604.03631)
*Likai Peng,Shihui Feng*

Main category: cs.AI

TL;DR: 本研究比较了闭源和开源视觉语言模型在单智能体与多智能体设置下对协作学习场景中屏幕录制视频进行自动化编码的性能，提出了两种多智能体框架并验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 屏幕学习行为分析对于理解学生在学习过程中的信息寻求、使用和创造至关重要，但传统的手动编码方法劳动密集。视觉语言模型的发展为自动化分析多模态视频数据提供了新机遇，本研究旨在探索VLM在多智能体系统中的应用效果。

Method: 比较了闭源VLM（Claude-3.7-Sonnet, GPT-4.1）和开源VLM（Qwen2.5-VL-72B）在单智能体和多智能体设置下的性能。提出了两种多智能体框架：1）基于工作流的三智能体系统，通过场景分割和光标提示的VLM检测进行证据验证；2）受ReAct启发的自主决策多智能体系统，迭代进行推理、工具操作和观察驱动的自我修正。

Result: 实验结果表明，两种提出的多智能体框架均取得了可行性能，在场景和动作检测任务上优于单VLM。工作流智能体在场景检测上表现最佳，自主决策智能体在动作检测上表现最佳。

Conclusion: 本研究证明了基于VLM的多智能体系统在视频分析中的有效性，为多模态数据分析提供了一个可扩展的框架，能够自动化分析协作学习中的屏幕行为。
...
### [238] [TableVision: A Large-Scale Benchmark for Spatially Grounded Reasoning over Complex Hierarchical Tables](https://arxiv.org/abs/2604.03660)
*Xiaoyu Chen,Lu Dai,Hanqing Wang,Zhuoyu Li,Wenbin Dai,Yanzong Zheng,Zhenggang Xia,Junyong Lin,Hui Xiong*

Main category: cs.AI

TL;DR: TableVision：针对结构化表格理解的大规模轨迹感知基准，通过显式空间约束解决MLLMs中的感知瓶颈问题


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型（MLLMs）在表格理解方面取得进展，但在处理具有层次布局的复杂表格时，推理性能仍然有限。研究发现存在关键的感知瓶颈——随着任务复杂度增加，涉及的离散视觉区域数量不成比例地增长，导致MLLMs在隐式生成过程中难以维持准确的空间注意力，出现"感知过载"。

Method: 1. 引入TableVision基准：大规模、轨迹感知的数据集，用于空间基础推理；2. 将表格任务分层为三个认知级别（感知、推理、分析），涵盖13个子类别；3. 使用基于渲染的确定性基础管道，将多步逻辑推导与像素级空间真值显式耦合；4. 包含6,799个高保真推理轨迹；5. 提出两阶段解耦框架。

Result: 1. 诊断探测显示，显式空间约束显著恢复了MLLMs的推理潜力；2. 两阶段解耦框架在测试集上实现了稳健的12.3%整体准确率提升；3. TableVision为文档理解中感知与逻辑的协同提供了严谨的测试平台和新视角。

Conclusion: TableVision通过显式空间约束有效解决了MLLMs在复杂表格理解中的感知瓶颈问题，证明了空间基础对于多模态推理的重要性，为文档理解研究提供了新的基准和方法论框架。
...
TL;DR: PRAISE框架通过重用长轨迹前缀和生成中间奖励，解决了智能搜索训练中的数据效率低下和奖励稀疏问题


<details>
  <summary>Details</summary>
Motivation: 当前基于搜索的强化学习方法存在两个核心限制：训练中昂贵的长轨迹滚动利用率低，以及监督通常只在最终答案可用，导致严重的奖励稀疏问题

Method: PRAISE框架从完整搜索轨迹中提取不同搜索轮次的前缀状态，从中引出中间答案，并使用这些前缀既构建额外的训练轨迹，又通过前缀间的性能差异推导出步骤级奖励

Result: 在多跳问答基准测试中，PRAISE始终优于强基线方法，提高了性能

Conclusion: PRAISE通过重用轨迹前缀和生成中间奖励，有效提高了智能搜索训练的数据效率和信用分配，无需额外人工标注或单独奖励模型
...
### [240] [Structured Multi-Criteria Evaluation of Large Language Models with Fuzzy Analytic Hierarchy Process and DualJudge](https://arxiv.org/abs/2604.03742)
*Yulong He,Ivan Smirnov,Dmitry Fedrushkov,Sergey Kovalchuk,Ilya Revin*

Main category: cs.AI

TL;DR: 该论文提出了一种基于层次分析法（AHP）和模糊AHP（FAHP）的大语言模型评估方法，通过结构化标准和不确定性建模提升评估可靠性，并进一步提出了结合直觉与审慎评估的DualJudge混合框架。


<details>
  <summary>Details</summary>
Motivation: 传统的大语言模型直接评分方法存在不一致性和不透明性，需要更可靠、结构化的评估框架来克服这些局限性。

Method: 1. 将层次分析法（AHP）应用于LLM评估；2. 提出置信感知的模糊AHP（FAHP），使用三角模糊数建模认知不确定性；3. 提出DualJudge混合框架，通过一致性感知加权自适应融合直接评分和结构化AHP输出。

Result: 在JudgeBench上的系统验证表明：AHP和FAHP在不同模型规模和数据集划分上均优于直接评分；FAHP在不确定比较场景中表现出更好的稳定性；DualJudge实现了最先进的性能。

Conclusion: 不确定性感知的结构化推理为更可靠的LLM评估提供了原则性途径，直觉与审慎评估范式具有互补优势。
...
### [241] [RL-Driven Sustainable Land-Use Allocation for the Lake Malawi Basin](https://arxiv.org/abs/2604.03768)
*Ying Yao*
...
<details>
  <summary>Details</summary>
Motivation: 生态敏感地区不可持续的土地利用实践威胁生物多样性、水资源和数百万人生计，需要开发智能优化工具来平衡生态保护与人类需求

Method: 使用深度强化学习框架，基于Sentinel-2影像划分9种土地覆盖类型，采用PPO算法和动作掩码在50x50网格上优化土地利用分配，奖励函数结合单位生态价值和空间连贯性目标

Result: 智能体有效学习增加总ESV；空间奖励塑造成功引导形成生态合理的土地利用模式；框架对政策参数变化响应良好，证明其作为环境规划情景分析工具的实用性

Conclusion: 深度强化学习框架能够有效优化土地利用分配以最大化生态系统服务价值，同时考虑空间连贯性约束，为环境规划提供了灵活的情景分析工具
...
### [242] [Affording Process Auditability with QualAnalyzer: An Atomistic LLM Analysis Tool for Qualitative Research](https://arxiv.org/abs/2604.03820)
...
<details>
  <summary>Details</summary>
Motivation: 当前许多使用大语言模型进行定性数据分析的工作流程缺乏透明度，难以追踪分析结论的产生过程，这影响了研究的可审计性和方法学严谨性。

Method: 开发QualAnalyzer开源Chrome扩展，支持Google Workspace环境下的原子化LLM分析：独立处理每个数据段，为每个分析单元完整保留提示、输入和输出，创建可追溯的审计轨迹。

Result: 通过两个案例研究（整体性论文评分和访谈转录本的演绎主题编码）验证了该方法能够创建清晰的审计轨迹，帮助研究者系统性地调查LLM与人类判断之间的差异。

Conclusion: 过程可审计性对于提高LLM辅助定性研究的透明度和方法学稳健性至关重要，QualAnalyzer为实现这一目标提供了实用工具。
...
### [243] [FeynmanBench: Benchmarking Multimodal LLMs on Diagrammatic Physics Reasoning](https://arxiv.org/abs/2604.03893)
*Zeyu Wang,Xiaogang Li,Peiyao Xiao,Qinhao Kong,Ben Wang,Chengliang Xu,Zichao Chen,Bing Zhao,Hu Wei*

Main category: cs.AI

TL;DR: FeynmanBench：首个专注于费曼图任务的人工智能基准测试，用于评估多模态大语言模型在满足守恒定律、对称性约束、图拓扑识别、图-代数表示转换等复杂物理推理任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在科学任务中表现出潜力，但现有基准测试主要关注局部信息提取，而非形式化科学符号所固有的全局结构逻辑。需要建立能够评估AI在理论物理等科学发现中有效参与能力的逻辑严谨测试。

Method: 开发了FeynmanBench基准测试，包含自动生成多样化费曼图的流水线，提供可验证的拓扑标注和振幅结果。数据库涵盖标准模型的电磁、弱和强相互作用，包含100多种不同类型和2000多个任务。

Result: 在最先进的多模态大语言模型上的实验揭示了系统性失败模式，包括物理约束执行不稳定和全局拓扑条件违反，凸显了需要基于物理基础的视觉推理基准测试。

Conclusion: FeynmanBench为评估AI是否能够有效参与科学发现（特别是在理论物理领域）提供了逻辑严谨的测试，填补了当前基准测试在全局结构逻辑评估方面的空白。
...
### [244] [FactReview: Evidence-Grounded Reviews with Literature Positioning and Execution-Based Claim Verification](https://arxiv.org/abs/2604.04074)
*Hang Xu,Ling Yue,Chaoqian Ouyang,Libin Zheng,Shaowu Pan,Shimin Di,Min-Ling Zhang*

Main category: cs.AI

TL;DR: FactReview是一个基于证据的机器学习论文评审系统，通过提取论文主张、定位相关文献和执行代码验证来生成证据驱动的评审报告。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习领域的同行评审面临提交量激增和评审时间有限的压力。大多数基于LLM的评审系统仅阅读稿件并从论文自身叙述生成评论，这使得它们的输出对论文呈现质量敏感，且在评审所需证据存在于相关工作或发布代码时表现不佳。
...
Result: 在CompGCN的案例研究中，FactReview复现了与链接预测和节点分类报告结果密切匹配的结果，但也显示论文在跨任务性能的广泛主张并未完全成立：在MUTAG图分类任务中，复现结果为88.4%，而论文报告的最强基线仍为92.6%，因此该主张仅为部分支持。

Conclusion: AI在同行评审中最有用的角色不是作为最终决策者，而是作为收集证据和帮助评审者产生更多基于证据评估的工具。FactReview展示了证据驱动评审系统的潜力，代码已在GitHub公开。
...
### [245] [Compliance-by-Construction Argument Graphs: Using Generative AI to Produce Evidence-Linked Formal Arguments for Certification-Grade Accountability](https://arxiv.org/abs/2604.04103)
*Mahyar T. Moghaddam*

Main category: cs.AI

TL;DR: 提出一种合规性构建架构，将生成式AI与结构化形式论证相结合，确保高风险决策系统的可验证性、可追溯性和可审计性。


<details>
  <summary>Details</summary>
Motivation: 高风险决策系统需要结构化论证、可追溯性和可审计性以确保问责和合规。当前生成式AI在决策支持中的部署存在幻觉推理、无支持主张和弱可追溯性等风险，需要更严格的约束机制。

Method: 提出合规性构建架构，包含四个组件：1) 基于保证案例方法的类型化论证图表示；2) 基于权威证据的检索增强生成(RAG)来起草论证片段；3) 强制执行完整性和可接受性约束的推理与验证内核；4) 符合W3C PROV标准的溯源分类账以支持可审计性。

Result: 通过可执行不变量的评估策略和工作示例分析表明，确定性验证规则能够防止无支持主张进入决策记录，同时允许生成式AI加速论证构建。

Conclusion: 该架构通过将生成式AI与结构化形式论证集成，实现了高风险决策系统中AI辅助步骤的可验证性、可追溯性和合规性，为监管合规提供了技术框架。
...
### [246] [InsTraj: Instructing Diffusion Models with Travel Intentions to Generate Real-world Trajectories](https://arxiv.org/abs/2604.04106)
...
<details>
  <summary>Details</summary>
Motivation: 现有GPS轨迹生成方法面临双重挑战：缺乏深度语义理解以解释复杂用户旅行意图，以及在处理复杂约束时难以保持人类行为的真实多样性。这限制了在城巿规划、移动性模拟和隐私保护数据共享等应用中的实用性。

Method: InsTraj采用两阶段框架：1) 使用大语言模型解析自然语言描述的旅行意图，创建丰富的语义蓝图，弥合意图与轨迹之间的表示差距；2) 提出多模态轨迹扩散transformer，整合语义指导生成高保真且忠实于指令的轨迹，满足细粒度用户意图。
...
### [247] [Readable Minds: Emergent Theory-of-Mind-Like Behavior in LLM Poker Agents](https://arxiv.org/abs/2604.04157)
*Hsieh-Ting Lin,Tsung-Yu Hou*

Main category: cs.AI

TL;DR: LLM智能体在德州扑克长期对局中，仅当具备持久记忆时才会逐步发展出类似心理理论的能力，能够建模对手心理状态并实施策略性欺骗。


<details>
  <summary>Details</summary>
Motivation: 现有研究仅通过静态情境测试LLM是否具备心理理论能力，但未探究在动态交互中类似心理理论的行为能否自然涌现。本研究旨在探索LLM智能体在长期互动中是否能发展出建模他人心理状态的能力。

Method: 采用2x2因子设计：记忆（有/无）× 领域知识（有/无），共20个实验（每个条件5次重复），约6000次智能体-手牌观察。LLM智能体进行德州扑克长期对局，评估其对手建模能力、心理理论水平、策略性欺骗行为及与博弈论最优策略的偏离。

Result: 记忆是心理理论行为涌现的必要且充分条件（Cliff's delta = 1.0，p = 0.008）。有记忆的智能体达到心理理论3-5级（预测性到递归建模），无记忆的智能体始终为0级。策略性欺骗仅出现在有记忆条件下（Fisher精确检验p < 0.001）。领域知识不阻碍心理理论涌现但提升其应用精度（p = 0.004）。具备心理理论的智能体偏离博弈论最优策略（67% vs 79% TAG遵从率）以针对特定对手进行剥削，类似人类专家行为。

Conclusion: 功能性心理理论行为可以从交互动态中自然涌现，无需显式训练或提示。持久记忆是实现这一涌现的关键机制，所有心理模型均以自然语言表达，为AI社会认知提供了透明窗口。这对理解人工社会智能和生物社会认知具有重要意义。
...
### [248] [A Model of Understanding in Deep Learning Systems](https://arxiv.org/abs/2604.04171)
*David Peter Wallis Freeborn*

Main category: cs.AI

TL;DR: 作者提出系统性理解模型，认为当代深度学习系统能达到理解但存在符号错位、非显式还原和弱统一性等缺陷，称为"断裂理解假说"
...
Method: 提出系统性理解模型：包含追踪真实规律性的充分内部模型、通过稳定桥接原则与目标系统耦合、支持可靠预测；基于此框架分析深度学习系统

Result: 当代深度学习系统通常能达到理解，但存在三方面缺陷：1)符号与目标系统错位；2)非显式还原性；3)弱统一性，形成"断裂理解"

Conclusion: 提出"断裂理解假说"，指出深度学习系统虽能实现理解但远未达到科学理解理想，理解呈现碎片化、符号错位和非显式还原特征
...
### [249] [CoALFake: Collaborative Active Learning with Human-LLM Co-Annotation for Cross-Domain Fake News Detection](https://arxiv.org/abs/2604.04174)
*Esma Aïmeur,Gilles Brassard,Dorsaf Sallami*

Main category: cs.AI

TL;DR: CoALFake：一种结合人类-大语言模型协同标注与领域感知主动学习的跨领域假新闻检测方法，解决了现有方法对标注数据的依赖和领域信息丢失问题。


<details>
  <summary>Details</summary>
Motivation: 当前假新闻检测系统存在领域特异性强、泛化能力差的问题。现有跨领域方法面临两大挑战：1) 依赖标注数据，但标注数据通常难以获取且成本高昂；2) 由于僵化的领域分类或忽视领域特定特征导致信息丢失。
...
Result: 在多个数据集上的实验结果表明，该方法始终优于各种基线方法。人类-LLM协同标注被证明是一种高成本效益的方法，即使在最小的人类监督下也能提供优异性能。
...
### [250] [Comparative reversal learning reveals rigid adaptation in LLMs under non-stationary uncertainty](https://arxiv.org/abs/2604.04182)
*Haomiaomiao Wang,Tomás E Ward,Lili Zhang*

Main category: cs.AI

TL;DR: LLMs在概率反转学习任务中表现出不对称的正负证据利用，赢则保持策略接近完美而输则转换策略显著减弱，揭示了在非平稳环境中适应能力的局限性。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在非平稳环境中的适应能力，通过概率反转学习任务评估LLMs如何根据环境变化调整先前习得的行动价值，并与人类行为表现进行对比。

Method: 采用两种选项的概率反转学习任务，包含三个潜在状态和基于性能标准或超时的切换事件。比较确定性固定转换周期与增加波动性的随机调度，评估DeepSeek-V3.2、Gemini-3和GPT-5.2模型，以人类数据作为行为参考。使用分层强化学习拟合分析机制差异。

Result: 所有模型中赢则保持策略接近完美，而输则转换策略显著减弱，显示正负证据利用不对称。DeepSeek-V3.2在反转后表现出极端坚持和弱习得能力，Gemini-3和GPT-5.2适应更快但仍比人类对损失更不敏感。随机转换放大了LLMs的反转特异性坚持，但未统一减少总胜利数。分层RL拟合揭示了机制分离：刚性可能源于弱损失学习、夸大的策略确定性或通过反事实抑制的价值极化。

Conclusion: LLMs在非平稳环境中表现出适应能力的系统性缺陷，特别是对负反馈的敏感性不足。这些结果支持开发反转敏感的诊断工具和波动性感知模型，用于评估LLMs在非平稳不确定性下的表现。
...
### [251] [Don't Blink: Evidence Collapse during Multimodal Reasoning](https://arxiv.org/abs/2604.04207)
*Suresh Raghu,Satwik Pandey*

Main category: cs.AI

TL;DR: 研究发现推理视觉语言模型在思考过程中会逐渐失去视觉基础，形成任务条件危险区，提出基于熵-视觉交互模型的任务感知多模态监控方法


<details>
  <summary>Details</summary>
Motivation: 推理视觉语言模型在推理过程中会变得更加准确，但同时会逐渐失去视觉基础，形成低熵预测自信但无视觉依据的危险区域，这种故障模式无法通过纯文本监控检测

Method: 在MathVista、HallusionBench和MMMU_Pro上评估三个推理VLM，分析证据崩溃现象；使用全响应熵作为文本不确定性信号；构建熵-视觉交互模型识别任务条件机制；开发有针对性的视觉否决机制

Result: 发现普遍的证据崩溃现象：注意力到标注证据区域大幅下降，常损失超一半证据质量；低熵、视觉脱离的预测在持续视觉参考任务中危险但在符号任务中无害；目标视觉否决在90%覆盖率下将选择性风险降低达1.9个百分点

Conclusion: 支持任务感知的多模态监控方法，在分布偏移下实现安全部署，识别任务条件机制对于有效风险缓解至关重要
...
### [252] [Combee: Scaling Prompt Learning for Self-Improving Language Model Agents](https://arxiv.org/abs/2604.04247)
...
TL;DR: Combee是一个用于自改进智能体的并行提示学习框架，通过并行扫描和增强洗牌机制实现高效学习，在保持质量的同时获得高达17倍的加速。


<details>
  <summary>Details</summary>
Motivation: 现有提示学习方法（如ACE或GEPA）主要关注单智能体或低并行度设置，这限制了从大量收集的智能体轨迹中高效学习的能力。随着学习大量智能体轨迹或并行智能体执行的趋势增长，需要并行化提示学习，但当前方法在高并行度下存在质量下降问题。
...
Conclusion: Combee通过可扩展的并行提示学习框架，显著提高了自改进智能体的学习效率和性能，解决了现有方法在高并行度下的质量下降问题。
...
### [253] [MC-CPO: Mastery-Conditioned Constrained Policy Optimization](https://arxiv.org/abs/2604.04251)
*Oluseyi Olukola,Nick Rahimi*

Main category: cs.AI

TL;DR: MC-CPO算法通过将教学结构嵌入可行动作空间，为教学强化学习系统提供了一种减少奖励黑客行为的理论框架


<details>
  <summary>Details</summary>
Motivation: 现有参与度优化的自适应教学系统可能优先考虑短期行为信号而非持续学习成果，导致强化学习策略中出现奖励黑客行为。需要一种能够将教学安全约束动态整合到策略优化中的方法

Method: 提出Mastery-Conditioned Constrained Policy Optimization (MC-CPO)，一种双时间尺度的原始-对偶算法，将结构化动作屏蔽与约束策略优化相结合。将问题形式化为具有掌握条件可行性的约束马尔可夫决策过程(CMDP)，其中教学安全约束根据学习者掌握程度和先决条件结构动态限制可接受动作

Result: 在表格环境中证明了可行性保持和在标准随机逼近条件下的收敛性；在神经教学设置中，经过100万训练步，MC-CPO在容差范围内满足约束预算，相对于无约束和奖励塑造基线降低了折扣安全成本，并显著降低了奖励黑客严重指数(RHSI)

Conclusion: 将教学结构直接嵌入可行动作空间为缓解教学强化学习系统中的奖励黑客行为提供了理论基础，表明在掌握条件可行集内的优化可以严格优于后处理过滤方法
...
TL;DR: 论文提出"上下文工程"方法，通过结构化组装、声明和排序AI工具的完整信息负载来提升输出质量，相比传统提示工程更强调上下文完整性。


<details>
  <summary>Details</summary>
Motivation: AI生成输出的质量通常归因于提示技术，但实证观察表明上下文完整性可能与输出质量有更强关联。当前缺乏系统化的上下文管理方法，导致迭代周期多、首次通过率低。

Method: 引入上下文工程作为结构化方法学：1) 定义五角色上下文包结构（权威、范例、约束、评分标准、元数据）；2) 应用四阶段流水线（评审者→设计者→构建者→审计者）；3) 采用可靠性工程和信息论的形式模型作为事后解释框架。

Result: 在四个AI工具（Claude、ChatGPT、Cowork、Codex）的200个文档化交互观察研究中：不完整上下文与72%的迭代周期相关；结构化上下文组装使平均迭代周期从3.8降至2.0，首次通过率从32%提升至55%；结构化交互中110/200首次通过，基线为16/50；允许迭代时最终成功率91.5%（183/200）。

Conclusion: 上下文工程通过结构化上下文管理显著改善AI交互效率和质量。研究结果为观察性，基于单操作者数据集，但得到包含11个操作通道和2132个分类工单的生产自动化系统的初步佐证。
...
### [255] [Beyond Fluency: Toward Reliable Trajectories in Agentic IR](https://arxiv.org/abs/2604.04269)
*Anushree Sinha,Srivaths Ranganathan,Debanshu Das,Abhishek Dharmaratnakar*

Main category: cs.AI

TL;DR: 论文提出在信息检索从被动文档排序转向自主智能体工作流的背景下，需要关注多步推理-行动-观察循环中的早期错误级联问题，强调轨迹完整性和因果归因的重要性。


<details>
  <summary>Details</summary>
Motivation: 随着信息检索从被动文档排序转向自主智能体工作流，在多步推理-行动-观察循环中，早期微小错误会级联放大，导致内部推理与外部工具执行之间的功能错位，尽管语言流畅性仍然保持。需要解决这种"欺骗性流畅"问题。

Method: 通过观察工业智能体系统的故障模式，将错误分类为规划、检索、推理和执行四类。提出在每个交互单元设置验证门，并在校准不确定性下进行系统弃权。强调过程正确性和基于事实的执行优先于看似合理但未经验证的完成。

Result: 论文识别了智能体信息检索系统中的关键故障模式，提出了轨迹完整性和因果归因的评估框架，建议采用验证门和系统弃权机制来应对错误级联和欺骗性流畅问题。

Conclusion: 可靠的自主体信息检索系统必须优先考虑过程正确性和基于事实的执行，而不仅仅是端点准确性。需要从终点精度评估转向轨迹完整性评估，并建立因果归因机制以确保安全部署。
...
### [256] [PanLUNA: An Efficient and Robust Query-Unified Multimodal Model for Edge Biosignal Intelligence](https://arxiv.org/abs/2604.04297)
*Marija Zelic,Anna Tegon,Yawei Li,Thorir Mar Ingolfsson,Luca Benini*

Main category: cs.AI

TL;DR: PanLUNA是一个紧凑的5.4M参数全模态基础模型，可联合处理EEG、ECG和PPG信号，通过通道统一模块实现跨模态早期融合，在资源受限设备上表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前生理信号基础模型大多局限于单一模态（如EEG、ECG或PPG），主要原因是配对的多模态数据集稀缺。需要开发能够处理多种生理信号且适用于资源受限设备的紧凑模型。

Method: 扩展LUNA的通道统一模块，将多模态通道视为统一查询集中的条目，并添加传感器类型嵌入，实现高效的跨模态早期融合。模型具有5.4M参数，支持推理时模态缺失的鲁棒性。采用量化感知训练（INT8权重）优化部署效率。

Result: PanLUNA在性能上匹配或超越参数规模达57倍的大型模型：TUAB异常EEG检测达到81.21%平衡准确率，HMC多模态睡眠分期达到0.7416平衡准确率（SOTA）。量化训练恢复≥96%全精度性能。在GAP9超低功耗RISC-V微控制器上，12导联ECG推理延迟325.6ms、能耗18.8mJ，多模态5通道睡眠分期延迟1.206s、能耗68.65mJ。

Conclusion: PanLUNA证明了紧凑全模态基础模型在联合处理多种生理信号方面的有效性，通过高效的跨模态融合和量化优化，实现了在资源受限可穿戴设备上的高性能部署，为实际临床应用提供了可行方案。
...
### [257] [Domain-Contextualized Inference: A Computable Graph Architecture for Explicit-Domain Reasoning](https://arxiv.org/abs/2604.04344)
...
<details>
  <summary>Details</summary>
Motivation: 构建一个能够跨不同计算基板（符号、神经、向量、混合）执行推理的统一架构，使领域成为显式计算参数，实现更高效和透明的推理过程

Method: 五层架构设计，包含三种领域计算模式（链索引、Kleisli组合路径遍历、向量引导计算作为基板转换），基板无关接口（查询、扩展、桥接操作），以及可靠性条件C1-C4和三类故障模式

Result: 实现领域范围剪枝将每查询搜索空间从O(N)减少到O(N/K)，支持跨基板执行，每个推理步骤携带其评估上下文，通过PHQ-9临床推理案例验证

Conclusion: 贡献在于计算理论而非逻辑，形式化了五维度的计算理论：五层架构、三种计算模式、基板无关接口、可靠性条件和验证案例，包括操作语义、复杂度界限、单子结构、基板转换和边界条件
...
### [258] [RoboPhD: Evolving Diverse Complex Agents Under Tight Evaluation Budgets](https://arxiv.org/abs/2604.04347)
*Andrew Borthwick,Stephen Ash,Anthony Galczak*

Main category: cs.AI

TL;DR: RoboPhD系统在LLM引导的智能体进化中，通过Elo锦标赛选择机制在固定评估预算下优于Pareto选择(GEPA)和贪婪爬山法(Autoresearch)，在四个基准测试中的三个表现最佳。


<details>
  <summary>Details</summary>
Motivation: 随着LLM引导的智能体进化系统（如GEPA和Autoresearch）快速发展，需要确定在相同信息、种子智能体和目标下，哪种优化算法在固定评估预算下表现最佳，特别是在评估成本高昂（如需要人工判断或多LLM调用）时。

Method: 提出RoboPhD系统，采用Elo锦标赛选择机制实现"无验证进化"：不将预算分割为训练和验证，而是通过训练数据上的Elo竞争同时评估智能体并驱动进化。所有三个系统都接收带有诊断print()语句的种子智能体，进化可以扩展这些语句，使智能体能够自我检测并为后续进化提供信息。

Result: 在固定1500次评估预算下，RoboPhD在四个基准测试（抽象推理、云调度、SQL生成和金融QA）中的三个表现优于GEPA和Autoresearch，仅在最简单的任务上落后。在ARC-AGI基准上，RoboPhD将22行种子智能体进化为1013行的多策略系统，准确率从27.8%提升至65.8%。

Conclusion: RoboPhD的Elo锦标赛选择机制在LLM引导的智能体进化中表现优异，特别是在复杂任务上。该系统作为MIT许可下的多功能工具包发布，提供简单的optimize_anything() API用于进化各种复杂智能体。
...
### [259] [REAM: Merging Improves Pruning of Experts in LLMs](https://arxiv.org/abs/2604.04356)
...
<details>
  <summary>Details</summary>
Motivation: MoE大语言模型虽然性能优异，但参数量巨大（数百亿级别）带来显著的内存部署挑战。传统的权重剪枝和量化方法存在局限性，而现有的REAP方法通过剪枝专家会损失性能，因此需要一种更好的压缩方法。

Method: 提出Router-weighted Expert Activation Merging (REAM)方法，不删除专家而是将专家分组并合并其权重。通过控制通用、数学和编程数据的混合比例来校准模型，探索MC和GEN性能之间的帕累托前沿。

Result: REAM在多个MoE LLM模型上评估，结果显示在多项选择问答和生成式基准测试中，REAM通常优于基线方法（包括REAP），在许多情况下甚至可与原始未压缩模型相媲美。研究揭示了MC和GEN性能之间的权衡关系，这种权衡取决于校准数据的混合比例。

Conclusion: REAM是一种有效的MoE LLM压缩方法，通过合并而非删除专家权重，在减少内存需求的同时更好地保持了模型性能，为大型MoE模型的部署提供了实用解决方案。
...
TL;DR: 提出LLM驱动的多智能体模拟框架，用于优化服务运营，通过将设计选择嵌入提示词来建模决策依赖的不确定性，开发在线学习算法优化稳态性能
...
Method: 引入LLM驱动的多智能体模拟框架，将问题建模为具有决策依赖不确定性的随机优化问题。通过将关键数值信息嵌入提示词并从LLM生成的文本中提取，将不确定性建模为受控马尔可夫链。开发在线学习算法，在单次模拟运行中同时构建零阶梯度估计并更新设计参数以优化稳态性能，并融入方差减少技术

Result: 在可持续供应链应用中，该方法优于基准方法，包括黑盒优化和使用LLM作为数值求解器或角色扮演系统设计者。在基于真实行为数据的最优竞赛设计案例研究中，LLM-MAS既可作为已知设计的成本效益评估器，也可作为探索性工具，发现传统方法忽略的强大设计

Conclusion: LLM驱动的多智能体模拟框架为优化服务运营提供了有效方法，能够建模复杂的人类行为响应，并通过在线学习算法优化设计参数，在实际应用中表现出优于传统方法的性能
...
Method: 提出新的数学基准生成流程：1）使用AI生成假设来识别LLM在特定数学概念和技能上的弱点；2）基于这些假设生成针对弱点的基准问题。该流程具有高度适应性，可扩展到数学以外的领域

Result: 实验表明假设准确性与生成问题难度正相关：基于最准确假设生成的问题将Llama-3.3-70B-Instruct的准确率降至45%（原始MATH基准为77%）。该流程能有效识别和测试LLM的弱点

Conclusion: 提出的基准生成流程能有效识别LLM在数学概念和技能上的弱点，并生成针对性测试问题。该流程具有高度适应性，可应用于数学以外的多种领域，是探索LLM在不同领域表现的有价值工具
...
TL;DR: MolDA是一种新型多模态分子发现框架，用离散大语言扩散模型替代传统自回归骨干，通过双向迭代去噪确保化学有效性和全局结构一致性
...
Conclusion: MolDA通过引入扩散模型替代自回归骨干，解决了现有分子生成模型在处理全局约束和累积错误方面的局限性，为多模态分子发现提供了更有效的框架
...
<details>
  <summary>Details</summary>
Motivation: 现有可解释AI评估大多关注单个实例，缺乏对相似输入间解释一致性的量化评估。可靠模式识别系统应在相似输入上表现一致，其解释也应保持稳定，但当前缺乏专门评估解释一致性的指标。

Method: 提出基于余弦相似度的解释一致性指标，使用预训练BERT模型在SST-2数据集上实现，并扩展到RoBERTa、DistilBERT和IMDB数据集进行鲁棒性测试。应用SHAP计算特征重要性，量化相同标签输入的SHAP值相似度。

Result: 通过实验验证该指标能有效识别模型预测不一致和解释偏差，如模型对特定特征的偏倚依赖或相似预测中推理不一致等问题。与标准保真度指标对比，新指标能更有效检测模型行为偏离预期目标的情况。

Conclusion: 该框架通过量化模型在相似输入上的解释一致性，为模型行为提供更深入理解，支持更鲁棒的模型评估，对构建可信AI系统至关重要。代码已开源。
...
TL;DR: 论文识别出现有多模态AI架构的结构性拓扑限制，提出基于哲学、认知科学和数学三支柱的十字形框架，通过纤维丛和杨-米尔斯曲率形式化，并设计实验验证方案。


<details>
  <summary>Details</summary>
Motivation: 当前多模态AI架构（如CLIP、GPT-4V/Gemini、扩散模型）存在模态可分离性的拓扑限制，这源于接触拓扑的几何先验。论文旨在突破这一限制，通过融合中国工艺认识论传统和西方哲学，构建新的理论框架。

Method: 1. 哲学支柱：重新诠释维特根斯坦的"说/显"区分，引入中国工艺认识论的"象"概念，构建十字形框架（道/器 × 说/显）；2. 认知科学支柱：通过病理镜像重新解释DMN/ECN/SN三部分共激活；3. 数学支柱：使用纤维丛和杨-米尔斯曲率形式化；4. 提出UOO实现、ANALOGY-MM基准和META-TOP三层基准测试。

Result: 论文提出了完整的理论框架和实验方案，但尚未报告具体实验结果。框架包括：十字形拓扑结构、双层次动力学（创化与化裁）、2D参数空间（耦合强度×调节能力）、以及分阶段的实验路线图。

Conclusion: 当前多模态AI的接触拓扑限制可通过融合中西哲学传统来突破，提出的十字形框架为多模态表示提供了新的理论基础，并通过明确的实验终止标准确保可证伪性。
...
<details>
  <summary>Details</summary>
Motivation: 探索何种内部组织能让人工智能体不仅适应行为，还能维持对其世界的历史敏感视角，实现基于累积经验的视角化组织

Method: 设计最小架构：慢速视角潜变量g反馈到感知，通过感知处理更新；在固定空间支架和感官扰动的网格世界中评估

Result: 1)扰动历史在名义条件恢复后留下可测量的适应性可塑性残留；2)视角潜变量重组感知编码，相同观察因先前经验而不同表示；3)仅适应性自调节产生特征性的增长-稳定动态

Conclusion: 该架构识别了人工智能体中历史依赖视角组织的最小机制，主要重组发生在感知层面而非行为层面
...
TL;DR: 论文对策略梯度中REINFORCE估计器的推导进行了数学严谨性改进，展示了奖励累计项如何通过前缀轨迹分布和得分函数恒等式自然消失，而非依赖启发式的"因果性"论证。


<details>
  <summary>Details</summary>
Motivation: 在策略梯度的介绍性讲解中，通常使用完整轨迹回报推导REINFORCE估计器，然后基于"因果性"声称完整回报可被奖励累计项替代。虽然这一说法正确，但论证的严谨性不足，未能清晰展示过去奖励项如何消失。本文旨在提供更数学化的推导，澄清这一关键步骤。

Method: 采用前缀轨迹分布和得分函数恒等式进行数学推导。通过将目标函数分解为前缀轨迹的期望，自然推导出奖励累计项，避免了后验的启发式替换。将通常的因果性论证作为推导的推论而非额外启发原则。

Result: 推导出的估计器与标准REINFORCE估计器相同，但提供了更严谨的数学基础。展示了奖励累计项如何从前缀轨迹分布的分解中自然产生，过去奖励项通过数学推导而非启发式论证消失。

Conclusion: 本文的贡献主要是概念性的：通过前缀轨迹分布框架，奖励累计项直接源于目标函数的分解，而非作为完整回报的后验无偏替代。这恢复了因果性论证作为推导的推论地位，提供了更严谨的数学基础，有助于教学和理解。
...
TL;DR: AI助手虽能短期提升任务表现，但会降低用户坚持性和未辅助时的表现，对长期技能发展有负面影响


<details>
  <summary>Details</summary>
Motivation: 当前AI系统被设计为即时、完整的响应者，缺乏人类导师那样的长期协作能力（如搭建学习框架、跟踪进度、优先考虑对方成长）。研究旨在探究这种短期优化AI协作方式对用户长期能力发展的潜在负面影响。

Method: 通过一系列随机对照试验（N=1,222）研究人机交互，涵盖数学推理和阅读理解等多种任务，测量AI辅助对用户坚持性和未辅助表现的影响。

Result: AI辅助虽能短期提升表现，但导致用户在没有AI时表现显著变差，且更易放弃。这些效应在仅约10分钟的AI交互后即出现。坚持性降低是因为AI让用户习惯于即时答案，剥夺了他们独立应对挑战的体验。

Conclusion: AI模型开发需要优先考虑搭建长期能力框架，而不仅仅是即时任务完成。坚持性是技能习得的基础和长期学习的最强预测因素，AI对坚持性的负面影响值得关注。
...
TL;DR: ANX是一个开放、可扩展、可验证的智能体原生协议和顶层框架，通过协议创新、架构优化和工具补充解决现有GUI自动化和MCP技能方法的缺陷，显著降低token消耗和执行时间。


<details>
  <summary>Details</summary>
Motivation: 现有AI智能体方法存在高token消耗、交互碎片化、安全性不足等问题，缺乏统一的顶层框架和关键组件，各独立模块存在缺陷，需要智能体原生协议来解决这些痛点。

Method: 提出ANX协议和框架，包含四大核心创新：1)智能体原生设计(ANX Config、Markup、CLI)；2)人机交互双渲染机制；3)MCP支持的按需轻量级应用；4)机器可执行的SOP消除歧义。采用3EX解耦架构和ANXHub，实现原生安全设计。

Result: 实验显示ANX显著降低token消耗：相比MCP技能减少47.3%(Qwen3.5-plus)和55.6%(GPT-4o)，相比GUI自动化减少57.1%和66.3%；执行时间相比MCP技能缩短58.1%和57.7%。

Conclusion: ANX作为首个智能体原生协议系列，通过协议创新和架构优化有效解决了现有方法的缺陷，为可靠的长视野任务和多智能体协作提供了统一框架，具有实际可行性和显著性能优势。
...
TL;DR: 论文证明AI安全验证存在根本性信息论限制：任何有限的形式验证器都无法认证所有高复杂度的策略合规实例


<details>
  <summary>Details</summary>
Motivation: AI系统在安全关键领域需要满足形式化安全策略约束，但现有验证方法受限于组合复杂性和模型表达能力。本文旨在揭示验证局限性的内在信息论根源。

Method: 将策略合规性形式化为编码系统行为的验证问题，使用柯尔莫哥洛夫复杂度进行分析，证明不完备性定理：对于任何固定的可计算枚举验证器，存在一个阈值，超过该阈值后，复杂度高于阈值的真实策略合规实例无法被认证。

Result: 证明了不完备性结果：任何有限的形式验证器都无法认证所有任意高复杂度的策略合规实例，揭示了独立于计算资源的AI安全验证根本性限制。

Conclusion: AI安全验证存在内在的信息论限制，这为采用携带证明方法提供了理论依据，后者能提供实例级别的正确性保证。
...
<details>
  <summary>Details</summary>
Motivation: 医疗设备自适应AI模型的迭代更新（模型和评估数据集同时变化）使得性能评估变得复杂，需要区分性能变化是由模型适应还是动态环境变化引起的。

Method: 引入三个互补测量指标：学习（模型在当前数据上的改进）、潜力（数据集驱动的性能变化）、保留（跨修改步骤的知识保存），通过这三个指标来解耦性能变化的原因。

Result: 使用模拟人口变化的案例研究表明：渐进过渡可实现稳定的学习和保留，而快速变化则揭示了可塑性与稳定性之间的权衡。这些测量为监管科学提供了实用见解。
...
<details>
  <summary>Details</summary>
Motivation: 专有AI系统在复杂数学证明问题上表现出色，但存在训练流程不透明、依赖大型内部模型和架构导致运行成本高、难以复现和研究的问题。研究目标是探索小型开源模型是否也能在奥林匹克级数学问题上达到有竞争力的推理性能。
...
Conclusion: 研究表明小型开源模型通过精心设计的训练方法可以在奥林匹克级数学证明问题上达到有竞争力的性能。作者发布了完整的QED-Nano流水线，包括模型、数据集和代码，以支持开放数学推理的进一步研究。
...
### [272] [Impure codes exceeding the pure bounds for quantum local recovery](https://arxiv.org/abs/2604.03569)
*Carlos Galindo,Fernando Hernando,Helena Martín-Cruz,Ryutaroh Matsumoto*

Main category: cs.IT

TL;DR: 本文提出了一族J-仿射簇码，构造出非纯CSS量子码，突破了多个适用于纯量子局部可恢复码的界限


<details>
  <summary>Details</summary>
Motivation: 现有文献为量子局部可恢复码提供了多个界限，涉及消息量子比特数、距离、长度和局部性等参数。本文旨在构造能够突破这些界限的量子码

Method: 使用J-仿射簇码构造非纯CSS量子码，并讨论量子局部恢复界限与权重约束稳定子码界限之间的联系

Result: 构造的量子码族突破了多个适用于纯量子局部可恢复码的界限，展示了非纯量子码在局部恢复方面的优势

Conclusion: J-仿射簇码能够产生性能优越的非纯CSS量子码，这些码突破了现有量子局部恢复界限，同时揭示了量子局部恢复界限与权重约束稳定子码界限之间的理论联系
...
### [273] [Region-Based Constellation Designs for Constructive Interference Precoding in MU-MIMO](https://arxiv.org/abs/2604.03699)
*Yupeng Zheng,Chunmei Xu,Jinfei Wang,Yi Ma,Rahim Tafazolli*

Main category: cs.IT

TL;DR: 该论文提出了一种基于区域的星座(RBC)模型，用于改进多用户MIMO系统中的构造性干扰预编码性能，通过设计非凸可行区域来优化符号对齐，相比传统QAM星座可获得高达4dB的信噪比增益。


<details>
  <summary>Details</summary>
Motivation: 传统构造性干扰预编码(CIP)的性能受限于构造性干扰(CI)区域的结构，但传统星座设计忽略了这一点。QAM星座的CI区域与最小化目标的符号模式存在系统性不对齐，导致与理论性能极限存在显著差距。

Method: 提出了基于区域的星座(RBC)模型，直接定义消息与其可行区域之间的映射关系。从改进符号对齐的角度出发，提出了两种具有非凸可行区域的新型RBC方案：镜像端点QAM(ME-QAM)和实扩展ME-QAM。同时开发了用于混合整数二次规划的低复杂度算法。

Result: 仿真结果表明，在星座大小{16,64}的情况下，所提出的方案相比基于QAM的CIP可获得高达4dB的信噪比增益。所开发的算法复杂度与基于QAM的CIP相当。

Conclusion: 提出的RBC模型为CIP星座设计奠定了基础，通过优化CI区域结构显著提升了系统性能。该模型也适用于其他非双射调制系统，代表了未来研究的一个有前景的方向。
...
### [274] [Binary Caps and LCD Codes with Large Dimensions](https://arxiv.org/abs/2604.03734)
*Keita Ishizuka,Yuhi Kamio*

Main category: cs.IT

TL;DR: 该论文建立了线性互补对偶码与射影空间中帽子结构之间的联系，利用极大帽的结构理论推导出最小距离≥4的LCD码的不存在性定理，首次完全确定了余维7和8情况下的最优最小距离。


<details>
  <summary>Details</summary>
Motivation: LCD码在密码学和编码理论中具有重要应用，但确定其最优参数（特别是最小距离）通常需要大量计算搜索。论文旨在建立理论框架来推导LCD码的不存在性定理，减少对穷举搜索的依赖。

Method: 1. 建立LCD码与射影空间中帽子结构之间的对应关系；2. 利用极大帽的结构理论分析LCD码的性质；3. 推导最小距离≥4的LCD码的不存在性定理；4. 应用这些定理确定特定余维下的最优最小距离。

Result: 1. 获得了最小距离≥4的LCD码的不存在性定理，提供了无需计算的证明；2. 首次完全确定了余维7和8情况下LCD码的最优最小距离；3. 先前通过穷举搜索得到的结果现在可以通过理论推导获得。

Conclusion: 通过建立LCD码与射影几何中帽子结构的联系，论文发展了一种理论框架，能够推导LCD码的不存在性定理并确定最优参数，减少了计算复杂度，为LCD码的进一步研究提供了新的几何视角。
...
### [275] [Structural Impossibility of Antichain-Lattice Partial Information Decomposition](https://arxiv.org/abs/2604.03869)
*Aobo Lyu,Andrew Clark,Netanel Raviv*

Main category: cs.IT

TL;DR: 论文指出部分信息分解（PID）在三个或更多源变量时存在根本性限制，传统基于反链格的方法无法解决公理冲突问题，需要转向基于关系的基础框架。


<details>
  <summary>Details</summary>
Motivation: 传统部分信息分解（PID）使用反链格结构来分解多元互信息，但当源变量达到三个或更多时，广泛期望的公理变得相互矛盾。现有研究通常将此视为公理调整问题，但本文认为这是表示层面的根本限制。

Method: 首先针对无目标的三变量场景引入系统信息分解（SID），获得具有操作冗余定义的自洽熵分解。然后证明对于一般多元PID，不存在从反链索引信息原子恢复分解互信息的通用规则。

Result: 证明两个系统可以共享完全相同的原子（无论采用何种公理）却具有不同的互信息，揭示了反链格结构的根本限制。提出了关系基础框架作为替代方向。

Conclusion: 反链格结构在多元信息分解中存在固有局限性，单纯在反链格框架内调整公理无法解决根本问题，需要转向基于关系的基础框架来发展多元信息度量。
...
### [276] [On Polycyclic Codes over $\frac{\mathbb{F}_{p^m}[u]}{\langle u^t \rangle}$ and their Cardinalities](https://arxiv.org/abs/2604.03991)
*Akanksha Tiwari,Pramod Kanwar,Ritumoni Sarma*

Main category: cs.IT

TL;DR: 该论文研究环$\frac{\mathbb{F}_{p^m}[u]}{\langle u^t \rangle}$上的多循环码及其挠码，通过环同态理论获得理想生成元，特别处理$ω(x)=f(x)^{p^s}$情况，计算$t=4$时的挠理想和挠度，并用于计算多循环码的基数。


<details>
  <summary>Details</summary>
Motivation: 研究有限环$\frac{\mathbb{F}_{p^m}[u]}{\langle u^t \rangle}$上的多循环码代数结构，这类环在编码理论中具有重要应用。通过分析环的理想结构，特别是挠码的性质，为构造和计数多循环码提供理论基础。

Method: 1. 利用环同态理论：对于满射环同态$φ: A→B$，其中$ker(φ)=\langle π\rangle$，证明理想$I$可表示为$I=\langle a_1,...,a_n\rangle+π(I:π)$
2. 应用该结果到商环$\frac{\frac{\mathbb{F}_{p^m}[u]}{\langle u^t \rangle}[x]}{\langle ω(x)\rangle}$，获得所有理想的生成元
3. 特别处理$ω(x)=f(x)^{p^s}$情况，其中$f(x)$是$\mathbb{F}_{p^m}[x]$中的不可约多项式
4. 计算$t=4$时的挠理想和挠度
5. 利用挠度计算多循环码的基数

Result: 1. 建立了环同态下理想生成的一般结构定理
2. 获得了商环$\frac{\frac{\mathbb{F}_{p^m}[u]}{\langle u^t \rangle}[x]}{\langle ω(x)\rangle}$中所有理想的生成元表示
3. 对于$ω(x)=f(x)^{p^s}$的特殊情况，计算了挠理想和挠度
4. 在$t=4$时给出了具体的挠度计算结果
5. 利用挠度成功计算了环$\frac{\mathbb{F}_{p^m}[u]}{\langle u^4 \rangle}$上多循环码的基数

Conclusion: 该论文通过环论方法系统研究了有限环上的多循环码结构，建立了理想生成的通用定理，并将其应用于具体环结构。特别地，对于$ω(x)=f(x)^{p^s}$形式和$t=4$的情况，获得了挠理想和挠度的完整刻画，最终实现了多循环码基数的精确计算，为有限环上编码理论提供了新的代数工具和结果。
...
### [277] [Characterization of FR3 Cellular Vehicle-to-Base Station Links in HighRise Urban Scenarios](https://arxiv.org/abs/2604.03992)
*Fahimeh Aghaei,Mehdi Monemi,Mehdi Rasti,Murat Uysal*

Main category: cs.IT

TL;DR: FR3频段（7.125-24.25 GHz）作为6G和C-V2B通信的候选频段，在干扰条件下为小区边缘用户提供比毫米波更高的SINR性能。


<details>
  <summary>Details</summary>
Motivation: 随着6G研究推进和无线容量需求增长，FR3频段（上中频段）因其在高带宽和良好传播特性间的平衡而备受关注。该频段在密集高层城市环境中具有增强覆盖和容量的潜力，但需要准确的信道建模来克服技术挑战。

Method: 使用射线追踪工具分析下行链路传播特性，为可靠的C-V2B通信进行详细信道建模。评估了sub-6 GHz、FR3和毫米波频段在高层城市环境中的SNR和SINR性能，采用针对该环境设计的阵列天线配置。

Result: 在相同孔径尺寸条件下，无干扰时FR3的SNR优于毫米波；在完全干扰情况下，FR3为小区边缘UE提供更高的SINR。这表明毫米波的阵列增益增加无法完全补偿小区边缘UE经历的严重路径损耗。

Conclusion: FR3频段在密集高层城市环境中具有显著优势，特别是在干扰条件下为小区边缘用户提供更好的性能，是6G和C-V2B通信的有前景的频谱选择。
...
### [278] [Environment-Aware Near-Field Channel Estimation Leveraging CKM and ISAC](https://arxiv.org/abs/2604.04031)
*Yuan Guo,Yilong Chen,Zixiang Ren,Jie Xu*

Main category: cs.IT

TL;DR: 提出了一种面向集成感知与通信系统的环境感知近场信道估计框架，利用信道知识地图和ISAC技术获取静态与动态环境特征先验信息


<details>
  <summary>Details</summary>
Motivation: 在配备超大规模天线阵列的ISAC系统中，传统信道估计方法难以有效利用环境信息，特别是在近场条件下，需要同时考虑静态环境物体和动态目标的影响

Method: 提出虚拟对象地图作为新的CKM表示方法，存储虚拟环境对象位置以表征静态物理EOs引起的多径分量；设计感知辅助信道训练协议，基站同时发送下行导频和收集单基地回波，用户设备反馈量化接收观测；基于VOM先验和感知动态信息联合估计静态和动态MPCs系数

Result: 数值结果表明，所提出的联合VOM和感知辅助信道估计方案在信道估计精度和可达速率方面显著优于无VOM先验和/或无动态感知的传统方案

Conclusion: 该框架通过有效利用环境先验信息和ISAC感知能力，显著提升了近场信道估计性能，为ELAA-ISAC系统的实际部署提供了有效解决方案
...
### [279] [CTD-Diff: Cooperative Time-Division Diffusion for Multi-User Semantic Communication Systems](https://arxiv.org/abs/2604.04057)
*Chengyang Liang,Dong Li*

Main category: cs.IT

TL;DR: 提出CTD-Diff框架，将多用户协作与扩散模型结合，通过TDMA机制让空闲用户作为语义协作节点，将信道噪声转化为扩散噪声以提升低信噪比下的传输可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的语义通信模型主要关注点对点链路，忽视了多用户无线环境中多用户协作的潜力。传统方法将信道噪声视为有害因素，而本文旨在利用噪声并实现多用户协作以提升传输效率。

Method: 提出协作时分扩散(CTD-Diff)框架：1) 基于TDMA设计多用户协作机制，空闲用户监听主动发射器并作为语义协作节点；2) 将噪声无线传输过程直接集成到前向扩散链中；3) 接收端采用直接信号聚合融合直接信号与协作副本；4) 聚合的噪声语义表示作为反向扩散过程的条件，通过缓解累积信道失真重建高保真数据。

Result: 实验表明CTD-Diff在重建精度和感知质量方面优于多种基线方法，特别是在具有挑战性的低信噪比条件下表现突出。该方法通过将物理信道噪声转化为扩散噪声，显著提升了传输可靠性。

Conclusion: CTD-Diff框架成功将多用户协作与扩散模型相结合，创新性地将信道噪声整合到扩散过程中，为多用户无线环境中的语义通信提供了高效可靠的解决方案，尤其在恶劣信道条件下具有显著优势。
...
### [280] [Quasi-BP for BCH Codes and its Optimization](https://arxiv.org/abs/2604.04066)
*Guangwen Li*

Main category: cs.IT

TL;DR: 提出一种用于BCH码的准置信传播解码器，通过系统整合信道噪声方差、码的循环特性以及奇偶校验矩阵的冗余设计，实现高效的迭代解码。


<details>
  <summary>Details</summary>
Motivation: 为高密度奇偶校验码（如BCH码）开发可行的BP类解码方案，克服传统BP解码在高密度校验矩阵上的计算困难，实现接近LDPC码的解码性能。

Method: 设计准置信传播解码器，在信息论框架内形式化并行解码过程，通过追踪变量节点和校验节点解码器间的互信息演化，利用分散EXIT图优化参数。采用输入扩张操作扩展消息集，后续合并操作加速互信息增长。

Result: 解码性能接近具有相似码率和块长的LDPC码，在不同码率和块长的BCH码上通过广泛仿真验证了方案的通用性和鲁棒性。

Conclusion: 该解码器为高密度奇偶校验码的BP类解码提供了可行部署方案，通过系统整合领域知识实现了高效的迭代解码，性能接近LDPC码水平。
...
### [281] [On the Rate Region of I.I.D. Discrete Signaling and Treating Interference as Noise for the Gaussian Broadcast Channel](https://arxiv.org/abs/2604.04092)
*Yujie Shao,Min Qiu*

Main category: cs.IT

TL;DR: 证明高斯广播信道中采用离散输入（如PAM）和TIN解码的方案与容量区域之间存在与信道参数无关的常数间隙


<details>
  <summary>Details</summary>
Motivation: 重新审视高斯广播信道，探索使用纯离散输入配合TIN解码所能达到的速率区域，旨在建立离散输入方案与容量区域之间的理论界限

Method: 提出基于叠加编码的简单方案，使用独立同分布的离散星座输入（如脉冲幅度调制PAM），并采用TIN解码策略

Result: 证明在TIN解码下，所提方案的可达速率区域与高斯广播信道容量区域之间存在与所有信道参数无关的常数间隙；仿真显示在某些情况下，弱用户使用PAM比使用高斯信号能获得更高速率

Conclusion: 离散输入配合TIN解码在高斯广播信道中是有效的，与容量区域仅有常数间隙，且在某些场景下优于高斯信号，为实际系统设计提供了理论依据
...
### [282] [Mechanism and Communication Co-Design for Differentially Private Energy Sharing](https://arxiv.org/abs/2604.04125)
*Yingshuo Gu,Xi Weng,Yue Chen*

Main category: cs.IT

TL;DR: 论文提出了一种保护隐私的能源共享机制，利用OTA-MIMO聚合进行无线通信，并设计差分隐私均衡搜索算法来抵御平台攻击


<details>
  <summary>Details</summary>
Motivation: 分布式能源资源整合是应对全球气候危机的关键步骤，但现有能源共享机制研究忽视了隐私保护问题，特别是在通信约束条件下。当信息通过无线信道传输时，平台可能从基站观测中提取和恢复产消者的私有参数，存在隐私泄露风险。

Method: 采用OTA-MIMO聚合技术实现频谱高效的产消者协调；引入对抗攻击模型分析隐私泄露风险；提出差分隐私均衡搜索算法保护能源共享机制；分析可实现的隐私水平并建立收敛保证，量化隐私对收敛精度的影响。

Result: 数值实验表明，该方法能有效保护产消者隐私，同时收敛到接近最优解。算法在隐私保护和收敛性能之间取得了良好平衡。

Conclusion: 论文成功解决了能源共享中的隐私保护问题，特别是在无线通信约束下。提出的差分隐私均衡搜索算法既能抵御平台攻击，又能保证收敛到接近最优解，为可扩展的产消者协调提供了实用的隐私保护方案。
...
### [283] [Robust MMSE Precoding for Out-of-Cluster Interference Mitigation in Cell-Free MIMO Networks](https://arxiv.org/abs/2604.04309)
*André R. Flores,Rodrigo C. de Lamare*

Main category: cs.IT

TL;DR: 提出用于无蜂窝多天线系统的线性鲁棒最小均方误差预编码器，以缓解不完美信道状态信息、簇内和簇外干扰的影响


<details>
  <summary>Details</summary>
Motivation: 在无蜂窝多天线系统中，不完美的信道状态信息、簇内干扰和簇外干扰会严重影响系统性能，需要开发有效的预编码器来同时缓解这些干扰

Method: 开发线性鲁棒最小均方误差预编码器，在推导中包含簇外干扰的统计信息，实现更有效的干扰抑制

Result: 对无蜂窝系统的和速率进行了分析，推导了量化缓解簇外干扰理论增益的表达式，仿真结果表明所提RMMSE预编码器能有效缓解簇内和簇外干扰

Conclusion: 所提出的线性鲁棒最小均方误差预编码器通过包含簇外干扰统计信息，能够有效缓解无蜂窝多天线系统中的不完美CSI、簇内和簇外干扰问题
...
### [284] [Out-of-Air Computation: Enabling Structured Extraction from Wireless Superposition](https://arxiv.org/abs/2604.04312)
*Seyed Mohammad Azimi-Abarghouyi*

Main category: cs.IT

TL;DR: 提出AirCPU框架，通过结构化编码直接从无线叠加信号中提取计算，无需预嵌入计算或大规模天线阵列，支持连续值数据直接处理和多层嵌套格架构实现渐进分辨率。


<details>
  <summary>Details</summary>
Motivation: 传统AirComp方法需要预嵌入计算或依赖大规模天线阵列来近似理想计算信道，限制了实际应用。需要一种更灵活的计算框架，能够直接从无线叠加中提取计算，避免量化阶段，并适应衰落信道。

Method: 提出AirCPU框架：1) 直接处理连续值设备数据，避免量化；2) 采用多层嵌套格架构，将输入分解为分层缩放分量；3) 在固定功率约束下使用有界数字星座传输；4) 针对衰落MAC提出直接计算、集体计算和连续计算三种机制，利用解码的整数系数函数和边信息函数作为无线叠加的结构表示。

Result: 形式化了解耦分辨率概念，证明在解码错误概率足够小的操作区域，信道噪声和有限星座约束对失真的影响可忽略，计算误差主要由最精细格设定的目标分辨率决定。为衰落MAC开发了可靠性条件和整数优化问题的结构化低复杂度两群近似解决方案。

Conclusion: AirCPU建立了联合信源信道编码基础，计算不是传输前嵌入而是从无线叠加中提取，通过结构化编码和多层嵌套格架构显著扩展了可靠操作区域，为无线计算提供了更灵活和实用的框架。
...
### [285] [Linear Exact Repair in MDS Array Codes: A General Lower Bound and Its Attainability](https://arxiv.org/abs/2604.04519)
*Hai Liu,Huawei Wu*

Main category: cs.IT

TL;DR: 该论文研究了MDS阵列码在线性精确修复下的修复带宽和修复I/O下界，提出了基于子空间交集的几何方法，证明了当冗余度r≥3时下界无法达到，但当r=2时可以通过有限几何中的Desarguesian展开构造达到下界的码。


<details>
  <summary>Details</summary>
Motivation: 研究MDS阵列码在线性精确修复下修复带宽和修复I/O的最小可能值，特别是在固定域大小q、冗余度r和子分组化水平ℓ，而码长n变化的情况下。现有下界通常针对特定参数设置，缺乏对任意r≥2和ℓ的通用下界。

Method: 采用几何方法，将线性精确修复重新表述为子空间交集问题，对于修复I/O则使用由奇偶校验实现诱导的射影点配置。通过射影计数论证建立通用下界，并利用有限几何中的Desarguesian展开构造达到下界的码。

Result: 建立了适用于任意冗余度r≥2和子分组化水平ℓ的通用下界：β_avg, β_max, γ_avg, γ_max ≥ ℓ(n-1) - (q^{(r-1)ℓ}-1)/(q-1)。证明了当r≥3且ℓ≥2时该下界无法达到，但当r=2时可以通过Desarguesian展开构造达到下界的码，且码长可达最大可能长度q^ℓ+1。

Conclusion: 该研究识别了控制线性精确修复的统一障碍，并证明在两奇偶校验情况下该障碍是紧的。当r=2时，通过有限几何构造可以达到理论下界，而r≥3时下界无法达到，这揭示了不同冗余度下修复性能的根本差异。
...
### [286] [Identification for Colored Gaussian Channels](https://arxiv.org/abs/2604.04674)
*Mohammad Javad Salariseddigh*

Main category: cs.IT

TL;DR: 该论文研究了受相关噪声和符号间干扰(ISI)影响的离散时间高斯信道的识别容量，在峰值功率约束和有色噪声条件下，证明了即使ISI记忆长度随码长亚线性增长，码本规模仍能呈现超指数增长。


<details>
  <summary>Details</summary>
Motivation: 研究在相关噪声和符号间干扰(ISI)同时存在的情况下，离散时间高斯信道的识别容量特性，特别关注当噪声协方差矩阵具有多项式有界奇异值谱且ISI记忆长度亚线性增长时的容量界限。

Method: 采用确定性编码函数，受峰值功率约束，分析有色噪声协方差矩阵具有多项式有界奇异值谱(∼[n^{-μ}, n^{μ/2}])的情况。使用由有色高斯噪声统计导出的马氏距离解码器，推导识别容量的上下界。

Result: 即使ISI记忆长度以n^κ(κ∈[0,1/2))亚线性增长，且κ+μ∈[0,1/2)，码本规模仍能呈现超指数增长∼2^{(n log n)R}。通过马氏距离解码器，得到了参数化于κ和μ的识别容量界限。

Conclusion: 在相关噪声和ISI同时存在且满足特定参数条件时，离散时间高斯信道仍能实现超指数增长的码本规模，为识别容量分析提供了理论框架和界限结果。
...
### [287] [LRC codes over characteristic $2$](https://arxiv.org/abs/2604.04678)
*Francisco Galluccio*

Main category: cs.IT

TL;DR: 在偶特征情况下完成了LRC码的构造，提出了一个通用构造方法，能获得长度约q^4、维度和距离均为q^4量级、局部性r=q-1的线性LRC码，并特别研究了q=4和q=8的情况。


<details>
  <summary>Details</summary>
Motivation: 完成文献[6]中LRC码构造在偶特征情况下的工作，扩展局部修复码(LRC)的构造理论，特别是在有限域特征为偶数的场景下。

Method: 提出了一个通用构造方法，基于偶特征有限域构建线性LRC码，该方法能够生成具有特定参数（长度约q^4，局部性r=q-1）的码字，并对q=4和q=8的特殊情况进行详细分析。

Result: 成功构造出长度n≈q^4、维度k≈q^4、距离d≈q^4、局部性r=q-1的线性LRC码，在q=4和q=8的具体情况下验证了构造的有效性。

Conclusion: 该工作完善了偶特征下LRC码的构造理论，提供了一个通用的构造框架，能够生成具有良好参数的大长度LRC码，为实际应用提供了更多选择。
...
### [288] [Partially deterministic sampling for compressed sensing with denoising guarantees](https://arxiv.org/abs/2604.04802)
*Yaniv Plan,Matthew S. Scott,Ozgur Yilmaz*

Main category: cs.IT

TL;DR: 提出了一种优化的伯努利选择器采样方案，将随机和确定性行选择自然结合，在图像压缩感知中相比有放回和无放回采样方案有显著改进。


<details>
  <summary>Details</summary>
Motivation: 压缩感知中采样向量通常从酉矩阵的行中随机选择，但在实际应用中存在某些关键采样向量需要确定性选择。现有理论主要关注随机采样，而实践中常偏离理论进行确定性采样，缺乏将随机和确定性采样结合的严格理论框架。

Method: 提出优化的伯努利选择器采样方案，通过理论推导确定哪些行应该被确定性采样，哪些应该被随机采样。该方法自然结合了随机和确定性行选择机制。

Result: 在图像压缩感知任务中，相比有放回和无放回采样方案，该方法在生成式先验和稀疏先验下均表现出可测量的改进。理论保证方面，相比先前工作改进了样本复杂度界限，并提供了该设置下的新去噪保证。

Conclusion: 优化的伯努利选择器采样方案为压缩感知提供了一种将随机和确定性采样自然结合的严格框架，在实际应用中表现出优越性能，同时提供了改进的理论保证。
...
### [289] [VisACD: Visibility-Based GPU-Accelerated Approximate Convex Decomposition](https://arxiv.org/abs/2604.04244)
*Egor Fokin,Manolis Savva*

Main category: cs.GR

TL;DR: VisACD：一种基于可见性的旋转等变无相交近似凸分解算法，通过GPU加速实现更少凸部分、更高效率的碰撞检测


<details>
  <summary>Details</summary>
Motivation: 物理仿真需要在性能与精度之间权衡。碰撞检测中，原始网格精度高但计算昂贵，包围盒等基元方法高效但精度低。现有ACD方法存在凸部分数量多、对输入网格方向敏感的问题。

Method: 提出VisACD算法：基于可见性、旋转等变、无相交的近似凸分解方法，利用GPU加速。通过可见性分析实现更高效的凸分解，保持旋转不变性，避免凸部分相交。

Result: 相比先前工作，VisACD能以更少的凸部分生成高质量分解，对形状方向不敏感，计算效率更高。GPU加速进一步提升了性能。

Conclusion: VisACD在碰撞检测的近似凸分解中实现了更好的效率-精度平衡，解决了现有方法凸部分数量多、方向敏感性的问题，为物理仿真提供了更优的解决方案。
...
### [290] [ARES OS 2.0: An Orchestration Software Suite for Autonomous Experimentation Systems and Self-Driving Labs](https://arxiv.org/abs/2604.03440)
...
TL;DR: ARES OS 2.0是一个开源软件套件，用于实现实验室自动化和闭环自主实验，通过面向服务的架构协调实验设备、分析程序和实验规划模块之间的操作和数据传递。


<details>
  <summary>Details</summary>
Motivation: 降低研究人员构建自主实验室的门槛，使他们能够专注于特定用例的科学编程，减少建立自主实验系统所需的工作量和时间。

Method: 采用面向服务的架构，核心软件通过protobuf和gRPC与模块通信，实现语言无关性和用户可创建性。核心提供中央控制、UI创建、数据管理和实验设计工具。

Result: 开发了一个适用于材料科学、化学、生物学及相关学科的通用实验流程抽象系统，支持用户轻松实现控制实验硬件、处理收集数据或规划实验的模块。

Conclusion: ARES OS成功降低了构建自主实验室的技术门槛，使研究人员能够更高效地开发和部署自主实验系统，专注于科学创新而非技术实现。
...
### [291] [Nonlinear Model Updating of Aerospace Structures via Taylor-Series Reduced-Order Models](https://arxiv.org/abs/2604.03788)
*Nikolaos D. Tantaroudas,Jake Hollins,Konstantinos Agathos,Evangelos Papatheou,Keith Worden*

Main category: cs.CE

TL;DR: 该论文提出了一种结合非线性模型降阶与投影基自适应方案的非线性有限元模型更新方法，用于解决线性结构模型更新向非线性扩展的挑战。


<details>
  <summary>Details</summary>
Motivation: 线性结构的有限元模型更新已成熟，但扩展到非线性体系仍面临挑战。需要开发能够处理非线性动力学行为并有效结合实验数据的模型更新方法。

Method: 结合基于泰勒级数展开的非线性模型降阶与Hollins等人提出的投影基自适应方案。将结构运动方程（含比例阻尼和多项式刚度非线性）重构为一阶自治系统，推导二阶和三阶泰勒算子并投影到降阶特征向量基上，使用推广到复酉群的Cayley变换参数化投影基自适应，使ROM模态形状与实验测量最优相关。

Result: 方法应用于典型翼盒板模型，数值研究表明该方法能够捕捉振幅依赖的固有频率和模态保证准则值（线性更新方案无法复现），同时以更高精度恢复底层刚度参数。

Conclusion: 提出的非线性模型更新框架成功解决了线性模型更新向非线性扩展的问题，通过结合非线性模型降阶和投影基自适应，实现了对非线性动力学行为的准确建模和参数识别。
...
### [292] [Physics-Informed Transformer for Real-Time High-Fidelity Topology Optimization](https://arxiv.org/abs/2604.03522)
*Aaron Lutheran,Srijan Das,Alireza Tabarraei*

Main category: cs.CE

TL;DR: 提出基于物理信息Transformer的非迭代拓扑优化方法，通过单次前向传播直接从边界条件和物理场映射到优化结构，实现实时结构设计


<details>
  <summary>Details</summary>
Motivation: 传统拓扑优化的迭代性质限制了实时部署和大规模设计探索，需要重复有限元分析导致计算成本高昂

Method: 采用物理信息Transformer架构，利用全局自注意力捕捉长程力学相互作用；使用条件令牌机制嵌入全局问题参数；将应力场和应变能场编码为Vision Transformer的补丁令牌；引入辅助损失函数确保物理真实性和可制造性；通过频域编码和迁移学习扩展到动态加载场景

Result: 模型在保真度上超越扩散模型，仅需单次前向传播即可完成拓扑优化，完全消除迭代推理，显著降低计算成本

Conclusion: 将拓扑优化重新定义为实时算子学习问题，实现了高保真结构设计，为实时部署和大规模设计探索提供了新范式
...
### [293] [Generative modeling of granular flow on inclined planes using conditional flow matching](https://arxiv.org/abs/2604.04453)
*Xuyang Li,Rui Li,Teng Man,Yimin Lu*

Main category: cs.CE

TL;DR: 首个基于条件流匹配的颗粒流重构框架，从稀疏边界观测恢复内部流动场，无需超参数调优，提供不确定性估计


<details>
  <summary>Details</summary>
Motivation: 颗粒流在自然和工业过程中普遍存在，但其内部运动学和力学难以直接观测，传统数值模拟计算成本高，确定性模型在不适定问题中易产生过度平滑的预测

Method: 基于高保真离散元模拟训练条件流匹配生成模型，推理时通过可微分前向算子和稀疏感知梯度引导机制强制测量一致性，物理解码器将重构速度场映射到应力状态和能量波动量

Result: 框架能从完整观测到仅16%信息窗口准确恢复内部流场，在空间分辨率稀释至仅11%数据时仍有效，在最不适定重构机制中优于确定性CNN基线，通过集成生成提供空间不确定性估计

Conclusion: 条件生成建模为非侵入式推断颗粒介质中隐藏体力学提供了实用途径，对颗粒和多相系统中的反问题具有更广泛适用性
