...
Motivation: 现有3DGS方法在具有丰富视角依赖效应的场景中网格提取困难，传统方法依赖多视图技术、迭代网格提取或大型预训练模型，牺牲了3DGS的固有效率

Method: 引入自监督置信度框架，可学习置信值动态平衡光度与几何监督；提出惩罚每个基元颜色和法线方差的损失函数；改进外观模型，解耦D-SSIM损失的各个项

Result: 在无界网格提取方面达到最先进结果，同时保持高效率

Conclusion: 通过自监督置信度框架和增强的监督信号，解决了3DGS在视角依赖效应丰富场景中的网格提取难题，实现了高效且高质量的网格重建
...
TL;DR: SITH是一个无需数据、无需训练的可解释性框架，通过分析CLIP视觉transformer的权重空间来提供细粒度的语义解释，并支持基于权重的模型编辑。


<details>
  <summary>Details</summary>
Motivation: 现有基于激活的可解释性方法存在数据集依赖、易受数据偏见影响、解释粒度粗糙等问题，需要开发更可靠、细粒度的模型内部机制分析方法。

Method: 提出SITH框架，直接分析CLIP视觉transformer的权重空间：对每个注意力头的value-output矩阵进行奇异值分解，使用COMP算法将奇异向量解释为稀疏、语义连贯的人类可理解概念组合。

Result: SITH能产生连贯、忠实的内在解释，通过重建保真度和可解释性实验验证；支持精确、可解释的权重空间模型编辑，无需重新训练即可提升下游性能；揭示了微调主要通过重新加权稳定语义基而非学习全新特征。

Conclusion: SITH为视觉语言模型提供了数据无关、训练无关的细粒度可解释性框架，支持基于权重的模型编辑，并揭示了模型适应的机制，为理解大规模部署的模型内部机制提供了新工具。
...
TL;DR: ReDiPrune是一种无需训练、即插即用的视觉token剪枝方法，在视觉-语言投影器之前直接选择信息丰富的视觉token，显著减少计算量同时提升多模态大语言模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的多模态大语言模型需要处理大量视觉token，计算成本高昂。投影后的token剪枝方法会损失细粒度空间和语义信息，需要在视觉编码器输出阶段进行更有效的token选择。

Method: ReDiPrune在视觉编码器和投影器之间插入，使用轻量级规则对每个token评分，综合考虑文本条件相关性和最大-最小多样性，确保选择的token既与查询相关又非冗余，无需重新训练或修改架构。

Result: 在4个视频和5个图像基准测试中，ReDiPrune持续改善准确率-效率权衡。例如，在EgoSchema数据集上使用LLaVA-NeXT-Video-7B模型，仅保留15%视觉token即可获得+2.0%绝对准确率提升，同时将TFLOPs计算量减少6倍以上。

Conclusion: ReDiPrune是一种高效、无需训练的多模态token剪枝方法，通过在视觉编码器输出阶段直接选择信息丰富的token，显著提升模型效率同时保持甚至改善性能，为多模态大语言模型的实际部署提供了实用解决方案。
...
Method: 提出双向相关图域适应框架：1) 通过虚拟域桥接进行知识迁移，利用双向相关图合成标注和未标注图像，采用固定比例和渐进动态MixUp策略生成虚拟图像，使用双重双向CutMix实现知识转移；2) 采用原型对齐和伪标签校正缓解确认偏差，使用可学习原型余弦相似度分类器进行双向原型对齐，并进行原型伪标签校正

Result: 在三个公开多域数据集上的实证评估表明该方法具有优越性，特别是在标注样本非常有限的情况下仍表现出色

Conclusion: BCMDA框架有效解决了混合域半监督医学图像分割中的域偏移和确认偏差问题，在有限标注数据下实现了优异性能
...
TL;DR: 该研究提出了LLaVA-LE，一个专门用于月球表面和次表面特征分析的多模态视觉语言模型，通过构建大规模月球数据集LUCID并进行两阶段训练，显著提升了在月球地形分析任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态视觉语言模型在视觉-文本联合推理方面取得了进展，但在行星科学领域的应用仍未被充分探索。主要障碍是缺乏将真实行星图像与详细科学描述配对的大规模数据集。本研究旨在填补这一空白，开发专门用于月球探索的视觉语言模型。

Method: 1. 构建LUCID数据集：包含96k高分辨率全色图像配详细地形描述，以及81k个从约20k图像中提取的问答对；2. 采用两阶段训练课程：第一阶段进行领域特定地形描述的概念对齐，第二阶段进行指令调优的视觉问答训练；3. 设计涵盖月球地形分析多个推理复杂度的评估基准。

Result: LLaVA-LE相比基础LLaVA实现了3.3倍的整体性能提升，相比第一阶段模型提升2.1倍，推理得分达到1.070，超过了评判者自身的参考分数。这表明领域特定的多模态数据和指令调优能有效推进视觉语言模型在行星探索中的应用。

Conclusion: 通过构建大规模月球多模态数据集和采用两阶段训练策略，成功开发了专门用于月球探索的视觉语言模型LLaVA-LE。该模型在月球地形分析任务上表现出色，证明了领域特定数据和指令调优对推进行星科学中多模态模型应用的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D物体理解和生成方法忽视了现实场景中普遍存在的重复物体信息，这些重复和相似物体对提供了互补线索，可用于提升3D感知的一致性。

Method: 提出Lookalike3D多视角图像Transformer，利用大型图像基础模型的强语义先验来区分物体对；构建3DTwins数据集，包含基于ScanNet++的76k手动标注的相同、相似和不同物体对。

Result: 在3DTwins数据集上相比基线方法实现了104% IoU的提升，显著改善了相似物体检测性能，并展示了在下游任务如联合3D物体重建和部件协同分割中的应用效果。

Conclusion: 将重复和相似物体转化为强大的线索，能够实现一致、高质量的3D感知，为室内场景理解提供了新的有效方法。
...
TL;DR: 3D高斯泼溅（3DGS）在实时渲染方面表现出色，但其几何测量潜力未被充分利用。本文提出一种基于3DGS的立体测量方法，通过在不同视图中选取共轭点并进行三角测量，实现高精度3D点测量，无需专业立体工作站。


<details>
  <summary>Details</summary>
Motivation: 3DGS在实时渲染和新视角合成方面已达到最先进水平，但其在精确几何测量方面的应用尚未充分开发。传统立体测量方法需要昂贵的立体工作站和专业操作技能，而基于网格的直接测量方法往往存在不完整和不准确的问题。因此，需要一种更便捷、准确的测量方法。

Method: 提出基于3DGS的立体测量方法：利用3DGS能够精确渲染源视图并平滑插值中间视图的特性，用户可以在不同视图中直观地选取共轭点。通过三角测量这些共轭点，可以精确生成3D点测量结果。该方法支持多视图交会（超过两个视图）以提高测量精度，并实现了基于Web的概念验证应用。

Result: 在多个无人机航拍数据集上的实验表明，该方法能够实现高精度点测量，精度达到或超过传统立体测量方法。在明确定义的点上，RMSE达到1-2厘米范围。在具有挑战性的薄结构上，网格方法的RMSE为0.062米，而本方法达到0.037米。在网格重建不佳的尖锐角落处，本方法成功测量所有点（RMSE 0.013米），而网格方法完全失败。

Conclusion: 基于3DGS的立体测量方法提供了一种高效、准确的几何测量解决方案，无需专业立体工作站或特殊操作技能。该方法在薄结构和尖锐特征等挑战性场景中表现优异，显著优于基于网格的直接测量方法，为3DGS在精确测量领域的应用开辟了新途径。
...
TL;DR: 提出QuatRoPE方法，通过线性复杂度的位置编码和注意力层点积计算显式空间关系，解决3D场景空间推理中数据稀缺和可扩展性问题。


<details>
  <summary>Details</summary>
Motivation: 3D场景空间推理对智能体发展至关重要，但3D场景-语言配对数据稀缺，从头训练模型困难。现有方法要么因绝对位置编码难以从过早融合特征中提取空间关系，要么因显式编码所有对象间关系（二次复杂度）导致可扩展性差。

Method: 提出QuatRoPE位置嵌入方法：1）输入长度与对象数量呈线性关系；2）通过注意力层点积显式计算成对空间关系；3）使用整体向量编码3D坐标保证空间一致性。同时提出IGRE机制，限制QuatRoPE仅影响对象相关token，减少对LLM原有位置嵌入的干扰。

Result: 大量实验验证了方法的有效性。代码和数据已在GitHub开源。

Conclusion: QuatRoPE通过线性复杂度的位置编码和注意力层显式关系计算，解决了3D空间推理中的可扩展性和关系提取问题，同时保持LLM原有能力。
...
TL;DR: 该研究使用语义模糊图像作为可解释性探针，通过CLIP嵌入空间中的概念插值生成连续模糊图像谱，比较人类与机器学习分类器在概念边界上的差异，发现机器分类器更偏向"兔子"而人类更接近CLIP嵌入。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索人类和机器学习模型在视觉证据模糊时如何划定概念边界，以及两者是否在相同位置划定这些边界。经典的鸭兔错觉揭示了人类大脑在模糊视觉证据下必须做出决策，但机器分类器是否以相同方式划定这些边界尚不清楚。

Method: 方法包括：1）使用语义模糊图像作为可解释性探针；2）开发心理物理学框架，在CLIP嵌入空间中插值概念以生成连续模糊图像谱；3）精确测量人类和机器分类器放置语义边界的位置和方式；4）比较人类与机器分类器的敏感度和偏差。

Result: 结果显示：1）机器分类器更偏向于看到"兔子"；2）人类更接近用于合成的CLIP嵌入；3）引导尺度对人类敏感度的影响比对机器分类器更强；4）受控模糊可以作为诊断工具，弥合人类心理物理学分析、图像分类和生成图像模型之间的差距。

Conclusion: 结论是受控模糊框架能够作为诊断工具，连接人类心理物理学分析、图像分类和生成图像模型，为人类-模型对齐、鲁棒性、模型可解释性和图像合成方法提供见解，揭示了人类与机器在概念边界划定上的系统性差异。
...
Result: 与基于标记的运动捕捉和测力板数据相比，OpenCap Monocular实现了低运动学误差（旋转自由度平均绝对误差4.8°；骨盆平移3.4 cm），在旋转精度上比仅回归的计算机视觉基线提高48%，在平移精度上提高69%。在行走中估计地面反作用力的准确性可与之前的两相机系统相媲美或更好。

Conclusion: OpenCap Monocular能够以临床有意义的准确性估计重要的动力学结果，已通过智能手机应用、Web应用和安全云计算部署，实现了免费、可访问的单智能手机生物力学评估。
...
TL;DR: TIGeR模型通过统一的地理-时间嵌入空间，实现基于图像、地理位置和时间的联合推理，支持地理定位、拍摄时间预测和地理时间感知检索三种任务。
...
Method: 提出TIGeR模型，基于多模态transformer将图像、地理位置和时间映射到统一的地理-时间嵌入空间。模型支持灵活的输入配置（单模态和多模态查询），使用相同表示执行三种任务：地理定位、拍摄时间预测和地理时间感知检索。

Result: TIGeR在多个任务上显著优于基线方法和最先进方法：时间预测（季节预测提升16%，日间预测提升8%），地理时间感知检索召回率提升14%。模型能更好地在视觉外观大幅变化下保持位置身份，实现基于场景时空而非纯视觉相似性的检索。

Conclusion: 统一的地理-时间建模方法能有效支持复杂的时空推理任务，TIGeR模型通过联合学习图像、地理位置和时间表示，在多个基准任务上取得显著性能提升，证明了统一时空表示的有效性。
...
TL;DR: 这篇综述论文比较了合成心脏MRI生成方法，重点评估了GANs、VAEs、扩散模型和流匹配技术在保真度、实用性和隐私保护方面的表现，强调需要集成评估驱动的临床工作流程框架。


<details>
  <summary>Details</summary>
Motivation: 解决心脏MRI数据稀缺、标注不足、厂商差异和隐私泄露风险等问题，通过合成数据生成来克服医疗影像数据的局限性。

Method: 系统综述和比较分析现有CMRI生成方法，包括GANs、VAEs、扩散概率模型和流匹配技术，特别关注掩码条件生成、厂商风格条件化和隐私保护机制。

Result: 分析显示掩码条件生成能提高结构保真度，扩散和流匹配模型具有强边界保持能力，解剖约束的合成数据能提升多厂商环境下的分割准确性和鲁棒性。
...
TL;DR: DRoPS：一种利用静态预扫描作为几何和外观先验的动态场景重建方法，通过表面对齐的高斯原语网格结构和CNN参数化运动，显著提升新视角渲染质量和3D跟踪精度


<details>
  <summary>Details</summary>
Motivation: 现有方法从2D基础模型提取先验并施加手工正则化，但在极端新视角和高度关节运动场景下重建效果不佳。本文旨在利用动态对象的静态预扫描作为显式几何和外观先验，有效约束解空间并确保序列中的几何一致性。

Method: 1) 建立网格结构和表面对齐模型：将高斯原语组织成锚定在物体表面的像素网格；2) 利用原语网格结构，通过CNN参数化运动，注入强隐式正则化并关联邻近点运动。

Result: 广泛实验表明，该方法在渲染质量和3D跟踪精度方面显著优于当前最先进方法，特别是在极端新视角和高度关节运动场景下表现优异。

Conclusion: DRoPS通过利用静态预扫描作为几何先验，结合表面对齐的高斯网格结构和CNN参数化运动，有效解决了动态场景重建中的挑战，在渲染质量和3D跟踪方面实现了显著提升。
...
TL;DR: AVControl：基于LTX-2音频视觉基础模型的轻量可扩展框架，通过并行画布和LoRA适配器实现多模态视频音频控制，无需架构修改
...
Method: 基于LTX-2音频视觉基础模型，每个控制模态作为独立的LoRA在并行画布上训练，将参考信号作为注意力层的额外token，仅需LoRA适配器无需架构修改

Result: 在VACE基准测试中，在深度和姿态引导生成、修复、外绘方面优于所有基线，在相机控制和音频视觉基准上取得竞争性结果；支持多种独立训练模态，计算和数据效率高

Conclusion: AVControl提供了一种轻量、可扩展的多模态控制框架，通过并行画布和LoRA适配器有效解决了结构化控制问题，实现了计算和数据高效的多模态视频音频生成控制
...
TL;DR: 提出Calibri方法，通过引入单个学习缩放参数和进化算法优化约100个参数，显著提升扩散变换器性能并减少推理步骤


<details>
  <summary>Details</summary>
Motivation: 挖掘扩散变换器在生成任务中的隐藏潜力，通过分析去噪过程发现引入学习缩放参数可显著提升DiT块性能，但需要参数高效的方法进行优化

Method: 提出Calibri方法：1）引入单个学习缩放参数优化DiT块；2）将DiT校准建模为黑盒奖励优化问题；3）使用进化算法高效求解；4）仅修改约100个参数

Result: Calibri在各种文本到图像模型中一致提升性能，尽管设计轻量；显著减少图像生成所需的推理步骤，同时保持高质量输出

Conclusion: Calibri是一种参数高效的DiT校准方法，通过最小参数修改显著提升生成质量并加速推理，展现了扩散变换器的优化潜力
...
TL;DR: 提出XAI引导的编码器塑形框架，通过生成注意力场来优化复杂腹主动脉瘤CT图像分割，解决模型关注无关结构或忽略低对比度目标的问题


<details>
  <summary>Details</summary>
Motivation: 复杂腹主动脉瘤CT图像分割常因模型关注无关结构或忽略薄层低对比度目标而失败，需要优化模型的注意力机制

Method: 提出XAI引导的编码器塑形框架：1) 从最终编码器块计算密集的基于归因的注意力场；2) 将预测概率质量与注意力场对齐以促进关注与输出的一致性；3) 将注意力场输入轻量级细化路径和置信度先验，在推理时调制logits以抑制干扰并保留细微结构

Result: 在临床验证的挑战性病例上评估，相比基础SAM设置，该方法带来显著改进，表明通过XAI引导显式优化编码器关注是复杂场景中可靠分割的有效原则

Conclusion: 将归因引导整合到表示和解码中是提升复杂腹主动脉瘤CT图像分割性能的实用有效方法，通过优化编码器关注机制解决分割失败问题
...
<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型通常依赖数十亿样本数据集，这限制了研究进展。现有方法虽然开始通过提高监督质量来缩小差距，但只解决了对比预训练中的部分弱点。需要找到监督信号的适当平衡点。

Method: 采用多方面的训练框架，结合三个关键创新：1）文本条件自蒸馏方法，对齐文本无关和文本条件特征；2）集成解码器的编码器，使用VQA目标使编码器超越字幕式查询；3）基于不确定性的加权机制，自动平衡所有异构损失。

Result: 仅用3000万图像训练，在数据高效方法中达到最先进水平：MSCOCO检索提升2.2点，细粒度检索提升2.0点，基于问题的检索提升5.9点，同时与数十亿规模模型保持竞争力。

Conclusion: GoldiCLIP通过平衡监督信号的多方面训练框架，显著提高了视觉语言模型的数据效率，仅用少量数据就能达到与大规模数据集训练模型相竞争的性能。
...
TL;DR: 该论文提出了一个基于深度学习的针道感染分类方法，通过注意力机制和高效冗余重建卷积技术，实现了对针道图像的高精度分类，性能优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 针道是外固定器与骨骼连接的接口，容易发生感染，导致患者疼痛和并发症。目前缺乏基于视觉特征的针道感染自动识别方法，现有研究主要关注开放性伤口而非针道接口。需要开发能够准确分类针道感染状态的深度学习模型，以改善患者管理体验。

Method: 1. 收集并构建针道伤口感染数据集；2. 提出基于注意力的深度学习模型，强调相关区域并减少针体干扰；3. 引入高效冗余重建卷积（ERRC）方法，增强特征图丰富度同时减少参数数量；4. 将针道图像分为两组：A组显示炎症或感染迹象，B组无明显并发症。

Result: 模型性能优异：AUC达到0.975，F1分数为0.927，仅需5.77M参数。模型能够基于视觉感染迹象准确区分针道状态，与医疗专业人员评估一致，优于基线方法。

Conclusion: 深度学习在基于视觉特征区分针道感染方面具有巨大潜力，提出的注意力模型和ERRC方法有效解决了针道图像分类的复杂性。模型性能优异且参数效率高，但需要更多数据进行进一步验证。
...
<details>
  <summary>Details</summary>
Motivation: 当前人群计数领域主要使用密度图和点回归两种范式，现有对抗攻击研究主要关注单一范式内的攻击，跨范式攻击尚未探索。考虑到该领域的安全影响，需要研究能够同时攻击两种架构的对抗攻击方法

Method: 提出综合多任务损失优化框架：对点回归模型采用场景密度特定的高置信度logit抑制；对密度图方法采用峰值目标密度图抑制；结合模型无关的感知约束确保扰动有效且对人眼不可察觉

Result: 攻击效果显著，平均MAE比干净图像增加7倍，同时保持竞争性视觉质量；成功在7个SOTA人群模型间转移，转移比在0.55到1.69之间；在攻击有效性和不可察觉性之间取得良好平衡

Conclusion: 首次实现了跨密度图和点回归两种人群计数范式的对抗攻击，提出的多任务损失优化框架在攻击效果和视觉质量之间达到良好平衡，为人群计数模型的安全性评估提供了新工具
...
Method: 提出DCARL分治自回归框架：1) 使用专门的无需时间压缩训练的关键帧生成器建立长距离全局一致的结构锚点；2) 采用插值生成器以自回归方式合成密集帧，使用重叠片段，利用关键帧提供全局上下文，单个干净的前一帧确保局部连贯性。

Result: 在大规模互联网长轨迹视频数据集上训练，在视觉质量（更低的FID和FVD）和相机一致性（更低的ATE和ARE）方面优于最先进的自回归和分治基线方法，能够稳定生成长达32秒的高保真长轨迹视频。

Conclusion: DCARL通过结合分治方案的结构稳定性和视频扩散模型的高保真生成能力，有效解决了长轨迹视频生成中的视觉漂移和可控性问题，为世界建模提供了重要技术突破。
...
TL;DR: WAFT-Stereo是一种基于扭曲的立体匹配方法，无需成本体积即可实现高性能，在多个基准测试中排名第一，同时速度提升1.8-6.7倍


<details>
  <summary>Details</summary>
Motivation: 传统立体匹配方法依赖成本体积，计算复杂且效率低。本文旨在证明成本体积并非必要，可以通过更高效的扭曲操作实现同等甚至更好的性能

Method: 提出WAFT-Stereo方法，采用基于扭曲的架构替代传统的成本体积设计。通过特征扭曲和匹配实现立体对应，简化了计算流程

Result: 在ETH3D、KITTI和Middlebury公开基准测试中排名第一。在ETH3D基准上零样本误差降低81%，速度比竞争方法快1.8-6.7倍

Conclusion: 成本体积不是立体匹配的必要组件，基于扭曲的方法可以实现更好的性能和效率平衡，为立体匹配领域提供了新的设计思路
...
Method: 使用包含多发性硬化、卒中、脑肿瘤、其他异常和正常对照的MRI和CT数据集，要求模型同时生成诊断、亚型、成像模态、专用序列和解剖平面等多重输出。采用四方面评估：带弃权的判别分类、校准度、结构化输出有效性和计算效率。通过多阶段框架控制选择偏倚，确保公平比较。

Result: 技术成像属性（如模态和平面）已接近解决，但诊断推理特别是亚型预测仍具挑战性。肿瘤分类最可靠，卒中中等可解，多发性硬化和罕见异常仍困难。少量样本提示能提升多个模型性能，但增加令牌使用、延迟和成本。Gemini-2.5-Pro和GPT-5-Chat诊断性能最强，Gemini-2.5-Flash提供最佳效率-性能权衡。开源模型中MedGemma-1.5-4B表现最突出，在少量样本提示下接近多个专有模型的零样本性能，且保持完美的结构化输出。

Conclusion: 研究为多模态大语言模型在神经影像中的性能、可靠性和效率权衡提供了实用见解，支持该领域的标准化评估。技术成像属性已基本解决，但诊断推理仍需改进，特别是复杂疾病和罕见异常的识别。
...
TL;DR: CORA是一个用于心血管风险评估的3D视觉基础模型，通过病理中心、合成驱动的自监督框架直接从CCTA体积数据学习，在多项诊断和解剖任务上优于现有方法，并能改善30天主要不良心脏事件的风险分层。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉疾病是全球心血管死亡的主要原因，虽然CCTA可用于非侵入性评估，但深度学习在CCTA分析中的临床应用受到专家标注数据稀缺的限制。现有的无标签预训练策略（如掩码图像建模）偏向于全局解剖统计，难以捕捉冠状动脉斑块的空间局部病理特征。

Method: 提出CORA模型，采用病理中心、合成驱动的自监督框架。通过解剖引导的病变合成引擎，模型被明确训练以检测模拟的血管异常，使表示学习偏向于临床相关的疾病特征而非主导的背景解剖结构。在12,801个未标注CCTA体积数据上训练，并耦合成像编码器与大语言模型形成多模态框架。

Result: 在来自9家独立医院的多中心数据集上全面评估，在斑块表征、狭窄检测和冠状动脉分割等诊断和解剖任务中，CORA始终优于最先进的3D视觉基础模型，性能提升高达29%。多模态框架显著改善了30天主要不良心脏事件（MACE）的风险分层。

Conclusion: CORA作为一个可扩展和可扩展的基础模型，为统一解剖评估和心血管风险预测建立了基础，解决了现有方法在捕捉局部病理特征方面的局限性，并展示了在临床风险分层中的实际应用价值。
...
TL;DR: 本文提出了跨域遥感视觉定位任务CD-RSVG，构建首个大规模数据集OptSAR-RSVG，并提出OptiSAR-Net++框架解决跨域特征建模、计算效率和细粒度语义判别等挑战。
...
Method: 提出OptiSAR-Net++框架：1) 补丁级低秩适应专家混合(PL-MoE)用于高效跨域特征解耦；2) 采用CLIP对比范式结合动态对抗负采样，将生成回归转化为高效跨模态匹配；3) 文本引导双门融合模块(TGDF-SSA)和区域感知辅助头增强语义-视觉对齐和空间建模。

Result: 在OptSAR-RSVG和DIOR-RSVG基准测试中达到最先进性能，在定位精度和效率方面具有显著优势。

Conclusion: 提出的CD-RSVG任务和OptiSAR-Net++框架有效解决了跨域遥感视觉定位问题，通过创新的特征建模和高效计算架构实现了优越性能。
...
TL;DR: 提出用于垂体瘤手术视频阶段识别的综合框架，结合自监督表示学习、鲁棒时序建模和可扩展数据标注策略，在测试集上达到90%准确率，优于现有方法。
...
Method: 1. 开发协作式在线平台供外科医生上传手术视频、接收自动阶段分析并贡献数据；2. 在251个未标注PTS视频上使用自监督框架预训练ResNet-50模型；3. 在81个标注手术数据集上微调，采用包含焦点损失、渐进层解冻和动态采样的改进训练方案以处理类别不平衡和手术变异性。

Result: 在保留测试集上达到90%的准确率，优于当前最先进方法，并在不同手术案例中表现出强大的泛化能力。

Conclusion: 该框架通过结合自监督学习、鲁棒时序建模和可扩展数据标注策略，成功实现了垂体瘤手术视频的准确阶段识别，同时通过协作平台促进了大规模数据收集和持续模型改进。
...
TL;DR: 评估自监督学习在膝关节骨关节炎诊断和预后建模中的效果，发现图像自监督学习在诊断任务中表现有限，而多模态图像-文本自监督学习在预后预测中显著优于ImageNet预训练


<details>
  <summary>Details</summary>
Motivation: 研究自监督学习是否比ImageNet预训练初始化能更好地改进膝关节骨关节炎的诊断和预后建模，特别是比较图像自监督学习和多模态图像-文本自监督学习的效果

Method: 比较两种自监督学习方法：(1) 在OAI、MOST和NYU队列的膝关节X光片上进行的图像自监督预训练；(2) 在未筛选的医院膝关节X光片与放射科医生印象配对数据上进行的多模态图像-文本自监督预训练。评估它们在Kellgren-Lawrence分级预测（诊断）和4年结构发病与进展预测（预后）中的表现

Result: 诊断任务中自监督学习效果有限：图像自监督在线性探测时有所改进，但在全微调时未超越ImageNet预训练；多模态自监督因预训练数据严重偏倚（93%为KL 3级）而未改善分级性能。预后任务中多模态初始化显著优于ImageNet基线：在外部验证中预测4年结构发病和进展表现更好（MOST AUROC: 0.701 vs. 0.599，10%标记数据）

Conclusion: 未筛选的医院图像-文本数据由于严重偏倚可能不适合学习诊断任务，但当下游任务与预训练数据分布一致时，能为预后建模提供强信号。自监督学习在医学影像中的应用效果取决于任务与预训练数据的对齐程度
...
TL;DR: 该论文提出了一个大规模的真实世界偏振反射和材料数据集，使用配备交叉/平行偏振的8相机346光源光舞台采集，包含218个日常物体，超过120万张高分辨率图像，具有漫反射-镜面反射分离和解析推导的材料属性。


<details>
  <summary>Details</summary>
Motivation: 准确建模真实世界材料的光反射是逆向渲染的核心挑战，主要由于缺乏真实的测量反射率数据。现有方法严重依赖合成数据集，这些数据集具有简化的光照和有限的材料真实性，导致模型无法泛化到真实世界图像。

Method: 使用配备交叉/平行偏振的8相机346光源光舞台采集大规模真实世界偏振反射和材料数据集。数据集涵盖218个日常物体，跨越五个采集维度：多视角、多光照、偏振、反射分离和材料属性，产生超过120万张高分辨率图像，具有漫反射-镜面反射分离和解析推导的漫反射率、镜面反射率和表面法线。

Result: 使用该数据集训练和评估了最先进的逆向和正向渲染模型，在固有分解、重光照和稀疏视图3D重建方面展示了显著改进，包括材料分离、光照保真度和几何一致性方面的提升。

Conclusion: 该工作为基于物理的材料理解建立了新的基础，并实现了超越合成训练机制的真实世界泛化能力。
...
TL;DR: 提出TIGFlow-GRPO两阶段生成框架，通过流匹配建模轨迹分布，结合行为规则对齐，提升轨迹预测的社会合规性和物理可行性
...
Method: 两阶段框架：第一阶段使用TIG增强的条件流匹配预测器建模细粒度视觉空间交互；第二阶段通过Flow-GRPO后训练，将确定性流展开重构为随机ODE-to-SDE采样进行轨迹探索，结合视图感知的社会合规性和地图感知的物理可行性复合奖励

Result: 在ETH/UCY和SDD数据集上，TIGFlow-GRPO提高了预测精度和长时域稳定性，同时生成更具社会合规性和物理可行性的轨迹

Conclusion: 该框架为动态多媒体环境中连接基于流的轨迹建模与行为感知对齐提供了有效方法，实现了更符合实际行为规则的轨迹预测
...
<details>
  <summary>Details</summary>
Motivation: 传统显著性图和扫描路径方法无法捕捉原始注视的细粒度时间动态，且现有模型局限于短时窗口（3-5秒），无法捕获真实世界内容中的长程行为依赖

Method: 使用自回归扩散模型生成具有连续空间坐标和高分辨率时间戳的注视轨迹，模型以显著性感知的视觉潜在空间为条件

Result: 定量和定性评估表明，该方法在长程时空准确性和轨迹真实性方面显著优于现有方法

Conclusion: 提出的生成框架能够有效预测无限时域内的原始注视轨迹，解决了传统方法的时空动态捕捉不足和长程依赖建模局限问题
...
TL;DR: TIES：一种基于tau引导的层间高效选择框架，通过动态平衡注意力大小和排名一致性来减少VLA模型的视觉token处理，在保持性能的同时显著降低推理延迟


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在机器人操作任务中存在显著的推理延迟问题，主要源于密集视觉token的处理。当前token减少方法主要依赖注意力大小作为静态选择标准，但研究发现高注意力token是任务依赖性的，甚至可能降低策略性能。

Method: 提出TIES框架，基于层间token排名一致性进行动态token选择。通过自适应平衡注意力大小和排名一致性，无需额外训练即可实现鲁棒的token选择。

Result: 在CogACT + SIMPLER基准测试中，TIES将平均成功率提高了6%，同时减少了78%的token使用量，并在不同解码器和基准测试中表现出强大的泛化能力。

Conclusion: TIES通过动态token选择框架有效解决了VLA模型的推理延迟问题，挑战了传统基于注意力大小的静态选择假设，为高效视觉语言动作模型提供了新的解决方案。
...
TL;DR: xLARD是一个自校正文本到图像生成框架，利用多模态大语言模型通过可解释的潜在奖励指导生成过程，解决复杂提示的语义对齐问题


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型在处理复杂提示时面临细粒度语义和空间关系对齐的挑战，这是因为前馈生成机制需要在完全理解输出前就预测对齐。相比之下，评估生成图像更为可行，这种不对称性启发了自校正框架的开发。

Method: 提出xLARD框架，包含轻量级校正器，基于模型生成参考的结构化反馈来精炼潜在表示。核心创新是可微分映射机制，将潜在编辑映射到可解释的奖励信号，使不可微分的图像级评估能够提供连续的潜在级指导。

Result: 在多样化的生成和编辑任务上的实验表明，xLARD能够改善语义对齐和视觉保真度，同时保持生成先验。该方法在复杂提示理解方面表现出色。

Conclusion: xLARD通过利用多模态大语言模型的评估能力指导生成过程，实现了文本到图像生成的自校正，为解决复杂提示对齐问题提供了有效解决方案，代码已开源。
...
Method: 提出PASDiff（物理感知语义扩散）方法：1）利用逆强度加权和Retinex理论引入光度约束，可靠恢复可见度和自然色彩分布；2）提出风格无关结构注入（SASI）技术，从现成的人脸先验中提取结构特征，同时过滤其固有的光度偏差，实现身份特征与物理约束的无缝融合；3）构建WildDark-Face真实世界基准数据集（700张低光照人脸图像）。

Result: 大量实验表明，PASDiff显著优于现有方法，在自然光照、色彩恢复和身份一致性方面实现了优越的平衡。

Conclusion: PASDiff通过物理感知的扩散框架和风格无关的结构注入，有效解决了低光照人脸恢复中的多重退化问题，在真实世界场景中表现出色。
...
TL;DR: MoE-GRPO：基于强化学习的专家路由优化框架，用于提升视觉语言模型中混合专家的多样性和性能


<details>
  <summary>Details</summary>
Motivation: 混合专家（MoE）通过稀疏激活参数降低Transformer计算开销，已扩展到视觉语言模型（VLM）。但传统的确定性top-K路由机制可能忽略更优的专家组合，导致专家过拟合。需要改进专家选择的多样性。

Method: 提出MoE-GRPO框架：1）将专家选择建模为序列决策问题；2）使用组相对策略优化（GRPO）通过探索和奖励反馈学习自适应路由策略；3）引入模态感知路由器指导，通过抑制对特定模态不常激活的专家探索来增强训练稳定性和效率。

Result: 在多模态图像和视频基准测试中，MoE-GRPO持续优于标准top-K路由及其变体，通过促进更多样化的专家选择来缓解专家过拟合，实现任务级专家专业化。

Conclusion: MoE-GRPO通过强化学习优化专家路由，解决了传统确定性路由的局限性，提高了专家选择的多样性和模型性能，为MoE-based VLM提供了更有效的路由机制。
...
TL;DR: 提出基于MAML的K-shot 3D左心房壁分割框架，通过元学习结合辅助任务和边界感知损失，在小样本下提升薄壁结构分割精度和跨域鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 左心房壁的LGE-MRI分割面临薄壁几何结构、低对比度和专家标注稀缺的挑战，需要开发能够在小样本条件下准确分割且具有跨域鲁棒性的方法。

Method: 采用模型无关元学习（MAML）框架，在元训练阶段结合左心房壁主任务和左右心房腔辅助任务，使用边界感知复合损失函数强调薄壁结构精度，支持K-shot（K=5,10,20）3D分割。

Result: 在保留测试集上，MAML优于监督微调模型（5-shot时DSC 0.64 vs 0.52，HD95 5.70 vs 7.60mm），20-shot时接近全监督参考（0.69 vs 0.71 DSC）。在未见域偏移和本地队列中保持鲁棒性，性能随K增加而提升。

Conclusion: MAML框架在小样本适应中实现了更准确可靠的薄壁边界分割，有望在最小额外标注需求下促进心房重构评估的临床转化。
...
TL;DR: EWAD：首个事件驱动的视频异常检测框架，通过构建事件流基准数据集和创新的密度感知采样、调制建模及知识蒸馏方法，显著提升异常检测性能。
...
Conclusion: 该工作首次将事件视觉确立为统一的视频异常检测研究方向，通过构建基准数据集和提出创新框架，为事件驱动的异常检测开辟了新途径。
...
TL;DR: C2W-Tune：一种两阶段腔壁迁移框架，利用高精度左心房腔模型作为解剖先验，改善3D LGE-MRI中薄壁左心房壁的分割精度。


<details>
  <summary>Details</summary>
Motivation: 3D LGE-MRI中左心房壁的准确分割对于壁厚映射和纤维化量化至关重要，但由于壁薄、解剖结构复杂和对比度低而具有挑战性。现有方法难以精确描绘薄壁边界。

Method: 提出C2W-Tune两阶段框架：第一阶段使用3D U-Net（ResNeXt编码器和实例归一化）预训练分割左心房腔，学习稳健的心房表示；第二阶段迁移权重并适应左心房壁分割，采用渐进式层解冻策略以保留心内膜特征同时实现壁特异性细化。

Result: 在2018 LA分割挑战数据集上，相比从头训练的基线：壁Dice从0.623提升至0.814，1mm表面Dice从0.553提升至0.731；HD95从2.95mm降至2.55mm，ASSD从0.71mm降至0.63mm。即使在监督减少（70个训练样本）的情况下，仍获得Dice 0.78和HD95 3.15mm，优于通常报告Dice 0.6-0.7的多类别基准。

Conclusion: 基于解剖的任务迁移结合受控微调显著提高了3D LGE-MRI中薄左心房壁分割的边界准确性，为壁厚映射和纤维化量化提供了更可靠的工具。
...
TL;DR: 提出一种基于视图空间射线分组策略的新方法，通过聚类被同一射线相交的高斯函数并施加约束，在4D场景中显式保持高斯函数跨时间的局部几何结构，实现更物理合理的运动建模。


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的动态场景重建方法在建模真实运动方面存在挑战，特别是对于单目视频数据集，高斯函数运动与真实物理动力学不一致会破坏局部几何结构，导致重建质量下降。大多数方法依赖光流或2D轨迹等外部先验来强制时间一致性。

Method: 提出视图空间射线分组策略：聚类被同一射线相交的高斯函数，仅考虑α混合权重超过阈值的高斯函数。对这些分组施加约束以保持一致的空间分布，从而保持局部几何结构。该方法强制更物理合理的运动模型，确保局部几何随时间保持稳定，无需外部指导。

Result: 将方法集成到两个不同的基线模型中，在具有挑战性的单目数据集上进行广泛实验。结果显示，该方法显著优于现有方法，实现了优越的时间一致性和重建质量。

Conclusion: 提出的视图空间射线分组策略能够有效保持4D场景中高斯函数的局部几何结构，实现更物理合理的运动建模，无需依赖外部先验，显著提升了动态3D场景重建的时间一致性和质量。
...
TL;DR: 提出一种分布式车辆控制方法，通过本地信息实现紧急车辆快速通行，同时最小化对普通车辆的影响，具有高可扩展性和实时决策能力。


<details>
  <summary>Details</summary>
Motivation: 现有紧急车辆快速通行方法存在两个根本限制：集中式数学求解器计算成本高且仅适用于小规模场景；强化学习方法通过集中训练学习但模型对不同交通条件的可扩展性有限。需要克服高计算成本和缺乏可扩展性的问题。

Method: 提出可扩展的分布式车辆控制方法，车辆仅使用本地信息（而非全局信息）在线分布式调整驾驶行为。证明仅使用本地信息的分布式方法近似等价于使用全局信息的方法。进一步提出分布式冲突解决机制，通过避免决策冲突保证车辆安全。

Result: 基于真实交通数据集的仿真实验表明，与现有方法相比，所提方法实现更快的决策速度、对普通车辆影响更小，在不同交通密度和道路配置下保持更强的可扩展性。

Conclusion: 提出的分布式方法克服了现有集中式和强化学习方法的局限性，提供了实时近似最优决策、自然适应不同交通条件、消除单点故障风险、提供确定性安全保障，并具有强可扩展性。
...
TL;DR: SGREC是一种利用查询驱动场景图作为结构化中介的可解释零样本指代表达理解方法，通过VLM构建场景图、LLM推理目标对象，在多个基准测试中达到最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：基于CLIP的VLM方法难以捕捉细粒度视觉细节和复杂对象关系；LLM擅长高级语义推理但无法直接处理视觉特征。需要一种能结合两者优势、提供可解释性的零样本REC方法。

Method: 提出SGREC方法：1) 使用VLM构建查询驱动的场景图，显式编码空间关系、描述性标题和对象交互；2) 利用场景图作为结构化中介，弥合低级图像区域与LLM所需高级语义理解之间的差距；3) LLM从场景图提供的结构化文本表示中推理目标对象，并提供详细解释确保推理过程的可解释性。

Result: 在多个零样本REC基准测试中取得最佳性能：RefCOCO val (66.78%)、RefCOCO+ testB (53.43%)、RefCOCOg val (73.28%)，展示了强大的视觉场景理解能力。

Conclusion: SGREC通过查询驱动场景图作为结构化中介，有效结合了VLM的视觉理解能力和LLM的语义推理能力，实现了高性能且可解释的零样本指代表达理解，解决了现有方法的局限性。
...
Result: 在Synthesis NeRF基准上，PSNR从21.45dB提升到23.70dB（精细调优版达24.52dB），训练时间保持10-15分钟；在THuman 2.0数据集上，仅用8张输入图像达到27.37-34.00dB。
...
TL;DR: 提出GDPO-Listener框架，通过自回归流匹配架构和分组奖励解耦策略优化，解决对话中听众3D头部运动生成的"回归到均值"问题，实现高表达性的说话和倾听动作生成，并支持语义文本控制。


<details>
  <summary>Details</summary>
Motivation: 当前虚拟人合成方法在生成对话中听众的3D头部运动时存在"回归到均值"问题，导致动作趋于静态面部表情，缺乏复杂非语言动作的参数空间，无法生成高度表达性的倾听动作。

Method: 1. 引入自回归流匹配架构实现稳定的监督学习；2. 应用分组奖励解耦策略优化，通过对不同FLAME参数组进行独立的奖励归一化，显式激励高方差表达性生成；3. 支持显式语义文本控制实现可定制响应。

Result: 在Seamless Interaction和DualTalk数据集上的广泛评估表明，该方法在长期运动方差、视觉表达性和语义可控性方面优于现有基线方法。

Conclusion: GDPO-Listener框架成功解决了对话中听众3D头部运动生成的挑战，通过创新的架构设计和优化策略实现了高度表达性的说话和倾听动作生成，为虚拟人交互提供了更自然的非语言行为。
...
TL;DR: VideoTIR：基于强化学习的多级工具包调用方法，通过TAGPO策略优化和轨迹合成框架，解决MLLMs在长视频理解中的幻觉问题，实现高效准确的长视频问答。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在长视频理解中存在幻觉问题，主要源于文本与视觉令牌的不平衡。虽然基于SFT的工具调用方法可以缓解此问题，但需要大量高质量细粒度数据且工具调用轨迹受限。

Method: 提出VideoTIR方法，利用强化学习促进多级工具包的合理使用。采用Zero-RL和SFT冷启动策略，使MLLMs能够检索并聚焦于有意义的视频片段/图像/区域。引入TAGPO策略优化减少冗余工具调用，通过逐步奖励分配和失败轨迹重用来提高调用效率。开发基于沙箱的轨迹合成框架生成高质量轨迹数据。

Result: 在三个长视频问答基准测试上进行广泛实验，证明了该方法的有效性和效率。

Conclusion: VideoTIR通过强化学习驱动的多级工具包调用，结合TAGPO优化和轨迹合成框架，显著提升了MLLMs在长视频理解中的准确性和效率，有效缓解了幻觉问题。
...
TL;DR: CARE是一个无需训练的医疗图像可控修复框架，通过双潜在分支策略和风险感知自适应控制器，在保持解剖结构一致性与生成先验增强之间实现平衡控制。


<details>
  <summary>Details</summary>
Motivation: 现有医疗图像修复方法通常需要任务特定重训练，且缺乏对忠实重建与先验驱动增强之间权衡的可控性。过度激进的修复可能引入幻觉细节或改变诊断重要结构，这在临床环境中尤其成问题。

Method: 提出CARE框架：1) 双潜在修复策略：一个分支强制数据保真和解剖一致性，另一个分支利用生成先验恢复缺失或退化信息；2) 风险感知自适应控制器：基于修复不确定性和局部结构可靠性动态调整各分支贡献，实现保守或增强导向的修复模式，无需额外模型训练。

Result: 在噪声和不完整医疗图像场景中，CARE实现了强大的修复质量，同时更好地保留了临床相关结构，减少了不合理重建的风险。

Conclusion: 该方法为更安全、更可控、更易于部署的医疗图像修复提供了实用步骤。
...
Method: 1. 构建BiHumanML3D双语数据集（LLM辅助标注+人工校正）；2. 提出BiMD模型，采用跨语言对齐策略显式对齐不同语言的语义表示

Result: BiMD在BiHumanML3D上取得FID 0.045 vs 0.169，R@3 82.8% vs 80.8%，显著优于单语扩散模型和翻译基线，支持零样本代码切换

Conclusion: 提出的数据集和跨语言对齐策略对于跨语言动作合成至关重要且有效，为双语文本到动作生成提供了可靠基准
...
Result: 在20年全球MODIS MCD43A4反射率记录上，压缩比达95:1（0.44GB），光谱保真度高（平均R²>0.98，平均RMSE=0.021）。在Sentinel-2影像上，连续时间参数化在2km云遮挡下恢复无云动态（R²>0.85）。在HiGLASS生物物理产品上达到近乎完美精度（R²>0.98）。

Conclusion: GeoNDC为行星尺度地球观测提供了统一的AI原生表示，作为原始档案的补充，集成了查询、重建和压缩功能，形成紧凑、分析就绪的数据层。
...
<details>
  <summary>Details</summary>
Motivation: 传统多模态虚假信息检测器存在两个主要问题：1）不透明黑盒模型难以解释；2）对新型操纵手法脆弱。需要开发既可解释又能适应新威胁的检测框架。

Method: 采用"先构建后推理"范式：1）构建人类可理解的概念节点图，包括通过多模态大语言模型自动发现和验证的新型高级概念；2）在概念图上应用分层注意力机制推断声明真实性，生成可解释的证据到结论推理链。

Result: PCGR在多模态虚假信息检测精度和鲁棒性方面达到最先进水平，在粗粒度检测和细粒度操纵识别任务中均优于先前方法，对新兴操纵类型表现出更强的适应性。

Conclusion: PCGR通过结构化、概念化的推理方法，成功解决了传统检测器的可解释性和鲁棒性问题，为多模态虚假信息检测提供了既准确又透明的解决方案。
...
TL;DR: MoRGS提出了一种在线高斯运动推理框架，通过光流监督和运动置信度机制，解决现有在线4D重建方法中高斯运动无法反映真实场景动态的问题。


<details>
  <summary>Details</summary>
Motivation: 现有在线4D重建方法虽然高效且视觉质量好，但无法学习反映真实场景动态的逐高斯运动。在没有明确运动线索的情况下，外观和运动仅通过光度损失优化，导致逐高斯运动追逐像素残差而非真实3D运动。

Method: 提出MoRGS框架：1) 利用稀疏关键视图上的光流作为轻量级运动线索，超越光度监督来正则化逐高斯运动；2) 学习逐高斯运动偏移场，协调投影3D运动与跨视图和时间观测流之间的差异；3) 引入逐高斯运动置信度，分离动态和静态高斯，加权高斯属性残差更新，抑制静态区域的冗余运动。

Result: 大量实验表明，MoRGS在在线方法中实现了最先进的重建质量和运动保真度，同时保持流式处理性能。

Conclusion: MoRGS通过显式建模逐高斯运动并引入光流监督和运动置信度机制，有效提升了在线4D重建的质量和运动保真度，解决了现有方法中运动优化不准确的问题。
...
TL;DR: GaussFusion通过几何感知的视频生成改进3D高斯泼溅重建，解决浮游物、闪烁和模糊等常见伪影，支持优化式和前馈式方法，在保持实时性能的同时实现SOTA新视角合成效果。


<details>
  <summary>Details</summary>
Motivation: 解决3D高斯泼溅在真实场景重建中的常见伪影问题，包括浮游物、闪烁和模糊，这些伪影主要源于相机位姿误差、不完整覆盖和噪声几何初始化。

Method: 提出几何感知的视频到视频生成器，通过渲染包含深度、法线、不透明度和协方差的Gaussian primitive视频缓冲区，生成时间连贯、无伪影的帧。同时引入伪影合成管道模拟多样化退化模式以确保鲁棒性和泛化性。

Result: 在新视角合成基准测试中达到最先进性能，高效变体能够以21 FPS实时运行并保持相似性能，支持交互式3D应用。

Conclusion: GaussFusion通过几何感知的视频生成有效解决了3D高斯泼溅的伪影问题，在保持实时性能的同时显著提升了重建质量，为交互式3D应用提供了实用解决方案。
...
Method: 开发闭环事件-SVE测量系统：1) SVE分支采用烟雾感知融合策略生成HDR图；2) 使用多线索烟雾似然图分离粒子发射与烟雾散射；3) 事件相机分支利用HDR图作为绝对强度参考抑制烟雾伪影；4) 立体事件3D管道通过特征提取和三角测量估计分离高度和等效粒径。

Result: 系统最大校准误差0.56%，在硼基推进剂实验中获取多模态等效半径统计，成功捕捉传统传感器难以观测的快速分离瞬态现象。

Conclusion: 该框架为烟雾遮蔽HDR条件下的微秒级分辨率3D燃烧测量提供了实用且校准一致的方法。
...
TL;DR: 提出一种从单目视频进行高质量动态高斯溅射的方法，通过SE(3)B样条运动基函数显式建模动态高斯的连续位置和方向变形


<details>
  <summary>Details</summary>
Motivation: 现有方法在从单目视频进行动态高斯溅射时存在局限性，需要更有效地建模复杂运动并避免过拟合训练视角

Method: 使用SE(3)B样条运动基函数建模连续变形，设计自适应控制机制动态调整运动基函数和控制点数量，开发软分段重建策略减轻长间隔运动干扰，采用多视角扩散模型提供多视角线索

Result: 在大量实验中，该方法在新视角合成方面优于现有最先进方法

Conclusion: 提出的方法通过显式建模连续变形、自适应控制、软分段重建和多视角扩散模型，实现了高质量的单目视频动态高斯溅射
...
Method: 提出GIFT框架：1) 引入定向多样性来量化帧在相关性条件下的独特性，形成统一的不可替代性评分；2) 采用预算感知细化策略，通过自适应迭代过程，首先选择不可替代性最高的核心帧集，然后随着预算扩展，优先在这些选择周围构建关键的时间上下文。

Result: 在LLaVA-Video-7B上的广泛实验表明，GIFT在长视频基准测试中相比均匀采样实现了最高12.5%的平均改进。
...
Result: Z-Erase成功克服了生成崩溃问题，在广泛任务中实现了最先进的性能表现

Conclusion: Z-Erase是首个专门针对单流文本到图像模型的概念擦除方法，通过流解耦框架和自适应调制算法，有效解决了生成崩溃问题，为单流架构的安全应用提供了重要工具
...
TL;DR: 论文提出Token-Reweighting (ToR)策略，解决多模态大语言模型在强化学习可验证奖励训练中感知与推理token耦合的优化难题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的响应混合了感知相关token（视觉内容基础）和推理相关token（推理链构建），这两种token类型体现了不同但相互依赖的能力。现有方法单独优化任一种token类型都会导致性能不佳，需要解决这种内在耦合问题。

Method: 提出可插拔的Token-Reweighting (ToR)策略，通过识别两种类型的关键token并在RLVR训练期间动态重新加权，显式建模感知与推理token的相互依赖关系。该策略可应用于现有方法（如GRPO和DAPO）之上。

Result: ToR在多个多模态推理基准测试中带来一致的性能提升，实现了最先进的性能，同时具备准确的视觉基础和连贯的推理能力。

Conclusion: 通过显式建模感知与推理token的相互依赖关系，Token-Reweighting策略有效解决了多模态大语言模型在强化学习可验证奖励训练中的优化挑战，为多模态推理系统提供了有效的训练框架。
...
Method: 提出分层因果丢弃(HCD)：1) 使用通道级因果掩码强制特征稀疏性，分离因果特征与虚假特征；2) 采用基于矩阵的互信息(MMI)目标，最小化潜在特征与领域标签的互信息，同时最大化与类别标签的共享信息；3) 引入StyleMix驱动的VICReg模块确保稳定性，防止掩码意外过滤重要因果数据。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对最终模型阶段注意力漂移的充分解释性，作者发现幻觉源于深层注意力回归到早期层的初始视觉噪声

Method: 提出CLVA（跨层视觉锚点），一种无需训练的方法，通过强化关键中间层特征同时抑制回归噪声，利用注意力动态捕获的锚点将深层注意力拉回正确视觉区域

Result: 在多种架构和基准测试中表现出色，计算时间和GPU内存没有显著增加

Conclusion: 输出可靠性依赖于在中间层而非最终层获取视觉锚点，CLVA通过跨层视觉锚点有效解决了多模态大语言模型的物体幻觉问题
...
TL;DR: THEMIS是一个新颖的多任务基准测试，专门用于评估多模态大语言模型在真实学术场景中的视觉欺诈推理能力，包含4000多个问题，涵盖7个场景和5种欺诈类型，对16个领先模型进行了严格测试。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在真实世界学术欺诈的复杂性方面存在不足，无法全面评估多模态大语言模型在实际学术场景中的视觉欺诈推理能力，需要建立一个更贴近现实、更具挑战性的评估框架。

Method: THEMIS基准包含三个主要创新：(1)基于真实撤稿案例和多模态合成数据构建了4000多个问题，涵盖7个真实学术场景，其中60.47%为复杂纹理图像；(2)系统覆盖5种欺诈类型和16种细粒度操作，每个样本通常经历多个堆叠操作；(3)建立欺诈类型到5个核心视觉欺诈推理能力的映射，实现多维度能力评估。

Result: 在16个领先的多模态大语言模型上的实验表明，即使是表现最好的GPT-5模型，总体准确率也仅为56.15%，证明了该基准的严格性和挑战性，能够有效揭示不同模型在核心能力上的具体优势和弱点。

Conclusion: THEMIS基准填补了现有评估体系与真实世界学术欺诈复杂性之间的关键差距，为多模态大语言模型在复杂真实世界欺诈推理任务的发展提供了严格的测试平台，有望推动该领域的进步。
...
TL;DR: Pixelis：一个直接在像素空间操作的视觉语言智能体，通过可执行操作（缩放/裁剪、分割、跟踪、OCR、时序定位）学习，采用三阶段训练方法，在六个基准测试中平均相对提升4.08%


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言系统是被动的静态观察者，只能描述像素而无法行动，在分布偏移下无法安全改进。这种被动性限制了通用化、物理基础视觉智能的发展。通过行动而非静态描述进行学习，对于超越精心策划的数据至关重要。

Method: 三阶段训练方法：1）监督微调：从思维链-行动轨迹中学习像素工具语法，使用掩码模仿损失加权操作/参数标记，辅助头部稳定像素基础参数；2）好奇心-一致性奖励微调：优化双驱动目标，结合预测误差好奇心和相邻步骤一致性，在KL锚点下加入适度效率先验，产生简短、有效、结构化的工具链；3）像素测试时强化学习：通过检索邻居、对完整轨迹而非答案进行投票，向简短高保真示例更新，同时使用KL-to-EMA安全控制约束漂移。

Result: 在六个公共图像和视频基准测试中，Pixelis实现了持续改进：相对于相同的8B基线，平均相对增益为+4.08%（在VSI-Bench上峰值达到+6.03%），同时产生更短、可审计的工具链，并在测试时学习中保持走廊内KL距离。

Conclusion: 在像素而非抽象标记中行动，将多模态感知锚定在物理世界中，将视觉推理与可行动结果联系起来，并实现无需外部反馈的具身适应。这种方法为物理基础的视觉智能提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 多模态学习依赖大规模标注数据，成本高昂。主动学习通过选择性标注信息丰富的样本来缓解这一问题。然而，现有多模态主动学习方法通常假设模态重要性在训练过程中保持不变，忽略了模态相对价值和样本难度随训练进程变化的动态特性，导致对多模态学习动态性不敏感。

Method: 提出RL-MBA强化学习框架，将样本选择建模为马尔可夫决策过程。包含两个关键组件：1) 自适应模态贡献平衡(AMCB)：通过强化反馈动态调整模态权重；2) 证据融合的难度感知策略调整(EFDA)：基于不确定性的证据融合估计样本难度，优先选择信息丰富的样本。策略适应模态贡献、不确定性和多样性，奖励机制鼓励准确率提升和平衡。

Result: 在Food101、KineticsSound和VGGSound数据集上的实验表明，RL-MBA在有限标注预算下持续优于强基线方法，同时提高了分类准确性和模态公平性。

Conclusion: RL-MBA通过强化学习框架有效解决了多模态主动学习中模态重要性动态变化的问题，自适应地平衡模态贡献并感知样本难度，在有限标注资源下实现了更好的性能和公平性。
...
Method: 提出多阶段强化学习(MSRL)方法：1) 从大规模文本偏好数据学习通用的奖励推理能力；2) 通过基于描述的强化学习阶段逐步迁移到多模态任务；3) 引入跨模态知识蒸馏提升偏好泛化能力。

Result: MSRL显著提升了生成式多模态奖励模型的性能：在VL-RewardBench上从66.6%提升到75.9%，在GenAI-Bench上从70.2%提升到75.7%，且无需额外的多模态偏好标注。

Conclusion: MSRL方法有效解决了多模态奖励模型训练的数据稀缺问题，通过分阶段的知识迁移实现了可扩展的强化学习训练，显著提升了模型在视觉理解和生成任务上的性能。
...
TL;DR: 提出一种基于莫尔干涉图案的轻量级数据增强方法，通过解析数学公式实时生成结构化扰动，无需外部数据且计算成本极低，显著提升视觉Transformer的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有数据增强方法（如基于扩散的合成或复杂特征混合）存在计算开销大或需要外部数据集的问题，需要探索更轻量、无需外部数据的增强方案。

Method: 提出基于莫尔干涉图案的程序化增强方法，使用封闭形式的数学公式实时生成覆盖广泛空间频率的结构化扰动，直接在内存中合成并与训练图像混合，随后立即丢弃，实现无存储的增强流程。

Result: 该方法计算成本极低（每张图像0.0026秒），在ImageNet-C、ImageNet-R和对抗基准测试中，使用视觉Transformer的实验显示其持续提升鲁棒性，优于标准增强基线和现有无需外部数据的增强方法。
...
TL;DR: AnyDoc是一个统一的HTML/CSS文档生成框架，通过自动合成大规模数据集DocHTML，并采用多模态大语言模型微调和高度感知强化学习，实现多种文档生成任务。


<details>
  <summary>Details</summary>
Motivation: 推动AI驱动内容创作中文档生成的发展，解决现有手工制作文档数据集覆盖范围有限、规模不足的问题，实现跨多种文档类别的统一生成框架。

Method: 1) 建立可扩展的数据合成管道，自动生成HTML/CSS格式文档，创建包含265,206个样本、111个类别、32种风格的大规模数据集DocHTML；2) 基于该数据集微调多模态大语言模型，实现意图到文档、文档反渲染、元素到文档三种生成任务；3) 引入高度感知强化学习后训练程序，通过基于预测与目标文档高度差异的奖励函数惩罚内容溢出问题。

Result: 定性和定量实验表明，AnyDoc在所有三种任务上都优于通用多模态大语言模型和任务特定的基线方法，有效解决了内容溢出问题并提升了整体性能。

Conclusion: AnyDoc通过统一的数据合成管道、大规模数据集构建、多模态模型微调和高度感知强化学习，成功实现了跨多种文档类别的统一生成框架，为文档生成领域提供了有效的解决方案。
...
TL;DR: AirSplat是一个训练框架，将3D视觉基础模型的几何先验适配到无姿态的新视角合成任务中，通过自一致姿态对齐和评分式不透明度匹配提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 尽管3D视觉基础模型在视觉几何估计方面表现出强大的零样本能力，但直接应用于通用的无姿态新视角合成仍然具有挑战性。需要一种方法能够有效利用这些模型的几何先验来实现高质量、无需姿态信息的新视角合成。

Method: 提出AirSplat框架，包含两个关键技术贡献：1) 自一致姿态对齐(SCPA)：训练时的反馈循环，确保像素对齐监督以解决姿态-几何不一致问题；2) 评分式不透明度匹配(ROM)：利用稀疏视角NVS教师模型的局部3D几何一致性知识来过滤退化的基元。

Result: 在大规模基准测试上的实验结果表明，该方法在重建质量方面显著优于最先进的无姿态NVS方法，展示了同时实现视觉几何估计和高质量视角合成的潜力。

Conclusion: AirSplat展示了将3D视觉基础模型适配到无姿态新视角合成任务中的潜力，能够同时实现视觉几何估计和高质量视角合成，为3DVFMs的实际应用提供了新的方向。
...
TL;DR: DAPASS框架通过全景置信度引导去噪和上下文分辨率对抗模块，在无源数据条件下解决全景语义分割中的几何失真和标注成本问题，在室内外基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 全景语义分割在自动驾驶和虚拟现实等应用中至关重要，但面临两个核心挑战：全景投影固有的严重几何失真和密集标注的高昂成本。虽然无监督域适应（UDA）从标注丰富的针孔相机数据集提供了一种可行方案，但许多实际任务存在更严格的源数据不可用（SFUDA）约束，这显著加剧了域偏移问题，导致不可靠的伪标签和性能严重下降，特别是对于少数类别。

Method: 提出DAPASS框架，包含两个协同模块：1）全景置信度引导去噪（PCGD）模块，通过强制扰动一致性并结合邻域级置信度来过滤噪声，生成高保真、类别平衡的伪标签；2）上下文分辨率对抗模块（CRAM），通过对抗性地对齐高分辨率裁剪的细粒度细节与低分辨率上下文的全局语义，显式解决尺度变化和失真问题。

Result: 在室外（Cityscapes-to-DensePASS）和室内（Stanford2D3D）基准测试中达到最先进性能，分别获得55.04%（+2.05%）和70.38%（+1.54%）的mIoU。

Conclusion: DAPASS框架有效解决了源数据不可用条件下的全景语义分割挑战，通过创新的去噪和对抗对齐机制，显著提升了伪标签质量和模型性能，为实际应用提供了可行的解决方案。
...
TL;DR: 提出鲁棒主成分完成（RPCC）框架，通过间接识别稀疏分量支持来处理前景遮挡背景的问题，使用变分贝叶斯推理解决，无需后处理阈值化。


<details>
  <summary>Details</summary>
Motivation: 传统鲁棒主成分分析（RPCA）假设稀疏前景与低秩背景相加，但实际应用中前景经常遮挡或替换背景元素，存在模型不匹配问题。

Method: 提出RPCC框架，通过确定稀疏分量支持间接识别稀疏分量，采用完全概率贝叶斯稀疏张量分解，使用变分贝叶斯推理求解，收敛到硬分类器。

Result: 在合成数据上获得接近最优估计，在真实彩色视频数据集上实现鲁棒前景提取，在高光谱数据集上实现异常检测，性能优于传统方法。

Conclusion: RPCC框架有效解决了前景遮挡问题，通过变分贝叶斯推理实现无需阈值化的稀疏分量识别，在多种应用场景中表现优异。
...
TL;DR: 提出了EgoXtreme数据集，这是首个大规模第一人称视角的6D物体姿态估计数据集，包含工业维护、体育和紧急救援三个极端场景，旨在解决现实世界应用中运动模糊、动态光照和视觉遮挡等挑战。


<details>
  <summary>Details</summary>
Motivation: 现有6D物体姿态估计基准数据集无法捕捉真实世界第一人称应用中的极端挑战，如严重运动模糊、动态光照和视觉遮挡，导致实验室数据与现实应用之间存在显著差距。

Method: 创建了EgoXtreme数据集，包含三个具有感知模糊性的挑战性场景：工业维护（极端光照）、体育（剧烈运动模糊）和紧急救援（烟雾遮挡）。数据集完全从第一人称视角采集，评估了最先进的通用姿态估计器在极端条件下的性能。

Result: 评估显示现有通用姿态估计器在极端条件下（特别是低光照环境）泛化能力失效；简单的图像恢复（如去模糊）在极端条件下无正面改善；基于跟踪的方法显示出性能提升，表明在快速运动场景中利用时间信息有意义。
...
TL;DR: SAVe是一个自监督的音频-视觉深度伪造检测框架，通过在真实视频上生成身份保持、区域感知的自混合伪操作来模拟篡改伪影，同时通过音频-视觉对齐组件检测唇语同步异常，实现无需合成数据的深度伪造检测。


<details>
  <summary>Details</summary>
Motivation: 当前多模态深度伪造检测面临两个主要挑战：1）检测器主要依赖精心策划的合成伪造数据进行训练，这引入了数据集和生成器偏差；2）多模态伪造会表现出微妙的视觉伪影和跨模态不一致性，这些难以检测。现有方法在可扩展性和对未见操作的鲁棒性方面存在局限。

Method: SAVe采用完全自监督的方法，仅在真实视频上进行训练。主要包含两个核心组件：1）生成身份保持、区域感知的自混合伪操作，模拟篡改伪影，使模型能够学习跨多个面部粒度级别的互补视觉线索；2）音频-视觉对齐组件，通过建模唇语同步来检测音频-视觉伪造特有的时间错位模式。

Result: 在FakeAVCeleb和AV-LipSync-TIMIT数据集上的实验表明，SAVe在域内性能上具有竞争力，并展现出强大的跨数据集泛化能力。这突显了自监督学习作为多模态深度伪造检测可扩展范式的有效性。

Conclusion: SAVe框架通过自监督学习在真实视频上训练，避免了合成数据依赖带来的偏差问题，能够有效检测多模态深度伪造中的视觉伪影和跨模态不一致性，为可扩展的深度伪造检测提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有解耦数据集蒸馏方法主要依赖粗粒度类别标签监督，在细粒度数据集上会导致蒸馏样本：(1)保留大的类内差异但类间差异细微，(2)同类样本过于相似，限制了局部判别线索并损害识别性能。

Method: FD²通过定位判别区域和构建细粒度表示进行蒸馏。预训练阶段使用反事实注意力学习聚合判别表示更新类别原型；蒸馏阶段采用细粒度特征约束对齐样本与类别原型并排斥其他原型，同时使用相似性约束使同类样本的注意力多样化。

Result: 在多个细粒度和通用数据集上的实验表明，FD²能够无缝集成到解耦数据集蒸馏框架中，在大多数设置下提升了性能，显示出强大的可迁移性。

Conclusion: FD²是针对细粒度数据集蒸馏的有效框架，通过细粒度特征约束和相似性约束解决了现有方法在细粒度数据上的局限性，提高了蒸馏样本的判别能力和多样性。
...
TL;DR: 论文提出用排序优化替代传统的二元DPO方法，以更好地处理视觉语言模型中对视觉内容忠实度的评估，特别是在长视频描述生成任务中。


<details>
  <summary>Details</summary>
Motivation: 传统DPO的二元"赢家通吃"方法不适合视觉语言模型，因为即使是不太优选的回答也可能对视觉输入保持高度忠实。标准Bradley-Terry DPO公式缺乏这种细微差别，过度强调获胜回答而忽略"失败"回答可能仍保持高视觉保真度。

Method: 提出排序优化作为替代方法，更精确地定位回答对视觉输入的忠实度。专注于视频-文本对齐任务，通过重复的标题降级方法大规模生成具有挑战性的全序标题链。

Result: 排序优化在长内容生成和评估方面优于二元DPO。重要的是，研究发现这些方法需要微调视觉编码器才能有效，挑战了DPO纯粹作为语言重加权过程的观点。

Conclusion: 对于视觉语言模型，排序优化比二元DPO更适合处理视觉忠实度的细微差别，并且视觉编码器的微调对于这些方法的有效性至关重要。
...
TL;DR: 提出语义解缠的统一3D异常检测模型，通过条件特征重建解决多类别统一模型中的类别间纠缠问题，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 统一3D异常检测模型在多类别学习中存在类别间纠缠问题，导致潜在特征重叠，模型在重建时采用错误的语义先验，从而产生不可靠的异常分数。
...
<details>
  <summary>Details</summary>
Motivation: 当前大规模视频数据集主要关注通用人类活动，缺乏针对物理技能学习所需的细粒度活动覆盖深度。需要专门的数据集来支持动作细节理解和个性化教学反馈。

Method: 构建SportSkills数据集，包含55种不同体育项目的36万+教学视频和63万+视觉演示，配有解释动作技巧的教学旁白。基于该数据集开发了错误条件教学视频检索任务，将表示学习与可操作反馈生成相结合。

Result: SportSkills使模型能够理解物理动作间的细粒度差异，相比传统活动中心数据集训练的同模型，性能提升高达4倍。专业教练的正式评估表明，检索方法显著提升了视频模型为用户查询个性化视觉教学的能力。

Conclusion: SportSkills填补了物理技能学习领域大规模数据集的空白，首次实现了错误条件教学视频检索的大规模任务制定，为个性化技能学习提供了有效工具，在细粒度动作理解和个性化教学反馈方面取得了重要进展。
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法主要关注语义感知，但在实例定位任务上迁移效果差，需要完全微调才能获得良好性能。实例感知是3D感知的基本组成部分，填补这一空白对于推进真正的3D基础模型至关重要。
...
Conclusion: PointINS通过几何感知的自监督学习成功增强了点云表示的实例感知能力，显著提升了实例定位任务的性能，是迈向真正3D基础模型的重要一步。
...
TL;DR: 提出基于红外物理知识的对抗训练方法KGAT，通过建模不同类别间的热辐射关系来提升红外目标检测的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 红外目标检测在复杂环境中具有广泛应用，但对常见损坏和对抗样本脆弱，现有数据驱动方法未充分考虑红外图像特性，鲁棒性有限

Method: 基于不同类别灰度值排序建模热辐射关系，量化各类间热辐射关系的稳定性，提出知识引导对抗训练KGAT，将红外物理知识嵌入对抗训练过程

Result: 在三个红外数据集和六个主流红外目标检测模型上的实验表明，KGAT能有效提升干净精度和对对抗攻击及常见损坏的鲁棒性

Conclusion: 利用红外物理知识特别是不同类别间的相对热辐射关系，可以有效指导对抗训练，显著提升红外目标检测的鲁棒性
...
TL;DR: ET-SAM是一个基于SAM的高效框架，通过双解码器设计实现场景文本检测和布局分析，显著提升推理速度并利用异构标注数据进行训练。
...
Method: 提出ET-SAM框架，包含轻量级点解码器生成词热图以减少前景点数量，设计联合训练策略利用多级、词级和行级标注数据，引入三组可学习任务提示来缓解数据集差异。

Result: 相比之前基于SAM的架构，ET-SAM实现约3倍推理加速，在HierText上获得竞争性性能，在Total-Text、CTW1500和ICDAR15上平均提升11.0% F-score。

Conclusion: ET-SAM通过高效的点解码器和联合训练策略，成功解决了SAM-based方法在场景文本检测和布局分析中的推理延迟和异构数据利用问题，实现了速度与精度的平衡。
...
TL;DR: FreeLOC：一种无需训练、层自适应的框架，通过视频相对位置重编码和分层稀疏注意力解决长视频生成中的分布外问题


<details>
  <summary>Details</summary>
Motivation: 预训练的视频扩散模型通常在短片段上训练，直接用于长视频推理会导致视觉质量显著下降。这主要源于两个分布外问题：帧级相对位置分布外和上下文长度分布外。
...
Conclusion: FreeLOC通过解决长视频生成中的两个关键分布外问题，提供了一种无需训练的高效框架，显著提升了长视频生成的质量和一致性。
...
Method: 提出VolDiT模型，将扩散Transformer扩展到原生3D数据：1) 使用体素块嵌入将3D数据转换为token；2) 采用全局自注意力机制直接处理3D token；3) 设计时间步门控控制适配器，将分割掩码映射为可学习的控制token，在去噪过程中调制Transformer层。

Result: 在高分辨率3D医学图像合成任务中，与基于U-Net的3D潜在扩散模型相比，VolDiT展现出改进的全局一致性、更优的生成保真度和增强的可控性。

Conclusion: 完全基于Transformer的扩散模型为体素医学图像合成提供了灵活的基础框架，在全局连贯性、生成质量和可控性方面优于传统卷积架构。
...
<details>
  <summary>Details</summary>
Motivation: 现有身份保持视频生成方法通常针对单一身份参考进行设计和优化，这限制了创作灵活性，无法充分适应多样化的现实世界输入格式。依赖单一来源也构成了一个不适定场景，导致模型难以在新颖上下文中忠实再现身份。
...
Result: 广泛评估验证AnyID在不同任务设置下实现了超高身份保真度以及卓越的属性级可控性。模型通过人类评估构建的偏好数据集进行微调，在身份保真度和提示可控性两个关键标准上表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理电影心脏MRI（CMR）这种具有时间维度的3D医学图像时，通常将空间和时间分解处理或通过辅助机制（如解剖掩码）强制时间一致性，这些策略引入了结构偏差，可能限制全局上下文整合，导致细微的时空不连续或生理上不一致的心脏动力学

Method: 提出CardioDiT：完全4D潜在扩散框架，包含时空VQ-VAE编码2D+t切片为紧凑潜在表示，扩散变换器将完整3D+t体积作为整体建模，在整个生成过程中耦合空间和时间维度

Result: 在公共CMR数据集和大型私有队列上评估，与具有渐进增强时空耦合的基线比较，结果显示改进的切片间一致性、时间相干运动和真实的心脏功能分布
...
TL;DR: CIV-DG：一种利用条件工具变量解决医学AI中结构性选择偏倚的因果框架，通过放松标准IV方法的随机分配假设，在患者人口统计学驱动的医院选择场景中实现跨站点泛化。


<details>
  <summary>Details</summary>
Motivation: 医学AI的跨站点泛化能力受到选择偏倚的根本性损害，这种结构性机制导致患者人口统计学特征（如年龄、严重程度）非随机地决定医院分配。传统的域泛化方法主要针对图像级分布偏移，无法解决站点特定变异与诊断标签之间的虚假相关性。
...
Conclusion: CIV-DG框架通过条件工具变量成功解决了医学AI中的结构性选择偏倚问题，为处理患者人口统计学驱动的医院选择场景提供了有效的因果方法，显著提升了跨站点泛化性能。
...
TL;DR: 本文综述了高光谱成像在自动驾驶中的应用挑战、技术选择标准和定制算法开发，并以HSI-Drive数据集的最新实验结果为例进行分析。


<details>
  <summary>Details</summary>
Motivation: 高光谱成像在自动驾驶领域具有应用潜力，但面临非受控光照条件、大景深范围、动态场景、实时性要求和嵌入式平台计算资源有限等多重挑战，需要系统分析技术选择和算法开发策略。

Method: 分析多种高光谱成像视觉系统技术，以HSI-Drive数据集最新版本的实验结果为案例进行研究，探讨适合自动驾驶应用的技术选择和定制算法开发方法。

Result: 通过HSI-Drive数据集的实验分析，展示了高光谱成像技术在自动驾驶应用中的具体表现，为技术选择和算法优化提供了实证依据。

Conclusion: 高光谱成像在自动驾驶中的应用需要综合考虑环境挑战和系统约束，选择合适的技术并开发能够充分利用光谱和空间信息的定制算法，HSI-Drive数据集为相关研究提供了重要实验基础。
...
### [80] [SDD-YOLO: A Small-Target Detection Framework for Ground-to-Air Anti-UAV Surveillance with Edge-Efficient Deployment](https://arxiv.org/abs/2603.25218)
...
<details>
  <summary>Details</summary>
Motivation: 地面到空中视角的小型无人机检测面临像素占比极低、空中背景杂乱和实时性要求严格等挑战。现有YOLO检测器主要针对通用目标检测，对亚像素目标特征分辨率不足，且部署复杂度高。

Method: 1. 提出SDD-YOLO框架，专为G2A反无人机监视设计；2. 引入P2高分辨率检测头（4倍下采样）捕捉微目标的细粒度空间细节；3. 集成YOLO26架构改进：DFL-free、NMS-free架构简化推理；4. 采用MuSGD混合训练策略，结合ProgLoss和STAL，缓解稀疏小目标信号的梯度振荡；5. 构建DroneSOD-30K大规模G2A数据集（约3万标注图像，涵盖多种气象条件）。

Result: 1. SDD-YOLO-n在DroneSOD-30K数据集上达到86.0% mAP@0.5，比YOLOv5n基线提升7.8个百分点；2. 推理性能：NVIDIA RTX 5090上226 FPS，Intel Xeon CPU上35 FPS；3. 展示了未来边缘部署的卓越效率。

Conclusion: SDD-YOLO通过高分辨率检测头、简化架构和先进训练策略，有效解决了G2A小目标无人机检测的挑战，在精度和速度方面均显著优于基线方法，具备实际部署潜力。
...
TL;DR: CHIRP数据集和CORVID方法：用于野生鸟类个体重识别的计算机视觉数据集与管道，支持多种视觉任务和生物相关指标评估


<details>
  <summary>Details</summary>
Motivation: 长期监测个体动物行为对保护生物学和进化生物学至关重要，但野外种群的自动化行为监测仍然具有挑战性，主要原因是缺乏覆盖多种计算机视觉任务的数据集来提取有生物学意义的个体动物测量数据

Method: 1) 引入CHIRP数据集，从瑞典拉普兰的野生西伯利亚松鸦长期种群中收集，支持重识别、动作识别、2D关键点估计、目标检测和实例分割；2) 提出CORVID方法，基于彩色腿环的分割和分类进行鸟类个体识别，采用概率基础的ID跟踪方法，通过检测到的彩色环组合与数据库匹配；3) 引入应用特定基准测试，使用生物学相关指标（喂食率、共现率）评估模型在实际应用中的性能

Result: CORVID在应用特定基准测试中优于最先进的重识别方法，CHIRP数据集为计算机视觉任务提供了全面的基准测试平台，支持从伦理批准的生物学研究中整理真实世界数据集

Conclusion: 这项工作为社区提供了从伦理批准的生物学研究中整理真实世界数据集的蓝图，以弥合计算机视觉研究与生物学应用之间的差距，CHIRP数据集和CORVID方法为野生鸟类个体行为监测提供了有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有监督方法缺乏对新器械的灵活性且需要大量标注数据，需要一种无需训练即可准确估计未见手术器械6D位姿的方法

Method: 两阶段流程：1) 检测阶段：生成掩膜提议，通过预训练特征提取器评分，跨视图匹配并三角化为3D实例候选；2) 位姿估计阶段：迭代精炼位姿假设，使用特征度量评分和跨视图注意力，最终通过多视图、遮挡感知轮廓配准进行精炼

Result: 在MVPSP数据集上评估，达到毫米级精度，与受控条件下的监督方法相当，同时保持对未见器械的完全泛化能力

Conclusion: 提出了一种新颖灵活的流程，有效结合基础模型、多视图几何和轮廓精炼，无需任务特定训练即可实现手术器械的高精度6D位姿估计
...
TL;DR: DeepFAN是一种基于Transformer的模型，用于肺结节良恶性分类，通过超过10K病理确认结节训练，并在多读者多病例临床试验中验证，显著提升初级放射科医生的诊断性能。


<details>
  <summary>Details</summary>
Motivation: CT广泛应用导致肺结节检出增多，但现有深度学习方法未能全面整合全局与局部特征，且缺乏临床试验验证。需要开发能有效辅助初级放射科医生、提高诊断一致性的工具。

Method: 开发基于Transformer的DeepFAN模型，在超过10K病理确认肺结节数据集上训练。进行多读者多病例临床试验，涉及12名初级放射科医生和400个病例，评估模型辅助诊断效果。使用可解释性分析评估特征贡献。

Result: DeepFAN在内部测试集AUC为0.939，临床试验数据集AUC为0.954。辅助后，读者平均AUC提升10.9%，准确率提升10.0%，敏感性提升7.6%，特异性提升12.6%。结节级读者间诊断一致性从一般提升到中等（κ值：0.313 vs 0.421）。可解释性分析显示全局特征贡献高于局部特征。

Conclusion: DeepFAN能有效辅助初级放射科医生诊断肺结节，可能有助于统一诊断质量、减少不确定肺结节的不必要随访。模型在临床试验中验证了其临床实用性。
...
<details>
  <summary>Details</summary>
Motivation: 皮肤疾病是全球性的公共卫生问题，在孟加拉国等人口密集国家，皮肤科专家和诊断设备严重不足，缺乏及时检测和治疗可能导致严重后果。利用人工智能和机器学习技术，通过图像处理和计算机视觉方法检测皮肤疾病具有重要现实意义。

Method: 开发了一个专注于常见皮肤疾病检测的公开数据集，包含五种孟加拉国流行皮肤病：接触性皮炎、白癜风、湿疹、疥疮和癣。数据集包含1612张图像（其中250张为原始图像，其余为增强图像），收集自孟加拉国Faridpur医学院门诊患者。应用多种机器学习和深度学习模型对数据集进行分类性能评估。

Result: 创建了包含302张皮炎、381张湿疹、301张疥疮、316张癣和312张白癜风图像的数据集。虽然数据收集具有地域性，但这些疾病在南亚等许多国家都很常见，使该数据集在全球机器学习皮肤病学应用中具有潜在价值。研究还报告了不同模型的分类性能。

Conclusion: 该研究为解决皮肤疾病检测资源不足问题提供了有价值的公开数据集，并展示了机器学习在皮肤病诊断中的应用潜力。数据集虽然基于孟加拉国数据，但所包含的疾病具有全球普遍性，特别适用于南亚地区。研究期望能吸引更多机器学习研究者和从业者关注自动化疾病诊断领域。
...
TL;DR: 该研究评估了9个开源多模态大语言模型在面部验证任务中的公平性，发现FaceLLM-8B显著优于通用模型，且偏见模式与传统面部识别系统不同。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型被探索用作面部验证系统，但其人口统计学公平性尚未得到充分研究。本研究旨在评估这些模型在不同种族和性别群体上的公平性表现。

Method: 使用IJB-C和RFW面部验证协议，评估9个开源MLLM（来自6个模型家族，参数规模2B-8B）。测量了四个种族群体和两个性别群体的验证准确率（使用EER和TMR），并使用四种基于FMR的公平性指标量化人口统计学差异。

Result: FaceLLM-8B（唯一的面部专用模型）在两个基准测试中显著优于通用MLLM。观察到的偏见模式与传统面部识别系统不同，不同群体在不同基准和模型下受影响程度不同。最准确的模型不一定最公平，整体准确率低的模型可能因在所有群体上产生均匀的高错误率而显得公平。

Conclusion: MLLM在面部验证任务中存在人口统计学公平性问题，其偏见模式与传统系统不同。需要专门的面部验证模型和更全面的公平性评估方法，不能仅依赖整体准确率作为公平性指标。
...
### [86] [A Unified Spatial Alignment Framework for Highly Transferable Transformation-Based Attacks on Spatially Structured Tasks](https://arxiv.org/abs/2603.25230)
*Jiaming Liang,Chi-Man Pun*

Main category: cs.CV

TL;DR: 提出空间对齐框架（SAF）解决变换对抗攻击在结构化任务（语义分割、目标检测）中效果不佳的问题，通过同步变换标签与输入实现空间对齐，显著提升攻击迁移性。


<details>
  <summary>Details</summary>
Motivation: 现有变换对抗攻击（TAAs）在分类任务中表现出良好的迁移性，但在语义分割、目标检测等结构化任务中效果不佳甚至失败。研究发现，结构化任务的标签具有空间结构，而现有TAAs在应用空间变换时未能同步调整标签，导致空间错位和错误梯度。

Method: 提出统一的空间对齐框架（SAF），包含空间对齐（SA）算法。该算法在应用空间变换时同步变换标签，确保输入和标签的空间一致性，从而生成更有效的对抗样本。将变换分为非空间变换和空间变换两类，重点关注空间变换时的标签同步问题。

Result: 实验表明SAF显著提升TAAs在结构化任务中的攻击效果：在Cityscapes数据集上，平均mIoU从24.50降至11.34；在Kvasir-SEG数据集上，从49.91降至31.80；在COCO数据集上，平均mAP从17.89降至5.25。

Conclusion: 空间对齐对于变换对抗攻击在结构化任务中的成功至关重要。提出的SAF框架通过同步变换输入和标签解决了空间错位问题，显著提升了攻击的迁移性和有效性，为结构化任务的对抗攻击提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位方法普遍采用基于对比学习的图像检索框架，存在三个主要问题：1) 依赖大批量和困难负样本挖掘；2) 忽略地图的几何结构；3) 街景与卫星图像覆盖不匹配导致目标模糊。需要更有效的空间推理方法。

Method: 提出Just Zoom In方法，将CVGL重新定义为在城市级卫星地图上的自回归缩放过程。模型从粗粒度卫星视图开始，通过一系列缩放决策选择目标分辨率的终端卫星单元，无需对比损失或困难负样本挖掘。

Result: 在包含众包街景和高分辨率卫星图像的真实基准上，Just Zoom In实现SOTA性能：50米内Recall@1提升5.5%，100米内Recall@1提升9.6%，显著优于最强的对比检索基线。

Conclusion: 自回归的由粗到细空间推理方法在跨视角地理定位中表现出色，验证了序列空间推理的有效性，为CVGL提供了新的范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型的内存机制将环境视为静态画布，当动态物体暂时离开视野后重新出现时，常出现冻结、扭曲或消失的问题，需要同时处理静态背景和动态物体的记忆机制

Method: 提出混合记忆范式，构建HM-World数据集（59K高保真视频片段，17个场景，49个不同主体），设计HyDRA架构将记忆压缩为token并使用时空相关性驱动的检索机制，选择性关注相关运动线索

Result: 在HM-World数据集上的大量实验表明，该方法在动态物体一致性和整体生成质量方面显著优于现有最先进方法

Conclusion: 混合记忆范式能有效解决动态物体隐藏后重新出现的失真问题，HyDRA架构通过选择性记忆检索机制成功保持隐藏物体的身份和运动连续性
...
TL;DR: TITAnD将轨迹异常检测转化为视觉问题，使用超光谱轨迹图像统一表示稠密和稀疏轨迹，通过循环因子化Transformer实现高效的多月异常检测。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法同时处理稠密GPS轨迹和稀疏停留点轨迹：稠密方法计算成本高（二次复杂度）无法处理多月数据，稀疏方法丢弃了精细证据。两种模态需要不同架构，阻碍知识迁移。

Method: 提出TITAnD框架：1) 将轨迹表示为超光谱轨迹图像（HTI），即"天×一天内时间"的网格，通道编码空间、语义、时间和运动学信息；2) 设计循环因子化Transformer（CFT），沿两个时间轴分解注意力，编码人类日常的循环归纳偏置，大幅降低计算成本。

Result: TITAnD在稀疏和稠密基准测试中均取得最佳AUC-PR，超越UNet等视觉模型，比标准Transformer快11-75倍且内存相当，首次实现稠密多月轨迹异常检测。

Conclusion: 视觉化重构和结构感知建模对轨迹异常检测至关重要。TITAnD统一了稠密和稀疏轨迹表示，通过循环因子化注意力高效建模人类日常模式，为多月轨迹分析提供了可行方案。
...
TL;DR: PixelSmile：基于扩散模型的细粒度面部表情编辑框架，通过全对称联合训练解耦表情语义，结合强度监督和对比学习实现精确线性控制


<details>
  <summary>Details</summary>
Motivation: 解决细粒度面部表情编辑中长期存在的内在语义重叠问题，实现连续、可控的精确表情编辑

Method: 构建FFE数据集（连续情感标注），建立FFE-Bench评估基准；提出PixelSmile扩散框架，采用全对称联合训练解耦表情语义，结合强度监督和对比学习，通过文本潜在插值实现线性表情控制

Result: PixelSmile实现了优越的解耦效果和鲁棒的身份保持，在结构混淆、编辑精度、线性可控性以及表情编辑与身份保持的权衡方面表现优异，支持平滑表情混合

Conclusion: PixelSmile框架有效解决了细粒度面部表情编辑中的语义重叠问题，实现了连续、可控、细粒度的表情编辑，同时保持身份特征，为表情编辑提供了新的解决方案
...
<details>
  <summary>Details</summary>
Motivation: LiDAR点云数据具有极高的稀疏性，这阻碍了高效的上下文建模，限制了现有压缩方法的速度和性能。需要一种既能处理稀疏几何细节又能保持高效性的压缩方案。

Method: 提出包含两个轻量级模块的框架：1) 几何重稠密化模块：迭代稠密化编码的稀疏几何，在稠密尺度提取特征，然后稀疏化特征进行预测编码；2) 跨尺度特征传播模块：利用多分辨率级别的占用线索指导分层特征传播，实现跨尺度信息共享并减少冗余特征提取。此外，引入整数推理管道确保跨平台一致性。

Result: 实验表明该方法在保持实时速度的同时获得了有竞争力的压缩性能，避免了现有神经压缩方法中观察到的熵编码崩溃问题，并进一步加速了编码过程。

Conclusion: FastPCC框架通过创新的几何重稠密化和跨尺度特征传播技术，实现了高效的LiDAR点云无损压缩，在速度和性能之间取得了良好平衡，为实时应用提供了可行的解决方案。
...
TL;DR: 提出Vega模型，一个统一的视觉-语言-世界-动作模型，用于基于指令的自动驾驶生成和规划，通过结合自回归和扩散范式处理多模态输入
...
Method: 首先构建大规模驾驶数据集InstructScene（约10万场景，含多样化驾驶指令和对应轨迹），然后提出Vega模型：使用自回归范式处理视觉输入和语言指令，扩散范式生成未来预测和轨迹，采用联合注意力实现模态间交互，不同模态使用独立投影层

Result: 实验表明该方法不仅实现优越的规划性能，还展现出强大的指令跟随能力，为更智能和个性化的驾驶系统铺平道路

Conclusion: Vega模型通过统一的多模态架构成功解决了指令驱动的个性化驾驶问题，结合自回归和扩散范式实现了有效的视觉-语言-世界-动作集成
...
<details>
  <summary>Details</summary>
Motivation: 现有V2V和V2I协同感知局限于地面协作，无法完全解决大规模遮挡和复杂环境中的远距离感知问题，需要车辆与无人机协同的跨视角感知方案。

Method: 通过地面车辆和无人机搭载多视角LiDAR和RGB相机采集数据，覆盖城市街道、大学校园和乡村道路等多种交通场景，构建包含超过56K LiDAR帧、56K多视角图像和700K标注3D边界框的数据集。

Result: 建立了单智能体3D目标检测、协同3D目标检测和目标跟踪的基准测试，通过多个先进模型的综合评估验证了V2U协同在增强感知鲁棒性和远距离感知能力方面的有效性。
...
TL;DR: 该论文提出了MacroData数据集和MacroBench基准测试，用于解决多参考图像生成任务中的数据瓶颈和评估标准化问题。


<details>
  <summary>Details</summary>
Motivation: 当前多参考图像生成模型在输入参考图像数量增加时性能严重下降，根本原因是现有数据集以单参考或少数参考对为主，缺乏结构化、长上下文监督来学习密集的参考间依赖关系。

Method: 1. 构建MacroData大规模数据集：包含40万个样本，每个样本最多10张参考图像，系统组织为四个互补维度（定制化、插图、空间推理、时间动态）；2. 提出MacroBench基准测试：包含4000个样本，评估生成一致性跨分级任务维度和输入规模。

Result: 在MacroData上微调显著改善了多参考图像生成性能；消融研究揭示了跨任务协同训练的协同效益以及处理长上下文复杂性的有效策略。

Conclusion: MacroData和MacroBench解决了多参考图像生成领域的数据瓶颈和评估标准化问题，为多主题组合、叙事插图和新颖视角合成等实际应用提供了重要支持。
...
<details>
  <summary>Details</summary>
Motivation: VGGT等视觉Transformer的全局注意力层存在二次计算复杂度问题，现有稀疏化加速技术往往导致显著的精度下降。作者假设这种下降源于注意力头对稀疏化的敏感性存在异质性，而现有方法对所有头采用统一的稀疏模式
...
Conclusion: 通过量化注意力头对稀疏化的异质性敏感性并相应调整稀疏模式，可以显著改善VGGT等视觉Transformer在高稀疏度下的性能保持能力，为高效视觉Transformer设计提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个核心限制：1）由于过度依赖单参考帧导致用户控制不灵活；2）在多角色场景中实例可控性差导致错位；3）细粒度区域细节保真度下降。需要开发一个能够提供灵活用户控制、精确多实例对齐和高细节保真度的框架。
...
Result: 大量实验证明，InstanceAnimator在用户控制灵活性、视觉质量和实例一致性方面实现了卓越的多实例着色效果，优于现有方法。

Conclusion: InstanceAnimator通过创新的画布引导条件、实例匹配机制和自适应解耦控制模块，成功解决了多实例草图视频着色中的关键挑战，实现了前所未有的用户控制灵活性和高质量的着色效果。
...
TL;DR: 提出CLIP-RD关系知识蒸馏框架，通过垂直关系蒸馏和交叉关系蒸馏方法，显式建模教师-学生嵌入间的多向关系依赖，提升轻量学生模型对CLIP教师模型结构关系的保持能力


<details>
  <summary>Details</summary>
Motivation: CLIP模型需要大量计算和内存资源，现有CLIP蒸馏方法未能显式建模教师-学生嵌入间的多向关系依赖，限制了学生模型保持教师编码的结构关系的能力

Method: 提出关系知识蒸馏框架CLIP-RD，包含两种新方法：垂直关系蒸馏（VRD）在分布层面强制教师-学生蒸馏强度跨模态一致性；交叉关系蒸馏（XRD）对跨模态教师-学生相似度分布施加双向对称性约束

Result: CLIP-RD通过联合建模多向关系结构，促进学生嵌入几何与教师嵌入的忠实对齐，性能超越现有方法0.8个百分点

Conclusion: 显式建模多向关系依赖的关系知识蒸馏框架能有效提升CLIP蒸馏效果，促进学生模型更好地保持教师模型的结构关系
...
TL;DR: 提出PTM-ST框架解决多模态数据集蒸馏中教师模型动态知识捕获不足的问题，通过分阶段教师建模和捷径轨迹构建策略提升蒸馏稳定性和表达能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以捕捉教师模型后期训练阶段中复杂动态演化的知识，导致学生性能下降和蒸馏数据质量受损，存在显著的跨阶段性能差距和不稳定的教师轨迹问题。

Method: 提出PTM-ST（分阶段教师模型与捷径轨迹）框架，采用阶段感知的教师建模和基于捷径的轨迹构建策略，精确拟合教师在不同训练阶段的学习动态。

Result: PTM-ST显著减轻优化振荡和阶段间知识差距，同时降低存储开销。在Flickr30k和COCO数据集上超越现有方法，Flickr30k上实现最高13.5%绝对提升，平均增益9.53%。

Conclusion: PTM-ST通过分阶段蒸馏框架有效解决了多模态数据集蒸馏中的动态知识捕获问题，提升了蒸馏过程的稳定性和表达能力，为高效知识压缩和迁移提供了新思路。
...
TL;DR: FSGNet：一种用于红外小目标检测的轻量级框架，通过频率感知和语义引导机制解决U-Net中的语义退化问题
...
Method: 提出FSGNet框架，包含三个核心模块：1）多方向交互注意力模块，在编码器中捕获细粒度和方向性特征；2）多尺度频率感知模块，利用快速傅里叶变换过滤目标相似杂波；3）全局池化模块，捕获高层语义信息并通过全局语义引导流传播到各解码器阶段

Result: 在四个公开IRSTD数据集上的实验表明，FSGNet实现了优越的检测性能，同时保持高效率，突出了其实用性和鲁棒性

Conclusion: FSGNet通过频率感知和语义引导机制有效解决了U-Net中的语义退化问题，在红外小目标检测中实现了精确的定位和高效的性能，具有实际应用价值
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视觉基础模型的编码器-解码器分割方法需要微调编码器，牺牲了多任务编码器共享的优势。需要一种既能保持编码器冻结共享，又能实现快速分割的方法

Method: 提出Plain Mask Decoder (PMD)，一个基于Transformer的快速分割解码器，直接在冻结的VFM特征上操作。由此构建的PMT模型保持了编码器-解码器架构的简单性和低延迟，同时保持编码器表示不变且可共享

Result: 在图像分割基准测试中，PMT匹配了冻结编码器的最先进性能，同时运行速度提升约3倍。在视频分割中，性能与完全微调的方法相当，同时比最先进的冻结编码器模型快8倍

Conclusion: PMT成功解决了编码器共享与分割性能之间的权衡问题，提供了一种既保持编码器冻结共享又能实现快速高效分割的解决方案，适用于图像和视频分割任务
...
Conclusion: LaMP通过显式嵌入3D场景流作为潜在运动先验，有效解决了现有VLA模型在复杂物理交互学习中的局限性，显著提升了机器人操作的性能和鲁棒性。
...
TL;DR: 该论文提出了一个分层框架，将视觉语言模型（VLMs）的3D空间理解学习分解为四个渐进复杂层次，并构建了自动化流水线生成大规模3D空间VQA数据进行监督微调，同时开发了RGB-D VLM以增强空间理解能力。


<details>
  <summary>Details</summary>
Motivation: 实现人类水平的空间智能需要视觉语言模型从2D观察中推断3D结构、识别3D空间中的物体属性和关系，并进行高级空间推理。当前VLMs在这些能力上存在不足，需要系统性的方法来提升3D空间理解能力。

Method: 1. 提出分层框架将3D空间理解分解为四个层次：几何感知、物体级理解、关系推理和抽象推理；2. 构建自动化流水线处理约500万张图像（包含4500万个物体）生成3D空间VQA对用于监督微调；3. 开发RGB-D VLM，将度量尺度点云图作为辅助输入以增强空间理解。

Result: 在多个空间理解和推理基准测试中达到最先进性能，超越了专用空间模型和大型专有系统（如Gemini-2.5-pro和GPT-5）。分析显示分层任务级别之间存在清晰的依赖关系，为多级任务设计如何促进3D空间智能的出现提供了新见解。

Conclusion: 提出的分层框架和自动化数据生成方法有效提升了VLMs的3D空间理解能力，证明了多级任务设计在促进空间智能涌现中的重要性，为开发更强大的空间感知VLMs提供了系统性的方法。
...
TL;DR: VideoWeaver：首个多模态多视角视频到视频转换框架，通过共享4D潜在空间和扩散时间步训练实现视角一致性，支持机器人学习中的世界随机化


<details>
  <summary>Details</summary>
Motivation: 现有视频到视频转换方法只能处理单视角，而具身AI任务通常需要多视角同步摄像头支持策略学习。独立应用单视角模型会导致跨视角外观不一致，标准Transformer架构因跨视角注意力的二次成本无法扩展到多视角设置

Method: 1. 首先训练为基于流的单视角V2V模型；2. 通过Pi3空间基础模型将所有视角锚定在共享4D潜在空间中，确保视角一致性；3. 在不同扩散时间步训练不同视角，使模型学习联合和条件视角分布；4. 支持基于现有视角的自回归新视角合成

Result: 在单视角转换基准测试中达到或优于最先进水平；首次实现物理和风格一致的多视角转换，包括对机器人学习世界随机化至关重要的挑战性第一人称和异构摄像头设置

Conclusion: VideoWeaver是首个多模态多视角V2V转换框架，通过共享潜在空间和扩散时间步训练解决了多视角一致性问题，为具身AI策略迁移到新环境提供了有效的世界随机化工具
...
TL;DR: DC-Reg：基于DC规划的全局最优点云配准框架，通过整体凹下界估计显著加速分支定界搜索，实现大错配和部分重叠下的鲁棒配准。


<details>
  <summary>Details</summary>
Motivation: 现有全局最优点云配准方法面临两个主要问题：1）同时估计变换参数和对应关系的非凸耦合目标函数容易陷入局部最优；2）现有全局求解器由于下界估计过于宽松导致收敛时间过长。需要一种既能保证全局最优性又能高效收敛的鲁棒配准方法。

Method: 提出DC-Reg框架，核心创新是基于差分凸（DC）规划推导出变换参数和对应关系耦合目标函数的整体凹下界估计器。与先前依赖逐项松弛（如McCormick包络）的方法不同，该方法捕捉了变量间的联合结构交互。该公式化使得能够通过搜索框顶点处的高效线性分配问题（LAP）计算紧致下界。框架支持2D相似变换和3D刚体配准，后者使用旋转不变特征实现高效性而不牺牲最优性。

Result: 在合成数据和3DMatch基准测试上的实验结果表明，DC-Reg相比最先进的全局技术实现了显著更快的收敛速度，以及对极端噪声和离群点具有更优越的鲁棒性。该方法在保持全局最优性的同时大幅提升了计算效率。

Conclusion: DC-Reg通过基于DC规划的整体凹下界估计，为具有部分重叠和大错配的点云配准问题提供了一个高效且鲁棒的全局最优解决方案。该方法在理论和实验上均优于现有全局配准技术，为实际应用中的鲁棒配准提供了实用工具。
...
<details>
  <summary>Details</summary>
Motivation: 自回归模型在图像生成方面取得了显著进展，但其计算密集和序列特性阻碍了设备端部署，导致高延迟。现有方法在处理视觉合成时面临两个关键挑战：高保真图像所需的大规模token词汇量，以及空间冗余导致的均匀区域高度可预测性与物体边界高不确定性之间的矛盾。

Method: CIAR采用云-设备协作框架，核心是设备端token不确定性量化器，使用连续概率区间而非传统离散解集来处理大规模视觉词汇。结合区间增强解码模块，通过分布对齐训练策略加速解码同时保持视觉保真度和语义一致性。
...
TL;DR: GridVAD：一种无需训练的零样本视频异常检测框架，利用视觉语言模型作为异常提议器，通过空间和时间模块进行接地和传播，实现像素级异常分割。


<details>
  <summary>Details</summary>
Motivation: 直接使用视觉语言模型作为视频监控异常检测器存在脆弱性：缺乏校准的异常先验会导致漏检和误报。问题不在于VLM本身，而在于如何使用它。VLM应作为异常提议器，生成开放集候选描述，然后由专门的空间和时间模块进行接地和跟踪。

Method: 提出"提议-接地-传播"原则，实现为GridVAD训练免费流程：1）VLM对视频片段的网格化表示进行推理，生成自然语言异常提议；2）自一致性整合过滤幻觉，仅保留多次独立采样中重复出现的提议；3）Grounding DINO将每个存活提议锚定为边界框；4）SAM2在异常间隔内将其传播为密集掩码。每片段VLM调用预算固定为M+1次，与视频长度无关。

Result: 在UCSD Ped2数据集上，GridVAD达到最高像素级AUROC（77.59），超过部分微调的TAO（75.11），在对象级RBDC上比其他零样本方法高出5倍以上。自一致性整合提供可控的精度-召回权衡：过滤改善所有像素级指标，对象级召回略有损失。效率实验显示GridVAD比均匀每帧VLM查询效率高2.7倍，同时产生密集分割掩码。

Conclusion: GridVAD成功展示了将VLM作为异常提议器而非直接检测器的有效性，通过专门的空间和时间模块实现稳健的零样本异常检测。该方法无需领域特定训练，提供像素级异常掩码，在精度和效率方面均优于现有方法。
...
TL;DR: AdaSFormer：一种用于室内单目语义场景补全的序列化Transformer框架，通过自适应感受野、中心相对位置编码和卷积调制层归一化解决Transformer在室内场景中的内存和细节重建问题。
...
Conclusion: AdaSFormer通过创新的序列化Transformer设计有效解决了室内MSSC中Transformer的内存和细节重建问题，在多个基准数据集上取得了最优结果。
...
TL;DR: KGFP是一种基于表示的监控框架，通过检测目标检测器内部特征与视觉基础模型嵌入之间的语义错位，来预测安全关键目标检测失败，而非传统OOD检测方法。


<details>
  <summary>Details</summary>
Motivation: 安全关键环境中的目标检测器可能会无声地失败（如漏检行人等关键对象），传统OOD检测方法只能识别陌生输入，无法直接预测检测器本身的功能性失败。

Method: 采用双编码器架构和角度距离度量，测量目标检测器内部特征与视觉基础模型嵌入之间的语义错位。当检测器超出其能力范围或视觉基础模型遇到新输入时，两种嵌入会发散，产生高角度信号来标记不安全图像。

Result: 在COCO行人检测任务中，KGFP作为选择性预测门，将接受图像中行人召回率从64.3%提升到84.5%（5% FPR下），在六个COCO-O视觉域上保持强劲性能，大幅优于OOD基线方法。

Conclusion: KGFP通过监测目标检测器与视觉基础模型之间的语义一致性，能够有效预测安全关键检测失败，为安全关键应用提供了可靠的运行时监控框架。
...
TL;DR: 该论文针对真实世界图像修复任务，构建大规模数据集并训练开源模型，同时提出包含464张真实退化图像的评估基准RealIR-Bench，以缩小与闭源模型的性能差距。


<details>
  <summary>Details</summary>
Motivation: 现有修复模型受限于训练数据的规模和分布，在真实世界场景中泛化能力差。虽然大型图像编辑模型（如闭源模型Nano Banana Pro）在修复任务中表现出色，但需要大量数据和计算成本。需要开发高性能的开源替代方案。

Method: 1. 构建覆盖九种常见真实世界退化类型的大规模数据集；2. 训练最先进的开源修复模型；3. 提出RealIR-Bench评估基准，包含464张真实退化图像和专注于退化去除与一致性保持的评估指标。

Result: 实验表明，所提出的模型在开源方法中排名第一，达到了最先进的性能水平，显著缩小了与闭源替代方案的差距。

Conclusion: 通过构建大规模数据集和专门的评估基准，成功开发了高性能的开源图像修复模型，为真实世界图像修复任务提供了有效的解决方案。
...
TL;DR: 该论文提出时间步感知质量解耦(TQD)方法，通过调整训练过程中的时间步选择策略，解决视频数据中视觉质量与运动强度负相关的困境，无需完美数据即可训练出高质量视频生成模型。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型依赖高质量数据，但视频数据存在"运动-视觉质量困境"：视觉质量与运动强度呈负相关，难以同时获得高视觉质量和高运动质量的数据。传统方法需要完美数据，限制了模型训练。

Method: 提出时间步感知质量解耦(TQD)：1) 分析视频扩散模型的分层学习动态，对质量退化样本进行梯度分析；2) 发现质量不平衡数据在适当时间步能产生与完美数据相似的梯度；3) 引入训练过程中的时间步选择概念，调整数据采样分布以匹配模型学习过程：运动丰富数据偏向高时间步采样，高视觉质量数据偏向低时间步采样。

Result: 实验表明：1) TQD仅使用分离的不平衡数据训练即可超越使用更好数据的传统训练方法；2) 挑战了视频生成中完美数据的必要性；3) 在高质量数据上训练时也能提升模型性能；4) 在不同数据场景下均表现出有效性。

Conclusion: TQD方法通过时间步感知的质量解耦，有效解决了视频数据中的运动-视觉质量困境，无需依赖完美数据即可训练高质量视频生成模型，为视频生成领域的数据利用提供了新思路。
...
TL;DR: 提出了首个羽毛球全场比赛密集标注数据集BFMD，包含19场比赛、1687个回合、16751次击球事件，每击球都有标注。开发了基于VideoMAE的多模态字幕生成框架，通过语义反馈机制提升字幕质量。
...
Method: 1) 构建BFMD数据集：包含19场转播比赛（单打和双打），超过20小时比赛时间，提供分层标注（比赛片段、回合事件、密集回合级多模态标注）；2) 开发基于VideoMAE的多模态字幕生成框架，引入语义反馈机制，利用击球语义指导字幕生成。

Result: 实验结果表明，多模态建模和语义反馈相比仅使用RGB的基线方法显著提升了击球字幕质量。进一步展示了BFMD在分析全场比赛战术模式时间演化方面的潜力。

Conclusion: BFMD数据集填补了羽毛球全场比赛密集标注数据的空白，提出的多模态字幕生成框架有效提升了击球字幕质量，为比赛级战术分析提供了重要基础。
...
TL;DR: PAWS：从野外第一人称视频中的手-物体交互直接提取物体铰接结构，无需3D监督数据或人工标注


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的铰接感知方法严重依赖高质量3D数据和人工标注的监督训练，限制了方法的可扩展性和多样性。需要一种能够从大规模野外视频中直接学习铰接结构的方法。

Method: PAWS方法直接从大规模野外第一人称视频中的手-物体交互中提取物体铰接结构。该方法避免了传统方法对3D监督数据和人工标注的依赖。

Result: 在HD-EPIC和Arti4D等公开数据集上评估，相比基线方法取得了显著改进。提取的铰接结构有益于下游任务，包括微调3D铰接预测模型和实现机器人操作。

Conclusion: PAWS提供了一种从野外视频中直接学习物体铰接结构的新方法，减少了对监督数据的依赖，提高了方法的可扩展性，并为机器人操作等下游任务提供了实用价值。
...
TL;DR: 该论文针对当前地球观测大模型忽视垂直维度的问题，提出了首个高度感知的遥感理解评估框架，包括数据生成管道、两个基准测试集和一个基线模型，验证了高度感知在复杂遥感场景中的必要性。


<details>
  <summary>Details</summary>
Motivation: 当前地球观测大模型普遍忽视关键的"垂直"维度，限制了它们在复杂遥感几何和灾害场景中的推理能力，因为在这些场景中物理空间结构往往比平面视觉纹理更重要。

Method: 1) 开发可扩展的VLM驱动数据生成管道，利用系统提示工程和元数据提取；2) 构建两个互补基准：GeoHeight-Bench用于相对高度分析，GeoHeight-Bench+用于更全面的地形感知推理；3) 提出首个高度感知遥感LMM基线GeoHeightChat，通过将视觉语义与隐式注入的高度几何特征相结合。

Result: 提出的基线模型GeoHeightChat作为强有力的概念验证，表明将视觉语义与隐式注入的高度几何特征协同结合，能有效缓解"垂直盲点"，成功在现有光学模型中解锁了交互式高度推理的新范式。

Conclusion: 该研究填补了地球观测大模型中高度感知能力的空白，通过创新的评估框架和基线模型，证明了高度信息在复杂遥感场景理解中的关键作用，为未来高度感知遥感模型的发展奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有生物多样性识别方法通常将分类学视为扁平标签空间，未能编码生物分类的层次结构，导致在噪声和模态缺失情况下鲁棒性不足

Method: 提出两种端到端变体：CLiBD-HiR引入层次信息正则化（HiR）在分类学层级间塑造嵌入几何；CLiBD-HiR-Fuse额外训练轻量级融合预测器，支持图像、DNA或联合推理

Result: 在大规模生物多样性基准测试中，相比强大多模态基线，分类准确率提升超过14%，在部分和损坏DNA条件下获得特别显著的改进

Conclusion: 显式编码生物层次结构结合灵活融合是构建实用生物多样性基础模型的关键，该方法为生态学、保护和环境监测提供了更鲁棒的解决方案
...
TL;DR: LanteRn框架让多模态模型能在潜在空间中直接进行视觉推理，通过生成和关注连续的视觉思维嵌入，避免了像素空间的不必要计算
...
Method: 引入LanteRn框架，增强视觉语言变换器，使其能在推理过程中生成和关注连续的视觉思维嵌入。采用两阶段训练：监督微调将视觉特征锚定在潜在状态，然后通过强化学习将潜在推理与任务级效用对齐
...
TL;DR: 提出Trace框架，通过单锚帧轨迹设计实现视频中目标物体运动路径编辑，解决相机运动下轨迹控制的挑战


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法主要关注外观修改或依赖点跟踪轨迹控制，后者在相机运动视频中用户难以提供推理时的轨迹控制，需要更实用易用的物体中心运动编辑方法

Method: 两阶段流水线：1)跨视图运动变换模块将首帧路径设计映射到相机运动下的帧对齐边界框轨迹；2)运动条件视频重合成模块遵循这些轨迹重新生成物体同时保留输入视频其余内容

Result: 在多样化真实世界视频上的实验表明，相比最近的图像到视频和视频到视频方法，该方法产生更连贯、真实且可控的运动编辑效果

Conclusion: Trace框架提供了一种实用、易用的物体中心运动编辑方法，通过单锚帧轨迹设计实现可控的视频物体运动路径编辑，有效处理相机运动场景
...
TL;DR: 本文提出两种简单改进方法解决V&L模型组合性学习不足的问题：1) 使用短概念中心字幕部分与图像对齐；2) 引入参数无关的跨模态注意力池化获取概念中心视觉嵌入，在保持零样本和检索能力的同时实现组合性任务SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有对比视觉语言模型在组合性表示学习方面存在局限。先前方法通常通过生成定制训练数据获取困难负样本来解决，但这些方法存在三个主要问题：1) 通常针对单一基准测试；2) 缺乏泛化能力；3) 会严重损害模型的基本V&L能力（如零样本或检索性能）。本文旨在找到更根本的解决方案。

Method: 提出两种核心改进：1) 使用标准NLP工具获取短概念中心字幕部分，并与图像对齐；2) 引入参数无关的跨模态注意力池化机制，从图像编码器中获取概念中心视觉嵌入。结合简单的辅助对比损失，在不增加推理成本的情况下提升组合性学习能力。

Result: 在标准组合性基准测试中达到SOTA性能，同时保持或改进了强大的零样本和检索能力。方法具有通用性，不针对特定基准，且不会损害模型的基本V&L功能。

Conclusion: 通过解决组合性学习的两个根本原因（长训练字幕不需要组合性表示，以及全局池化导致绑定信息丢失），提出了简单有效的解决方案，在提升组合性性能的同时保持了模型的通用能力，为V&L模型的组合性学习提供了实用且可扩展的方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有真实世界数据集覆盖有限，先前合成数据集缺乏遮挡、手臂细节和深度信息，限制了手部姿态估计的性能和鲁棒性提升

Method: 创建包含250万张单手和410万张手物交互RGB-D图像的AnyHand数据集，提供丰富的几何标注；开发轻量级深度融合模块，可集成到现有RGB模型中

Result: 在RGB设置下，使用AnyHand扩展训练集显著提升FreiHAND和HO-3D基准性能；在HO-Cap数据集上表现出更强的泛化能力；RGB-D模型在HO-3D基准上达到最优性能

Conclusion: AnyHand数据集通过提供大规模、多样化的合成数据有效解决了手部姿态估计的数据瓶颈，深度融合模块进一步提升了RGB-D模型的性能，展示了合成数据和深度信息整合的价值
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注自然图像合成，未能系统评估模型在真实商业设计任务中的结构化、多约束要求下的表现。需要专门针对商业视觉内容创建的系统性评估框架。

Method: 构建包含5种代表性文档类型（幻灯片、图表、网页、海报、科学图表）和4个关键能力维度（文本渲染、布局控制、属性绑定、知识推理）的基准测试，形成20个多样化评估任务。包含400个精心设计的提示和8000个人工验证的检查清单问题。

Result: 对26个流行图像生成系统（包括最先进的商业API和领先的开源模型）进行大规模基准测试，结果显示当前生成模型与专业视觉内容创建需求之间存在显著能力差距。

Conclusion: BizGenEval可作为真实世界商业视觉内容生成的标准化基准，揭示当前模型的局限性，并为未来模型开发提供指导方向。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频时序定位任务中，由于粗粒度的识别能力不足，需要进行任务特定的微调。但这种微调导致模型记忆数据集特定的捷径而非真正基于视觉内容进行定位，造成跨域泛化能力差。需要一种低成本的方法来引导模型进行以对象为中心的视觉推理。

Method: 提出SlotVTG框架，引入轻量级槽适配器，通过槽注意力将视觉标记分解为抽象槽并重建原始序列。利用自监督视觉模型的对象性先验促进语义一致的槽形成，引导MLLMs进行以对象为中心的输入接地视觉推理。

Result: 在标准VTG基准测试的跨域评估中，该方法显著提升了跨域鲁棒性，同时保持了具有竞争力的域内性能，且计算开销最小。

Conclusion: SlotVTG通过低成本的方式将MLLMs引导至以对象为中心的视觉推理，有效解决了视频时序定位中的跨域泛化问题，为提升多模态模型的细粒度时空理解能力提供了有效途径。
...
TL;DR: LIGHT：一种基于数据驱动的扩散模型，通过异步去噪调度和模态特定噪声水平实现接触感知指导，无需手动设计先验，提高人-物交互动画的真实性和泛化能力


<details>
  <summary>Details</summary>
Motivation: 生成真实的人-物交互动画具有挑战性，需要同时建模动态人类动作和多样物体几何。现有基于扩散的方法通常依赖手工设计的接触先验或人为施加的运动学约束来改善接触质量，这限制了方法的泛化能力和数据驱动特性。

Method: 提出LIGHT方法，基于扩散强迫技术，将表示分解为模态特定组件，并为每个组件分配个体化噪声水平和异步去噪调度。在这种范式下，较干净的组件通过交叉注意力指导较嘈杂的组件，实现无需辅助分类器的指导。该方法通过训练时使用广泛的合成物体几何数据进行增强，鼓励接触语义对几何多样性的不变性。

Result: 实验表明，节奏诱导的指导比传统的无分类器指导更有效地模拟接触先验的益处，同时实现更高的接触保真度、更真实的HOI生成，以及对未见物体和任务的更强泛化能力。

Conclusion: LIGHT提供了一种数据驱动的替代方案，减少了对手动设计先验的依赖，通过异步去噪调度和模态特定噪声水平实现接触感知指导，在生成真实人-物交互动画方面表现出优越性能。
...
TL;DR: PSDesigner是一个模仿人类设计师工作流程的自动化图形设计系统，通过专业组件收集主题相关素材，自主推断并执行工具调用来操作设计文件，基于CreativePSD数据集学习专家设计流程。
...
Method: 提出PSDesigner系统，基于多个专业组件：1) 根据用户指令收集主题相关素材；2) 自主推断并执行工具调用来操作设计文件；3) 构建CreativePSD数据集，包含大量高质量PSD设计文件及操作轨迹，涵盖广泛设计场景和艺术风格。

Result: 广泛实验表明PSDesigner在多样化图形设计任务中优于现有方法，使非专业人士能够方便地创建生产质量的设计作品。

Conclusion: PSDesigner通过模拟人类设计师工作流程，解决了现有自动化设计系统灵活性和直观性不足的问题，成功将用户意图转换为可编辑设计文件，为图形设计自动化提供了有效解决方案。
...
TL;DR: MegaFlow：利用预训练视觉Transformer特征进行全局匹配的零样本大位移光流估计模型，无需特定架构设计或微调


<details>
  <summary>Details</summary>
Motivation: 现有光流估计方法依赖迭代局部搜索和领域特定微调，在大位移和零样本泛化场景下性能受限，需要一种更通用的解决方案

Method: 将光流估计构建为全局匹配问题，利用预训练全局Vision Transformer特征捕获大位移，然后进行少量轻量级迭代细化提升亚像素精度
...
TL;DR: RefAlign提出了一种用于参考到视频生成任务的表示对齐框架，通过将扩散Transformer的参考分支特征与视觉基础模型的语义空间显式对齐，解决了现有方法中的复制粘贴伪影和多主体混淆问题。


<details>
  <summary>Details</summary>
Motivation: 现有参考到视频生成方法通常将参考图像的VAE潜在表示与额外的高层语义或跨模态特征一起输入扩散Transformer。这些辅助表示虽然能提供语义指导并作为隐式对齐信号，但仍难以解决由异构编码器特征间的模态不匹配导致的复制粘贴伪影和多主体混淆问题。

Method: 提出RefAlign表示对齐框架，其核心是一个参考对齐损失函数。该损失函数将同一主体的参考特征和视觉基础模型特征拉近以提高身份一致性，同时将不同主体的对应特征推远以增强语义区分性。这一简单有效的策略仅在训练时应用，不增加推理时间开销。

Result: 在OpenS2V-Eval基准测试上的大量实验表明，RefAlign在TotalScore指标上优于当前最先进的方法，验证了显式参考对齐在参考到视频生成任务中的有效性。
...
TL;DR: 提出MuRF（多分辨率融合）策略，通过处理图像的多分辨率视图并融合特征来增强视觉基础模型的推理性能，无需额外训练。
...
Method: MuRF通过处理图像在多个分辨率下的视图，将冻结的视觉基础模型生成的特征进行融合，构建统一的表示。该方法与特定架构无关，是一种无需训练的基础性增强策略。

Result: MuRF在广泛的计算机视觉任务上得到验证，主要应用于DINOv2模型家族，并成功推广到对比学习模型如SigLIP2，证明了其通用性和有效性。

Conclusion: MuRF作为一种简单而通用的策略，能够有效利用多分辨率视图的互补优势，为视觉基础模型提供无需训练的性能增强，具有广泛的适用性。
...
TL;DR: LGTM提出了一种前馈式3D高斯溅射框架，通过预测紧凑高斯基元配合纹理贴图，将几何复杂度与渲染分辨率解耦，实现了无需逐场景优化的4K新视角合成。


<details>
  <summary>Details</summary>
Motivation: 现有前馈式3D高斯溅射方法预测像素对齐基元，导致基元数量随分辨率呈二次增长，限制了可扩展性，使得4K等高分辨率合成难以实现。

Method: LGTM框架预测紧凑的高斯基元并配合每个基元的纹理贴图，将几何复杂度与渲染分辨率解耦，使用更少的高斯基元实现高分辨率渲染。

Result: 该方法实现了高保真的4K新视角合成，无需逐场景优化，这是前馈方法之前无法达到的能力，同时显著减少了高斯基元的使用数量。

Conclusion: LGTM通过解耦几何复杂度和渲染分辨率，克服了前馈式3D高斯溅射方法的分辨率扩展障碍，为高分辨率新视角合成提供了可行方案。
...
TL;DR: ShotStream：一种用于交互式多镜头视频生成的因果架构，通过双缓存记忆机制和两阶段蒸馏策略实现实时叙事生成


<details>
  <summary>Details</summary>
Motivation: 当前双向架构在多镜头视频生成中存在交互性有限和延迟高的问题，无法满足实时交互式叙事的需求。需要一种能够支持用户动态指导叙事流程的实时生成系统。

Method: 1. 将任务重新定义为基于历史上下文的下一个镜头生成；2. 通过分布匹配蒸馏将双向文本到视频模型转化为因果学生模型；3. 引入双缓存记忆机制：全局上下文缓存保持镜头间一致性，局部上下文缓存保持镜头内一致性；4. 使用RoPE不连续性指示器区分两个缓存；5. 采用两阶段蒸馏策略：从基于真实历史镜头的镜头内自强制开始，逐步扩展到使用自生成历史的镜头间自强制。

Result: ShotStream在单个GPU上实现16 FPS的生成速度，达到亚秒级延迟。在质量上匹配或超过较慢的双向模型，能够生成连贯的多镜头视频，支持实时交互式叙事。

Conclusion: ShotStream通过因果架构、双缓存记忆机制和两阶段蒸馏策略，成功解决了多镜头视频生成中的交互性和延迟问题，为实时交互式叙事开辟了新途径。模型和代码已开源。
...
TL;DR: 研究发现抑郁症自动检测模型存在系统性偏差：模型利用半结构化访谈中固定的访谈员提示而非参与者语言来预测抑郁症，导致性能虚高


<details>
  <summary>Details</summary>
Motivation: 尽管基于医患对话的自动抑郁症检测研究取得了进展，但模型可解释性有限，现有研究往往报告高性能却未揭示预测依据。研究者旨在分析模型是否真正学习参与者的语言特征，还是利用了访谈协议中的系统性偏差

Method: 分析三个公开数据集（ANDROIDS、DAIC-WOZ、E-DAIC），通过对比模型在完整对话与仅参与者话语上的表现，识别半结构化访谈中访谈员提示导致的系统性偏差。研究考察了不同架构模型如何利用访谈员固定提示和位置信息进行预测

Result: 模型在完整对话中利用访谈员固定提示和位置信息区分抑郁与非抑郁被试，常获得高分类分数而无需使用参与者语言。当限制模型仅使用参与者话语时，决策证据分布更广泛且反映真实语言线索。半结构化协议确保一致性，但包含访谈员提示会因利用脚本伪影而虚增性能

Conclusion: 研究揭示了跨数据集、架构无关的系统性偏差，强调需要通过时间和说话者定位决策证据的分析方法，确保模型真正学习参与者的语言特征，而非访谈协议中的伪影
...
TL;DR: 通过微调小型开源LLM（12亿参数）用于系统综述的文献筛选，相比基础模型F1分数提升80.79%，与人工编码员一致性达86.40%，证明微调方法比单纯提示更有效。


<details>
  <summary>Details</summary>
Motivation: 传统系统综述需要人工筛选大量文献标题和摘要，耗时耗力。现有研究尝试使用大语言模型（LLM）提高效率，但仅通过提示（prompting）效果不一致，可能因为提示无法提供足够上下文。本研究旨在探索通过微调LLM来改进系统综述中的文献筛选效果。

Method: 使用人类已标注的8500多篇文献标题和摘要数据集，对一个小型开源LLM（12亿参数）进行微调，专门用于系统综述的文献筛选任务。比较微调后模型与基础模型在相同数据集上的性能表现。

Result: 微调模型性能显著提升：加权F1分数相比基础模型提高80.79%。在8277篇文献的完整数据集上，微调模型与人工编码员的一致性达到86.40%，真阳性率91.18%，真阴性率86.38%，且在多次推理运行中表现完全一致。

Conclusion: 微调LLM在系统综述的文献标题和摘要筛选中具有良好前景。相比单纯使用提示，针对特定任务进行微调能显著提升模型性能，为大规模系统综述的自动化筛选提供了有效解决方案。
...
TL;DR: 研究通过微调LLaMA 3.1-8B模型，探索其在芬兰语临床对话转录中的有效性，结果显示语义相似度高但n-gram重叠度低


<details>
  <summary>Details</summary>
Motivation: 临床文档对患者安全、诊断和护理连续性至关重要，但电子健康记录的管理负担导致医生职业倦怠，这在芬兰语等低资源语言中尤为突出。研究旨在探索领域对齐的自然语言处理模型在芬兰语医疗转录中的有效性。

Method: 使用Metropolia应用科学大学学生模拟临床对话的小型验证语料库微调LLaMA 3.1-8B模型，采用受控预处理和优化方法，通过七折交叉验证评估微调效果。
...
TL;DR: 该论文介绍了首个英语句子数据集，包含在缺乏该特征的AMR图上标注的UMR体（aspect）标签，建立了自动预测UMR体的基准实验。


<details>
  <summary>Details</summary>
Motivation: 体（aspect）对于完整捕捉句子意义至关重要，它描述事件内部的时间结构。然而，在语义表示框架中，体标注仍然稀疏，这阻碍了手动标注和自动预测系统的发展。

Method: 1. 创建新的英语句子数据集，在缺乏体特征的AMR图上标注UMR体标签；2. 设计基于UMR体格的标注方案和指南；3. 采用多步骤裁决流程确保标注一致性和质量；4. 使用三种建模方法进行基准实验。

Result: 建立了首个UMR体标注数据集，为自动UMR体预测提供了初始基准。通过三种建模方法的实验结果，为未来自动化系统开发奠定了基础。

Conclusion: 该数据集填补了语义表示中体标注的空白，为将体信息更广泛地整合到语义表示中提供了基础，并展示了数据集对未来自动化研究的实用性。
...
TL;DR: 葡萄牙语持续预训练中，合成重写与源数据质量的交互作用研究：高质量数据重写带来显著性能提升，低质量数据重写效果有限，且这种效应具有规模依赖性。


<details>
  <summary>Details</summary>
Motivation: 现有合成数据生成研究主要关注英语，且未系统控制源数据质量。本研究旨在探究在葡萄牙语持续预训练中，合成重写如何与源数据质量相互作用，以理解数据质量对重写效果的影响。

Method: 使用ClassiCC-PT葡萄牙语语料库（标注有STEM和教育质量分数），构建两个不同质量水平的10B token子集。使用7B指令调优模型将每个子集重写为四种风格，每个条件生成约40B token合成数据。在两个英语中心基础模型（1.1B和7B参数）上训练每个条件，并在PoETa V2（包含44个任务的葡萄牙语基准）上评估。

Result: 在7B规模下，重写高质量数据比未修改的相同数据带来+3.4 NPM增益，而重写低质量数据仅提供+0.5 NPM增益。在1.1B规模下，这种交互作用较弱，未修改的低质量数据与重写的高质量数据表现相当。合成重写主要作为质量乘数而非数据筛选的替代品，且这种效应具有规模依赖性。

Conclusion: 合成重写主要作为数据质量乘数而非数据筛选的替代品，其效果强烈依赖于源数据质量，且这种交互作用随模型规模变化。高质量源数据通过重写可获得显著性能提升，而低质量数据重写效果有限。
...
TL;DR: ARRoL是一种在线剪枝方法，通过训练轻量级质量头预测部分rollout的成功概率，在生成过程中剪枝rollout，同时引导剩余rollout更平衡以提高学习信号，显著提升RLVR方法的训练效率和推理准确性。
...
Method: 提出ARRoL（Accelerating RLVR via online Rollout Pruning）方法：1）训练轻量级质量头实时预测部分rollout的成功概率；2）基于预测进行早期剪枝决策；3）引导剩余rollout更平衡以增强学习信号；4）系统设计在推理引擎内部剪枝rollout，重新批处理剩余rollout进行对数概率计算和政策更新；5）质量头还可用于测试时缩放中加权候选以提高推理准确性。

Result: 在Qwen-3和LLaMA-3.2模型（1B-8B）上，ARRoL在GRPO和DAPO上平均准确率提升+2.30到+2.99，训练速度提升达1.7倍，测试时缩放中平均准确率额外提升达+8.33。

Conclusion: ARRoL通过在线剪枝rollout并引导剩余rollout更平衡，有效解决了RLVR方法的高计算成本和稀疏学习信号问题，显著提升了训练效率和推理准确性，为RLVR方法的实际应用提供了有效解决方案。
...
TL;DR: LogSigma系统在SemEval-2026任务3中通过基于学习同方差不确定性的多任务平衡方法，在维度方面情感分析任务上取得第一名


<details>
  <summary>Details</summary>
Motivation: 传统方面情感分析预测离散情感标签，而维度方面情感分析需要预测连续的效价和唤醒度分数。主要挑战在于不同语言和领域中效价和唤醒度的预测难度存在差异，需要自动平衡多个回归目标。

Method: 使用学习同方差不确定性方法，模型学习任务特定的对数方差参数来自动平衡训练过程中的每个回归目标。结合语言特定编码器和多种子集成技术。

Result: LogSigma在两个赛道中的五个数据集上均获得第一名。学习到的方差权重在不同语言间差异显著（德语0.66x，英语2.18x），表明最优任务平衡是语言依赖的，无法先验确定。

Conclusion: 基于学习同方差不确定性的多任务平衡方法能有效处理维度方面情感分析中不同语言和任务间的难度差异，证明了语言特定任务平衡的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM记忆量化方法存在局限：标准贪婪解码仅检测逐字记忆，忽略近逐字提取风险；概率提取方法虽能计算风险但仅适用于逐字情况，且近逐字风险量化成本过高（需约10万次蒙特卡洛采样）

Method: 提出解码约束束搜索方法，通过确定性搜索生成近逐字后缀，以约20次蒙特卡洛采样的成本计算近逐字提取风险的下界

Result: 方法揭示了标准方法无法检测的信息：更多可提取序列、更大的每序列提取质量，以及近逐字提取风险在不同模型规模和文本类型中的模式

Conclusion: 解码约束束搜索为高效量化LLM近逐字提取风险提供了实用工具，揭示了传统方法忽略的隐私和版权风险，对模型安全评估具有重要意义
...
TL;DR: 该论文研究如何通过数据选择、表示和高效适应策略，使机器翻译和质量评估系统适应专业领域，提出了多种方法提升领域适应性。
...
Result: 1. 小型针对性领域子集优于大型通用数据集，以更低计算成本达到强翻译质量；2. 质量评估方法在跨领域、跨语言和资源受限情况下均有改进；3. 对齐的分词-词汇表设置带来稳定训练和更好翻译质量；4. 质量评估引导的上下文学习方法无需参数更新即可提升翻译质量，优于标准检索方法。
...
### [137] [Exons-Detect: Identifying and Amplifying Exonic Tokens via Hidden-State Discrepancy for Robust AI-Generated Text Detection](https://arxiv.org/abs/2603.24981)
*Xiaowei Zhu,Yubing Ren,Fang Fang,Shi Wang,Yanan Cao,Li Guo*

Main category: cs.CL

TL;DR: Exons-Detect：一种基于外显子感知令牌重加权的免训练AI文本检测方法，通过识别和放大信息性外显子令牌，在双模型设置下测量隐藏状态差异，实现最先进的检测性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的快速发展模糊了人类书写与AI生成文本的界限，带来了错误信息传播、作者身份模糊和知识产权威胁等社会风险。现有免训练方法通常通过聚合令牌级信号获得全局分数，但假设令牌贡献均匀，在短序列或局部令牌修改下鲁棒性不足。

Method: 提出Exons-Detect，一种基于外显子感知令牌重加权的免训练AI文本检测方法。该方法在双模型设置下通过测量隐藏状态差异来识别和放大信息性外显子令牌，然后从重要性加权的令牌序列计算可解释的翻译分数。

Result: 实验评估表明，Exons-Detect实现了最先进的检测性能，并对对抗攻击和不同输入长度表现出强鲁棒性。在DetectRL数据集上，相对于先前最强基线，平均AUROC相对提升了2.2%。

Conclusion: Exons-Detect通过外显子感知令牌重加权有效解决了现有免训练检测方法的局限性，在AI生成文本检测方面表现出卓越性能和鲁棒性，为应对AI生成文本带来的社会风险提供了可靠解决方案。
...
### [138] [Imperative Interference: Social Register Shapes Instruction Topology in Large Language Models](https://arxiv.org/abs/2603.25015)
*Tony Mason*

Main category: cs.CL

TL;DR: 研究发现：相同语义内容但不同语言的系统提示指令在交互拓扑上存在差异（英语合作，西班牙语竞争），这种差异由社会语域（特别是祈使语气）在不同语言社群中的不同强制力所中介。


<details>
  <summary>Details</summary>
Motivation: 探究多语言模型在处理指令时是否存在语言依赖的差异，特别是相同语义内容在不同语言中是否会产生不同的交互拓扑结构，以及这种差异的潜在机制。

Method: 在四种语言和四种模型上进行指令级消融实验；将生产系统提示分解为56个块，使用22个手动编写的探针；通过将祈使语气重写为陈述语气来减少跨语言差异；进行排列检验等统计分析。

Result: 陈述语气重写单个指令块可将跨语言差异减少81%（p=0.029）；重写11个祈使块中的3个即可将西班牙语指令拓扑从竞争性转变为合作性，并对未重写的块产生溢出效应。

Conclusion: 模型将指令处理为社会行为而非技术规范；祈使语气的强制力具有语言依赖性，而陈述语气的事实描述则能跨语言传递；这暗示宪法AI原则若以祈使语气编写可能产生语言依赖的对齐问题。
...
### [139] [Closing the Confidence-Faithfulness Gap in Large Language Models](https://arxiv.org/abs/2603.25052)
*Miranda Muqing Miao,Lyle Ungar*

Main category: cs.CL

TL;DR: 研究发现LLM的口头置信度与实际准确性脱节，两者在表征空间中正交；推理过程会污染置信度方向，提出两阶段自适应引导方法改善校准对齐


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的口头置信度得分往往与其实际准确性脱节，但这种行为的几何关系机制尚不清楚，需要深入理解LLM内部如何编码和表达置信度信号

Method: 使用线性探针和对比激活加法（CAA）引导进行机制可解释性分析，发现校准信号和口头置信度信号线性编码但正交；提出两阶段自适应引导管道，读取模型内部准确性估计并引导口头输出与之匹配

Result: 在三个开源模型和四个数据集上一致发现：校准信号和口头置信度信号正交；推理过程会污染置信度方向（"推理污染效应"）；提出的自适应引导方法显著改善了所有评估模型的校准对齐

Conclusion: LLM的口头置信度与实际准确性脱节源于两者在表征空间中的正交关系，推理过程会进一步破坏置信度方向；通过读取内部准确性估计并引导口头输出的自适应方法可以有效改善校准对齐
...
### [140] [Do LLMs Know What They Know? Measuring Metacognitive Efficiency with Signal Detection Theory](https://arxiv.org/abs/2603.25112)
*Jon-Paul Cacioli*

Main category: cs.CL

TL;DR: 该论文提出了基于Type-2信号检测理论的LLM置信度评估框架，将模型的知识量（Type-1敏感性）与元认知能力（Type-2元认知敏感性）分离，通过meta-d'和M-ratio指标揭示不同LLM在"知道它们不知道什么"方面的真实能力差异。


<details>
  <summary>Details</summary>
Motivation: 传统LLM置信度评估使用校准指标（如ECE、Brier分数）混淆了两个不同能力：模型知道多少（Type-1敏感性）和模型知道它知道什么（Type-2元认知敏感性）。需要开发能够区分这两种能力的评估框架，以更准确地评估模型的元认知能力。

Method: 引入基于Type-2信号检测理论的评估框架，使用meta-d'（元认知敏感性）和M-ratio（元认知效率比）指标。在四个LLM（Llama-3-8B-Instruct、Mistral-7B-Instruct-v0.3、Llama-3-8B-Base、Gemma-2-9B-Instruct）上进行224,000个事实性QA试验评估，分析温度变化对Type-2准则和meta-d'的影响。

Result: 1. 元认知效率在不同模型间差异显著，即使Type-1敏感性相似；Mistral的d'最高但M-ratio最低。2. 元认知效率具有领域特异性，不同模型在不同领域表现最弱，这是聚合指标无法发现的。3. 温度操作改变Type-2准则但meta-d'保持稳定（四个模型中的两个），表明置信度策略与元认知能力可分离。4. AUROC_2和M-ratio产生完全相反的模型排名，证明这些指标回答根本不同的评估问题。

Conclusion: meta-d'框架能够区分哪些模型真正"知道它们不知道什么"与哪些仅仅因为准则设置而显得校准良好，这对模型选择、部署和人机协作具有直接意义。该框架为LLM元认知能力评估提供了更精细的分析工具。
...
### [141] [To Write or to Automate Linguistic Prompts, That Is the Question](https://arxiv.org/abs/2603.25169)
*Marina Sánchez-Torrón,Daria Akselrod,Jason Rauchwerk*

Main category: cs.CL

TL;DR: 自动提示优化与专家提示工程在语言任务中的首次系统比较，结果显示任务依赖性：术语插入中两者质量无显著差异，翻译中各方法在不同模型上表现不同，语言质量评估中专家提示错误检测更强而优化提升特征描述


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索自动提示优化是否能替代专家提示工程在语言任务中的应用。LLM性能对提示设计高度敏感，但之前缺乏对这两种方法在语言任务中的系统性比较。

Method: 研究方法包括：1）比较手工设计的零样本专家提示；2）基础DSPy签名；3）GEPA优化的DSPy签名。评估任务包括翻译、术语插入和语言质量评估，使用五种模型配置进行实验。

Result: 结果具有任务依赖性：在术语插入中，优化提示和手动提示的质量大多无统计显著差异；在翻译中，不同方法在不同模型上表现最佳；在语言质量评估中，专家提示在错误检测方面更强，而优化在特征描述方面有所改进。GEPA显著提升了最小DSPy签名的性能，大多数专家与优化的比较显示无统计显著差异。

Conclusion: 结论是自动提示优化在某些任务中可以达到与专家提示工程相当的效果，但比较存在不对称性：GEPA优化需要程序化搜索黄金标准分割，而专家提示原则上不需要标注数据，依赖领域专业知识和迭代优化。两种方法各有优势，选择取决于具体任务和资源约束。
...
### [142] [Prompt Attack Detection with LLM-as-a-Judge and Mixture-of-Models](https://arxiv.org/abs/2603.25176)
...
<details>
  <summary>Details</summary>
Motivation: 生产环境中，LLM系统面临提示攻击（越狱和提示注入）的安全风险。现有解决方案存在部署差距：轻量级分类器和基于规则的系统难以应对分布偏移，而高容量LLM法官又太慢或成本太高，无法满足实时执行的低延迟要求。

Method: 通过精心设计的提示和输出结构，引导轻量级LLM进行结构化推理过程，包括显式意图分解、安全信号验证、危害评估和自我反思。同时评估了混合模型（MoM）设置，即聚合多个LLM法官来提升检测性能。

Result: 实验表明，通用轻量级LLM（如gemini-2.0-flash-lite-001）可作为有效的低延迟法官用于实时护栏。该方法已在新加坡公共服务聊天机器人中作为集中式护栏服务部署。混合模型设置仅带来适度的性能提升。

Conclusion: 轻量级通用LLM通过结构化推理流程能够可靠地作为安全法官，满足生产环境的低延迟约束，填补了现有部署差距，为实时提示攻击检测提供了可行解决方案。
...
### [143] [Cross-Preference Learning for Sentence-Level and Context-Aware Machine Translation](https://arxiv.org/abs/2603.25183)
...
<details>
  <summary>Details</summary>
Motivation: 现有上下文感知机器翻译方法未能一致超越句子级翻译，因为上下文信号在不同句子中效益不均。现有训练目标未显式建模这种变异性，限制了模型自适应利用上下文的能力。

Method: 提出Cross-Preference Learning (CPL)偏好训练框架，整合句内偏好（同一条件下不同翻译质量比较）和跨条件偏好（不同条件下翻译质量比较），为偏好优化目标提供显式监督。

Result: 在多个公共上下文感知MT任务上验证，使用Qwen3-4B、Qwen3-8B和Llama-3-8B等模型，实验结果显示翻译质量和鲁棒性在两种输入条件下均获得一致提升，无需架构修改。

Conclusion: CPL框架通过显式建模句子级和上下文感知翻译的互补优势，有效解决了上下文信号效益不均的问题，为上下文感知机器翻译提供了更有效的训练方法。
...
### [144] [Probing the Lack of Stable Internal Beliefs in LLMs](https://arxiv.org/abs/2603.25187)
*Yifan Luo,Kangping Xu,Yanzhen Lu,Yang Yuan,Andrew Chi-Chih Yao*

Main category: cs.CL

TL;DR: LLMs在角色驱动应用中难以保持隐式一致性，即在多轮对话中维持未明确陈述的目标，这限制了其模拟真实人格特征的能力。


<details>
  <summary>Details</summary>
Motivation: 角色驱动的大型语言模型需要跨交互保持一致的行为倾向来模拟人类人格特征（如坚持性、可靠性），但现有LLMs缺乏稳定的内部表征来锚定其在扩展对话中的响应。

Method: 设计了20个问题式的谜语游戏范式，让LLM秘密选择一个目标，并对用户的猜测回答"是/否"。通过评估LLM在多轮交互中保持隐式一致性的能力，即能否持续坚持未明确陈述的目标。

Result: LLMs难以保持隐式一致性：除非在上下文中明确提供其选择的目标，否则它们的隐式"目标"会在不同轮次间发生偏移。

Conclusion: 这些发现突显了构建角色驱动LLMs的关键局限性，强调需要开发能够随时间锚定隐式目标的机制，这对于交互应用（如对话系统）中实现真实人格建模至关重要。
...
### [145] [A Catalog of Basque Dialectal Resources: Online Collections and Standard-to-Dialectal Adaptations](https://arxiv.org/abs/2603.25189)
*Jaione Bengoetxea,Itziar Gonzalez-Dios,Rodrigo Agerri*

Main category: cs.CL

TL;DR: 该论文提出了巴斯克语方言数据的系统化目录，包含在线原生方言数据和标准语到方言的适配数据，旨在解决方言NLP中的数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 方言NLP研究面临的主要限制是数据稀缺问题，特别是对于巴斯克语这样的语言。为了应对这一挑战，需要系统化地收集和整理可用的方言数据资源。

Method: 区分两种数据来源：1) 在线原生方言数据（新闻、推特、词典、地图集等）；2) 标准语到方言的适配数据。手动适配了XNLI数据集的测试集到三个巴斯克方言，并自动适配了物理常识数据集，然后进行人工评估。

Result: 创建了巴斯克语方言数据的综合目录，生成了高质量的平行评估数据集（XNLI测试集适配到三个方言），并通过人工评估验证了自动适配数据的质量，确定其可作为银标准数据使用。

Conclusion: 该研究提供了巴斯克语方言数据的系统化资源目录，为方言NLP研究奠定了基础，并展示了自动适配数据作为银标准数据的可行性，有助于缓解方言数据稀缺问题。
...
### [146] [A Decade-Scale Benchmark Evaluating LLMs' Clinical Practice Guidelines Detection and Adherence in Multi-turn Conversations](https://arxiv.org/abs/2603.25196)
*Andong Tan,Shuyu Dai,Jinglu Wang,Fengtao Zhou,Yan Lu,Xi Wang,Yingcong Chen,Can Yang,Shujie Liu,Hao Chen*

Main category: cs.CL

TL;DR: CPGBench是一个评估大语言模型在临床对话中识别和遵循临床实践指南能力的自动化基准框架，涵盖多国指南、24个专科的32,155条临床推荐，发现模型在指南检测和实际应用之间存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在医疗场景中应用日益广泛，但尚不清楚它们在对话中识别和遵循临床实践指南的能力如何。为确保基于证据的决策和改善患者结局，需要评估LLMs的临床指南检测和依从能力。

Method: 收集来自9个国家/地区和2个国际组织近十年发布的3,418份CPG文档，涵盖24个专科，提取32,155条临床推荐。为每条推荐生成多轮对话，评估8个主流LLMs的检测和依从能力。通过56名临床医生进行人工验证。

Result: 模型能正确检测71.1%-89.6%的推荐，但仅能正确引用3.6%-29.7%的对应标题，显示模型知道指南内容但不知其来源。依从率在21.8%-63.2%之间，表明知道指南和实际应用之间存在巨大差距。

Conclusion: CPGBench是首个系统揭示LLMs在对话中未能检测或遵循哪些临床推荐的基准。由于每条临床推荐可能影响大量人群且临床应用本质上是安全关键的，解决这些差距对于LLMs在真实世界临床实践中的安全负责任部署至关重要。
...
### [147] [Translation or Recitation? Calibrating Evaluation Scores for Machine Translation of Extremely Low-Resource Languages](https://arxiv.org/abs/2603.25222)
*Danlu Chen,Ka Sing He,Jiahe Tian,Chenghao Xiao,Zhaofeng Wu,Taylor Berg-Kirkpatrick,Freda Shi*

Main category: cs.CL

TL;DR: 论文提出FRED难度指标（Fertility Ratio、Retrieval Proxy、Pre-training Exposure、Corpus Diversity），用于解释极低资源机器翻译中性能差异的根本原因，发现训练-测试重叠和预训练暴露是主要因素，而非模型能力。


<details>
  <summary>Details</summary>
Motivation: 极低资源机器翻译领域存在性能报告的高度变异性，使得不同语言对的结果难以比较和解释。对于特定语言群体（如古代语言）的研究者来说，难以判断其他语境（如非洲或美洲原住民语言）报告的突破是源于方法优势还是基准收集的偶然性。需要建立数据集内在指标来标准化评估。

Method: 提出FRED难度指标：1) Fertility Ratio (F)：衡量tokenization覆盖率，反映词汇共享程度；2) Retrieval Proxy (R)：评估训练-测试重叠度；3) Pre-training Exposure (E)：量化预训练数据暴露程度；4) Corpus Diversity (D)：衡量语料库多样性。这些指标作为数据集内在特征，用于解释性能差异。

Result: FRED指标揭示：1) 性能变异性主要由训练-测试重叠和预训练暴露解释，而非模型能力；2) 某些语言（特别是灭绝语言和非拉丁原住民语言）存在tokenization覆盖率差（高token fertility）问题，这限制了从高资源语言的模型迁移；3) 这些指标为跨语言迁移提供了更透明的评估框架。

Conclusion: FRED难度指标为极低资源机器翻译社区提供了标准化的评估基础，通过量化数据集内在特征来解释性能差异，使跨语言比较更加可靠。这有助于区分方法改进与数据集特性，推动该领域更透明和可复现的研究。
...
TL;DR: 比较自然数据和合成数据对大型语言模型训练与评估的影响，以法语和意大利语的被动动词交替为例，发现自然数据训练模型在两种测试集上表现稳健，而合成数据训练模型无法可靠泛化到自然句子


<details>
  <summary>Details</summary>
Motivation: 研究自然数据和合成数据在训练和评估大型语言模型时的相对价值，探究哪种数据能更好地帮助模型掌握抽象的语言模式

Method: 使用Blackbird Language Matrices结构化数据集，比较基于自然句子（从Universal Dependencies提取）和合成句子的结构化模板，以法语和意大利语的被动动词交替为案例研究

Result: 在合成数据集上训练和测试的模型能达到上限性能，但无法可靠泛化到自然句子；而在自然数据上训练的模型在自然和合成测试集上都表现出稳健性能，能更好地捕捉抽象语言模式

Conclusion: 自然数据和结构化评估设置对于探测大型语言模型的句法和语义知识具有重要价值，自然数据训练能产生更稳健的泛化能力
...
### [149] [MolQuest: A Benchmark for Agentic Evaluation of Abductive Reasoning in Chemical Structure Elucidation](https://arxiv.org/abs/2603.25253)
*Taolin Han,Shuang Wu,Jinghang Wang,Yuhao Zhou,Renquan Lv,Bing Zhao,Wei Hu*

Main category: cs.CL

TL;DR: MolQuest是一个基于真实化学实验数据的分子结构解析评估框架，将结构解析定义为多轮交互任务，评估LLMs在复杂化学空间中的溯因推理和战略决策能力。


<details>
  <summary>Details</summary>
Motivation: 当前科学评估基准主要依赖静态、单轮问答格式，无法衡量LLMs在需要多步迭代和实验交互的复杂科学任务中的表现。需要系统评估LLMs在真实科学研究中的动态推理能力。

Method: 引入MolQuest框架，基于真实化学实验数据，将分子结构解析形式化为多轮交互任务。要求模型主动规划实验步骤、整合异质谱源（如NMR、MS），并迭代优化结构假设。系统评估LLMs在复杂化学空间中的溯因推理和战略决策能力。

Result: 当代前沿模型在真实科学场景中表现出显著局限性：即使是SOTA模型的准确率也仅约50%，而大多数其他模型的性能低于30%阈值。
...
<details>
  <summary>Details</summary>
Motivation: 当前需要评估大语言模型在多智能体协调和语用通信方面的能力，特别是在严格部分信息设置下，多个智能体必须通过自然语言协作解决复杂任务。

Method: 将问题形式化为多发送者语用推理任务，提供诊断框架将失败分解为空间基础、信念建模和语用通信错误，包括对前沿和开源模型行为失败模式的分类。

Result: 在包括8个开源模型和7个前沿推理模型的多样化模型集合中，更强的推理能力并不能可靠地转化为更好的协调能力：较小的开源模型经常匹配或超越前沿系统，改进的个体通信也不能保证成功的协作。

Conclusion: 多智能体协调仍然是当前语言模型尚未根本解决的挑战，需要专门的方法来评估和改进模型在这方面的能力。
...
TL;DR: 该论文提出了WSF-ARG+数据集，这是首个结合仇恨言论与核查价值信息的数据集，并引入了一种新颖的LLM-in-the-loop框架来标注需要核查的主张，提高了仇恨言论检测性能。


<details>
  <summary>Details</summary>
Motivation: 在线仇恨内容常使用事实性信息表达，协调的在线骚扰活动和极端主义宣传尤其如此。单独处理仇恨言论和错误信息会加深偏见、强化有害刻板印象、给旁观者带来心理困扰，并污染公共辩论。此外，内容审核者需要同时评估有害性和真实性，增加了工作负担。

Method: 1. 发布WSF-ARG+数据集，首次结合仇恨言论与核查价值信息；2. 引入新颖的LLM-in-the-loop框架，使用12个不同规模和架构的开源权重LLM进行测试；3. 通过广泛的人工评估验证框架效果；4. 分析包含核查价值主张的仇恨言论特征，并评估结合核查价值标签对LLM仇恨言论检测的改进。

Result: 1. LLM-in-the-loop框架在不影响数据标注质量的前提下减少了人工工作量；2. 包含核查价值主张的仇恨言论显示出显著更高的骚扰和仇恨程度；3. 结合核查价值标签将LLM仇恨言论检测的macro-F1提高了0.213（最佳情况）和平均0.154（大型模型）。

Conclusion: 该研究强调了联合处理仇恨言论和错误信息的重要性，提出的WSF-ARG+数据集和LLM-in-the-loop框架为内容审核提供了有效工具，显著提高了仇恨言论检测性能，同时减少了人工标注负担。
...
TL;DR: 提出SGPE算法和WWHO架构，针对复杂Abugida文字（如僧伽罗语和天城体）改进BPE分词器，减少61.7%的token数量，确保音节完整性


<details>
  <summary>Details</summary>
Motivation: 标准BPE分词器在处理复杂Abugida文字（如僧伽罗语、印地语）时存在严重问题：将复杂的连字（多码点字素簇）拆分为无意义的子字符单元，迫使LLM在推理时学习基本正字法结构，降低推理效率并增加成本，对全球南方国家造成显著的"Token税"

Method: 提出三层WWHO架构和SGPE算法，将文字的语言规则与统计压缩过程分离，实现无缝多语言分词。在3000万句清洗数据集上训练WWHO，并在149.995万句测试集上评估，使用僧伽罗语和天城体（印地语/梵语）作为复杂Abugida文字案例

Result: 僧伽罗语：SGPE实现1.274的Token-Word比率，每个token包含4.83个字符，相比OpenAI o200k base减少61.7%的token。印地语：TWR为1.181（相比o200k减少27.0%）。混合文字数据集：总体TWR为1.240，相比o200k base、Llama 4 Scout和DeepSeek V3分别减少36.7%、39.6%和60.2%的token，有效将可用上下文窗口扩展至4.38倍

Conclusion: SGPE算法显著改善复杂Abugida文字的分词效率，通过确保"语言零断裂保证"（有效音节从不跨token拆分），大幅减少token数量并扩展上下文窗口，为全球南方语言提供更高效经济的LLM处理方案
...
TL;DR: 论文探讨了生成式AI工具进入教育领域后，写作可能被外包和自动化，从而丧失其认知价值的问题，强调写作过程本身对人类深度学习的重要性。


<details>
  <summary>Details</summary>
Motivation: 随着ChatGPT等生成式AI工具进入课堂、工作场所和日常思维，写作面临沦为形式化、外包化和自动化的风险，可能失去其认知价值。作者关注当人们让机器代写时，人类思维发展过程中会失去什么。

Method: 论文采用跨学科研究方法，结合认知心理学、教育理论和真实课堂实践，分析写作过程的认知价值，探讨AI文本检测的可能性，以及教育者如何通过更智能的教学法而非禁令来适应这一变化。

Result: 研究发现写作过程（混乱、缓慢、常令人沮丧）是人类深度学习发生的关键场所。论文探索了当前AI文本检测的可能性，提出教育者应通过改进教学法而非禁止AI来适应，并指出识别机器生成语言的能力可能成为21世纪的关键素养。

Conclusion: 在写作可以被伪造的世界里，学习却无法被伪造。写作不仅是输出，更是我们学习思考的方式。教育需要重新思考如何利用AI工具，同时保护写作作为认知发展核心过程的本质价值。
...
TL;DR: 论文提出了Adaptive Chunking框架，通过五个内在指标评估文档分块质量，并引入两种新的分块器，显著提升了RAG系统的下游性能。
...
Method: 提出了Adaptive Chunking框架，包含五个新颖的内在、基于文档的指标：引用完整性、块内内聚性、文档上下文连贯性、块完整性、大小合规性。引入了两种新的分块器：LLM-正则表达式分割器和分割-合并递归分割器，以及针对性的后处理技术。

Result: 在法律、技术和社会科学领域的多样化语料库上，基于指标的自适应方法显著提高了下游RAG性能。在不改变模型或提示的情况下，答案正确率从62-64%提升到72%，成功回答的问题数量增加了30%以上（从49个增加到65个）。

Conclusion: 由互补的内在指标指导的、文档感知的自适应分块方法，为构建更健壮的RAG系统提供了实用且有效的途径。
...
TL;DR: 论文提出使用现成LLM作为令牌压缩器/解压器，通过自表达自编码框架将长文本转换为紧凑的内部语言表示（Z-tokens），实现高达18倍的令牌压缩率。
...
Method: 设计自表达自编码学习框架，微调预训练LLM将长文本转换为离散、可变长度的潜在代码（Z-tokens），使用轻量级LoRA适配器头实现内容自适应压缩：语义密集段分配更多Z-tokens，冗余/可预测区域则被激进压缩。

Result: 在Wikipedia、CNN/DailyMail、HotpotQA和Qulac风格长查询数据集上实现高达18倍的令牌压缩率，同时保持重建保真度和下游任务性能。支持提示压缩和Z-token空间中的自回归生成。

Conclusion: 现成LLM可作为优秀的令牌压缩器/解压器，通过内容自适应压缩实现高效长文本表示，为令牌高效的长上下文推理提供了可行路径。
...
Method: 提出Translation-Augmented Policy Optimization (TAPO)框架，基于GRPO构建。采用英语作为枢纽语言的显式对齐策略，遵循理解-推理范式。使用步骤级相对优势机制将理解与推理解耦，允许整合翻译质量奖励而不引入优化冲突。

Result: TAPO在多语言数学推理和翻译任务中均优于基线方法，能够很好地泛化到未见过的语言和领域外任务，有效协同语言理解与推理能力，且兼容多种模型。

Conclusion: TAPO框架通过创新的强化学习方法和理解-推理解耦机制，成功提升了LLMs在多语言数学推理中的性能，为解决多语言能力差距问题提供了有效方案。
...
TL;DR: 研究探讨如何通过提示工程最大化LLM在社会科学文本分类中的性能，发现适度增加提示上下文能显著提升准确性，但过度增加反而可能降低效果，且不同模型、任务间存在显著异质性。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在社会科学文本分类中能显著降低成本且有时性能媲美现有计算方法，但当前测试显示性能差异很大，因此需要探索如何最大化LLM的性能表现。

Method: 通过系统性地改变提示工程的三个方面：标签描述、指导性提示和少样本示例，研究提示上下文对性能的影响。在两个不同示例上进行测试，分析不同模型、任务和批量大小下的表现。

Result: 适度增加提示上下文能带来最高的性能提升，但进一步增加上下文通常只产生边际改善，有时甚至降低准确性。测试显示模型、任务和批量大小之间存在显著的异质性。

Conclusion: 每个LLM编码任务都需要单独验证，不能依赖通用规则。提示工程需要精细调整，过度增加上下文可能适得其反，实际应用中应根据具体任务进行优化。
...
TL;DR: 研究发现传统LLM生成合成数据的方法不适用于罗曼什语，因为LLM会混淆其6种方言变体。提出按资源梯度方向进行数据增强的新方法，在最低资源变体上超越Gemini 3 Pro达23 BLEU，首次实现各罗曼什语变体的流利翻译。


<details>
  <summary>Details</summary>
Motivation: 现有低资源机器翻译方法依赖LLM从高资源语言生成合成数据，但这种方法对罗曼什语失效，因为LLM会混淆其6种不同的方言变体。需要探索更适合罗曼什语特点的数据增强方法。

Method: 提出数据增强方向应与源语言和目标语言之间的资源梯度对齐的方法，而不是传统LLM生成方式。通过实验验证这种基于资源梯度方向的数据增强策略的有效性。

Result: 新方法在最低资源罗曼什语变体上超越Gemini 3 Pro达23 BLEU分数。人工评估确认该方法首次生成了各罗曼什语变体的流利翻译。

Conclusion: 对于像罗曼什语这样具有多种方言变体的语言，数据增强方向应与资源梯度对齐，而不是依赖LLM生成合成数据。该方法成功解决了罗曼什语机器翻译的独特挑战。
...
TL;DR: 对12种假新闻检测方法在10个公开数据集上的综合评估，涵盖传统机器学习、深度学习、Transformer和跨域架构，通过统一标签和跨域实验评估泛化能力


<details>
  <summary>Details</summary>
Motivation: 尽管假新闻检测技术有所进步，但大语言模型和社交媒体的放大效应使虚假信息传播更加复杂，需要系统评估现有方法的泛化能力和实际应用效果

Method: 评估12种代表性假新闻检测方法，包括传统机器学习、深度学习、Transformer和跨域架构；在10个不同体裁、来源、主题和标注标准的公开数据集上进行实验；统一标签为"真实"和"虚假"；进行域内、多域和跨域实验模拟真实场景

Result: 微调模型在域内表现良好但泛化能力差；跨域架构能减少性能差距但需要大量数据；大语言模型通过零样本和少样本学习展现出潜力；结果应视为在英文纯文本协议下的鲁棒性评估

Conclusion: 假新闻检测面临泛化挑战，跨域架构和大语言模型是重要方向，但需注意数据集混杂因素和预训练暴露问题，未来研究应关注更稳健的检测方法
...
<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型在生成视觉叙事时的连贯性问题，探索模型叙事与人类叙事在连贯性方面的差异

Method: 使用Visual Writing Prompts语料库，通过一组捕捉叙事连贯性不同方面的指标（包括指代、话语关系类型、主题连续性、角色持久性和多模态角色基础）计算叙事连贯性分数

Result: 视觉语言模型展现出与人类系统性不同的连贯性特征，单个指标的差异可能细微，但联合考虑时差异更明显

Conclusion: 尽管模型叙事表面流畅度接近人类，但在组织视觉叙事的话语结构方面存在系统性差异
...
TL;DR: 通过分析arXiv论文，研究发现LLM使用导致词汇模式变化（如标题中"beyond"和"via"增加，摘要中"the"和"of"减少），当前分类器难以准确识别具体LLM模型，真实世界的LLM使用具有异质性和动态性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是分析大型语言模型（LLMs）对学术论文写作风格的影响，特别是词汇使用模式的变化，以及当前分类器在识别具体LLM生成文本方面的局限性。

Method: 采用直接且高度可解释的线性方法，分析arXiv论文中的词汇使用模式，考虑不同模型和提示词的差异，定量评估LLM使用对学术写作的影响。

Result: 发现LLM使用导致词汇模式显著变化：标题中"beyond"和"via"使用频率增加，摘要中"the"和"of"使用频率减少；当前分类器在多类分类任务中难以准确识别具体LLM模型；真实世界的LLM使用呈现异质性和动态性特征。

Conclusion: LLM使用正在改变学术写作的词汇模式，但由于不同LLM之间的相似性和变化性，当前检测方法面临挑战，需要更精细的分析方法来理解和监测LLM对学术交流的影响。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在自动评分系统中的广泛应用，需要重新关注"幻觉"问题以及这些LLM评分方法对构念无关因素的鲁棒性。尽管现有自动评分系统通常能达到与人类评分员相当或更好的性能，但它们经常被证明容易受到构念无关因素和对抗性条件的影响。

Method: 本研究调查了构念无关因素对基于双架构LLM评分系统的影响。该系统设计用于评分情境判断测试中的短文式开放回答项目。研究考察了多种构念无关因素，包括：无意义文本填充、拼写错误、写作复杂度、文本重复以及离题回答。

Result: 评分系统对无意义文本填充、拼写错误和写作复杂度具有较好的鲁棒性。然而，重复大段文本会导致系统预测的分数降低，这与先前非LLM评分系统的研究结果相矛盾。离题回答则被评分系统严重惩罚。

Conclusion: 这些结果为未来基于LLM的评分系统的鲁棒性提供了令人鼓舞的支持，前提是在设计时充分考虑构念相关性。研究强调了在设计LLM评分系统时关注构念相关性的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力的提升，仅依靠人工监督进行改进的成本越来越高且可扩展性有限。当模型在某些领域接近人类水平时，人类反馈可能无法提供足够的信息信号来推动进一步改进。同时，模型自主决策和执行复杂行动的能力不断增强，使得模型开发过程的组件可以逐步自动化。这些挑战和机遇共同推动了自改进研究的兴趣增长。

Method: 提出了一个系统级的自改进语言模型统一框架，将自改进系统概念化为一个闭环生命周期，包含四个紧密耦合的过程：数据获取、数据选择、模型优化和推理精炼，以及一个自主评估层。模型在每个阶段都扮演核心角色：收集或生成数据、选择信息信号、更新参数和精炼输出，而自主评估层持续监控进展并指导跨阶段的改进循环。

Result: 论文系统性地回顾和分析了每个组件的代表性方法，从技术角度对现有自改进技术进行了组织和分类。通过提出的框架，为理解自改进语言模型的研究现状提供了系统化的视角。

Conclusion: 论文讨论了当前自改进方法的局限性，并展望了未来实现完全自改进语言模型的研究方向。提出的框架为系统化研究和开发自改进语言模型提供了理论基础和组织结构。
...
TL;DR: S2D2是一个无需训练的自推测解码框架，用于块扩散语言模型，通过将同一预训练模型同时用作草稿器和验证器，在块扩散解码中插入推测验证步骤，显著提升生成速度与准确性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 块扩散语言模型通过结合块级自回归解码和块内并行去噪，为实现比自回归更快的生成提供了有前景的路径。然而，在实际加速所需的少步数机制下，标准置信度阈值解码往往很脆弱：激进的阈值会损害质量，而保守的阈值则需要不必要的去噪步骤。现有方法要么需要额外训练，要么增加测试时计算成本。

Method: S2D2是一个无需训练的自推测解码框架，其核心观察是：当块大小减小到1时，块扩散模型会变成自回归模型，这使得同一预训练模型可以同时充当草稿器和验证器。S2D2在标准块扩散解码中插入推测验证步骤，并使用轻量级路由策略来决定何时验证值得其成本。这产生了一种混合解码轨迹，其中扩散并行提出标记，而自回归模式充当局部序列级批评器。

Result: 在三个主流块扩散模型家族中，S2D2始终优于强大的置信度阈值基线，改善了准确性与速度之间的权衡。在SDAR上，观察到相对于自回归解码最高4.7倍的加速，相对于调优的动态解码基线最高1.57倍的加速，同时准确性最高提升4.5个百分点。在LLaDA2.1-Mini上，S2D2与内置自校正保持互补，包括一个保守设置，其中比静态基线快4.4倍且准确性略高。

Conclusion: S2D2为块扩散语言模型提供了一个无需训练的自推测解码框架，通过利用同一模型在不同块大小下的双重角色（扩散和自回归），实现了显著的速度提升而不牺牲准确性，为快速文本生成提供了有效的解决方案。
...
TL;DR: 提出自然语言智能体约束系统(NLAH)和智能约束运行时(IHR)，将智能体约束逻辑从控制器代码中解耦为可移植的自然语言可执行规范


<details>
  <summary>Details</summary>
Motivation: 当前智能体性能越来越依赖约束工程，但约束设计通常深埋在控制器代码和运行时特定约定中，难以转移、比较和作为科学对象研究

Method: 引入自然语言智能体约束系统(NLAH)将约束行为表达为可编辑的自然语言规范，开发智能约束运行时(IHR)通过显式契约、持久化工件和轻量级适配器执行这些约束

Result: 在编码和计算机使用基准测试中，进行了操作性可行性、模块消融和代码到文本约束迁移的受控评估

Conclusion: 智能体约束逻辑可以外部化为可移植的可执行工件，自然语言约束系统和智能运行时为约束工程提供了标准化、可转移的研究框架
...
<details>
  <summary>Details</summary>
Motivation: 研究智能体智能（agentic intelligence），评估智能体在全新任务上的流畅自适应效率，避免依赖语言和外部知识，专注于核心知识先验。

Method: 设计基于核心知识先验的抽象回合制环境，通过大量人类测试者进行难度校准，建立基于人类行动基准的效率评分框架，包含环境构建、验证和校准的方法论。

Result: 人类测试者能解决100%的环境任务，而截至2026年3月，前沿AI系统的得分低于1%，显示出人类与当前AI系统在智能体智能方面的显著差距。

Conclusion: ARC-AGI-3提供了一个有效评估智能体智能的基准测试，揭示了当前AI系统在无需明确指令的探索、推断和规划任务上的局限性，为未来AGI研究提供了重要参考。
...
### [167] [When Is Collective Intelligence a Lottery? Multi-Agent Scaling Laws for Memetic Drift in LLMs](https://arxiv.org/abs/2603.24676)
*Hidenori Tanaka*

Main category: cs.AI

TL;DR: 论文提出Quantized Simplex Gossip(QSG)模型，揭示多智能体系统中通过相互上下文学习达成共识的微观机制，称为"模因漂移"，并推导了漂移诱导极化的标度律。


<details>
  <summary>Details</summary>
Motivation: 尽管基于大语言模型的多智能体系统越来越多地应用于重要决策场景，但尚不清楚其结果是源于集体推理、系统偏见还是随机性。近期命名游戏研究表明，即使个体智能体没有先验偏好，群体也能迅速打破对称性达成共识，需要揭示其微观机制。

Method: 提出最小模型Quantized Simplex Gossip(QSG)，其中智能体维护内部信念状态但通过采样输出相互学习。通过理论分析推导漂移主导和选择主导两种机制，建立标度律描述漂移诱导极化与群体规模、通信带宽、上下文适应率和智能体内部不确定性的关系，并在QSG模拟和LLM命名游戏实验中验证。

Result: QSG模型揭示了共识形成的微观机制：一个智能体的任意选择成为下一个智能体的证据，通过相互上下文学习复合形成共识。模型预测了从漂移主导(共识如彩票)到选择主导(弱偏见被放大)的交叉转变。推导的标度律在模拟和实验中均得到验证。

Conclusion: 该研究为研究多智能体系统中社会表征形成的集体机制提供了理论框架，揭示了通过模因漂移和相互上下文学习达成共识的微观过程，有助于理解LLM多智能体系统的集体行为动态。
...
TL;DR: 该研究使用演化博弈论建模AI安全治理，将用户信任视为动态监控过程而非一次性选择，发现三种长期演化机制：不安全AI不被采用、不安全AI被广泛采用、安全AI被广泛采用，只有最后一种是理想状态。


<details>
  <summary>Details</summary>
Motivation: 现有AI治理模型通常将用户信任视为一次性采用决策，而非由重复互动塑造的动态演化过程。随着AI能力和采用率增长，AI安全日益紧迫，需要更精细的信任演化模型来理解用户与开发者之间的互动如何影响AI安全结果。

Method: 使用演化博弈论建模用户与AI开发者之间的重复非对称互动，将信任定义为减少监控。采用三种互补方法：无限种群复制者动态分析、随机有限种群动态模拟、以及强化学习（Q-learning）仿真。研究在不同监控成本和制度机制下，用户信任策略与开发者选择安全（合规）或不安全（不合规）AI的协同演化。

Result: 研究发现三种稳健的长期演化机制：1) 不安全AI不被采用；2) 不安全AI被广泛采用；3) 安全AI被广泛采用。只有第三种是理想状态，它出现在不安全行为的惩罚超过安全额外成本，且用户仍能负担偶尔监控的情况下。结果表明，仅靠监管或盲目用户信任都不足以防止向不安全或低采用结果的演化漂移。

Conclusion: 研究结果正式支持强调透明度、低成本监控和有效制裁的治理提案。成功AI安全治理需要结合：对不安全行为的有意义惩罚、用户负担得起的监控机制、以及避免盲目信任。仅靠监管或用户信任都不足以确保安全AI的广泛采用，必须建立支持用户监控和开发者合规的制度环境。
...
TL;DR: 提出用于加密货币推文预测性语句分类的两阶段框架，结合人工与GPT标注、情感特征提取和数据增强，在五类加密货币上评估多种模型性能


<details>
  <summary>Details</summary>
Motivation: 加密货币在社交媒体上的普及引发了大量投机活动，需要系统识别推文中的预测性陈述以理解市场情绪和预测行为

Method: 两阶段分类框架：任务1为预测/非预测二元分类，任务2将预测性推文细分为增量、减量、中性三类；结合人工与GPT标注，使用SenticNet提取情感特征，采用GPT生成复述进行数据增强以解决类别不平衡

Result: GPT平衡显著提升模型性能，Transformer模型在任务1获得最高F1分数，传统机器学习模型在任务2表现最佳；情感分析显示不同加密货币的预测类别具有独特的情感模式

Conclusion: 提出的分类框架能有效识别加密货币推文中的预测性陈述，GPT增强方法改善不平衡数据性能，情感特征为理解加密货币市场情绪提供了新视角
...
<details>
  <summary>Details</summary>
Motivation: 在结合轻量模型和昂贵大模型的LLM系统中，路由是平衡性能与成本的有效策略。现有基于隐藏状态的探针路由在纯文本LLM中表现良好，但在多模态LLM中性能显著下降。研究发现视觉输入削弱了隐藏状态中正确性信号的可分离性，使得标准探针设计难以提取这些信号。
...
Conclusion: 提高隐藏状态质量是多模态大语言模型中实现有效路由的关键。提出的注意力探针和KL正则化LoRA探针能够有效解决视觉输入导致的正确性信号可分离性问题，为MLLM系统中的路由策略提供了新的解决方案。
...
TL;DR: LogitScope：一个轻量级框架，通过从概率分布计算标记级信息度量来分析LLM不确定性，无需标注数据或语义解释


<details>
  <summary>Details</summary>
Motivation: 传统评估方法在生成过程中对模型在单个标记位置的置信度提供有限洞察，需要更好的不确定性量化方法

Method: 引入LogitScope框架，通过在每个生成步骤计算熵和变熵等信息度量来分析LLM不确定性，采用惰性评估提高计算效率

Result: LogitScope能够揭示模型置信度模式、识别潜在幻觉、暴露高不确定性决策点，适用于不确定性量化、模型行为分析和生产监控
...
### [172] [Resisting Humanization: Ethical Front-End Design Choices in AI for Sensitive Contexts](https://arxiv.org/abs/2603.24853)
*Silvia Rossi,Diletta Huyskes,Mackenzie Jorgensen*

Main category: cs.AI

TL;DR: 论文探讨AI前端设计（特别是对话式用户界面）中人性化设计的伦理意义，指出这是价值驱动的选择，影响用户心理模型、信任校准和行为反应，可能误导用户期望、培养错误信任并削弱用户自主权。


<details>
  <summary>Details</summary>
Motivation: 当前AI伦理讨论主要关注后端问题（数据治理、模型训练、算法决策），而前端设计选择（交互方式、表现形式）的伦理意义被忽视。对话式用户界面中的人性化设计元素（对话交互、情感语言、个性模式、拟人隐喻）日益普遍，需要伦理审视。

Method: 结合人机交互、对话式AI和价值敏感设计研究，分析界面如何影响用户期望、信任和自主权。通过非营利组织Chayn（支持性别暴力幸存者）的两个AI系统案例研究，展示创伤知情设计原则如何指导伦理约束。

Result: 人性化AI前端设计是价值驱动的选择，深刻影响用户心理模型、信任校准和行为反应。界面可能在脆弱情境中误导用户期望、培养错误信任并削弱自主权。Chayn案例表明伦理考虑可驱动设计中的原则性约束，挑战当前AI产品以参与度为导向的规范。

Conclusion: 伦理前端AI设计是一种程序性伦理，通过交互选择而非仅系统逻辑来体现。需要更多关注前端设计的伦理影响，特别是在脆弱用户群体中，设计选择应体现价值敏感性和伦理约束。
...
### [173] [How Far Are Vision-Language Models from Constructing the Real World? A Benchmark for Physical Generative Reasoning](https://arxiv.org/abs/2603.24866)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型评估过于偏向感知真实性（视觉合理性），而忽略了模型是否真正理解构建物理世界所需的分步过程和物理依赖关系。这种能力对于自动化设计到建造流程至关重要，但现有基准很少测试模型是否掌握实际构建这些构件所需的逐步过程和物理依赖关系。

Method: 基于住宅木结构建筑领域（具有完全编码的工程标准和客观可验证的正确性）创建基准，收集了13种建筑风格的26,000多个结构，均验证至施工图标准（LOD 350）。开发了包含10项测试的确定性结构验证框架，支持迭代式智能体交互：模型观察中间建造状态，生成建造动作，并接收结构化环境反馈。

Result: 对最先进的视觉语言模型进行广泛实验，揭示了现有排行榜上基本不可见的显著能力差距。这些发现确立了物理有效性作为与视觉真实性正交的关键评估维度。

Conclusion: 物理生成推理是多模态智能中一个独特且未充分发展的前沿领域，DreamHouse基准突出了物理有效性作为与视觉真实性正交的关键评估轴的重要性。
...
### [174] [On the Foundations of Trustworthy Artificial Intelligence](https://arxiv.org/abs/2603.24904)
*TJ Dunham*

Main category: cs.AI

TL;DR: 论文证明平台确定性推理是可信AI的必要且充分条件，提出确定性论题和信任熵量化非确定性成本，构建纯整数推理引擎实现跨架构比特级相同输出，验证了AI信任本质上是算术问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统缺乏平台确定性，特别是IEEE 754浮点运算在不同硬件架构上产生不一致结果，这破坏了AI系统的可信属性（公平性、鲁棒性、隐私、安全性、对齐性），因为所有这些属性都预设了平台确定性。

Method: 提出确定性论题，引入信任熵量化非确定性成本；证明确定性-验证坍缩定理；构建纯整数推理引擎替代浮点运算，实现跨ARM和x86架构的比特级相同输出；通过82次跨架构测试（模型达67亿参数）和356次链上认证交易验证。

Result: 在82次跨架构测试中观察到零哈希不匹配；四个地理分布式节点产生相同输出；验证失败概率精确等于1 - 2^{-H_T}；证明平台确定性是AI系统所有主要可信属性的前提；99,000行Rust代码部署在三大洲。

Conclusion: 平台确定性推理是可信AI的必要且充分条件，AI信任本质上是一个算术问题；通过纯整数推理引擎可以解决浮点运算的非确定性问题，实现跨平台的可验证确定性。
...
### [175] [FinMCP-Bench: Benchmarking LLM Agents for Real-World Financial Tool Use under the Model Context Protocol](https://arxiv.org/abs/2603.24943)
*Jie Zhu,Yimin Tian,Boyang Li,Kehao Wu,Zhongzhi Liang,Junhui Li,Xianyin Zhang,Lifan Guo,Feng Chen,Yong Liu,Chi Zhang*

Main category: cs.AI

TL;DR: FinMCP-Bench是一个用于评估大语言模型通过金融模型上下文协议工具调用解决现实金融问题能力的新型基准测试


<details>
  <summary>Details</summary>
Motivation: 需要建立一个标准化、实用且具有挑战性的测试平台来评估大语言模型在解决现实金融问题方面的工具调用能力，以推动金融LLM智能体的研究进展

Method: 构建包含613个样本的基准测试，涵盖10个主要场景和33个子场景，包含真实和合成的用户查询；整合65个真实金融MCPs，设计单工具、多工具和多轮对话三种样本类型；提出明确的工具调用准确性和推理能力评估指标

Result: 创建了FinMCP-Bench基准测试，包含丰富的金融场景和工具调用任务；系统评估了主流大语言模型在金融工具调用方面的表现；提供了标准化的评估框架和指标

Conclusion: FinMCP-Bench为金融大语言模型智能体的研究提供了一个标准化、实用且具有挑战性的测试平台，能够全面评估模型在解决现实金融问题时的工具调用和推理能力
...
TL;DR: 该研究首次大规模分析了电商平台嵌入式购物AI的用户采用与使用模式，基于携程3100万用户数据发现：采用者主要为年长、女性、高活跃用户；AI聊天与传统搜索处于相同购买阶段但更早出现，用户常交替使用两者；AI主要用于探索性、难以关键词化的任务，作为传统搜索的补充而非替代。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型（LLM）在电商平台中的集成应用，缺乏关于消费者如何采用和使用这类平台嵌入式购物AI的大规模实证证据。现有研究主要关注通用AI工具的用户画像，而平台嵌入式AI助手的使用模式可能有所不同，需要实证研究来理解其实际采用特征和使用行为。

Method: 研究使用中国最大在线旅游平台携程（Ctrip）的3100万用户数据，分析其LLM驱动的AI助手"问道"（Wendao）。通过大规模用户行为数据分析，采用描述性统计方法，研究AI助手的采用模式、使用时机以及与搜索行为的交互关系。

Result: 发现三个核心规律：1）采用者主要为年长消费者、女性用户和高活跃度现有用户，与通用AI工具的年轻男性主导模式相反；2）AI聊天与传统搜索处于相同购买阶段但更早出现，用户常交替使用两种模式；3）消费者主要使用AI进行探索性、难以关键词化的任务，景点查询占聊天请求的42%，聊天意图随使用时机和后续购买品类系统性变化。

Conclusion: 平台嵌入式购物AI并非传统搜索的替代品，而是作为电商中探索性产品发现的补充界面。AI助手特别适用于难以通过关键词搜索的探索性任务，与搜索功能形成互补关系，共同支持消费者的购买决策过程。
...
TL;DR: ScratchMath是一个专门用于解释和分类手写数学草稿中错误的基准数据集，包含1720个中国中小学生数学样本，支持错误原因解释和分类两个任务。
...
Method: 引入ScratchMath基准数据集，包含1720个中国中小学生数学样本，支持错误原因解释(ECE)和错误原因分类(ECC)两个任务，定义了七种错误类型。采用严格的人机协作方法进行多阶段专家标注、审查和验证。

Result: 系统评估了16个领先的MLLMs，发现与人类专家相比存在显著性能差距，特别是在视觉识别和逻辑推理方面。专有模型明显优于开源模型，大型推理模型在错误解释方面表现出强大潜力。

Conclusion: ScratchMath填补了手写数学草稿错误诊断的空白，为教育评估提供了重要基准。所有评估数据和框架已公开，以促进进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM不确定性评估方法存在局限性：要么提供单一不确定性分数，要么依赖传统的偶然-认知二分法，这些方法无法为改进生成模型提供可操作的见解。现有研究表明这些方法不足以理解LLMs中的不确定性，需要更细粒度的分解框架。

Method: 提出了一个不确定性分解框架，将LLM不确定性分解为三个语义组件：1) 输入歧义（由模糊提示引起）；2) 知识缺口（由参数证据不足导致）；3) 解码随机性（源于随机采样）。通过一系列实验验证该框架的有效性。

Result: 实验表明这三个不确定性组件在不同模型规模和任务中的主导地位会发生变化。该框架为审计LLM可靠性和检测幻觉提供了更好的理解，能够实现更有针对性的干预。

Conclusion: 提出的三组件不确定性分解框架比传统方法提供了更细粒度和可操作的见解，有助于开发更值得信赖的LLM系统，为针对性干预铺平了道路。
...
TL;DR: 该论文提出了多视角故障归因范式，以解决多智能体系统中故障归因的模糊性问题，并引入了MP-Bench基准和相应评估协议。
...
Result: 实验表明，先前关于大语言模型在故障归因方面存在困难的结论很大程度上源于现有基准设计的局限性；多视角基准和评估协议对于实现现实可靠的多智能体系统调试至关重要。

Conclusion: 多视角故障归因是更实用的范式，MP-Bench基准和相关评估协议为多智能体系统的现实调试提供了必要支持，揭示了现有基准设计的不足并提出了改进方向。
...
TL;DR: 论文提出了一种理论框架，通过将高层能力与内部稳定性约束耦合，降低知识蒸馏的价值，从而减少能力转移与治理结构之间的不对称性。


<details>
  <summary>Details</summary>
Motivation: 前沿AI中的知识蒸馏、模型提取和行为转移已成为核心关注点。主要风险不仅在于复制，而在于有用能力可能比原始伴随的治理结构更廉价地被转移。论文旨在减少这种不对称性。

Method: 提出了一个约束耦合推理框架，包含四个要素：有界转换负担、路径负载积累、动态演化可行区域，以及能力-稳定性耦合条件。该框架在架构层面工作，省略了专有实现细节。

Result: 论文贡献是理论性的而非操作性的，提供了一个可证伪的架构论点、清晰的威胁模型，以及一组可实验验证的假设，用于未来蒸馏抵抗、对齐和模型治理的研究。

Conclusion: 通过将高层能力与内部稳定性约束耦合，可以降低蒸馏作为捷径的价值，从而减少能力转移与治理结构之间的不对称性。论文为蒸馏抵抗、对齐和模型治理提供了理论基础。
...
TL;DR: 提出SAKE方法解决自回归神经PDE模拟器上下文窗口选择问题，通过系统锚点识别候选集和膝关节感知选择，在PDEBench上实现低成本高精度选择
...
Method: 提出系统锚定膝关节估计(SAKE)方法：第一阶段从物理可解释的系统锚点识别小型结构化候选集，第二阶段在候选集内进行膝关节感知下游选择

Result: 在PDEBench八个家族上评估，SAKE是最强的匹配预算低成本选择器：67.8%精确选择率，91.7%误差在1以内，6.1%平均遗憾@膝关节，成本比0.051（节省94.9%搜索成本）

Conclusion: SAKE将上下文窗口选择形式化为独立算法问题，通过系统锚点和膝关节感知实现高效选择，显著降低搜索成本同时保持高性能
...
Method: 提出LEKIA 2.0情境化LLM架构，将认知层与执行层分离，从而解耦情境建模与干预执行。该设计使系统能够在持续交互中维护用户情境和同意边界的稳定表示。同时引入了从静态到动态的在线评估协议来评估过程控制能力。

Result: LEKIA在深度干预循环完成方面相比仅使用提示的基线方法实现了约31%的平均绝对改进。结果表明，外部情境结构是构建稳定、可控、情境化情感支持系统的关键使能条件。

Conclusion: 面向过程的情感支持的关键挑战不是简单地生成自然语言，而是为模型构建可持续更新的外部情境结构。LEKIA 2.0通过分离认知与执行层，实现了情境建模与干预执行的解耦，为构建稳定、可控、情境化的情感支持系统提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 压缩视觉语言模型(VLMs)广泛用于减少内存和计算成本，但压缩会引发对内部计算和安全行为是否保留的担忧，需要研究不同压缩方法如何从根本上改变模型内部机制

Method: 使用因果电路分析和crosscoder-based特征比较来检查剪枝和量化如何改变代表性VLMs的内部机制，并引入VLMSafe-420基准测试，将有害输入与匹配的良性反事实配对

Result: 剪枝通常保持电路结构完整但旋转和衰减内部特征，量化在更高层次修改电路但使存活特征更好对齐，剪枝导致真实拒绝行为急剧下降

Conclusion: 压缩方法的选择具有安全影响，剪枝会显著降低模型的安全拒绝行为，而量化对特征对齐的影响较小，需要更细致地考虑压缩方法对模型安全性的影响
...
TL;DR: 提出MP-MoE框架，通过结合强度损失和结构感知的矩阵轮廓目标，改善热带地区降水预报，缓解双惩罚效应


<details>
  <summary>Details</summary>
Motivation: 热带地区（如越南）降水预报面临挑战，复杂地形和对流不稳定性限制了数值天气预报模型的准确性。现有数据驱动的后处理框架大多依赖逐点目标函数，在轻微时间错位下存在"双惩罚"效应

Method: 提出矩阵轮廓引导的专家混合框架，将传统强度损失与结构感知的矩阵轮廓目标相结合。利用子序列级相似性而非逐点误差，实现更可靠的专家选择，减轻相位偏移导致的过度惩罚

Result: 在越南两个主要流域的降雨数据集上评估，涵盖1小时强度和12、24、48小时累积降雨。MP-MoE在强降雨事件的平均临界成功指数上优于原始数值天气预报和基线学习方法，同时显著降低动态时间规整值

Conclusion: MP-MoE框架能有效捕捉峰值降雨强度并保持风暴事件的形态完整性，证明了结构感知损失函数在改善降水预报中的有效性
...
TL;DR: 稀疏自编码器特征在视觉语言模型中缺乏模块化组合性，特征联合干预会导致输出漂移和精度下降，而非模块化电路干扰限制了基于干预的导向方法有效性。


<details>
  <summary>Details</summary>
Motivation: 验证稀疏自编码器特征是否形成模块化、可组合的推理单元这一假设，该假设是许多基于干预的导向方法的基础，但在多模态模型中尚未明确。

Method: 开发可复现的因果管道，在Qwen3-VL-8B中定位和测试稀疏视觉思维电路。使用包含七种任务类型和三个难度级别的合成基准，通过线性探针识别任务类型信息的中层解码器位置，在该层训练SAE，通过明确规则构建任务选择性特征集，进行推理时间缩放和消融实验，同时量化精度和漂移。

Result: 模块化假设经常失败：干预任务选择性特征集只能适度提高推理精度，而干预两个特征集的联合则会可靠地诱导输出漂移（预测的大幅意外变化）并降低精度，即使在范数匹配的扰动下也是如此。这种非模块化电路干扰与共享内部通路一致，其中特征联合会放大激活偏移。

Conclusion: 研究结果阐明了SAE特征可组合性的边界，并为更可靠的VLM控制提供了严格的诊断框架。通过自助子样本和置换控制验证，并在多个VLM家族和五个不同数据集上复现。
...
### [186] [When Sensing Varies with Contexts: Context-as-Transform for Tactile Few-Shot Class-Incremental Learning](https://arxiv.org/abs/2603.25115)
*Yifeng Lin,Aiping Huang,Wenxi Liu,Si Wu,Tiesong Zhao,Zheng-Jun Zha*

Main category: cs.AI

TL;DR: CaT-FSCIL：一种针对触觉感知中少样本类增量学习的上下文处理方法，通过将采集上下文分解为低维结构化分量和高维残差分量，分别用可逆变换规范化和不确定性条件原型校准来解决上下文变化问题。


<details>
  <summary>Details</summary>
Motivation: 少样本类增量学习（FSCIL）在仅有少量标注样本的采集上下文中特别敏感。触觉感知场景中，设备多样性、接触状态和交互设置等采集上下文缺乏标准化，导致性能下降。需要解决上下文变化对模型性能的影响。

Method: 1. 将采集上下文分解为结构化低维分量和高维残差分量；2. 低维分量受触觉交互特征影响，建模为近似可逆的"上下文即变换"族，通过伪上下文一致性损失优化逆变换规范化；3. 高维残差主要来自平台和设备差异，采用不确定性条件原型校准（UCPC）基于上下文不确定性校准有偏原型和决策边界。

Result: 在标准基准HapTex和LMT108上的综合实验证明了CaT-FSCIL的优越性，表明该方法能有效处理触觉感知中的上下文变化问题。

Conclusion: CaT-FSCIL通过将采集上下文分解为可处理的组件，分别用逆变换规范和不确定性校准方法解决上下文变化问题，为触觉感知中的少样本类增量学习提供了有效解决方案。
...
### [187] [RubricEval: A Rubric-Level Meta-Evaluation Benchmark for LLM Judges in Instruction Following](https://arxiv.org/abs/2603.25133)
*Tianjun Pan,Xuan Lin,Wenyan Yang,Qianyu He,Shisong Chen,Licai Qi,Wanqing Xu,Hongwei Feng,Bo Xu,Yanghua Xiao*

Main category: cs.AI

TL;DR: RubricEval是首个针对指令跟随评估的rubric级别元评估基准，揭示了现有评估方法在细粒度判断准确性上的不足，GPT-4o在Hard子集上仅达到55.97%准确率。


<details>
  <summary>Details</summary>
Motivation: 尽管基于rubric的评估已成为评估大语言模型指令跟随能力的主流范式，但其可靠性尚不明确，需要元评估。现有元评估主要关注响应层面，未能评估rubric评估所依赖的细粒度判断准确性。

Method: 提出RubricEval基准，包含：(1)首个指令跟随的rubric级别元评估基准；(2)涵盖多类别和模型来源的多样化指令和响应；(3)3,486个质量控制实例，以及更好区分评估者性能的Easy/Hard子集。

Result: 实验显示rubric级别评估远未解决：广泛使用的GPT-4o在Hard子集上仅达到55.97%准确率。rubric级别评估优于checklist级别，显式推理提高准确性，两者结合减少评估者间方差。通过建立的rubric分类法识别了常见失败模式。

Conclusion: RubricEval填补了指令跟随评估中rubric级别元评估的空白，揭示了现有评估方法的局限性，为可靠评估提供了可操作的见解和失败模式分析。
...
### [188] [UniAI-GraphRAG: Synergizing Ontology-Guided Extraction, Multi-Dimensional Clustering, and Dual-Channel Fusion for Robust Multi-Hop Reasoning](https://arxiv.org/abs/2603.25152)
*Jie Wang,Honghua Huang,Xi Ge,Jianhui Su,Wen Liu,Shiguo Lian*

Main category: cs.AI

TL;DR: UniAI-GraphRAG是一个增强的图检索增强生成框架，通过本体引导知识提取、多维社区聚类和双通道图检索融合，在复杂推理和多跳查询任务上优于现有开源方案。
...
Conclusion: UniAI-GraphRAG通过结构化知识组织和增强检索机制，有效提升了RAG系统在复杂任务上的性能，为跨领域应用提供了更强大的解决方案。
...
### [189] [The Competence Shadow: Theory and Bounds of AI Assistance in Safety Engineering](https://arxiv.org/abs/2603.25197)
*Umair Siddique*

Main category: cs.AI

TL;DR: AI辅助在物理AI系统安全工程中可能产生"能力阴影"——系统性限制人类推理范围，导致安全分析质量下降远超简单叠加估计。论文提出五维能力框架，证明AI辅助是协作设计问题而非软件采购决策。


<details>
  <summary>Details</summary>
Motivation: 随着AI助手融入物理AI系统的安全工程工作流，需要评估AI辅助是否真正提升安全分析质量，还是引入系统性盲点。安全工程具有多维性、情境依赖性、固有不完备性和专家分歧等特征，难以通过基准测试评估。

Method: 1) 建立五维能力框架：领域知识、标准专业知识、操作经验、情境理解和判断力；2) 形式化"能力阴影"概念——AI生成的安全分析系统性限制人类考虑范围；3) 定义四种典型人机协作结构；4) 推导闭式性能边界，证明能力阴影的乘性复合效应。

Result: 能力阴影导致的分析质量退化远超简单加性估计。AI辅助在安全工程中的效果完全取决于使用方式，同一工具可能提升或降低分析质量。推导出抗阴影工作流的非退化条件。

Conclusion: AI辅助安全工程是协作设计问题而非软件采购决策。需要从工具认证转向工作流认证，建立抗阴影工作流以确保物理AI系统的可信性。关键在于设计能够保持人类多维能力的工作流程。
...
TL;DR: 开发了一个基于3D骨骼运动的步态基础模型，使用深度相机记录3,414名成年人的五种运动任务，模型嵌入优于工程特征，能预测多种表型指标，证明步态可作为独立的多系统生物信号。


<details>
  <summary>Details</summary>
Motivation: 步态日益被视为重要生命体征，但目前方法将其视为特定病理的症状而非系统性生物标志物。需要建立步态作为独立多系统生物信号的基础模型。

Method: 开发步态基础模型，使用深度相机记录3,414名深度表型化成年人在五种运动任务中的3D骨骼运动数据，学习步态嵌入表示。

Result: 学习到的嵌入优于工程特征，能预测年龄(r=0.69)、BMI(r=0.90)和内脏脂肪面积(r=0.82)。嵌入显著预测了3,210个表型目标中的1,980个；调整年龄、BMI、VAT和身高后，步态在男性所有18个身体系统和女性17/18个系统中提供独立增益，改善临床诊断和药物使用预测。解剖学消融显示腿部主导代谢和衰弱预测，躯干编码睡眠和生活方式表型。

Conclusion: 步态是独立的多系统生物信号，为将其转化为消费级视频应用并作为可扩展、被动的生命体征集成提供了理论基础。
...
TL;DR: 提出SliderQuant框架，通过自适应滑动量化解决LLM不同层对量化敏感度差异的问题，在低比特量化设置下显著优于现有PTQ方法。


<details>
  <summary>Details</summary>
Motivation: 现有PTQ方法通常对所有层采用统一的量化策略，但研究发现LLM的浅层和深层对量化更敏感，特别是首尾层量化误差显著更大，需要针对不同层设计差异化的量化方案。

Method: 提出SliderQuant框架，包含两个核心组件：1) 层间滑动量化，针对浅层、中间层和深层设计三种滑动窗口；2) 层内滑动量化，采用增量策略量化每个窗口。通过少量可学习参数实现自适应滑动量化。

Result: 在基础语言生成、零样本常识推理、数学和代码任务上，对Llama/Llama2/Llama3/Qwen2.5、DeepSeek-R1蒸馏模型和大MoE模型进行测试，SliderQuant在仅权重量化和权重-激活量化上都优于现有PTQ方法（包括使用旋转变换的最新方法）。
...
TL;DR: DAGverse框架从科学论文中构建文档接地的语义有向无环图，通过半自动管道生成高质量数据集，在因果DAG案例研究中优于现有视觉语言模型。
...
Method: 利用包含显式DAG图的科学论文作为监督源，开发DAGverse框架，其核心DAGverse-Pipeline通过图分类、图重建、语义接地和验证四个步骤半自动生成高精度语义DAG。
...
TL;DR: 该论文提出了一个评估AI有害操纵的框架，通过在三个领域（公共政策、金融、健康）和三个地区（美国、英国、印度）对10,101名参与者进行人机交互研究，发现AI模型能够产生操纵行为并改变参与者的信念和行为，且操纵效果因领域和地区而异。
...
Method: 提出了一个基于情境特定的人机交互研究框架，在三个AI应用领域（公共政策、金融、健康）和三个地理区域（美国、英国、印度）对10,101名参与者进行实验，评估AI模型的操纵倾向和效能。

Result: 研究发现：1）测试的AI模型在被提示时能够产生操纵行为，并在实验环境中成功改变参与者的信念和行为；2）情境很重要，不同领域的AI操纵效果不同；3）地理区域间存在显著差异，一个地区的发现不能推广到其他地区；4）AI模型的操纵倾向与操纵效能之间没有一致的相关性。

Conclusion: 需要在高风险情境中评估AI操纵，不同领域和地区的评估结果不可泛化，操纵倾向和效能应分开研究。作者公开了测试协议和相关材料，并讨论了评估AI有害操纵的开放挑战。
...
TL;DR: 该研究分析了基于深度强化学习（DRL）的自动驾驶车辆在混合交通中的宏观交通流特性和燃油效率，发现相比传统IDM模型，DRRL控制能提升约7.52%的道路容量，并在高速时提升28.98%的燃油效率。


<details>
  <summary>Details</summary>
Motivation: 混合交通中自动驾驶车辆控制面临平衡安全、效率、舒适性、燃油效率和交通规则遵守等多重挑战。传统跟驰模型（如IDM）难以泛化到多样化交通场景且通常不考虑燃油效率，这促使采用学习型方法。虽然深度强化学习在微观跟驰条件下表现出色，但其宏观交通流特性尚未充分探索。

Method: 采用Twin Delayed Deep Deterministic Policy Gradient（TD3）算法控制自动驾驶车辆，使用NGSIM高速公路数据集进行训练，实现与人类驾驶车辆的真实交互。通过基本图（FD）评估交通性能，考虑驾驶员异质性、异质时距渗透水平和不同RL控制车辆比例。同时从宏观层面比较RL模型与IDM的燃油效率。

Result: 交通性能对安全时距分布和RL车辆比例敏感。从完全人类驾驶过渡到完全RL控制可将道路容量提升约7.52%。与IDM相比，RL控制的自动驾驶车辆在高速（>50 km/h）时平均燃油效率提升约28.98%，低速（<50 km/h）时提升1.86%。
...
<details>
  <summary>Details</summary>
Motivation: 研究算术谜题游戏中的难度结构决定因素，为自适应学习系统提供理论基础。算术谜题提供了研究数学推理任务难度的受控环境，这是自适应学习系统的核心挑战。

Method: 形式化整数算术谜题问题，开发精确的动态规划求解器，枚举可达目标并提取最小操作见证。构建包含340万实例的数据集，基于最小操作数定义难度，分析难度与求解器导出特征的关系。

Result: 基于袋和目标级别统计的基线机器学习模型能部分预测可解性，但无法可靠区分简单实例。难度完全由精确见证导出的一小组可解释结构属性决定，特别是最小构造中使用的输入值数量是难度的最小充分统计量。
...
TL;DR: 研究验证了基于5W3H的结构化意图表示框架PPS在多语言和多模型中的泛化能力，发现AI辅助扩展的5W3H提示与人工编写的效果相当，且能减少跨模型输出方差。


<details>
  <summary>Details</summary>
Motivation: 扩展先前仅针对中文的研究，验证结构化意图表示框架PPS是否能在不同语言和模型中泛化，并探索AI辅助扩展提示的可行性。

Method: 采用5W3H框架的PPS结构化意图表示，在三种语言（英语、日语、中文）和四种条件下（包括AI辅助扩展完整5W3H规范）进行实验，评估2160个模型输出，分析目标对齐和跨模型输出一致性。

Result: AI扩展的5W3H提示与人工编写的在目标对齐上无显著差异；结构化条件通常能减少或重塑跨模型输出方差；非结构化提示存在系统性双重膨胀偏差：人为高估综合分数和低估跨模型方差。

Conclusion: 结构化5W3H表示能提高跨语言和模型间的意图对齐和可访问性，特别是当AI辅助创作降低了非专家用户的使用门槛时。
...
TL;DR: 论文提出"推理安全"概念，构建九类不安全推理行为分类法，开发实时推理安全监控器，在450条推理链基准测试中达到84.88%步骤级定位准确率。


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全研究主要关注内容安全（有害、偏见、事实错误输出），而将推理链视为不透明的中间产物。作者认为推理过程本身的安全性是一个正交且同等关键的安全维度，需要确保模型的推理轨迹在逻辑一致性、计算效率和对抗鲁棒性方面安全。
...
Result: 1) 证实所有九类错误类型在实践中都存在，每种攻击都产生机制可解释的特征；2) 在450条推理链静态基准测试中，监控器达到84.88%步骤级定位准确率和85.37%错误类型分类准确率，显著优于幻觉检测器和过程奖励模型基线；3) 证明推理级监控既必要又实际可行。

Conclusion: 推理安全是大型推理模型安全部署的基础性问题，推理级监控是必要且实际可行的安全机制，为LLM安全研究开辟了新维度。
...
### [198] [Modernising Reinforcement Learning-Based Navigation for Embodied Semantic Scene Graph Generation](https://arxiv.org/abs/2603.25415)
*Roman Kueble,Marco Hueller,Mrunmai Phatak,Rainer Lienhart,Joerg Haehner*

Main category: cs.AI

TL;DR: 该论文提出了一种用于具身语义场景图生成的模块化导航组件，通过改进优化算法和动作表示来提升在有限动作预算下的探索效率和场景图完整性。


<details>
  <summary>Details</summary>
Motivation: 在有机计算中，语义世界模型对于目标驱动的自适应至关重要，但如何在有限动作预算内最大化模型质量和下游效用是核心挑战。语义场景图提供了结构化表示，但需要有效的探索策略来平衡信息增益与导航成本。

Method: 1) 用现代优化算法替换原有的策略优化方法；2) 重新设计离散动作表示，研究更紧凑和细粒度的动作集；3) 比较单头策略与分解的多头策略；4) 评估课程学习和基于深度的碰撞监督；5) 分析SSG完整性、执行安全性和导航行为。

Result: 1) 仅替换优化算法就在相同奖励塑造下将SSG完整性相对基线提升21%；2) 深度信息主要影响执行安全性（无碰撞运动），对完整性影响不大；3) 现代优化算法与细粒度分解动作表示结合实现了最强的完整性-效率权衡。

Conclusion: 通过现代优化算法和细粒度分解动作表示的结合，能够显著提升具身语义场景图生成的探索效率和场景完整性，为资源受限环境下的语义世界模型构建提供了有效解决方案。
...
TL;DR: 将模型重训练视为计算约束下的近似贝叶斯推断，提出"学习债务"概念，建立基于决策理论的重训练策略框架


<details>
  <summary>Details</summary>
Motivation: 传统上将模型重训练视为维护任务，但作者认为应将其理解为计算约束下的近似贝叶斯推断。连续更新的信念状态与冻结部署模型之间的差距形成"学习债务"，需要建立系统化的决策框架来优化重训练策略。

Method: 提出决策理论框架，将重训练决策建模为成本最小化问题，从损失函数推导出重训练阈值。建立基于证据的触发机制替代日历调度，使治理过程可审计。

Result: 开发出基于证据的重训练触发机制，能够替代传统的日历调度方法，使模型治理过程更加透明和可审计。

Conclusion: 模型重训练应被视为计算约束下的近似贝叶斯推断过程，通过决策理论框架可以建立优化的重训练策略，实现基于证据的触发机制和可审计的治理流程。
...
TL;DR: Voxtral TTS是一种多语言语音合成模型，仅需3秒参考音频即可生成自然语音，采用自回归语义令牌生成与流匹配声学令牌的混合架构，在人类评估中优于ElevenLabs Flash v2.5。
...
Method: 采用混合架构：结合自回归生成语义语音令牌和流匹配生成声学令牌；使用Voxtral Codec语音标记器，该标记器采用混合VQ-FSQ量化方案从头训练；支持多语言语音克隆。

Result: 在由母语者进行的人类评估中，Voxtral TTS在自然度和表达性方面优于ElevenLabs Flash v2.5，获得了68.4%的胜率；模型权重以CC BY-NC许可证发布。

Conclusion: Voxtral TTS是一个高效的多语言语音合成系统，仅需极短的参考音频即可生成高质量的语音，在语音克隆任务中表现出色，为语音合成领域提供了新的技术方案。
...
### [201] [Agent Factories for High Level Synthesis: How Far Can General-Purpose Coding Agents Go in Hardware Optimization?](https://arxiv.org/abs/2603.25719)
*Abhishek Bhandwaldar,Mihir Choudhury,Ruchir Puri,Akash Srivastava*

Main category: cs.AI

TL;DR: 多智能体两阶段流水线优化硬件设计，通过分解子核、ILP全局配置和专家智能体探索交叉优化，在12个HLS基准测试中实现平均8.27倍加速


<details>
  <summary>Details</summary>
Motivation: 探索通用编码智能体（无需硬件特定训练）能否从高级算法规范优化硬件设计，验证智能体规模化是否可作为HLS优化的实用有效维度

Method: 两阶段智能体工厂：阶段1将设计分解为子核，独立优化每个子核，使用ILP在面积约束下组装全局配置；阶段2在顶级ILP解上启动N个专家智能体，探索pragma重组、循环融合、内存重构等跨函数优化

Result: 在12个HLS-Eval和Rodinia-HLS内核上，智能体数量从1到10时获得平均8.27倍加速，更难基准测试表现更佳：streamcluster超过20倍，kmeans约10倍；智能体无需领域特定训练即可重新发现已知硬件优化模式

Conclusion: 智能体规模化是HLS优化的实用有效维度，全局优化能发现子核搜索遗漏的改进，最佳设计通常不来自顶级ILP候选方案
...
Method: 引入RC2强化学习框架，通过强制跨模态循环一致性：要求模型执行反向推理、切换模态，并通过前向推理可靠地重建答案，从而获得密集的无标签奖励信号

Result: 优化这种结构约束能减轻模态特定错误，将推理准确性提升高达7.6个百分点

Conclusion: 高级推理不仅来自数据扩展，还来自强制对世界形成结构一致的理解，跨模态不一致性提供了丰富的自然学习信号
...
### [203] [Quadratic Residue Codes over $\mathbb{Z}_{121}$](https://arxiv.org/abs/2603.24689)
*Tapas Chatterjee,Priya Jain*

Main category: cs.IT

TL;DR: 该论文构造了素数长度p满足特定模44同余条件的二次剩余码及其扩展码，讨论了它们的性质、Gray像和置换自同构群，并给出了两个具体参数的新码构造。


<details>
  <summary>Details</summary>
Motivation: 研究环Z_121上特定素数长度的二次剩余码的构造和性质，探索其扩展码的置换自同构群结构，为置换译码提供理论基础，并构造具有良好参数的新码。

Method: 通过生成幂等元定义环Z_121上满足p≡±1,±5,±7,±9,±19(mod 44)的素数长度二次剩余码，分析其扩展码性质，研究Gray像和置换自同构群（由移位、乘子和逆置换生成）。

Result: 成功构造了Z_121上特定素数长度的二次剩余码族，证明了扩展二次剩余码具有由移位、乘子和逆置换生成的大置换自同构群，使置换译码可行，并构造了参数为[55,5,33]和[77,7,44]的新码。

Conclusion: 该研究建立了环Z_121上二次剩余码的系统构造方法，揭示了其扩展码的丰富置换自同构结构，为置换译码提供了理论依据，并获得了具有良好参数的实际编码实例。
...
### [204] [Algebraic Expander Codes](https://arxiv.org/abs/2603.24788)
*Swastik Kopparty,Itzhak Tamo*

Main category: cs.IT

TL;DR: 代数扩展码：一种新的Tanner型编码结构，在局部约束为Reed-Solomon码时，即使局部码率r≤1/2也能保持全局码率有正下界，同时具有常数相对距离。


<details>
  <summary>Details</summary>
Motivation: 传统Tanner码的约束计数方法在局部码率r≤1/2时无法保证正全局码率，而低码率区域在需要代数局部约束（如Reed-Solomon局部性和Schur积性质）的应用中很重要。

Method: 通过在非交换子群轨道上评估结构化的多项式子空间来构造代码，该子群由平移和缩放生成。利用加性特征和估计证明稀疏陪集几何形成强谱扩展器，通过新的多项式度概念和多面体体积/维数计数进行码率分析。

Result: 构造了明确的代数扩展码族，对于任意固定的r∈(0,1)（包括r≤1/2），全局码率保持远离0，同时实现常数相对距离。

Conclusion: 代数扩展码解决了低码率区域Tanner码的局限性，为需要代数局部约束的应用提供了具有正全局码率和良好距离特性的编码方案。
...
### [205] [Information-Theoretic Limits of Node Localization under Hybrid Graph Positional Encodings](https://arxiv.org/abs/2603.25030)
*Zimo Yan,Zheng Xie,Chang Liu,Yiqin Lv,Runfan Duan*

Main category: cs.IT

TL;DR: 研究图学习中位置编码的可识别性，提出结合锚点距离剖面和量化低频谱特征的混合编码，建立信息论下界，实验验证编码能力受图结构影响


<details>
  <summary>Details</summary>
Motivation: 位置编码已成为图学习尤其是图Transformer的标准组件，用于区分结构相似的节点，但其基本可识别性尚未得到充分理解。需要研究在混合位置编码下的节点定位问题

Method: 提出混合位置编码方法，结合锚点距离剖面和量化低频谱特征。将节点定位问题建模为观测映射问题，建立信息论下界（不可能性区域），通过实验验证理论预测

Result: 理论分析确定了由锚点数量、谱维度和量化水平共同控制的不可能性区域。实验表明：在随机3-正则图上，经验交叉点与预测标度一致；在真实DDI图上，可识别性强烈依赖于图结构，DrugBank在测试编码下保持高度冗余，而Decagon派生图在足够丰富的谱信息下变得几乎单射

Conclusion: 位置编码不应仅仅被视为启发式架构组件，而应理解为图依赖的结构分辨率机制。编码能力受图结构特性影响，需要根据具体图结构设计合适的编码方案
...
### [206] [Variance Based Transmitter Localization in Vessel-Like Molecular Communication Channels](https://arxiv.org/abs/2603.25213)
*Dağhan Erdönmez,H. Birkan Yilmaz*

Main category: cs.IT

TL;DR: 提出一种利用分子信号时间方差估计发射器距离的闭式近似方法，无需发射时间信息，在毛细血管尺度下误差约1%


<details>
  <summary>Details</summary>
Motivation: 现有血管状分子通信通道中的发射器定位方法要么需要已知发射时间，要么需要多个紧密排列的接收器，这在现实场景中限制了其应用性

Method: 提出一种简单的闭式近似方法，利用接收分子信号的时间方差来估计发射器与接收器之间的距离，无需发射时间信息。该方法基于接收信号峰值附近的高斯近似，推导出明确的方差-距离关系

Result: 在物理相关的毛细血管尺度下的仿真结果表明，所提方法能够实现距离预测，误差在1%量级

Conclusion: 该方法为无需发射时间信息的发射器距离估计提供了一种有效的闭式近似解决方案，在医疗保健领域具有应用潜力
...
### [207] [List Estimation](https://arxiv.org/abs/2603.25280)
*Nikola Zlatanov,Amin Gohari,Farzad Shahrivari,Mikhail Rudakov*

Main category: cs.IT

TL;DR: 本文研究了k-列表估计问题，比较了集中式（单个观测产生k个候选估计）与分散式（k个智能体各自独立观测并输出MMSE估计）的性能差异，发现集中式方法在某些情况下能达到与分散式基准相当甚至更好的渐近性能。


<details>
  <summary>Details</summary>
Motivation: 传统估计方法通常从观测中输出单个点估计。本文研究k-列表估计，即从单个观测产生k个候选估计，以最接近真实向量的候选的期望平方距离作为性能度量。动机在于比较这种集中式方法与对称分散式MMSE基准的性能差异，其中k个智能体观测条件独立同分布的测量并各自输出MMSE估计。

Method: 在集中式方面，将最优k-列表估计等价于后验分布的固定速率k点向量量化，并在标准正则条件下推导了精确的高速率渐近展开，包含显式常数和衰减率k^{-2/d}。在分散式方面，基于单智能体MMSE误差的小球行为推导下界；特别地，当条件误差密度在原点有界时，基准失真不能快于k^{-2/d}衰减。进一步证明如果误差密度在原点为零，则分散式基准无法匹配集中式的k^{-2/d}指数，而集中式估计器仍保持该缩放。

Result: 高斯特例给出了显式公式，数值实验验证了预测的渐近行为。结果显示，在k的缩放中，单个观测结合k个精心选择的候选可以达到与基于MMSE的分散式基准（具有k个独立观测）渐近相当的性能，在某些机制下甚至严格优于分散式基准。

Conclusion: 本文建立了集中式k-列表估计与分散式MMSE基准之间的理论比较框架。关键发现是：单个观测配合k个候选估计在某些情况下能达到与k个独立观测的分散式系统相当的性能，这挑战了传统直觉，表明精心设计的候选列表可以有效地利用单个观测的信息。
...
### [208] [New bounds for codes over Gaussian integers based on the Mannheim distance](https://arxiv.org/abs/2603.25362)
*Minjia Shi,Xuan Wang,Junmin An,Jon-Lark Kim*

Main category: cs.IT

TL;DR: 研究高斯整数上具有Mannheim距离的线性码，建立了多个Mannheim度量下的经典界，推导了球体积公式，证明了若干紧界，讨论了自对偶码，并提出了解码算法。


<details>
  <summary>Details</summary>
Motivation: 研究高斯整数环上的线性码在Mannheim度量下的性质，探索与传统Hamming度量不同的纠错能力，为高斯整数编码理论建立理论基础。

Method: 1. 推导Mannheim球的体积显式公式；2. 建立球填充界和完美码参数约束；3. 证明多个紧界并构造达到这些界的码族；4. 研究自对偶码并利用Mannheim版本的Macwilliams恒等式获得最小距离上界；5. 设计高斯整数剩余环上的解码算法。

Result: 1. 获得了Mannheim球体积的显式公式；2. 建立了球填充界和双纠错完美码的参数约束；3. 证明了多个紧界并构造了达到这些界的码族；4. 获得了自对偶码最小Mannheim距离的上界；5. 提出了有效的解码算法，并展示了某些在Hamming度量下不可纠的错误在Mannheim度量下变得可纠。

Conclusion: Mannheim度量为高斯整数编码提供了新的理论框架，在某些情况下比Hamming度量具有更好的纠错性能，建立了完整的理论体系并提供了实用的解码方法。
...
### [209] [Investigating the Fundamental Limit: A Feasibility Study of Hybrid-Neural Archival](https://arxiv.org/abs/2603.25526)
*Marcus Armstrong,ZiWei Qiu,Huy Q. Vo,Arjun Mukherjee*

Main category: cs.IT

TL;DR: 该论文探索了基于大语言模型的归档系统可行性，提出了Hybrid-LLM架构，解决了硬件非确定性导致的"GPU蝴蝶效应"，量化了神经压缩率，发现LLM能捕获经典算法无法访问的语义冗余。


<details>
  <summary>Details</summary>
Motivation: 大语言模型理论上能够建模远超经典统计方法（如Lempel-Ziv）的信息密度，但将其用于无损压缩面临严重系统约束，包括硬件非确定性和计算成本过高。本研究旨在探索基于LLM的归档系统的可行性。

Method: 提出了Hybrid-LLM概念验证架构，用于研究基础模型在存储环境中的"熵容量"。通过新颖的对数量化协议解决了"GPU蝴蝶效应"问题，该效应指微观硬件非确定性阻碍数据恢复。实验测量了真实世界数据上的神经压缩率。

Result: 实验揭示了"检索式"密度（记忆文献上0.39 BPC）与"预测式"密度（未见新闻上0.75 BPC）之间的明显差异。虽然当前推理延迟（比Zstd慢约2600倍）限制了立即部署到超冷存储，但发现LLM成功捕获了经典算法无法访问的语义冗余。

Conclusion: LLM能够捕获经典算法无法访问的语义冗余，为未来语义文件系统研究建立了基准。尽管当前推理延迟限制了实际部署，但证明了基于LLM的压缩系统在理论上的可行性。
...
### [210] [Kakeya Conjecture and Conditional Kolmogorov Complexity](https://arxiv.org/abs/2603.25611)
*Nicholas G. Polson,Daniel Zantedeschi*

Main category: cs.IT

TL;DR: 该论文提出了一个基于正则可识别纤维化的算法复杂性信息论框架，研究纤维结构如何影响集合的信息压缩性，并建立了复杂性在纤维分解下的加性分裂定理。


<details>
  <summary>Details</summary>
Motivation: 研究集合在具有正则可识别纤维分解时的信息压缩性，特别是理解Kakeya猜想中自适应纤维选择如何破坏条件复杂性分裂，从而连接几何测度论、算法复杂性和信息论压缩。

Method: 使用算法维度和Lutz-Lutz点对集原理，将点态Kolmogorov复杂度转化为Hausdorff维度。在有效双Lipschitz、可识别且可计算的纤维化条件下，证明复杂性的加性分裂定理。

Result: 证明了在有效双Lipschitz、可识别且可计算的纤维化下，点的复杂度可以加性分解为纤维标签复杂度和沿纤维剩余复杂度之和（最多相差对数项），这通过Kolmogorov复杂度的链规则实现。

Conclusion: 正则可识别纤维分解的集合在环境维度上保持信息不可压缩性，除非纤维结构退化或自适应选择。自适应纤维选择是Kakeya猜想在高维中未解决的关键障碍，提出了连接几何测度论、算法复杂性和信息论压缩的正式研究计划。
